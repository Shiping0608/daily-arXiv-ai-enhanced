...
Method: Uses neural networks to discretize eigenfunctions, combining the Rayleigh quotient and Gram-Schmidt orthogonalization for eigenvalue analysis.

Result: The method is robust for irregular domains, parametric/nonlinear eigenproblems, and high-dimensional cases, and useful for spectral basis approximations.
...
<details>
  <summary>Details</summary>
Motivation: To solve nonlinear degenerate parabolic equations with hyperbolic and parabolic behaviors, overcoming stiffness and sharp wave fronts.

Method: Uses DG methods with stabilizing limiters on unstructured meshes and ETD-RK for time integration, leveraging Jacobian matrix for stiffness.

Result: Improved stability and larger time-step sizes demonstrated in 3D simplex meshes.

Conclusion: ETD-RK DG methods effectively handle stiffness and wave fronts in degenerate parabolic equations.
...
Method: Finite element discretization, Newton's method for nonlinear systems, and an augmented Lagrangian preconditioner with flexible GMRES.

Result: Numerical results show robustness and efficiency across varying parameters like viscosity, mesh size, and time step.

Conclusion: The proposed preconditioned solver is effective for the nonlinear systems in this context.
...
<details>
  <summary>Details</summary>
Motivation: Address the inefficiency in memory and computation for array antennas due to their finite translational symmetries.

Method: Uses a multilevel block Toeplitz structure for the impedance matrix, with nine distinct electrically connectable components, and integrates an in-house inverse solver.

Result: Demonstrated on a 32x32 array and two 9x9 arrays, reducing memory from O(N_x^2 N_y^2) to O(N_x N_y).

Conclusion: The method efficiently handles non-symmetric parts of arrays and significantly improves memory and computational performance.
...
<details>
  <summary>Details</summary>
Motivation: Transport phenomena are fundamental in many fields, but solving their seven-dimensional equations is computationally challenging due to dimensionality.

Method: The T²S² solver combines Spectral Collocation, Superconsistency, and Tensor Train format for exponential convergence, stabilization, and data compression.
...
Conclusion: This method makes previously intractable transport problems feasible, enabling efficient modeling of complex phenomena.
...
<details>
  <summary>Details</summary>
Motivation: To fairly assess and compare various tensor-based multivariate function approximation methods, guiding users on their processes, advantages, and limitations.

Method: Construct tensors from a diverse collection of multivariate functions, evaluate method performances (accuracy, computational time, parameter tuning), and compare methods using multiple criteria.

Result: A benchmark collection of tools for tensor approximation is provided, with detailed insights into the mLF approach and its implementation.

Conclusion: The paper aims to guide users in understanding and selecting appropriate tensor approximation methods, emphasizing fairness and practical utility over ranking.
...
Result: Achieves near machine-precision accuracy; speed-up depends on quadrature order, breaking even at ~1.15 times surface nodes.

Conclusion: The method is accurate and efficient for transient acoustic field evaluation.
...
Result: Theoretical error bounds show comparable or superior accuracy to deterministic methods, with numerical experiments confirming reduced runtime and approximation error.

Conclusion: The proposed algorithms are efficient, robust, and outperform state-of-the-art techniques in both synthetic and real-world datasets.
...
<details>
  <summary>Details</summary>
Motivation: To develop a high-order, compact AF scheme that maintains positivity and handles divergence-free constraints in magnetohydrodynamics simulations.

Method: Uses a quadratic reconstruction for nonconservative terms, LLF flux splitting for point values, and introduces limiters and a shock sensor for positivity and oscillation control.

Result: Demonstrates third-order accuracy, preserves positivity, and effectively captures shocks while controlling divergence errors.

Conclusion: The proposed AF scheme is effective for magnetohydrodynamics, balancing accuracy, positivity, and divergence control.
...
<details>
  <summary>Details</summary>
Motivation: To model nematic liquid crystals realistically by addressing anisotropy and physical constraints on Q-tensor eigenvalues, ensuring accurate solutions to nonlinear elliptic PDEs.

Method: Finite element approximation for nonlinear elliptic PDEs with non-homogeneous boundary conditions, using the Newton-Kantorovich theorem for discrete solution existence and uniqueness.

Result: Well-posedness of the discrete problem, optimal convergence rate in energy norm, and analysis of eigenvalue constraints' impact on error.

Conclusion: The framework effectively models nematic liquid crystals with rigorous theoretical backing, demonstrating practical and theoretical validity.
...
<details>
  <summary>Details</summary>
Motivation: Standard collision probability methods are overly conservative for tethered systems, leading to unrealistic risk estimates.

Method: Maximizes collision probability over all physically admissible tether shapes to account for spatial extent and configuration uncertainty.

Result: Applied to real-world events, the method provides more realistic risk estimates, distinguishing hazardous from benign encounters.
...
<details>
  <summary>Details</summary>
Motivation: To develop a generalized framework for sketching dimension reduction applicable to various sketch operators and sets like low-rank tensors.

Method: Uses norming set theory to control sketching dimensions and introduces the median sketch method, requiring only Õ(dim V) measurements.

Result: The median sketch efficiently embeds sets like low-rank tensors with sparse or tensor-structured measurements.

Conclusion: The framework and median sketch advance sketching theory for algebraic varieties and polynomial maps, offering practical efficiency.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenges of solving reaction-diffusion equations with variable exponents and understand the behavior of solutions, including blow-up phenomena.

Method: Analysis of a parabolic problem with a variable exponent $p(x,t)$, using functional analysis and PDE techniques to establish weak solutions.

Result: Existence and uniqueness of weak solutions are proven, along with upper and lower bounds for the blow-up time.

Conclusion: The study provides rigorous results for the problem, contributing to the understanding of reaction-diffusion systems with variable exponents.
...
<details>
  <summary>Details</summary>
Motivation: The well-posedness of the 3D Prandtl equation remains unsolved, highlighted as the third open problem in Oleinik and Samokhin's book. The study aims to tackle this in the steady case.

Method: A new approach using intrinsic vector fields and maximum principles is introduced, with analysis on curvature-type quantities from commutators to address symmetry breaking.

Result: The paper overcomes symmetry-breaking challenges and provides insights into the structural stability of profiles like the Blasius solutions.

Conclusion: The work advances understanding of the 3D Prandtl equation's well-posedness, particularly in the steady case, through innovative methods.
...
<details>
  <summary>Details</summary>
Motivation: To understand the asymptotic behavior of solutions near singular boundaries, which is less explored compared to smooth boundaries.

Method: Analyzing solutions to the singular Yamabe problem with negative constant scalar curvature near singular boundaries and deriving optimal metric estimates.

Result: Solutions are well approximated by solutions in tangent cones at singular boundary points.

Conclusion: The study provides insights into the behavior of solutions near singular boundaries and confirms their approximation by tangent cone solutions.
...
Result: The method uniquely identifies obstacle shape and location without initial guesses or background medium knowledge.

Conclusion: The approach effectively solves the inverse problem for rigid elastic obstacles, offering a robust and prior-free solution.
...
<details>
  <summary>Details</summary>
Motivation: The study is motivated by the need to understand periodic solutions in equations with singularities and variable exponents, which are common in physical and engineering applications.

Method: The approach uses a continuation theorem, a priori estimates, and the method of lower and upper solutions.

Result: The paper establishes conditions under which T-periodic solutions exist for the given equation.

Conclusion: The results extend understanding of periodic solutions in singular differential equations with variable exponents.
...
<details>
  <summary>Details</summary>
Motivation: To address the existence of normalized solutions for the generalized Kadomtsev-Petviashvili equation with prescribed L²-norm, a problem not previously studied.

Method: Analyzes the equation with Lagrange multiplier λ, focusing on subcritical (2<q<10/3) and supercritical (10/3<q<6) cases, and extends to combined nonlinearities (f(t)=μ|t|^{q-2}t+|t|^{p-2}t).

Result: Existence of normalized ground state solutions is proven for both cases. For combined nonlinearities, a second solution with positive energy is shown for a sequence of norms approaching zero.

Conclusion: This work pioneers the study of normalized solutions for the generalized Kadomtsev-Petviashvili equation, demonstrating existence and multiplicity under L² constraints.
...
<details>
  <summary>Details</summary>
Motivation: To generalize previous rigidity results for rotating Euler equation solutions by removing restrictions on angular velocity and vorticity sign, and to include irregular vortex patches.

Method: Analyzes uniformly rotating solutions of the 2D incompressible Euler equation with compactly supported vorticity, focusing on angular velocity ranges and boundary conditions.

Result: Proves radial symmetry for solutions when angular velocity lies outside a specific interval, even for irregular vortex patches.

Conclusion: The work broadens the scope of rigidity theorems, covering more general cases and irregular patches, enhancing understanding of rotating Euler solutions.
...
TL;DR: The paper extends Rivière's 1998 result on minimizing 3-harmonic maps in π₃(S²) to fractional harmonic maps, showing infinitely many homotopy classes with minimizers for s ∈ (0,1).


<details>
  <summary>Details</summary>
Motivation: The motivation is to generalize Rivière's surprising result about minimizing harmonic maps in specific homotopy classes to the fractional setting.

Method: The authors use the framework of fractional harmonic maps, specifically W^{s,3/s}-harmonic maps, to extend the theorem.

Result: They prove that for s ∈ (0,1), infinitely many homotopy classes in π₃(S²) admit minimizing W^{s,3/s}-harmonic maps.

Conclusion: This work generalizes Rivière's findings to fractional harmonic maps, revealing new insights into minimizing properties in homotopy classes.
...
<details>
  <summary>Details</summary>
Motivation: To extend the well-posedness range of the ILW equation and understand its behavior as a perturbation of the Benjamin-Ono equation.

Method: Treats ILW as a perturbation of the Benjamin-Ono equation, avoiding reliance on integrability, and establishes a general well-posedness framework for such perturbations.

Result: Global well-posedness for $s > -\frac12$ and convergence to Benjamin-Ono solutions in the infinite-depth limit.

Conclusion: The ILW equation is well-posed for $s > -\frac12$, and the method applies to other systems like the Smith equation.
...
Method: The study employs techniques for degenerate elliptic equations, imposing dimensional restrictions on ellipticity and growth conditions for source terms near boundary points.

Result: Proves boundary Hölder and Lipschitz regularity, and introduces an intrinsic second-order expansion for solutions in the characteristic half-space.

Conclusion: The findings advance understanding of degenerate elliptic equations in the Heisenberg group, offering new regularity results and expansions.
...
<details>
  <summary>Details</summary>
Motivation: To understand how minimizers of a functional behave in higher dimensions, particularly when symmetry breaks and complex patterns emerge.

Method: Uses the calibration method for free discontinuity problems to analyze minimizers.

Result: In 2D, symmetry breaking produces exotic minimizers with non-simple level sets, unlike the staircase patterns in 1D.

Conclusion: The study reveals unexpected complexity in minimizers when moving from one to two dimensions, highlighting the role of symmetry breaking.
...
<details>
  <summary>Details</summary>
Motivation: To analyze the phase separation in two-component Bose-Einstein condensates, focusing on the linearized system around a one-dimensional solution in an infinite strip.

Method: Study the inner limit system with zero and periodic boundary conditions, deriving optimal invertibility estimates for Fourier modes.

Result: Achieved optimal invertibility estimates without requiring orthogonality conditions.

Conclusion: The findings provide insights into phase separation dynamics in Bose-Einstein condensates under specified boundary conditions.
...
<details>
  <summary>Details</summary>
Motivation: Traditional PINNs struggle with complex spatial hierarchies and boundary conditions, motivating the need for a hybrid approach like BridgeNet.
...
Conclusion: BridgeNet advances computational physics, offering scalable and accurate solutions for applications in finance and complex system dynamics.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenges of solving the unbounded 3D Poisson equation with high accuracy and efficiency, avoiding the need for correction terms or Taylor expansions.

Method: Uses a super-potential formulation with the biharmonic Green's function and a separable Gaussian-sum approximation for FFT-based computation.

Result: Achieves machine-precision accuracy and outperforms existing Gaussian-sum-based methods in error and runtime.

Conclusion: The method is robust and efficient for free-space Poisson problems on uniform grids.
...
Method: The study employs the Vertex-Centered Finite Volume Method for spatial discretization and introduces the Linearly Implicit Extrapolation Method (LIMEX) with an error estimator. The BiCGSTAB method with Geometric Multigrid preconditioner is used to solve the linear system.

Result: The framework shows high scalability, supporting thousands of processors and billions of Degrees of Freedom (DoF) in parallel computations on a supercomputer.
...
<details>
  <summary>Details</summary>
Motivation: Core-shell nanoparticles, especially gold-core ones, are promising for photothermal therapy, imaging, and biosensing, but their thermal dynamics under pulse illumination need deeper understanding.

Method: Combines Mie theory with electronic temperature corrections and couples the two-temperature model with molecular dynamics to simulate heat transfer in nanoparticles with dense/porous silica shells under varying laser pulses.
...
Conclusion: Findings optimize nanoparticle design for photothermal applications and provide a framework for energy transfer in metal-dielectric nanostructures.
...
<details>
  <summary>Details</summary>
Motivation: Accurate outflow boundary conditions are critical in CFD for aerodynamic and acoustic predictions, but LBM suffers from artificial reflections.
...
Result: NN methods outperformed traditional Zou & He conditions in drag, lift, and Strouhal number predictions. NN-enhanced CBC also reduced density errors.

Conclusion: NN-integrated boundary conditions enhance LBM accuracy and efficiency for aerodynamic and acoustic simulations.
...
<details>
  <summary>Details</summary>
Motivation: To create a reliable SDBD reactor for consistent operation and material durability, enabling fundamental studies of discharge characteristics and dynamics for diverse applications.

Method: The reactor uses detachable stainless steel electrodes and quartz dielectric, with the grounded electrode immersed in transformer oil to suppress parasitic discharges. It operates with ns-pulsed and AC discharges at 10 kHz, analyzed via electrical measurements and ICCD imaging.

Result: The reactor exhibits non-linear power consumption with voltage, distinct discharge behaviors (ionization channels and glow-like discharges), and varying propagation lengths and velocities for ns-pulsed and AC operations. Ns-pulsed discharges are more reproducible.

Conclusion: The reactor is effective for fundamental SDBD studies, offering insights into discharge dynamics and potential applications in fields like flow control, biomedicine, and agriculture.
...
<details>
  <summary>Details</summary>
Motivation: To understand how superintense lasers interact with dense nitrogen plasma, aiming to improve high-current laser-plasma accelerators.

Method: Particle-in-cell simulations to analyze laser self-focusing and its effects on plasma dynamics.

Result: Identified scaling laws for beam diffraction, wakefield amplitude, and plasma structures.

Conclusion: Provides key insights for optimizing laser-plasma accelerators in high-density regimes.
...
<details>
  <summary>Details</summary>
Motivation: Understanding gas-phase chemical kinetics in microdischarges is crucial for controlling plasma-driven electrochemistry, but current knowledge is limited due to high electron densities and temperatures.

Method: A 0-D modeling approach, informed by experimental measurements, is used to analyze chemical kinetics. A new reaction scheme for microdischarges in water vapor is developed, tested with plasma electrolytic oxidation.

Result: H2O is highly dissociated during peak power density, with H and O dominating the neutral gas. Ionization degree is low (0.31%), and electronegativity is minimal. Key reactions are identified across 2000K–6000K.

Conclusion: The findings provide insights into microdischarge chemistry, applicable to similar plasma electrolysis systems, highlighting the importance of high-threshold energy reactions.
...
<details>
  <summary>Details</summary>
Motivation: To bridge the gap between near-axis models and global equilibria for practical stellarator design and optimization.
...
Conclusion: This method enhances stellarator design by integrating near-axis and global equilibrium approaches.
...
<details>
  <summary>Details</summary>
Motivation: To address the growing adoption of Concept-based Models in sensitive domains by identifying their limitations and improving reliability.

Method: Comprehensive survey aggregating challenges and architecture choices for Supervised and Unsupervised paradigms, along with recent advances.

Result: Identified key issues like concept leakage and robustness, and explored mitigation strategies.

Conclusion: Highlights open problems and promising future research to enhance the reliability and applicability of Concept-based Models.
...
<details>
  <summary>Details</summary>
Motivation: OOD detection is critical for reliable deep learning in open-world scenarios, but current methods lack explainability.

Method: Augments existing OOD detectors with MLNs, combining probabilistic reasoning and first-order logic for better explainability. Introduces an algorithm for learning logical constraints.

Result: MLNs significantly improve OOD detector performance across datasets while maintaining computational efficiency.

Conclusion: MLNs offer a promising approach for enhancing OOD detection with explainability and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Current approaches treat time as an input parameter, ignoring the sequential nature of deformation development, which limits accuracy.

Method: The model uses a triple-stream attention framework (temporal, feature, and batch attention) within a transformer architecture for autoregressive sequence modeling.

Result: Achieves 1.63% mean absolute percentage error and R2 of 0.999, outperforming traditional models. SHAP analysis highlights key predictive features like Young's modulus.

Conclusion: The work demonstrates transformers' viability in materials science, offering high accuracy and interpretability for engineering applications.
...
<details>
  <summary>Details</summary>
Motivation: Traditional fine-tuning for safety in LLMs is costly and impractical, prompting the need for simpler, inference-time control methods.

Method: SafeSteer employs category-specific steering vectors and a gradient-free unsupervised approach to adjust model outputs without contrastive data.

Result: The method effectively guides LLMs to generate safer content while preserving topic relevance and avoiding blanket refusals.

Conclusion: SafeSteer offers a simple, effective solution for safety adjustments in LLMs, aligning with the trend favoring simplicity in activation steering.
...
<details>
  <summary>Details</summary>
Motivation: Climate change has increased fire frequency and intensity, making accurate prediction crucial. Traditional binary classification oversimplifies the problem for end-users like firefighters, necessitating a localized approach.

Method: Proposes a new AI-based method for fire risk assessment tailored to departmental conditions, using state-of-the-art models on an unexplored dataset for metropolitan France.

Result: Presents the first national-scale AI benchmark for France, providing region-specific and actionable fire risk predictions.

Conclusion: Highlights the need for localized risk modeling and suggests future research directions, with supplementary materials available on GitHub.
...
<details>
  <summary>Details</summary>
Motivation: Challenges in deploying neural networks on edge devices due to high complexity, and limitations of existing verification methods.

Method: Encodes original and compressed networks into SMT formulas, classifies changes, and uses Minimal Unsat Cores for guided verification.

Result: Effective in handling quantization and pruning, with high proof reuse and faster verification than traditional methods.

Conclusion: MUC-G4 ensures safety and reliability of compressed networks, offering practical benefits.
...
<details>
  <summary>Details</summary>
Motivation: Overcome the limitation of physical reservoir computing, where backpropagation is impossible due to the black-box nature of physical reservoirs.

Method: Uses random perturbations in parameter space to approximate gradient updates, requiring only forward passes. Tested on simulated networks and experimental hardware.

Result: PGT achieves performance similar to backpropagation in cases where backpropagation is impractical.

Conclusion: PGT enables deeper integration of physical reservoirs into neural networks, promising energy efficiency gains in AI training.
...
<details>
  <summary>Details</summary>
Motivation: To understand the impact of the sampling distribution on DPO's training dynamics and optimization landscape.

Method: Analyzed the influence of response distribution on policy updates, designed a simplified alignment model, and conducted empirical experiments.

Result: High-quality responses amplify gradient signals, improving the optimization landscape and policy learning.

Conclusion: The findings justify the online DPO framework, emphasizing the importance of sampling distribution quality.
...
<details>
  <summary>Details</summary>
Motivation: Compound flood forecasting is complex and traditional methods are inefficient. Machine learning shows promise, but data scarcity limits progress.

Method: Introduces SF2Bench, integrating tide, rainfall, groundwater, and human activities. Evaluates six ML methods (e.g., CNNs, RNNs, Transformers) for flood forecasting.

Result: Experiments show varying performance of methods, highlighting their strengths in capturing temporal and spatial dependencies.

Conclusion: SF2Bench enables detailed analysis of flood drivers, and diverse ML methods offer insights for improved forecasting.
...
<details>
  <summary>Details</summary>
Motivation: Existing LLM-based symbolic regression methods over-rely on internal priors and lack systematic data understanding, limiting robustness and discovery capability.

Method: DrSR integrates data-driven analysis (e.g., structural relationships) with reflective learning, creating a feedback loop to refine equation generation.

Result: DrSR improves valid equation rates and outperforms classical and LLM-based methods in accuracy, generalization, and efficiency across interdisciplinary datasets.

Conclusion: DrSR's dual reasoning approach advances symbolic regression, demonstrating strong potential for scientific equation discovery.
...
<details>
  <summary>Details</summary>
Motivation: To address the bottleneck of bandwidth-limited satellite data downlinking by enabling on-board processing and prioritization of relevant disaster data.

Method: Uses a PANN inspired by memristor-based physical neural networks, with dynamic weights updated via physics constraints, eliminating the need for training.

Result: PANN matched or outperformed a state-of-the-art AI model in disaster detection, proving effective for resource-constrained on-board processing.

Conclusion: PANN offers a viable, training-free solution for real-time disaster detection and data prioritization in satellites.
...
<details>
  <summary>Details</summary>
Motivation: Limited adaptation data hinders efficient training of large backbone models, despite their potential. The paper aims to utilize backbone data to augment adaptation datasets.

Method: Proposes BAT, a method using backbone data for augmentation. Formulates and proves two key propositions, introduces a data selection scheme, and presents ALBAT algorithm.

Result: ALBAT effectively improves adaptation training in tasks like personalization and language generation with scarce data.

Conclusion: BAT and ALBAT offer a practical solution to leverage backbone data for better adaptation, validated by theoretical and empirical results.
...
<details>
  <summary>Details</summary>
Motivation: To understand how transformers perform relational reasoning, specifically transitive inference, under different learning regimes (IWL and ICL).

Method: Compare transformer behavior in transitive inference tasks across IWL (information stored in network parameters) and ICL (information used from input sequences). Analyze mechanistic differences and effects of pre-training.

Result: IWL generalizes transitively naturally, while ICL requires pre-training on structured tasks to achieve similar performance. ICL models develop match-and-copy strategies without hierarchical encoding.

Conclusion: Pre-training on structured tasks enables transformers to develop representations supporting in-context relational reasoning, mirroring human-like behaviors.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the need for efficient optimization in IoT by integrating Lyapunov Drift-Plus-Penalty with reinforcement learning for better performance.

Method: The LDPTRLQ algorithm combines Lyapunov Drift-Plus-Penalty with RL, balancing greedy optimization and long-term perspective through theoretical analysis.

Result: Simulations show LDPTRLQ outperforms baseline methods in compatibility and stability, validating theoretical claims.

Conclusion: LDPTRLQ effectively integrates Lyapunov Drift-Plus-Penalty with RL, offering superior performance and stability for IoT optimization.
...
<details>
  <summary>Details</summary>
Motivation: Machine learning struggles to generalize without structural priors, especially for dynamics poorly represented in training data.

Method: A multiple-trajectory training scheme for reservoir computers, applied to multistable dynamical systems.
...
<details>
  <summary>Details</summary>
Motivation: Clinical trials are costly and time-consuming; accurate outcome prediction can reduce costs and accelerate drug discovery. Existing deep learning models lack interpretability and are prone to label leakage.

Method: AutoCT autonomously generates, evaluates, and refines tabular features using Monte Carlo Tree Search for iterative optimization.

Result: AutoCT matches or outperforms state-of-the-art methods in clinical trial prediction tasks with limited self-refinement iterations.

Conclusion: AutoCT introduces a scalable, interpretable, and cost-efficient paradigm for clinical trial prediction.
...
<details>
  <summary>Details</summary>
Motivation: The transition to renewable energy requires precise power demand forecasting to ensure grid stability, necessitating tailored models for different consumer groups.

Method: Customer clusterisation and feature selection (temporal, socio-economic, weather data) are used. AI/ML algorithms for STLF and VSTLF are compared to identify the most effective approaches.

Result: The proposed forecasting models outperform simpler ones, showing the value of customized strategies and detailed weather data.

Conclusion: Customized forecasting for consumer groups and detailed weather data enhance accuracy, aiding grid stability in renewable energy transitions.
...
<details>
  <summary>Details</summary>
Motivation: Accurate forecasting is critical due to operational uncertainties like weather, breakdowns, and crew availability.

Method: Proposes a hybrid deep-learning framework using XGBoost and LSTM, integrating real operational records and synthetic breakdown data.

Result: XGBoost (MedAE 14.3%) and LSTM (MedAE 15.1%) perform well; SHAP analysis highlights rainfall, payload trends, and breakdown frequencies as key predictors.

Conclusion: The hybrid model reduces volatility and supports proactive fleet management. Future work will expand features for robustness.
...
<details>
  <summary>Details</summary>
Motivation: To improve the consistency and dependability of next-generation ensemble learning systems by leveraging softlog-softmax cascades and diversity.

Method: 1. Anatomical: Uses canonical elements for ensemble models without a priori choices, prioritizing diversity. 2. Physiological: Derives softlog for consistent weak logarithmic operations and intermediate decisions. 3. Neural information theory: Introduces softlog-based entropy and divergence for consistent information measures. 4. Performance tensor: Derives a tensor for reliable ensemble evaluation.

Result: Demonstrates that ensemble diversity is more beneficial for complex problems and provides consistent information measures and reliable evaluation.

Conclusion: The proposed 4-step process effectively enhances ensemble learning systems through diversity, softlog-softmax cascades, and robust evaluation metrics.
...
<details>
  <summary>Details</summary>
Motivation: To address concerns about the efficiency, cost, and sustainability of dynamic, multi-turn AI agent workflows, which replace static inference.

Method: Conducts a comprehensive system-level analysis of AI agents, quantifying resource usage, latency, energy, and power consumption across designs and scaling strategies.

Result: Agents improve accuracy with more compute but face diminishing returns, high latency variance, and unsustainable infrastructure costs.

Conclusion: Urges a shift toward compute-efficient agent design to balance performance with real-world deployability.
...
<details>
  <summary>Details</summary>
Motivation: Current RFT-based red teaming lacks a unified benchmark, and implementation details like PPO affect stability and reproducibility.

Method: RedRFT combines CleanRL and Tianshou strengths, offering modular PPO components and diversity metrics, with an ablation study on key components.

Result: The benchmark simplifies RFT implementation and evaluation, supporting modular experimentation and clarifying component influences.
...
<details>
  <summary>Details</summary>
Motivation: To simplify and improve the process of loss weight hyperparameter optimization in model training, avoiding multiple runs.

Method: Uses gradient-based optimization to treat loss weights as network parameters, leveraging a differentiable composite loss formulation and a novel regularization loss.

Result: YOTO consistently outperforms grid-search models in 3D estimation and semantic segmentation tasks.

Conclusion: YOTO effectively optimizes loss weights in one shot, demonstrating superior performance over traditional methods.
...
<details>
  <summary>Details</summary>
Motivation: To reduce the number of trainable parameters in neural networks without sacrificing performance by leveraging randomized connections.

Method: Each hidden unit combines fixed random weights and two modifiable weights for scaling and translation, applied before a nonlinearity.

Result: Experiments on MNIST and FashionMNIST show reasonable accuracy with fewer parameters due to regularization from randomized connections.

Conclusion: Half layers offer a promising approach to parameter efficiency in deep networks, especially after convolutional layers.
...
<details>
  <summary>Details</summary>
Motivation: To address the limitations of single-metric objectives like the Sharpe ratio by providing a modular and parameterized reward function for diverse investor preferences.

Method: Proposes a composite reward function with four terms (annualized return, downside risk, differential return, Treynor ratio), tuned via grid search, and analyzed for theoretical properties like monotonicity and boundedness.

Result: The framework offers a robust, multi-objective reward function for financial trading, with potential for extension to additional risk measures or adaptive weighting.

Conclusion: This approach provides a flexible and scalable blueprint for designing reward functions in complex trading environments.
...
<details>
  <summary>Details</summary>
Motivation: To extend decision tree models to hyperbolic spaces while preserving simplicity and leveraging existing Euclidean optimizations.

Method: Reformulates hyperDT in the Beltrami-Klein model, enabling thresholding operations and wrapping around Euclidean tree-based models.

Result: Achieves better flexibility, speed, and accuracy with a simpler, maintainable codebase.

Conclusion: The wrapper approach effectively bridges hyperbolic and Euclidean decision trees, enhancing practicality and performance.
...
<details>
  <summary>Details</summary>
Motivation: To understand the theoretical underpinnings of sample replay in continual learning, particularly its potential to mitigate forgetting, and to explore scenarios where replay might be harmful.

Method: Theoretical analysis in an over-parameterized continual linear regression setting, examining the role of replayed samples and task subspace relationships. Empirical validation with neural networks and SGD on benchmarks.

Result: Forgetting can be non-monotonic with replay samples; replay may increase forgetting in worst-case and distributional settings. Harmful replay is observed even in neural networks.

Conclusion: Sample replay's effectiveness depends on sample choice and task relationships, and it can sometimes worsen forgetting, challenging its universal utility.
...
<details>
  <summary>Details</summary>
Motivation: To address instability and performance decline when removing target networks in reinforcement learning, while leveraging existing literature and reducing memory usage.

Method: Uses a copy of the last linear layer of the online network as a target network, sharing remaining parameters, and applies iterated Q-learning to bridge performance gaps.

Result: iS-QL improves sample efficiency of target-free methods, maintains comparable training time, and reduces memory footprint.

Conclusion: iS-QL offers a scalable solution for reinforcement learning by balancing target-based and target-free benefits.
...
### [64] [Unsupervised Meta-Testing with Conditional Neural Processes for Hybrid Meta-Reinforcement Learning](https://arxiv.org/abs/2506.04399)
*Suzan Ece Ada,Emre Ugur*

Main category: cs.LG

TL;DR: UMCNP is a hybrid few-shot meta-RL method combining policy gradient and task inference, improving sample efficiency without extra meta-training samples by leveraging CNPs for task inference.
...
Method: Combines PPG meta-RL and CNPs for task inference, reusing meta-training samples offline and inferring task dynamics from a single rollout.

Result: Outperforms baselines in 2D-Point Agent and continuous control tasks, requiring fewer meta-testing samples.

Conclusion: UMCNP effectively adapts to unseen tasks with minimal samples, demonstrating superior efficiency and scalability.
...
TL;DR: The paper bridges the theoretical gap in self-supervised contrastive learning (CL) by linking it to a supervised variant (NSCL), showing their equivalence as class numbers grow, and analyzing the geometric structure of learned representations. It also introduces a bound for few-shot error and validates findings empirically.


<details>
  <summary>Details</summary>
Motivation: To establish theoretical foundations for self-supervised contrastive learning, which lacks comprehensive understanding despite empirical success.

Method: The authors analyze CL by approximating it with a supervised variant (NSCL), prove vanishing gaps between losses with increasing classes, characterize minimizers' geometric structure, and derive a few-shot error bound.

Result: The gap between CL and NSCL losses vanishes as class numbers increase; learned representations exhibit specific geometric properties; a tight few-shot error bound is introduced.

Conclusion: The study provides theoretical insights into CL, validates the equivalence of CL and NSCL, and demonstrates practical implications for few-shot learning.
...
### [66] [Leveraging Coordinate Momentum in SignSGD and Muon: Memory-Optimized Zero-Order](https://arxiv.org/abs/2506.04430)
...
TL;DR: The paper proposes zero-order (ZO) optimization methods, JAGUAR SignSGD and JAGUAR Muon, as efficient alternatives to traditional first-order optimizers for fine-tuning LLMs, offering memory reduction and strong convergence guarantees.


<details>
  <summary>Details</summary>
Motivation: Traditional first-order optimizers like SGD and Adam are computationally expensive for fine-tuning large language models (LLMs). The paper explores ZO methods as a resource-efficient alternative, especially for parameter-efficient techniques like LoRA.

Method: The authors introduce JAGUAR SignSGD, a momentum-based ZO algorithm, and JAGUAR Muon, a ZO extension of Muon optimizer. Both methods aim to reduce memory and computational costs while maintaining convergence quality.

Result: Experiments show the proposed ZO methods match or surpass the performance of first-order optimizers, with significant memory savings. Theoretical convergence guarantees are provided for both algorithms.

Conclusion: The study establishes ZO optimization as a practical and theoretically sound approach for fine-tuning LLMs under resource constraints, offering a balance between efficiency and performance.
...
### [67] [KOALA++: Efficient Kalman-Based Optimization of Neural Networks with Gradient-Covariance Products](https://arxiv.org/abs/2506.04432)
*Zixuan Xia,Aram Davtyan,Paolo Favaro*

Main category: cs.LG

TL;DR: KOALA++ is a scalable Kalman-based optimization algorithm for neural networks, improving upon KOALA by modeling structured gradient uncertainty efficiently without costly second-order calculations.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiency of second-order methods in neural network training while capturing richer gradient uncertainty structure.
...
Result: Achieves accuracy comparable or superior to state-of-the-art optimizers in tasks like image classification and language modeling, with first-order efficiency.

Conclusion: KOALA++ offers an efficient and effective alternative to traditional first- and second-order optimizers by better modeling gradient uncertainty.
...
### [68] [Grokking and Generalization Collapse: Insights from \texttt{HTSR} theory](https://arxiv.org/abs/2506.04434)
*Hari K. Prakash,Charles H. Martin*

Main category: cs.LG

TL;DR: The paper identifies a new phase in neural network training called 'anti-grokking,' distinct from pre-grokking and grokking, where test accuracy collapses while training accuracy remains high. The Heavy-Tailed Self-Regularization (HTSR) metric α effectively detects all three phases, unlike other metrics.


<details>
  <summary>Details</summary>
Motivation: To explore the grokking phenomenon in neural networks and uncover new phases of training behavior, particularly focusing on late-stage collapse (anti-grokking).

Method: A 3-layer MLP is trained on a subset of MNIST with and without weight decay. The HTSR metric α, measured using the WeightWatcher tool, is used to analyze layer quality and detect phases.

Result: Anti-grokking is identified as a distinct phase, marked by α < 2 and Correlation Traps. HTSR α outperforms other metrics in detecting all phases.

Conclusion: HTSR α is a robust metric for measuring generalization and detecting overfitting, with α ≈ 2 as a universal convergence target.
...
### [69] [RETRO SYNFLOW: Discrete Flow Matching for Accurate and Diverse Single-Step Retrosynthesis](https://arxiv.org/abs/2506.04439)
*Robin Yadav,Qi Yan,Guy Wolf,Avishek Joey Bose,Renjie Liao*

Main category: cs.LG

TL;DR: RETRO SYNFLOW (RSF) is a discrete flow-matching framework for single-step retrosynthesis, outperforming state-of-the-art methods with 60.0% top-1 accuracy and improved diversity.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of accurate and diverse single-step retrosynthesis prediction in organic chemistry.

Method: RSF uses a Markov bridge between target and reactant molecules, reaction center identification, and Feynman-Kac steering with Sequential Monte Carlo resampling.

Result: Achieves 60.0% top-1 accuracy, a 20% improvement over prior SOTA, and 19% better top-5 round-trip accuracy.

Conclusion: RSF enhances retrosynthesis planning with higher accuracy and diversity, validated by empirical results.
...
### [70] [Selective Matching Losses -- Not All Scores Are Created Equal](https://arxiv.org/abs/2506.04446)
...
<details>
  <summary>Details</summary>
Motivation: Accurate predictions are often critical in specific regions of a domain, but traditional losses treat all regions equally. The paper aims to address this by designing loss functions that prioritize high-importance regions.

Method: The authors construct selective matching loss functions using link functions (e.g., Sigmoid, hyperbolic sine) to emphasize high-sensitivity regions. They extend these to multi-class settings using composite Softmax functions.

Result: Selective losses outperform traditional losses in applications like dwell-time prediction, retrieval, ranking, and LLM fine-tuning, by focusing on high-importance regions.

Conclusion: Selective loss functions provide a flexible and effective way to prioritize accuracy in critical regions, improving performance in various machine learning tasks.
...
### [71] [Neurosymbolic Artificial Intelligence for Robust Network Intrusion Detection: From Scratch to Transfer Learning](https://arxiv.org/abs/2506.04454)
...
<details>
  <summary>Details</summary>
Motivation: To enhance robustness, interpretability, and generalization in NIDS by combining Neurosymbolic AI with advanced uncertainty quantification and transfer learning.

Method: Extends ODXU with score-based and metamodel-based uncertainty quantification, and develops a transfer learning strategy for cybersecurity applications.

Result: ODXU outperforms traditional neural models on the CIC-IDS-2017 dataset and shows effective transfer learning on ACI-IoT-2023 with minimal data.
...
### [72] [Behavioural vs. Representational Systematicity in End-to-End Models: An Opinionated Survey](https://arxiv.org/abs/2506.04461)
*Ivan Vegner,Sydelle de Souza,Valentin Forch,Martha Lewis,Leonidas A. A. Doumas*

Main category: cs.LG

TL;DR: The paper discusses the distinction between systematicity of behavior and representations in ML models, analyzes benchmarks for behavioral systematicity, and suggests ways to assess representational systematicity.
...
Result: Identifies limitations in current benchmarks for testing behavioral systematicity and proposes ways to evaluate representational systematicity.

Conclusion: Emphasizes the need to distinguish and assess both behavioral and representational systematicity for better generalization in ML models.
...
### [73] [Classifying Dental Care Providers Through Machine Learning with Features Ranking](https://arxiv.org/abs/2506.04474)
*Mohammad Subhi Al-Batah,Mowafaq Salem Alzboon,Muhyeeddin Alqaraleh,Mohammed Hasan Abu-Arqoub,Rashiq Rafiq Marie*

Main category: cs.LG

TL;DR: The study uses ML models to classify dental providers into two categories, achieving high accuracy (94.1% with Neural Networks) and highlighting the importance of feature selection, especially treatment-related metrics, for efficient classification.
...
Method: Employed 12 ML models (e.g., Neural Networks, Gradient Boosting) on a dataset of 24,300 instances with 20 features, using feature ranking methods (information gain, Gini index, ANOVA) and 10-fold cross-validation.

Result: Neural Networks achieved the highest accuracy (94.1%), with treatment-related metrics being top predictors. Models improved with more features, and ensemble methods handled missing data well.

Conclusion: Feature selection is crucial for ML efficiency in healthcare datasets. Integrating feature-ranking with advanced ML can optimize dental provider classification for targeted resource allocation.
...
### [74] [Comparative performance of ensemble models in predicting dental provider types: insights from fee-for-service data](https://arxiv.org/abs/2506.04479)
*Mohammad Subhi Al-Batah,Muhyeeddin Alqaraleh,Mowafaq Salem Alzboon,Abdullah Alourani*

Main category: cs.LG

TL;DR: Machine learning models, especially Neural Networks and Random Forest, excelled in classifying dental providers, improving healthcare resource allocation.


<details>
  <summary>Details</summary>
Motivation: To enhance service delivery for underserved populations by accurately classifying dental providers.

Method: Analyzed a 2018 dataset of 24,300 instances with 20 features using multiple ML algorithms (kNN, Decision Trees, SVM, SGD, Random Forest, Neural Networks, Gradient Boosting) and evaluated performance via 10-fold cross-validation.

Result: Neural Networks achieved the highest AUC (0.975) and classification accuracy (94.1%), followed by Random Forest (AUC: 0.948, CA: 93.0%).

Conclusion: Advanced ML models, particularly ensemble and deep learning, significantly improve dental provider classification, aiding better resource distribution for underserved communities.
...
### [75] [Orthogonal Gradient Descent Improves Neural Calibration](https://arxiv.org/abs/2506.04487)
*C. Evans Hedges*

Main category: cs.LG

TL;DR: Orthogonalizing gradients ($\perp$Grad) improves model calibration without losing accuracy, outperforming SGD in metrics like test loss and predictive entropy. It works well under input corruption and extended training, is optimizer-agnostic, and complements post-hoc calibration. Theoretically, it converges to solutions favoring confidence scaling over boundary improvement.


<details>
  <summary>Details</summary>
Motivation: To enhance model calibration while maintaining accuracy, addressing issues like overconfidence and poor generalization under corruption.
...
Result: $\perp$Grad matches SGD in accuracy but improves calibration (lower test loss, reduced overconfidence, higher entropy). Benefits persist under corruption and extended training.

Conclusion: $\perp$Grad is a practical, efficient method for better calibration, supported by theoretical guarantees and empirical success.
...
### [76] [Multiscale guidance of AlphaFold3 with heterogeneous cryo-EM data](https://arxiv.org/abs/2506.04490)
*Rishwanth Raghu,Axel Levy,Gordon Wetzstein,Ellen D. Zhong*

Main category: cs.LG

TL;DR: CryoBoltz combines cryo-EM density maps with protein structure prediction models to capture conformational diversity in dynamic biomolecular systems.


<details>
  <summary>Details</summary>
Motivation: Current protein structure prediction models lack the ability to capture conformational diversity, while cryo-EM struggles with converting raw data into atomic models.

Method: CryoBoltz integrates cryo-EM density maps with pretrained protein structure prediction models, using global and local constraints to guide sampling.

Result: The method successfully builds atomic models for diverse dynamic systems like transporters and antibodies.

Conclusion: CryoBoltz bridges the gap between cryo-EM and structure prediction, enabling accurate modeling of conformational states.
...
### [77] [Hierarchical Implicit Neural Emulators](https://arxiv.org/abs/2506.04528)
*Ruoxi Jiang,Xiao Zhang,Karan Jakhar,Peter Y. Lu,Pedram Hassanzadeh,Michael Maire,Rebecca Willett*

Main category: cs.LG

TL;DR: A multiscale implicit neural emulator improves long-term prediction accuracy in neural PDE solvers by using hierarchical future state representations and adaptive temporal downsampling.


<details>
  <summary>Details</summary>
Motivation: Neural PDE solvers often face challenges with error accumulation and stability over long time horizons, limiting their effectiveness for complex dynamical systems.

Method: The proposed method conditions on a hierarchy of lower-dimensional future state representations and leverages predictions several steps ahead at increasing compression rates for refinements. It adjusts temporal downsampling ratios to capture dynamics across granularities.

Result: Experiments on turbulent fluid dynamics demonstrate high short-term accuracy and long-term stability, outperforming autoregressive baselines with minimal computational overhead.

Conclusion: The multiscale implicit neural emulator effectively addresses stability and accuracy issues in neural PDE solvers, offering improved performance for long-term predictions.
...
### [78] [HALoS: Hierarchical Asynchronous Local SGD over Slow Networks for Geo-Distributed Large Language Model Training](https://arxiv.org/abs/2506.04531)
*Geon-Woo Kim,Junbo Li,Shashidhar Gandham,Omar Baldonado,Adithya Gangidi,Pavan Balaji,Zhangyang Wang,Aditya Akella*

Main category: cs.LG

TL;DR: HALoS is a hierarchical asynchronous optimization framework for training LLMs across distributed regions, reducing communication costs and improving efficiency while maintaining model quality.


<details>
  <summary>Details</summary>
Motivation: Training LLMs on geographically distributed accelerators faces high communication costs and uneven hardware utilization, necessitating a solution like HALoS.

Method: HALoS introduces local parameter servers (LPSs) within regions and a global parameter server (GPS) to merge updates, minimizing inter-region communication and leveraging fast intra-region links.

Result: HALoS achieves up to 7.5x faster convergence than synchronous baselines and 2.1x improvement over asynchronous methods, while preserving model accuracy.

Conclusion: HALoS demonstrates that hierarchical, server-side update accumulation is effective for scalable and efficient LLM training in heterogeneous, geo-distributed environments.
...
### [79] [NOBLE -- Neural Operator with Biologically-informed Latent Embeddings to Capture Experimental Variability in Biological Neuron Models](https://arxiv.org/abs/2506.04536)
*Luca Ghafourpour,Valentin Duruisseaux,Bahareh Tolooshams,Philip H. Wong,Costas A. Anastassiou,Anima Anandkumar*

Main category: cs.LG

TL;DR: NOBLE is a neural operator framework that predicts neural dynamics by learning from biophysically realistic neuron models, addressing variability and scalability issues in current approaches.


<details>
  <summary>Details</summary>
Motivation: Current neuron modeling approaches struggle with data variability and computational costs, limiting their scalability and accuracy.

Method: NOBLE uses a neural operator to map interpretable neuron features to somatic voltage responses, trained on realistic neuron models.

Result: NOBLE predicts neural dynamics with variability, achieves a 4200x speedup, and is validated on real experimental data.

Conclusion: NOBLE advances neuron modeling, enabling better understanding of brain function and applications in neuroAI.
...
### [80] [Neural MJD: Neural Non-Stationary Merton Jump Diffusion for Time Series Prediction](https://arxiv.org/abs/2506.04542)
*Yuanpei Gao,Qi Yan,Yan Leng,Renjie Liao*

Main category: cs.LG

TL;DR: Neural MJD, a neural network-based non-stationary Merton jump diffusion model, outperforms existing methods by explicitly modeling stochastic processes and handling abrupt changes in time series data.


<details>
  <summary>Details</summary>
Motivation: Deep learning models struggle with non-stationary data and abrupt changes due to their black-box nature. Neural MJD addresses this by explicitly modeling stochastic dynamics and jumps.

Method: The model combines a time-inhomogeneous Itô diffusion for non-stationary dynamics and a compound Poisson process for jumps. It uses a likelihood truncation mechanism and a novel Euler-Maruyama solver for tractable learning.
...
Conclusion: Neural MJD provides a robust framework for non-stationary time series prediction by integrating stochastic modeling with neural networks.
...
### [81] [Communication Efficient Adaptive Model-Driven Quantum Federated Learning](https://arxiv.org/abs/2506.04548)
*Dev Gurung,Shiva Raj Pokhrel*

Main category: cs.LG

TL;DR: The paper introduces a model-driven quantum federated learning (mdQFL) algorithm to address challenges like training bottlenecks, large-scale device involvement, and non-IID data in federated learning, achieving reduced communication costs and improved accuracy.


<details>
  <summary>Details</summary>
Motivation: The challenges of massive datasets, numerous devices, and non-IID data distributions in federated learning (FL) and quantum federated learning (QFL) hinder performance and efficiency.

Method: The authors propose mdQFL, an efficient and adaptable algorithm that includes training and update personalization, and test generalization in a QFL setting.

Result: Experiments show a 50% reduction in communication costs while maintaining or improving accuracy and local model training compared to standard QFL.

Conclusion: mdQFL effectively addresses FL challenges in QFL, offering practical benefits and theoretical insights, with potential applicability to broader FL scenarios.
...
### [82] [Unsupervised Machine Learning for Scientific Discovery: Workflow and Best Practices](https://arxiv.org/abs/2506.04553)
*Andersen Chang,Tiffany M. Tang,Tarek M. Zikry,Genevera I. Allen*

Main category: cs.LG

TL;DR: The paper proposes a standardized workflow for unsupervised learning in science to ensure reliable and reproducible discoveries, illustrated with an astronomy case study.


<details>
  <summary>Details</summary>
Motivation: Address the lack of standardization in unsupervised learning workflows for scientific discoveries.

Method: Introduces a structured workflow including question formulation, data preparation, modeling, validation, and documentation.

Result: Demonstrates the workflow's effectiveness through a case study on refining Milky Way globular clusters.

Conclusion: A carefully-designed workflow enhances validation and reproducibility in unsupervised learning for scientific discovery.
...
### [83] [Clustering and Median Aggregation Improve Differentially Private Inference](https://arxiv.org/abs/2506.04566)
*Kareem Amin,Salman Avestimehr,Sara Babakniya,Alex Bie,Weiwei Kong,Natalia Ponomareva,Umar Syed*

Main category: cs.LG

TL;DR: The paper introduces a method to improve differentially private (DP) language model inference by clustering input data and using medians for aggregation, enhancing text quality and privacy guarantees.


<details>
  <summary>Details</summary>
Motivation: Uniform sampling in DP language model inference degrades text quality, especially for heterogeneous topics. The paper aims to address this by improving batch selection and aggregation methods.

Method: Clusters input data before selecting inference batches and aggregates next-token predictions using medians instead of averages, leveraging reduced local sensitivity for better privacy guarantees.

Result: The method improves representativeness metrics (e.g., MAUVE) and downstream task performance, achieving higher-quality synthetic data at lower privacy cost.

Conclusion: Clustering and median-based aggregation enhance DP language model inference, offering better text quality and stronger privacy guarantees compared to prior methods.
...
### [84] [StatsMerging: Statistics-Guided Model Merging via Task-Specific Teacher Distillation](https://arxiv.org/abs/2506.04567)
*Ranjith Merugu,Bryan Bo Cao,Shubham Jain*

Main category: cs.LG

TL;DR: StatsMerging is a lightweight, learning-based model merging method that uses weight distribution statistics and SVD for task importance, improving accuracy and generalization without needing labels or test samples.


<details>
  <summary>Details</summary>
Motivation: To address memory constraints by merging multiple large models efficiently, leveraging weight distribution statistics for task-specific guidance.

Method: Uses SVD for task importance, a lightweight learner (StatsMergeLearner) for weight distribution modeling, and task-specific teacher distillation for heterogeneous architectures.

Result: Outperforms state-of-the-art in accuracy, generalization, and robustness across eight tasks.
...
### [85] [Scaling Laws for Robust Comparison of Open Foundation Language-Vision Models and Datasets](https://arxiv.org/abs/2506.04598)
*Marianna Nezhurina,Tomer Porian,Giovanni Pucceti,Tommie Kerssies,Romain Beaumont,Mehdi Cherti,Jenia Jitsev*

Main category: cs.LG

TL;DR: The paper demonstrates how scaling laws can be used to compare models and datasets for pre-training, focusing on CLIP and MaMMUT. It shows MaMMUT's superior scalability and efficiency, validated across multiple tasks and datasets.


<details>
  <summary>Details</summary>
Motivation: To enable systematic comparison of foundation models and datasets using scaling laws, avoiding misleading conclusions from single-scale evaluations.

Method: Derives full scaling laws for CLIP and MaMMUT using dense measurements across model and sample scales, and evaluates them on tasks like classification, retrieval, and segmentation.

Result: MaMMUT shows stronger improvement with scale and better sample efficiency than CLIP, consistently across tasks and datasets.

Conclusion: Scaling laws provide a robust method for model and dataset comparison, facilitating systematic improvement of foundation models. Open-sourced models and code support reproducibility.
...
### [86] [Ignoring Directionality Leads to Compromised Graph Neural Network Explanations](https://arxiv.org/abs/2506.04608)
*Changsheng Sun,Xinke Li,Jin Song Dong*

Main category: cs.LG

TL;DR: Direction-aware GNN explainability improves fidelity by preserving directional semantics, unlike symmetrization which misleads.


<details>
  <summary>Details</summary>
Motivation: Reliable explanations are crucial for GNNs in critical domains, but symmetrization loses directional info, compromising fidelity.

Method: Theoretical and empirical studies on preserving directional semantics in GNNs.

Result: Direction-aware methods significantly enhance explanation quality and faithfulness.

Conclusion: Direction-aware GNN explainability is essential, especially for security-critical applications.
...
### [87] [Exploring bidirectional bounds for minimax-training of Energy-based models](https://arxiv.org/abs/2506.04609)
...
<details>
  <summary>Details</summary>
Motivation: EBMs are difficult to train, and recent work linking them to GANs introduced instability. The authors aim to stabilize EBM training by using bidirectional bounds.

Method: The authors propose maximizing a lower bound and minimizing an upper bound for EBM training. They derive four bounds: two lower bounds (based on generator Jacobian singular values and mutual information) and two upper bounds (gradient penalty-like and diffusion process-based).

Result: Bidirectional bounds stabilize EBM training, leading to high-quality density estimation and sample generation.

Conclusion: Using bidirectional bounds is effective for stabilizing EBM training and improving performance, with trade-offs between the bounds explored.
...
### [88] [Composing Agents to Minimize Worst-case Risk](https://arxiv.org/abs/2506.04632)
...
<details>
  <summary>Details</summary>
Motivation: Modern agentic systems decompose tasks into subtasks handled by specialized AI agents, requiring risk-aware compositions to ensure safety, fairness, and privacy.

Method: The work defines worst-case risk as value-at-risk, proposes an efficient algorithm using union bounds and dynamic programming to approximate it, and proves asymptotic near-optimality for practical loss functions.

Result: The algorithm is evaluated on control benchmarks with reinforcement learning agents, showing effectiveness in approximating value-at-risk and identifying optimal compositions.

Conclusion: The framework successfully balances task success and risk minimization, providing a practical solution for deploying agentic systems in real-world scenarios.
...
### [89] [Inference economics of language models](https://arxiv.org/abs/2506.04645)
*Ege Erdil*

Main category: cs.LG

TL;DR: The paper presents a theoretical model optimizing the trade-off between cost per token and serial token generation speed for LLM inference, considering hardware constraints and parallelism setups.


<details>
  <summary>Details</summary>
Motivation: To address the economic and efficiency challenges of deploying large language models (LLMs) at scale by balancing cost and speed.

Method: Develops a theoretical model incorporating arithmetic, memory bandwidth, network bandwidth, and latency constraints, optimizing parallelism setups and batch sizes.

Result: Computes Pareto frontiers of serial speed versus cost per token for popular language models.

Conclusion: The model provides a framework to optimize LLM deployment by balancing cost and performance.
...
### [90] [Neural Network Reprogrammability: A Unified Theme on Model Reprogramming, Prompt Tuning, and Prompt Instruction](https://arxiv.org/abs/2506.04650)
...
<details>
  <summary>Details</summary>
Motivation: To bridge fragmented research areas like model reprogramming, prompt tuning, and prompt instruction by understanding their shared principle of manipulating model interfaces without altering parameters.

Method: Proposes a taxonomy for adaptation approaches based on four dimensions: manipulation format, location, operator, and output alignment.

Result: The framework consistently applies across data modalities and model architectures, revealing connections between techniques like in-context learning and chain-of-thought prompting.

Conclusion: Neural network reprogrammability is a fundamental paradigm for efficient model adaptation, with identified challenges and promising research directions.
...
### [91] [The Oversmoothing Fallacy: A Misguided Narrative in GNN Research](https://arxiv.org/abs/2506.04653)
*MoonJeong Park,Sunghyun Choi,Jaeseung Heo,Eunhyeok Park,Dongwoo Kim*

Main category: cs.LG

TL;DR: The paper challenges the overstated impact of oversmoothing in deep GNNs, attributing performance issues to vanishing gradients from transformation and activation, not aggregation. It shows deep GNNs can work with classical solutions like skip connections.


<details>
  <summary>Details</summary>
Motivation: To correct misconceptions about oversmoothing in GNNs and advocate for deeper architectures by clarifying the root causes of performance issues.

Method: Analyzes GNN core operations (aggregation, transformation, activation) and tests classical solutions (skip connections, normalization) in deep GNNs.

Result: Demonstrates that oversmoothing is conflated with vanishing gradients, and deep GNNs can perform well with proper techniques.

Conclusion: Oversmoothing's impact is overstated; deep GNNs are viable with classical solutions, opening new research directions.
...
### [92] [Noise-Resistant Label Reconstruction Feature Selection for Partial Multi-Label Learning](https://arxiv.org/abs/2506.04669)
*Wanfu Gao,Hanlin Pan,Qingqi Han,Kunpeng Liu*

Main category: cs.LG

TL;DR: A new Partial Multi-label Learning (PML) feature selection method addresses the 'Curse of dimensionality' by leveraging label noise-resistance and connectivity, improving label disambiguation and feature selection.


<details>
  <summary>Details</summary>
Motivation: Existing PML methods struggle with high-dimensional data, low-rank assumptions, and poor positive label identification, necessitating a more robust approach.

Method: The proposed method uses label noise-resistance for disambiguation, a reformed low-rank assumption for learning, and label connectivity for feature selection.

Result: Experiments on benchmark datasets show the method outperforms existing approaches.

Conclusion: The method effectively mitigates dimensionality issues and enhances label identification in PML.
...
### [93] [FedAPM: Federated Learning via ADMM with Partial Model Personalization](https://arxiv.org/abs/2506.04672)
*Shengkun Zhu,Feiteng Nie,Jinshan Zeng,Sheng Wang,Yuan Sun,Yuan Yao,Shangfeng Chen,Quanqing Xu,Chuanhui Yang*

Main category: cs.LG

TL;DR: FedAPM, an FL framework using ADMM, mitigates client drift by combining first- and second-order proximal terms, achieving stable convergence and outperforming SOTA methods in accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: In FL, non-i.i.d. data and multimodal inputs make single-model approaches impractical, and partial personalization worsens client drift. FedAPM addresses these issues.

Method: FedAPM uses ADMM with augmented Lagrangian, incorporating first- and second-order proximal terms for correction between local and shared models.

Result: FedAPM achieves faster, more accurate convergence, with 12.3% higher test accuracy, 16.4% better F1 score, and 18.0% higher AUC than SOTA methods, using fewer communication rounds.

Conclusion: FedAPM effectively mitigates client drift, improves FL performance, and demonstrates global convergence under mild assumptions.
...
### [94] [The cost of ensembling: is it always worth combining?](https://arxiv.org/abs/2506.04677)
*Marco Zanotti*

Main category: cs.LG

TL;DR: Ensembles improve forecasting accuracy but increase computational costs. Reducing retraining frequency lowers costs with minimal accuracy loss. Small ensembles (2-3 models) often suffice for near-optimal results.


<details>
  <summary>Details</summary>
Motivation: Address the trade-off between forecast accuracy and computational cost in ensemble learning for time series forecasting.

Method: Evaluated ten base models and eight ensemble configurations on M5 and VN1 datasets, considering point and probabilistic accuracy under varying retraining frequencies.

Result: Ensembles boost performance, especially probabilistically, but at high computational cost. Efficiency-driven ensembles balance accuracy and cost.

Conclusion: Careful ensemble design and retraining strategies enable accurate, robust, and cost-effective forecasts, supporting sustainable AI in forecasting.
...
### [95] [Urania: Differentially Private Insights into AI Use](https://arxiv.org/abs/2506.04681)
...
TL;DR: $Urania$ is a privacy-focused framework for analyzing LLM chatbot interactions using differential privacy (DP) techniques, ensuring meaningful insights while protecting user data.


<details>
  <summary>Details</summary>
Motivation: To address the need for extracting conversational insights from LLM chatbots without compromising user privacy, ensuring rigorous DP guarantees.

Method: Employs private clustering, keyword extraction (frequency-based, TF-IDF-based, LLM-guided), and DP tools like clustering, partition selection, and histogram-based summarization.
...
Conclusion: $Urania$ successfully balances data utility and privacy, offering a robust solution for private LLM interaction analysis.
...
### [96] [Towards Better Generalization via Distributional Input Projection Network](https://arxiv.org/abs/2506.04690)
*Yifan Hao,Yanxin Lu,Xinwei Shen,Tong Zhang*

Main category: cs.LG

TL;DR: DIPNet introduces a framework to project inputs into learnable distributions, smoothing the loss landscape and improving generalization in neural networks.


<details>
  <summary>Details</summary>
Motivation: Training loss alone is insufficient for assessing generalization in overparameterized models, and enforcing smoothness directly is challenging.

Method: DIPNet projects inputs into learnable distributions at each layer, inducing a smoother loss landscape and reducing local smoothness measures and Lipschitz constants.

Result: Empirical validation shows improved test performance across architectures (ViTs, LLMs, ResNet, MLPs) and scenarios (standard, adversarial, out-of-distribution, reasoning).

Conclusion: DIPNet is a general, effective approach for enhancing generalization in modern deep learning, easily integrated into existing models.
...
### [97] [Influence Functions for Edge Edits in Non-Convex Graph Neural Networks](https://arxiv.org/abs/2506.04694)
*Jaeseung Heo,Kyeongheung Yun,Seokwon Yoon,MoonJeong Park,Jungseul Ok,Dongwoo Kim*

Main category: cs.LG

TL;DR: The paper introduces a proximal Bregman response function for GNNs to predict edge influence, relaxing convexity assumptions and addressing message propagation effects for deletions and insertions.


<details>
  <summary>Details</summary>
Motivation: To improve GNN interpretability and robustness by accurately predicting edge influence without retraining, addressing limitations of existing methods.

Method: Proposes a proximal Bregman response function tailored for GNNs, relaxing convexity and capturing message propagation effects for edge deletions and insertions.
...
Conclusion: The method enhances GNN interpretability and robustness by providing a principled way to predict edge influence, applicable to various GNN characteristics and tasks.
...
### [98] [On the Mechanism of Reasoning Pattern Selection in Reinforcement Learning for Language Models](https://arxiv.org/abs/2506.04695)
*Xingwu Chen,Tianle Li,Difan Zou*

Main category: cs.LG

TL;DR: RLVR optimizes reasoning pattern selection, favoring high-success-rate patterns while maintaining stability. Theoretical analysis confirms convergence, with faster results for stronger models and slower for weaker ones, mitigated by SFT.


<details>
  <summary>Details</summary>
Motivation: To understand how RL improves reasoning abilities in models, focusing on RLVR's mechanisms.
...
Result: RLVR selects high-success-rate reasoning patterns, with convergence speed dependent on initial model strength. SFT can mitigate slower optimization in weaker models.

Conclusion: RLVR enhances reasoning by optimizing pattern selection, with theoretical and empirical support, offering insights for improving LLM fine-tuning.
...
### [99] [Enhanced Drought Analysis in Bangladesh: A Machine Learning Approach for Severity Classification Using Satellite Data](https://arxiv.org/abs/2506.04696)
*Tonmoy Paul,Mrittika Devi Mati,Md. Mahmudul Islam*

Main category: cs.LG

TL;DR: A machine learning framework using satellite data is proposed to classify drought severity in Bangladesh, addressing gaps in traditional indices and regional ML applications.


<details>
  <summary>Details</summary>
Motivation: Drought in Bangladesh affects agriculture and food security, but traditional indices like SPI and PDSI miss key factors, and ML models lack comprehensive regional integration.

Method: The framework uses unsupervised algorithms (K-means, Bayesian Gaussian Mixture) for clustering and classification models (KNN, Random Forest, Decision Tree, Naive Bayes) with weather data (2012-2024).

Result: The model successfully classifies drought severity but reveals regional variability in vulnerabilities.
...
### [100] [Explicit Density Approximation for Neural Implicit Samplers Using a Bernstein-Based Convex Divergence](https://arxiv.org/abs/2506.04700)
*José Manuel de Frutos,Manuel A. Vázquez,Pablo M. Olmos,Joaquín Míguez*

Main category: cs.LG

TL;DR: The paper introduces dual-ISL, a likelihood-free objective for training implicit generative models, offering convexity and continuity advantages over classical divergences like KL or Wasserstein distances. It provides theoretical guarantees, empirical benefits, and extends to multivariate settings.


<details>
  <summary>Details</summary>
Motivation: To address limitations of classical divergences (e.g., lack of convexity or continuity) in training implicit generative models, the authors propose dual-ISL, a rank-based metric with better theoretical and practical properties.

Method: The method interchanges target and model distributions in the ISL framework, yielding a convex optimization problem. It analyzes the rank-based discrepancy $d_K$, proves its properties, and extends it to multivariate settings via random projections.

Result: Dual-ISL shows faster convergence, smoother training, better mode collapse prevention, and provides explicit density approximations compared to classical ISL and other methods.

Conclusion: Dual-ISL is a robust and effective tool for training implicit generative models, with theoretical and empirical advantages over existing approaches.
...
### [101] [UNO: Unlearning via Orthogonalization in Generative models](https://arxiv.org/abs/2506.04712)
*Pinak Mandal,Georg A. Gottwald*

Main category: cs.LG

TL;DR: The paper introduces fast unlearning algorithms for generative models to selectively remove data influence without full retraining, ensuring efficiency and model fidelity.


<details>
  <summary>Details</summary>
Motivation: Addressing the need to unlearn specific data in generative models due to privacy, legal, or harmful content concerns without costly retraining.

Method: Proposes unlearning algorithms based on loss gradient orthogonalization to efficiently remove undesired data influence.

Result: Demonstrates significantly faster unlearning times compared to predecessors like gradient surgery, while maintaining model quality.

Conclusion: The proposed algorithms effectively balance forgetting undesired data, preserving model performance, and minimizing training steps.
...
### [102] [Multi-Layer GRPO: Enhancing Reasoning and Self-Correction in Large Language Models](https://arxiv.org/abs/2506.04746)
...
Method: MGRPO uses two GRPO layers: the first generates an initial response, and the second corrects errors in it, forming a self-correction loop.

Result: MGRPO outperforms GRPO on mathematical reasoning benchmarks by improving reasoning and self-correction.

Conclusion: MGRPO's self-correction mechanism provides implicit supervision, boosting performance without dense reward annotations.
...
### [103] [Log-Linear Attention](https://arxiv.org/abs/2506.04761)
...
<details>
  <summary>Details</summary>
Motivation: Address the limitations of quadratic-compute in Transformers and fixed-size hidden states in linear attention and state-space models.

Method: Develops log-linear attention, a mechanism with logarithmically growing hidden states, and applies it to existing architectures like Mamba-2 and Gated DeltaNet.
...
Conclusion: Log-linear attention offers a scalable and efficient alternative to traditional attention mechanisms, enhancing sequence modeling.
...
### [104] [OpenGT: A Comprehensive Benchmark For Graph Transformers](https://arxiv.org/abs/2506.04765)
*Jiachen Tang,Zhonghao Wang,Sirui Chen,Sheng Zhou,Jiawei Chen,Jiajun Bu*

Main category: cs.LG

TL;DR: The paper introduces OpenGT, a benchmark for Graph Transformers (GTs), addressing the lack of standardized evaluation and understanding of GT performance and design choices.


<details>
  <summary>Details</summary>
Motivation: GTs show promise but lack comprehensive benchmarks to understand their effectiveness and optimal design choices compared to GNNs.

Method: OpenGT provides standardized settings, evaluates diverse tasks/datasets, and compares GTs and GNNs.

Result: Key insights include challenges in model transfer, limitations of local attention, and efficiency trade-offs.

Conclusion: OpenGT aims to advance GT research with fairness, reproducibility, and generalizability, supported by an open library.
...
### [105] [Improved Regret Bounds for Linear Bandits with Heavy-Tailed Rewards](https://arxiv.org/abs/2506.04775)
...
### [106] [Kernel $k$-Medoids as General Vector Quantization](https://arxiv.org/abs/2506.04786)
*Thore Gerlach,Sascha Mücke,Christian Bauckhage*

Main category: cs.LG

TL;DR: The paper explores the connection between $k$-medoids clustering and Kernel Density Estimation (KDE) in Vector Quantization (VQ) using Quadratic Unconstrained Binary Optimization (QUBO), showing that KDE-QUBO is a special case of $k$-medoids-QUBO.


<details>
  <summary>Details</summary>
Motivation: To uncover the relationship between distance-based ($k$-medoids) and probability-based (KDE) VQ methods, which appear unrelated.

Method: Uses QUBO formulations for $k$-medoids and KDE-based VQ, comparing their structures and assumptions.

Result: Demonstrates that KDE-QUBO is a special case of $k$-medoids-QUBO under mild kernel feature map assumptions.

Conclusion: Reveals a deeper structural link between $k$-medoids and KDE, offering new insights into QUBO parameters in VQ.
...
### [107] [Adaptive Preconditioners Trigger Loss Spikes in Adam](https://arxiv.org/abs/2506.04805)
*Zhiwei Bai,Zhangchen Zhou,Jiajie Zhao,Xiaolong Li,Zhiyu Li,Feiyu Xiong,Hongkang Yang,Yaoyu Zhang,Zhi-Qin John Xu*

Main category: cs.LG

TL;DR: The paper investigates the cause of loss spikes in neural networks trained with Adam, attributing them to Adam's adaptive preconditioners rather than loss landscape sharpness.


<details>
  <summary>Details</summary>
Motivation: Loss spikes during training with Adam are common but poorly understood, prompting a deeper analysis of their root cause.

Method: The study identifies a critical regime where squared gradients are much smaller than second-order moment estimates, leading to instability. Theoretical and experimental validation is performed across various architectures.
...
Conclusion: Adam's adaptive preconditioners can inherently trigger loss spikes, verified across multiple neural network architectures.
...
### [108] [LogicPuzzleRL: Cultivating Robust Mathematical Reasoning in LLMs via Reinforcement Learning](https://arxiv.org/abs/2506.04821)
*Zhen Hao Wong,Jingwen Deng,Runming He,Zirong Chen,Qijie You,Hejun Dong,Hao Liang,Chengyu Shen,Bin Cui,Wentao Zhang*

Main category: cs.LG

TL;DR: A 'play to learn' framework uses reinforcement learning on logic puzzles to improve LLMs' structured reasoning, showing gains in multi-step problem-solving and transferable skills.


<details>
  <summary>Details</summary>
Motivation: Standard fine-tuning may limit LLMs to narrow heuristics; this work aims to foster general-purpose reasoning through puzzle-based training.

Method: Fine-tuning LLMs via reinforcement learning on seven custom logic puzzles, with binary feedback for correctness to encourage iterative problem-solving.

Result: Significant improvement in out-of-distribution performance, especially for mid-difficulty problems requiring multi-step reasoning, and gains in algebraic, geometric, and combinatorial logic.

Conclusion: Reinforcement learning over logic puzzles enhances LLMs' compositional generalization and robust reasoning without task-specific tools.
...
### [109] [From EHRs to Patient Pathways: Scalable Modeling of Longitudinal Health Trajectories with LLMs](https://arxiv.org/abs/2506.04831)
*Chantal Pellegrini,Ege Özsoy,David Bani-Harouni,Matthias Keicher,Nassir Navab*

Main category: cs.LG

TL;DR: The paper introduces EHR2Path, a novel method for modeling patient pathways by transforming EHR data into structured representations, improving prediction of future health trajectories with a token-efficient summary mechanism.


<details>
  <summary>Details</summary>
Motivation: Healthcare systems struggle with managing diverse patient data for personalized care, as existing methods are limited in scope and overlook longitudinal interactions.

Method: EHR2Path transforms EHR data into structured representations and uses a summary mechanism to embed long-term temporal context, optimizing for pathway prediction.

Result: EHR2Path outperforms baselines in next time-step prediction and longitudinal simulation, enabling detailed patient trajectory forecasts.

Conclusion: The approach advances predictive and personalized healthcare by efficiently modeling patient pathways and supporting diverse evaluation tasks.
...
### [110] [Sparse Autoencoders, Again?](https://arxiv.org/abs/2506.04859)
*Yin Lu,Tong He,Xuening Zhu,David Wipf*

Main category: cs.LG

TL;DR: The paper critiques traditional sparse autoencoders (SAEs) and variational autoencoders (VAEs), proposing a hybrid model that outperforms both in tasks like manifold dimension estimation and sparse representation without sacrificing reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: Despite the broad applicability of SAEs and VAEs, their limitations in handling structured data and sparse representations remain underaddressed. The paper aims to bridge these gaps.

Method: The authors introduce a hybrid model combining strengths of SAEs and VAEs, supported by theoretical proofs of its ability to recover structured data. Empirical tests on synthetic and real-world datasets validate its efficacy.

Result: The proposed model outperforms SAEs, VAEs, and diffusion models in tasks like manifold dimension estimation and sparse representation, while maintaining low reconstruction error.

Conclusion: The hybrid model addresses key weaknesses of traditional SAEs and VAEs, offering improved performance in diverse applications like image and language model activation analysis.
...
### [111] [Aligning Multimodal Representations through an Information Bottleneck](https://arxiv.org/abs/2506.04870)
*Antonio Almudévar,José Miguel Hernández-Lobato,Sameer Khurana,Ricard Marxer,Alfonso Ortega*

Main category: cs.LG

TL;DR: Contrastive losses in multimodal learning often fail to align representation spaces due to modality-specific information. The paper proposes a regularization term to improve alignment, supported by theory and experiments.


<details>
  <summary>Details</summary>
Motivation: Contrastive losses are ineffective for aligned representation learning because they don't remove modality-specific information, hindering multimodal alignment.

Method: Theoretical analysis via the Information Bottleneck Principle, empirical hyperparameter study, and introduction of a variational regularization term to enhance alignment.

Result: The proposed regularization term improves representational alignment in controlled experiments and real-world applications.

Conclusion: Addressing modality-specific information with a regularization term enhances the effectiveness of contrastive losses for multimodal representation learning.
...
### [112] [There Was Never a Bottleneck in Concept Bottleneck Models](https://arxiv.org/abs/2506.04877)
*Antonio Almudévar,José Miguel Hernández-Lobato,Alfonso Ortega*

Main category: cs.LG

TL;DR: The paper introduces Minimal Concept Bottleneck Models (MCBMs) to address the lack of a true bottleneck in Concept Bottleneck Models (CBMs), ensuring interpretability and valid interventions by using an Information Bottleneck objective.


<details>
  <summary>Details</summary>
Motivation: CBMs lack a true bottleneck, allowing components to encode extraneous information, which undermines interpretability and intervention validity.

Method: Proposes MCBMs with an Information Bottleneck (IB) objective via variational regularization to constrain representations to concept-relevant information.

Result: MCBMs enable concept-level interventions with theoretical guarantees, align with Bayesian principles, and offer design flexibility.

Conclusion: MCBMs improve interpretability and intervention validity over CBMs by enforcing a true bottleneck through IB regularization.
...
### [113] [Gaussian Process Diffeomorphic Statistical Shape Modelling Outperforms Angle-Based Methods for Assessment of Hip Dysplasia](https://arxiv.org/abs/2506.04886)
*Allen Paul,George Grammatopoulos,Adwaye Rambojun,Neill D. F. Campbell,Harinderjit S. Gill,Tony Shardlow*

Main category: cs.LG

TL;DR: The paper introduces GPDSSM, a semi-automated pipeline for classifying hip dysplasia using CT scans, outperforming angle-based methods with 96.2% AUC.


<details>
  <summary>Details</summary>
Motivation: Early diagnosis of hip dysplasia is crucial to prevent osteoarthritis, but current methods like angle-based measurements are time-consuming and less accurate.

Method: The GPDSSM combines Gaussian Process Latent Variable Model with diffeomorphism to create a statistical shape model, trained on 100 CT scans and tested on 92.

Result: GPDSSM achieves 96.2% AUC in classification, outperforming angle-based methods (91.2%), and identifies dysplastic variations in hip surfaces.

Conclusion: GPDSSM improves accuracy and efficiency in dysplasia diagnosis, reducing reliance on manual measurements and 2D scans.
...
### [114] [Dissecting Long Reasoning Models: An Empirical Study](https://arxiv.org/abs/2506.04913)
*Yongyu Mu,Jiali Zeng,Bei Li,Xinyan Guan,Fandong Meng,Jie Zhou,Tong Xiao,Jingbo Zhu*

Main category: cs.LG

TL;DR: The paper analyzes RL training for long-context reasoning, highlighting the roles of positive/negative samples, data inefficiency in policy optimization, and performance instability due to ambiguous problems.


<details>
  <summary>Details</summary>
Motivation: To address open questions and counterintuitive behaviors in RL training for long-context reasoning, focusing on sample roles, data efficiency, and performance stability.

Method: Systematic analysis of sample roles, strategies to improve data efficiency (e.g., relative length rewards, offline sample injection), and multiple evaluation runs to address instability.

Result: Negative samples enhance generalization; training on them alone rivals standard RL. Data inefficiency identified, mitigated by proposed strategies. Performance instability linked to ambiguous problems, reduced by multiple evaluations.

Conclusion: The study provides insights into RL training dynamics, offering practical solutions for data efficiency and stability in long-context reasoning models.
...
### [115] [Predicting ICU In-Hospital Mortality Using Adaptive Transformer Layer Fusion](https://arxiv.org/abs/2506.04924)
...
TL;DR: ALFIA is a modular, attention-based architecture for early identification of high-risk ICU patients, outperforming state-of-the-art methods by fusing multi-layer BERT features and enabling integration with other models for enhanced performance.


<details>
  <summary>Details</summary>
Motivation: Early identification of high-risk ICU patients is critical for efficient resource allocation and timely intervention.

Method: ALFIA combines LoRA adapters and adaptive layer-weighting to fuse multi-layer BERT features, trained on the cw-24 benchmark. It integrates with GBDTs and deep networks for further improvements.

Result: ALFIA achieves superior AUPRC and maintains a balanced precision-recall profile, outperforming existing tabular classifiers.

Conclusion: ALFIA provides a robust tool for risk stratification in critical-care settings by leveraging clinical text, enhancing early-warning capabilities.
...
### [116] [Agentic AI for Intent-Based Industrial Automation](https://arxiv.org/abs/2506.04980)
*Marcos Lima Romero,Ricardo Suyama*

Main category: cs.LG

TL;DR: The paper proposes a framework integrating Agentic AI with intent-based paradigms to simplify human-machine interaction in Industry 5.0, demonstrated via predictive maintenance.


<details>
  <summary>Details</summary>
Motivation: To reduce complexity in Industry 4.0 and align automation with human-centric, sustainable, and resilient principles of Industry 5.0.

Method: A conceptual framework combining Agentic AI and intent-based processing, tested using CMAPSS dataset and Google ADK.

Result: Feasibility shown for intent decomposition, agent orchestration, and autonomous decision-making, though data quality and explainability remain challenges.
...
### [117] [FPTQuant: Function-Preserving Transforms for LLM Quantization](https://arxiv.org/abs/2506.04985)
*Boris van Breugel,Yelysei Bondarenko,Paul Whatmough,Markus Nagel*

Main category: cs.LG

TL;DR: FPTQuant introduces lightweight function-preserving transforms (FPTs) to enable efficient INT4 quantization of LLMs without performance degradation, achieving up to 3.9x speed-up over FP.


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) consume significant energy during inference, and naive quantization degrades performance due to outliers. FPTQuant aims to maintain model function while improving quantization efficiency.

Method: FPTQuant uses four novel FPTs: pre-RoPE transform for queries/keys, value transform, MLP scaling transform, and dynamic scaling. These leverage transformer equivariances to shape activations for quantization without custom kernels.

Result: FPTQuant achieves INT4 quantization with minimal overhead, outperforming prior work in speed (3.9x faster than FP) while maintaining accuracy, trading only slight accuracy for 29% slower methods.

Conclusion: FPTQuant provides an efficient, accurate solution for LLM quantization, balancing speed and performance with lightweight transforms.
...
### [118] [Cautious Optimism: A Meta-Algorithm for Near-Constant Regret in General Games](https://arxiv.org/abs/2506.05005)
*Ashkan Soleymani,Georgios Piliouras,Gabriele Farina*

Main category: cs.LG

TL;DR: The paper introduces 'Cautious Optimism,' a framework accelerating no-regret learning in general games by adaptively pacing FTRL algorithms, achieving near-optimal regret guarantees without relying on monotonic step-sizes.


<details>
  <summary>Details</summary>
Motivation: To show that adaptive pacing for no-regret learning acceleration is not isolated and can be generalized, improving upon prior methods that depend on monotonic step-sizes.

Method: Proposes 'Cautious Optimism,' which takes any FTRL instance and accelerates it with minimal overhead, retaining uncoupledness (no knowledge of others' utilities).

Result: Achieves near-optimal O_T(log T) regret in self-play and maintains O(sqrt(T)) regret in adversarial settings.

Conclusion: Demonstrates a novel, efficient approach for fast learning in general games without monotonic step-size constraints.
...
### [119] [Towards Reasonable Concept Bottleneck Models](https://arxiv.org/abs/2506.05014)
*Nektarios Kalampalikis,Kavya Gupta,Georgi Vitanov,Isabel Valera*

Main category: cs.LG

TL;DR: CREAM is a new type of Concept Bottleneck Model that explicitly encodes concept-concept and concept-task relationships, using a regularized side-channel for performance while maintaining concept importance.


<details>
  <summary>Details</summary>
Motivation: To improve model reasoning by embedding expert-specified relationships and handling undesired information flows, while balancing task performance and concept reliance.

Method: CREAM architecturally embeds (bi)directed concept-concept and concept-task relationships, uses a regularized side-channel, and severs undesired information flows.

Result: CREAM achieves task performance comparable to black-box models while relying mainly on concepts, and its embedded relationships ease interventions and reduce concept leakage.

Conclusion: CREAM effectively balances expert-guided reasoning and task performance, offering a robust framework for interpretable models.
...
### [120] [Multi-Point Proximity Encoding For Vector-Mode Geospatial Machine Learning](https://arxiv.org/abs/2506.05016)
*John Collins*

Main category: cs.LG

TL;DR: MPP encoding converts geospatial shapes into vectors using scaled distances to reference points, outperforming rasterization in capturing geometric features and spatial relationships.


<details>
  <summary>Details</summary>
Motivation: To enable machine learning models to use vector-mode geospatial data (points, lines, polygons) by encoding shapes into vectors that retain essential geometric properties.

Method: MultiPoint Proximity (MPP) encoding, which measures scaled distances from a shape to reference points within a region of interest.

Result: MPP encoding is shape-centric, continuous, differentiates spatial objects, and captures pairwise spatial relationships more precisely than rasterization.

Conclusion: MPP encoding is a superior method for representing geospatial shapes in machine learning, outperforming rasterization in accuracy and versatility.
...
### [121] [Tuning the Right Foundation Models is What you Need for Partial Label Learning](https://arxiv.org/abs/2506.05027)
*Kuang He,Wei Tang,Tong Wei,Min-Ling Zhang*

Main category: cs.LG

TL;DR: The paper evaluates foundation models in partial label learning (PLL), proposes PartialCLIP for efficient fine-tuning, and highlights performance trends and limitations of current PLL approaches.


<details>
  <summary>Details</summary>
Motivation: To address the lack of focus on foundation models in PLL and improve generalizability of classifiers trained with inexact supervision.

Method: Empirical evaluation of 11 foundation models across 13 PLL approaches on 8 datasets under 3 scenarios, plus the introduction of PartialCLIP for fine-tuning.

Result: Foundation models significantly boost PLL performance, approaches show similar results, stability across ambiguity levels, but are sensitive to model selection.

Conclusion: Current PLL approaches have limitations; text-embedding initialization and label filtering with CLIP are effective. Insights provided for future generalizable PLL models.
...
### [122] [Identifying and Understanding Cross-Class Features in Adversarial Training](https://arxiv.org/abs/2506.05032)
*Zeming Wei,Yiwen Guo,Yisen Wang*

Main category: cs.LG

TL;DR: The paper studies adversarial training (AT) through class-wise feature attribution, identifying cross-class features as key for robust classification. It shows AT initially learns these features, then shifts to class-specific features, linking this to robust overfitting and soft-label training advantages.
...
Method: The study uses theoretical evidence from a synthetic data model and systematic experiments across various model architectures and settings to analyze feature learning in AT.

Result: AT initially learns cross-class features for robustness, then shifts to class-specific features, causing robust overfitting. This provides insights into soft-label training advantages and robust overfitting.

Conclusion: The findings refine understanding of AT mechanisms, offering new perspectives for future research. Code is available for reproducibility.
...
TL;DR: The paper introduces TIMING, a method to enhance Integrated Gradients (IG) for time series XAI by incorporating temporal awareness, outperforming existing baselines.


<details>
  <summary>Details</summary>
Motivation: Current XAI methods for time series overlook directional impact and fail to evaluate it properly, leading to suboptimal identification of significant points.

Method: Proposes TIMING, an enhancement of IG for time series, incorporating temporal awareness to avoid out-of-distribution samples and maintain theoretical properties.

Result: TIMING outperforms existing time series XAI baselines under novel metrics (CPD and CPP).

Conclusion: TIMING effectively addresses limitations of conventional IG in time series XAI, improving attribution accuracy.
...
TL;DR: iN2V extends node2vec to inductive settings, enabling embeddings for unseen nodes and improving node classification by 1-6 points.


<details>
  <summary>Details</summary>
Motivation: Overcome the limitation of transductive node embedding methods like node2vec, which require all nodes during training.

Method: Combines post-hoc embedding computation for unseen nodes with modified N2V training.

Result: Improves node classification by 1 point on average (up to 6 points).
...
TL;DR: D3M monitors post-deployment model deterioration by tracking predictive model disagreements, ensuring low false positives and high true positives under shifts.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of detecting when to retrain models in dynamic environments without label access, focusing on shifts that degrade performance.

Method: Proposes D3M, a monitoring algorithm based on predictive model disagreement, with sample complexity bounds for performance guarantees.

Result: Effective on benchmark and real-world datasets, proving viable as an alert mechanism for high-stakes ML pipelines.

Conclusion: D3M provides a practical solution for PDD monitoring, balancing false positives and true positives under data shifts.
...
TL;DR: NIMO combines neural networks and linear models to maintain interpretability while improving predictive accuracy.


<details>
  <summary>Details</summary>
Motivation: Neural networks are powerful but hard to interpret, while linear models are interpretable but less expressive. NIMO aims to merge their strengths.

Method: NIMO uses a neural network to learn nonlinear corrections to linear predictions, with an optimization algorithm based on profile likelihood and adaptive ridge regression for sparsity.

Result: NIMO recovers interpretable linear coefficients and outperforms other hybrid models in predictive accuracy.

Conclusion: NIMO successfully balances interpretability and performance, making it a practical choice for regression and classification tasks.
...
TL;DR: Extends HiPPO theory to handle measurement noise, providing an uncertainty-aware initialization for state space models without runtime overhead.


<details>
  <summary>Details</summary>
Motivation: HiPPO assumes noise-free data, which is often unrealistic. This work addresses this limitation by incorporating noise into the framework.

Method: Reinterprets HiPPO as a linear stochastic control problem, reformulates it to handle noisy data, and derives a new dynamics initialization.

Result: The proposed initialization improves noise resistance in state-space models during training and inference.

Conclusion: The work successfully extends HiPPO to noisy settings, enhancing robustness without computational cost.
...
TL;DR: Proposes a kernelized KL divergence estimator with importance sampling correction for stable and efficient semi-implicit variational inference, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Addressing high variance and bias in KL divergence training for high-dimensional semi-implicit variational inference.

Method: Introduces a kernelized KL divergence estimator with nonparametric smoothing and importance sampling correction, connecting theoretically to Stein variational gradient descent.

Result: Empirical results show superior performance and training efficiency compared to existing SIVI methods.

Conclusion: The method provides stable, efficient, and high-performing semi-implicit variational inference with reduced bias and variance.
...
### [129] [Privacy Amplification Through Synthetic Data: Insights from Linear Regression](https://arxiv.org/abs/2506.05101)
*Clément Pierquin,Aurélien Bellet,Marc Tommasi,Matthieu Boussard*

Main category: cs.LG

TL;DR: The paper explores privacy amplification in synthetic data generation, showing risks when the model's seed is exposed but benefits when using random inputs.
...
Method: Investigates through linear regression, analyzing scenarios where the generative model's seed is controlled by an adversary versus using random inputs.

Result: Negative results if the seed is exposed (leaks as much as the model), but privacy amplifies with random inputs.

Conclusion: Linear regression findings can guide future general privacy bounds for synthetic data.
...
TL;DR: The paper introduces an Isolation Forest-based federated anomaly detection system (FLiForest) for edge/IoT devices, achieving high accuracy (96%) and low memory usage (<160 KB).


<details>
  <summary>Details</summary>
Motivation: To address privacy and efficiency in embedded systems by leveraging federated learning frameworks for anomaly detection in resource-constrained environments.

Method: Developed FLiForest, an unsupervised anomaly detection algorithm based on Isolation Forests, implemented using federated learning frameworks (Python and MicroPython).

Result: Achieved 96% accuracy in anomaly detection and 78% precision, with memory usage under 160 KB during training.

Conclusion: The system is effective for edge/IoT deployments, balancing privacy, efficiency, and low resource consumption.
...
TL;DR: The paper explores links between generative diffusion models and continuous-state associative memory, using Morse-Smale systems to approximate and analyze their universal properties and stability.


<details>
  <summary>Details</summary>
Motivation: To understand the connections between generative diffusion models and associative memory models, and to characterize transitions between generation and memory as noise levels change.

Method: Uses Morse-Smale dynamical systems as universal approximators for gradient-based associative memory and diffusion models, analyzing structural stability and bifurcations.
...
<details>
  <summary>Details</summary>
Motivation: Existing RLVR methods lack fine-grained reward signals for intermediate reasoning steps, limiting LLM optimization.

Method: \name estimates step-level rewards via tree sampling and uses group-relative rewards from GRPO for training.
...
TL;DR: The paper introduces LPMT, a framework combining diffusion-based re-ranking with locality preservation to improve instance retrieval by maintaining local consistency and enabling global similarity propagation.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based re-ranking methods lose discriminative power over distance due to diminishing positive signals. LPMT aims to address this by preserving locality while measuring manifold distances accurately.

Method: LPMT uses Bidirectional Collaborative Diffusion (BCD) to integrate diffusion processes across graphs, Locality State Embedding (LSE) for local consistency, and Thermodynamic Markovian Transition (TMT) for global retrieval.

Result: Experiments show LPMT effectively improves instance retrieval across diverse tasks.

Conclusion: LPMT successfully balances local consistency and global similarity propagation, enhancing retrieval performance.
...
<details>
  <summary>Details</summary>
Motivation: To understand how transformers enable in-context learning and provide guarantees beyond optimization-based methods.

Method: Constructs transformers capable of universal function approximation for any task class, using only in-context examples.

Result: Demonstrates reliable prediction without weight updates, applicable to non-convex and non-linear problems.
...
TL;DR: The paper introduces HAR contrastive loss and SHARP framework to address degree bias in GNNs by adding more positive pairs and adaptive weighting, improving performance across datasets.


<details>
  <summary>Details</summary>
Motivation: GNNs exhibit degree bias in node classification, and existing GCL methods fail to adequately address low-degree nodes due to limited positive pairs and noisy information.

Method: Proposes HAR contrastive loss, which increases positive pairs using node labels and adaptively weights pairs based on learning hardness, alongside the SHARP framework for broader application.

Result: SHARP outperforms baselines on four datasets, showing improved performance globally and across degree levels.
...
<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing distributed robust kernel learning schemes by introducing a decentralized framework that enhances robustness and convergence.

Method: Utilizes a networked system represented as a graph, employs a robust loss function with a windowing function and scaling parameter, and derives convergence bounds using kernel-based integral operator techniques.

Result: Local robust estimators approximate the regression function with high confidence bounds in mean square distance, RKHS norm, and generalization error. Optimal learning rates are achieved with proper parameter selection.

Conclusion: The algorithm's robustness and convergence are enhanced by the scaling parameter, providing a unified decentralized learning framework with theoretical guarantees.
...
TL;DR: The paper presents a unified framework for Shapley value estimators, including KernelSHAP, and provides the first theoretical guarantees for KernelSHAP. It validates the approach on datasets and improves scalability for high-dimensional data.


<details>
  <summary>Details</summary>
Motivation: Exact Shapley value computation is computationally expensive, and existing randomized estimators lack theoretical guarantees for KernelSHAP. The paper aims to address this gap.

Method: The authors develop a broad framework for Shapley value estimators, including KernelSHAP, and prove non-asymptotic theoretical guarantees. They benchmark the approach on Decision-Tree models and improve scalability for high-dimensional datasets.

Result: The framework achieves low mean squared error with modest sample sizes and outperforms the KernelSHAP library on datasets like MNIST and CIFAR10.

Conclusion: The paper provides the first theoretical guarantees for KernelSHAP and offers a scalable, unified framework for Shapley value estimation, improving accuracy and efficiency.
...
TL;DR: Diagonal Batching improves RMTs by enabling parallelism across segments, reducing inference time and cost for long-context tasks without retraining.


<details>
  <summary>Details</summary>
Motivation: Transformer models face inefficiencies in long-context inference due to quadratic time and linear memory complexity. RMTs address this but introduce sequential bottlenecks.

Method: Diagonal Batching is introduced as a scheduling scheme to parallelize RMT execution while maintaining exact recurrence, avoiding sequential constraints.
...
Conclusion: Diagonal Batching makes RMTs more practical for real-world long-context applications by reducing latency and inference costs.
...
TL;DR: PTSD combines diffusion models and MCMC to improve neural samplers, outperforming diffusion-based methods in efficiency.


<details>
  <summary>Details</summary>
Motivation: Current neural samplers lag behind MCMC methods like Parallel Tempering in efficiency, and PT has limitations like dependent samples and high computational costs for new samples.

Method: PTSD trains diffusion models sequentially across temperatures, using PT's advantages. It combines high-temperature models to generate approximate samples, refined with MCMC for training the next model.

Result: PTSD generates well-mixed, uncorrelated samples efficiently, improving target evaluation efficiency over diffusion-based neural samplers.

Conclusion: PTSD effectively addresses weaknesses of both neural samplers and PT, offering a more efficient and reusable sampling method.
...
TL;DR: The paper introduces a stable, parallelizable version of the Mesa layer, derived from an in-context regression objective, and demonstrates its superior performance in language modeling, especially for long-context tasks, at the cost of increased inference compute.


<details>
  <summary>Details</summary>
Motivation: Transformers dominate sequence modeling but have linear memory and compute scaling. Recent RNN models like DeltaNet and Mamba offer constant costs. This work aims to unify these models under an in-context regression objective and improve performance.

Method: The paper introduces a numerically stable, chunkwise parallelizable Mesa layer, optimized using a fast conjugate gradient solver for in-context loss minimization at every time point.

Result: The proposed Mesa layer achieves lower perplexity and better downstream performance than previous RNNs, particularly in long-context tasks, though it requires additional inference compute.

Conclusion: Optimal test-time training via sequential optimization within the network improves performance, aligning with trends of increasing test-time compute for better results.
...
TL;DR: The paper evaluates sparse autoencoders (SAEs) in a controlled MNIST setting, identifies limitations due to quasi-orthogonality assumptions, and proposes a multi-iteration SAE (MP-SAE) to extract correlated features effectively.


<details>
  <summary>Details</summary>
Motivation: Current shallow SAE architectures struggle with extracting correlated features due to quasi-orthogonality assumptions, limiting interpretability in hierarchical settings like handwritten digit generation.

Method: The authors introduce a multi-iteration SAE by unrolling Matching Pursuit (MP-SAE), which allows residual-guided extraction of correlated features and guarantees monotonic reconstruction improvement.

Result: MP-SAE successfully extracts correlated features in hierarchical settings, overcoming the limitations of traditional SAEs.

Conclusion: The proposed MP-SAE advances SAE capabilities for interpretability by addressing the quasi-orthogonality constraint and improving feature extraction in complex scenarios.
...
TL;DR: The paper analyzes the convergence behavior of a single-layer Transformer with self-attention, feedforward networks, and residual connections, showing linear convergence under proper initialization and highlighting the role of residual connections in stability.


<details>
  <summary>Details</summary>
Motivation: Despite the empirical success of Transformers, their theoretical foundations, especially training dynamics and component interdependencies, are underdeveloped.

Method: The study examines a structurally complete single-layer Transformer, analyzing gradient descent convergence and the impact of residual connections.

Result: Gradient descent exhibits linear convergence, influenced by the attention layer's output matrix singular values. Residual connections improve optimization stability.

Conclusion: The findings extend to multi-layer Transformers, with empirical results supporting the theoretical insights on residual connections' benefits.
...
TL;DR: The paper explores learnability in strategic classification, focusing on genuine agent improvements, and provides new characterizations and positive results for proper and improper learning in various settings.


<details>
  <summary>Details</summary>
Motivation: Machine learning is widely used in societal decision-making, and understanding how agents react to learning algorithms is crucial. While most research focuses on deceptive behaviors, this paper investigates learnability when agents genuinely improve.

Method: The study introduces an asymmetric variant of minimally consistent concept classes, examines Euclidean ball improvement sets, and addresses learning under bounded noise models and online settings.

Result: The paper characterizes proper learning in the realizable setting, provides positive results for natural improvement regions, and resolves open questions from prior work.

Conclusion: The findings advance understanding of learnability with improvements, offering practical insights for strategic classification in real-world applications.
...
TL;DR: The paper introduces Recurrent Hamiltonian Echo Learning (RHEL), a physics-grounded algorithm for training deep state space models (SSMs) efficiently, matching BPTT performance without explicit Jacobian computation.


<details>
  <summary>Details</summary>
Motivation: To develop learning algorithms for SSMs that adhere to physical principles, enabling efficient simulation and scalable design.

Method: Proposes RHEL, which computes gradients as finite differences of Hamiltonian system trajectories, requiring only three forward passes. Introduces Hamiltonian Recurrent Units (HRUs) and extends to hierarchical Hamiltonian SSMs (HSSMs).

Result: RHEL matches BPTT performance on time-series tasks with sequences up to ~50k, demonstrating scalability and efficiency.

Conclusion: RHEL enables scalable, energy-efficient physical systems with self-learning capabilities for sequence modeling, bridging physics and ML.
...
TL;DR: The paper provides a tight analysis of error feedback schemes (EF and EF²¹) in distributed learning, comparing their convergence rates and performance against compressed gradient descent.


<details>
  <summary>Details</summary>
Motivation: Communication bottlenecks in distributed learning due to compressed information exchange degrade convergence, prompting the need for precise analysis of error feedback methods.

Method: The study uses Lyapunov functions to derive optimal convergence rates for EF and EF²¹, with matching lower bounds, in a simplified theoretical setting.

Result: The analysis yields sharp performance guarantees and enables a rigorous comparison between EF, EF²¹, and compressed gradient descent.

Conclusion: The principled approach clarifies the effectiveness of error feedback schemes and provides a framework for fair comparison in distributed learning.
...
TL;DR: The paper introduces CocktailEdit, a framework for flexible and precise time series editing, combining point-level and segment-level controls while maintaining temporal coherence.


<details>
  <summary>Details</summary>
Motivation: Current methods struggle with simultaneously controlling point-level and segment-level properties in time series generation, limiting practical applications.

Method: CocktailEdit uses confidence-weighted anchor control for point-wise constraints and classifier-based control for segment-level statistical properties, integrating with diffusion-based models.

Result: The framework achieves precise local control and temporal coherence, validated across diverse datasets and models.

Conclusion: CocktailEdit bridges generative modeling and real-world editing needs, offering a flexible solution for human-in-the-loop time series tasks.
...
TL;DR: Fast-DataShapley is a one-pass training method for efficiently computing Shapley values of training data without retraining, improving performance and speed.


<details>
  <summary>Details</summary>
Motivation: The need to fairly reward data providers and protect their rights, coupled with the computational inefficiency of existing Shapley value methods.

Method: Leverages weighted least squares to train a reusable explainer model, with three methods to reduce training overhead.

Result: Performance improved by 2.5x, and training speed increased by two orders of magnitude.

Conclusion: Fast-DataShapley offers an efficient, scalable solution for evaluating data contributions in AI.
...
TL;DR: The paper introduces SAILOR, a method for imitation learning that improves upon behavioral cloning (BC) by enabling agents to recover from mistakes using learned world and reward models.


<details>
  <summary>Details</summary>
Motivation: BC's limitation is its inability to recover from mistakes outside the expert's demonstrated states. The goal is to teach agents to plan and recover independently.

Method: The approach, SAILOR, learns a world model and a reward model from expert demonstrations, enabling planning for recovery without additional human corrections.

Result: SAILOR outperforms state-of-the-art BC methods across multiple tasks, even with 5-10x more BC demonstrations, and shows robustness to reward hacking.

Conclusion: SAILOR effectively addresses BC's recovery limitations, demonstrating superior performance and robustness in imitation learning tasks.
...
TL;DR: The paper analyzes the sample efficiency of test-time strategies for LLMs, showing self-consistency requires more samples than best-of-$n$, and proves self-correction with verifier feedback enables Transformers to handle multi-task settings.


<details>
  <summary>Details</summary>
Motivation: To theoretically understand the sample efficiency of test-time strategies like self-consistency, best-of-$n$, and self-correction, and extend Transformer capabilities to multi-task settings.

Method: Establishes separation results for sample efficiency, presents expressiveness results for self-correction with verifier feedback, and empirically validates findings.

Result: Self-consistency needs Θ(1/Δ²) samples, best-of-$n$ needs Θ(1/Δ); self-correction enables multi-task learning without prior task knowledge.

Conclusion: Theoretical and empirical results highlight the practical effectiveness of self-correction methods and extend Transformer representation theory to multi-task settings.
...
TL;DR: SiftAttention introduces a threshold-based filtering method for approximate attention, reducing GPU memory bandwidth usage while maintaining model quality.


<details>
  <summary>Details</summary>
Motivation: Addressing inefficiencies in GPU inference due to memory bandwidth limitations during attention computations, particularly with top-$k$ operations.

Method: Replaces top-$k$ operations with a threshold-based filtering step, dynamically estimating thresholds per prompt to reduce data movement.

Result: Outperforms existing methods in preserving model quality and reducing memory bandwidth usage.

Conclusion: SiftAttention offers an efficient alternative to traditional approximate attention methods for GPU-based inference.
...
TL;DR: The paper introduces a framework for learning normalized energy models inspired by diffusion generative models, using a dual score matching objective to ensure consistent energies across noise levels. It achieves state-of-the-art cross-entropy on ImageNet64 and demonstrates strong generalization.


<details>
  <summary>Details</summary>
Motivation: Learning probability models from data is challenging due to dimensionality issues. The paper aims to address this by leveraging diffusion generative models to learn normalized energy models.

Method: Modifies a score network architecture to compute energy while preserving inductive biases. Uses a dual score matching objective (denoising and a novel secondary objective) to ensure consistent energies across noise levels.

Result: Achieves competitive cross-entropy on ImageNet64, shows strong generalization (log probabilities are independent of training images), and reveals variability in image probability and local dimensionality.

Conclusion: The framework effectively learns normalized energy models, generalizes well, and challenges traditional assumptions about image probability and dimensionality.
...
Method: Uses adaptive difficulty for online data selection and an attention-based framework for efficient difficulty estimation. Introduces rollout replay to reuse recent rollouts.

Result: Achieves 25%-65% reduction in RL fine-tuning time while matching performance of the original GRPO algorithm.
...
TL;DR: LSM-2 with AIM introduces a self-supervised learning approach for handling incomplete wearable sensor data without imputation, achieving top performance across tasks.


<details>
  <summary>Details</summary>
Motivation: Wearable sensor data often has significant missingness, challenging traditional SSL models that assume complete data.
...
Result: Achieves best performance in classification, regression, and generative tasks, with robust scaling and high performance under targeted missingness.

Conclusion: AIM is a reliable solution for real-world wearable data applications, especially in clinical contexts.
...
<details>
  <summary>Details</summary>
Motivation: QPI imaging is powerful but extracting single-scatterer patterns from multi-scatterer images is an ill-posed problem.

Method: A two-step learning strategy: (1) train a variational autoencoder for kernel representation, (2) align latent representations of observations with pre-learned kernels.

Result: Achieves higher accuracy and better generalization to unseen kernels compared to a direct one-step baseline.

Conclusion: The proposed AI framework effectively addresses the ill-posed QPI kernel extraction problem.
...
TL;DR: The paper introduces the Kinetics Scaling Law, showing smaller models' effectiveness is overestimated due to overlooked memory bottlenecks. Sparse attention models outperform dense ones, improving accuracy in both low and high-cost regimes.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiency of smaller models in test-time scenarios, where prior work ignores memory access bottlenecks and overestimates their performance.

Method: Holistic analysis of models (0.6B to 32B parameters) to derive the Kinetics Scaling Law, proposing sparse attention to reduce per-token costs.
...
Conclusion: Sparse attention is crucial for test-time scaling, as it enables longer generations and better accuracy, unlike parameter scaling in training.
...
TL;DR: The paper introduces grafting, a method to edit pretrained diffusion transformers (DiTs) for exploring new architectures with minimal compute, achieving comparable quality to full pretraining.


<details>
  <summary>Details</summary>
Motivation: Evaluating architectural decisions in models requires costly pretraining, limiting exploration. The paper aims to study new designs using existing pretrained models.

Method: Grafting edits pretrained DiTs by replacing operators (e.g., attention with gated convolution) and restructuring architectures (e.g., parallel blocks).

Result: Hybrid designs achieve good quality (FID: 2.38-2.64) with <2% pretraining compute. Grafting also speeds up text-to-image models (1.43x) with minimal quality drop.

Conclusion: Grafting enables efficient exploration of new diffusion model designs, from operator replacements to architecture restructuring, using pretrained DiTs.
...
<details>
  <summary>Details</summary>
Motivation: The key-value (KV) cache in Transformer LLMs bottlenecks generation cost, not the number of tokens. Compressing the KV cache can enhance efficiency and accuracy.

Method: Proposes DMS, a method for sparsifying KV caches with minimal training (1K steps), delaying token eviction to preserve critical information.

Result: DMS achieves 8× compression, outperforming training-free sparse attention, and boosts accuracy on benchmarks like AIME 24, GPQA, and LiveCodeBench.

Conclusion: Inference-time hyper-scaling with DMS is practical and effective, improving LLM accuracy without increasing runtime or memory load.
...
### [159] [GARG-AML against Smurfing: A Scalable and Interpretable Graph-Based Framework for Anti-Money Laundering](https://arxiv.org/abs/2506.04292)
*Bruno Deprez,Bart Baesens,Tim Verdonck,Wouter Verbeke*

Main category: cs.SI

TL;DR: GARG-AML is a graph-based method for detecting smurfing in money laundering, offering a transparent and efficient single-metric solution.


<details>
  <summary>Details</summary>
Motivation: Money laundering, especially smurfing, is a major global issue, but current detection methods lack transparency and adoption in businesses.
...
Result: Outperforms state-of-the-art smurfing detection methods in synthetic and open-source networks.

Conclusion: GARG-AML demonstrates the effectiveness of fundamental network properties for fraud detection, balancing efficiency and transparency.
...
### [160] [kTULA: A Langevin sampling algorithm with improved KL bounds under super-linear log-gradients](https://arxiv.org/abs/2506.04878)
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of sampling from distributions with super-linearly growing log-gradients, particularly in deep learning where global Lipschitz continuity is often unmet.

Method: Proposes kTULA, a novel tamed Langevin dynamics algorithm, and provides theoretical guarantees for its convergence in KL divergence and Wasserstein-2 distance.

Result: Achieves a non-asymptotic convergence bound in KL divergence with a rate of $2-\overline{\epsilon}$, improving existing literature, and demonstrates applicability in high-dimensional sampling and neural network optimization.

Conclusion: kTULA effectively addresses the sampling problem with theoretical guarantees, showcasing its utility in practical scenarios like deep learning and optimization.
...
### [161] [Can Artificial Intelligence Trade the Stock Market?](https://arxiv.org/abs/2506.04658)
*Jędrzej Maskiewicz,Paweł Sakowski*

Main category: q-fin.TR

TL;DR: The paper compares Deep Reinforcement Learning (DRL) algorithms (DDQN and PPO) with Buy and Hold in stock trading, showing DRL's superior risk-adjusted returns.


<details>
  <summary>Details</summary>
Motivation: To evaluate DRL's effectiveness in stock market trading and its ability to manage risk compared to traditional methods.

Method: Uses DDQN and PPO algorithms, testing on currency pairs, S&P 500, and Bitcoin (2019-2023 daily data).

Result: DRL outperforms Buy and Hold, managing risk by avoiding unfavorable trades.

Conclusion: DRL provides a significant advantage over classical supervised learning methods in trading.
...
### [162] [Improving AI-generated music with user-guided training](https://arxiv.org/abs/2506.04852)
*Vishwa Mohan Singh,Sai Anirudh Aryasomayajula,Ahan Chatterjee,Beste Aydemir,Rifat Mehreen Amin*

Main category: cs.SD

TL;DR: The paper proposes a human-computation approach to improve AI music generation by incorporating user feedback via a genetic algorithm, showing incremental performance gains.


<details>
  <summary>Details</summary>
Motivation: Current AI music generation models lack personalization due to fixed datasets, making them less responsive to subjective user preferences.

Method: A genetic algorithm aggregates user ratings to fine-tune the model, using feedback as the loss function.

Result: Pilot tests showed rating increases of 0.2 (first iteration) and 0.39 (second iteration) over baseline.

Conclusion: Human-computation enhances AI music generation by iteratively improving model performance based on user input.
...
### [163] [Survey on the Evaluation of Generative Models in Music](https://arxiv.org/abs/2506.05104)
*Alexander Lerch,Claire Arthur,Nick Bryan-Kinns,Corey Ford,Qianyi Sun,Ashvala Vinay*

Main category: cs.SD

TL;DR: An interdisciplinary review of evaluation methods for generative music systems, covering subjective, objective, qualitative, and quantitative approaches.


<details>
  <summary>Details</summary>
Motivation: To systematically assess and compare the diverse evaluation methods used for generative music systems.

Method: Review of common evaluation targets, methodologies, and metrics from musicological, engineering, and HCI perspectives.

Result: Identifies advantages and challenges of various evaluation approaches.

Conclusion: Highlights the need for interdisciplinary collaboration to improve evaluation practices in generative music research.
...
### [164] [Reactive Transport Simulation of Silicate-Rich Shale Rocks when Exposed to CO2 Saturated Brine Under High Pressure and High Temperature](https://arxiv.org/abs/2506.05122)
...
Result: Significant dissolution and precipitation of minerals (quartz, clay, feldspar) near the brine-rock interface, with only 1.00% porosity reduction, insufficient for pore filling.

Conclusion: Reactive transport modeling is feasible but shows limited mineral precipitation, indicating challenges for complete pore sealing in CO2 storage.
...
### [165] [Estimating properties of a homogeneous bounded soil using machine learning models](https://arxiv.org/abs/2506.04256)
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of estimating soil properties like diffusivity and hydraulic conductivity from water moisture data.

Method: Simulated data from solving an initial-boundary value problem using the Fokas method, tested with various machine learning models under full, noisy, and limited data conditions.

Result: SVMs and NNs performed best, with high accuracy and robustness, especially for diffusivity prediction.

Conclusion: Machine learning, particularly SVMs and NNs, is effective for soil property estimation from water moisture data.
...
### [166] [Interpretable LLMs for Credit Risk: A Systematic Review and Taxonomy](https://arxiv.org/abs/2506.04290)
*Muhammed Golec,Maha AlabdulJalil*

Main category: q-fin.RM

TL;DR: A systematic review and taxonomy of LLM-based approaches for credit risk assessment, covering model architectures, data types, explainability, and applications.


<details>
  <summary>Details</summary>
Motivation: To organize and analyze the growing body of research on LLM-based credit risk models, focusing on interpretability and future trends.

Method: Reviewed 60 relevant papers (2020-2025) using PRISMA strategy, classifying them into model architectures, data types, explainability mechanisms, and application areas.

Result: Identified key trends and gaps in LLM-based credit scoring, emphasizing interpretability techniques like chain-of-thought prompts.

Conclusion: The paper serves as a foundational reference for AI and finance researchers, highlighting future directions for LLM-based credit risk systems.
...
### [167] [Quantum simulation of the Hubbard model on a graphene hexagon: Strengths of IQPE and noise constraints](https://arxiv.org/abs/2506.05031)
*Mohammad Mirzakhani,Kyungsun Moon*

Main category: quant-ph

TL;DR: Quantum simulation of the Hubbard model on graphene using Qiskit shows promise but faces noise challenges on current hardware.


<details>
  <summary>Details</summary>
Motivation: To explore quantum computing's potential for simulating strongly correlated electron systems like the Hubbard model on graphene.

Method: Used Iterative Quantum Phase Estimation (IQPE) and adiabatic evolution algorithms in noiseless and noisy simulations, including a custom noise model for IBM hardware.

Result: Noiseless simulations matched exact diagonalization results, but noise degraded accuracy, revealing hardware limitations.

Conclusion: Quantum computing holds promise for material simulation, but current noise levels hinder practical accuracy.
...
### [168] [TQml Simulator: Optimized Simulation of Quantum Machine Learning](https://arxiv.org/abs/2506.04891)
*Viacheslav Kuzmin,Basil Kyriacou,Mateusz Papierz,Mo Kordzanganeh,Alexey Melnikov*

Main category: quant-ph

TL;DR: The paper benchmarks simulation techniques for quantum machine learning circuits, introducing TQml Simulator, which outperforms Pennylane's default.qubit by 2- to 100-fold.


<details>
  <summary>Details</summary>
Motivation: Advancing quantum machine learning research by improving simulation speed for hardware-efficient circuits.

Method: Numerical benchmarking of universal and gate-specific techniques, developing TQml Simulator to optimize simulation methods per layer.

Result: TQml Simulator shows 2- to 100-fold performance gains over Pennylane's default.qubit, varying by circuit and hardware.

Conclusion: Tailored simulation methods per layer significantly enhance performance, making TQml Simulator a valuable tool for quantum machine learning.
...
### [169] [Trustworthiness Preservation by Copies of Machine Learning Systems](https://arxiv.org/abs/2506.05203)
*Leonardo Ceragioli,Giuseppe Primiero*

Main category: cs.LO

TL;DR: The paper introduces a calculus to verify trustworthiness in ML system copies, defining four distinct notions of trustworthiness and analyzing their relations and compositions.
...
Method: A calculus is developed to model and verify probabilistic complex queries over data, defining four trustworthiness notions (Justifiably, Equally, Weakly, Almost Trustworthy) and studying their relations.

Result: The study provides a computational tool to check trustworthiness of copied systems by analyzing their behavior relative to the original.

Conclusion: The proposed calculus and notions offer a practical approach to verify trustworthiness in ML system copies, contributing to responsible AI.
...
### [170] [Knowledge-guided Contextual Gene Set Analysis Using Large Language Models](https://arxiv.org/abs/2506.04303)
...
TL;DR: cGSA is an AI-driven framework improving gene set analysis by integrating clinical context, outperforming baseline methods by 30% and enhancing precision and interpretability.


<details>
  <summary>Details</summary>
Motivation: Conventional GSA methods lack clinical context, producing redundant or irrelevant results, which reduces reliability and reproducibility.

Method: cGSA combines gene cluster detection, enrichment analysis, and large language models for context-aware pathway prioritization.

Result: Benchmarked on 102 gene sets, cGSA outperforms baselines by 30%, with expert validation confirming its precision. Case studies in melanoma and breast cancer highlight its utility.

Conclusion: cGSA effectively addresses limitations of traditional GSA, offering biologically meaningful and context-specific insights.
...
### [171] [hdl2v: A Code Translation Dataset for Enhanced LLM Verilog Generation](https://arxiv.org/abs/2506.04544)
...
Method: hdl2v translates or compiles VHDL, Chisel, and PyMTL3 into Verilog, enriching the dataset for training LLMs.

Result: hdl2v improves a 32B-parameter model's performance by up to 23% in VerilogEvalV2 and boosts a data augmentation-based approach by 63%.

Conclusion: hdl2v effectively addresses the Verilog data scarcity and enhances LLM performance, with potential for further dataset expansion in future work.
...
### [172] [QiMeng: Fully Automated Hardware and Software Design for Processor Chip](https://arxiv.org/abs/2506.05007)
...
TL;DR: QiMeng is a novel system for fully automated processor chip design, leveraging a domain-specific Large Processor Chip Model (LPCM) to address challenges like knowledge gaps and data scarcity. It includes hardware and software design agents and has shown promise in early applications.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome challenges in processor chip design, such as physical constraints, resource demands, and ecosystem diversity, by using AI and domain-specific LLMs.

Method: QiMeng uses a three-layer approach: a bottom-layer LPCM for knowledge representation and inference, middle-layer hardware and software design agents, and top-layer applications.

Result: Early components of QiMeng have been successfully applied, demonstrating significant advantages in automated hardware/software design.

Conclusion: QiMeng provides a feasible solution for automated processor chip design, with future work focusing on full integration and iterative design processes.
...
### [173] [FlashDMoE: Fast Distributed MoE in a Single Kernel](https://arxiv.org/abs/2506.04667)
...
<details>
  <summary>Details</summary>
Motivation: Existing MoE implementations suffer from low GPU utilization, high latency, and inefficiencies due to CPU-managed scheduling and frequent kernel launches.

Method: FlashDMoE integrates expert computation and inter-GPU communication into a single persistent GPU kernel, enabling fine-grained pipelining and device-initiated communication.

Result: FlashDMoE achieves up to 6x lower latency, 5.7x higher throughput, 4x better weak scaling efficiency, and 9x higher GPU utilization compared to baselines.

Conclusion: Principled GPU kernel-hardware co-design is crucial for optimizing large-scale distributed ML workloads, as demonstrated by FlashDMoE.
...
### [174] [Nombre Effectif de Partis Politiques en Afrique: Une Nouvelle Méthode pour un Calcul Objectif et Institutionnellement Neutre](https://arxiv.org/abs/2506.04279)
*Adama Nouboukpo,Kodzo Michel Aladji,Muktar Bappa*

Main category: physics.soc-ph

TL;DR: The paper introduces two new measures for calculating the effective number of parties in Africa, focusing on geographical and demographic dimensions to address the limitations of traditional methods like the ENP index.


<details>
  <summary>Details</summary>
Motivation: Traditional measures like the ENP index fail to capture the nuanced realities of African political landscapes, such as dominant parties, fluid affiliations, and ethnic/regional cleavages.

Method: The paper proposes two 'apolitical' or 'institutionally neutral' measures, incorporating geographical (territorial area) and demographic (population size) dimensions, with a minimum threshold of two parties.

Result: The new models provide a simpler and more contextually relevant framework for analyzing political dynamics in Africa, especially in data-scarce environments.
...
### [175] [What does making money have to do with crime?: A dive into the National Crime Victimization survey](https://arxiv.org/abs/2506.04240)
*Sydney Anuyah*

Main category: physics.soc-ph

TL;DR: The study analyzes how socioeconomic and demographic factors influence violent vs. property crime victimization using data from 1992-2022. Higher income/education reduces violent crime odds, while men, youth, and minorities face higher risks. Geographic analysis shows suburban models predict best, while rural crime remains unpredictable.


<details>
  <summary>Details</summary>
Motivation: To understand how income, education, employment, and demographics shape crime victimization types (violent vs. property).

Method: Uses National Crime Victimization Survey data (1992-2022), balanced classification splits, and logistic regression models (Group A: socioeconomic factors; Group B: added demographic controls). Evaluated by F1-score.

Result: Higher income/education lowers violent crime odds; men, youth, and minorities face higher risks. Suburban models predict best (accuracy 0.607, F1 0.590); rural crime remains unpredictable.

Conclusion: Findings highlight the need for targeted interventions: educational investments in cities, economic support in rural areas, and demographic-aware prevention strategies.
...
### [176] [ChemReservoir -- An Open-Source Framework for Chemically-Inspired Reservoir Computing](https://arxiv.org/abs/2506.04249)
*Mehmet Aziz Yirik,Jakob Lykke Andersen,Rolf Fagerberg,Daniel Merkle*

Main category: cs.CE

TL;DR: ChemReservoir is an open-source framework for chemically-inspired reservoir computing, addressing limitations of previous DNA-focused tools by offering general applicability, testing, and reproducibility.


<details>
  <summary>Details</summary>
Motivation: Previous reservoir computing tools were limited to DNA chemistry and lacked maintenance, necessitating a versatile, open-source solution.

Method: Developed ChemReservoir, a framework for constructing and analyzing chemically-inspired reservoirs, with enhanced testing and evaluation.

Result: Demonstrated stable performance in memory capacity tasks across various cycle-based reservoir topologies.

Conclusion: ChemReservoir provides a robust, general-purpose tool for chemically-inspired reservoir computing, overcoming prior limitations.
...
### [177] [Contextual Integrity in LLMs via Reasoning and Reinforcement Learning](https://arxiv.org/abs/2506.04245)
...
TL;DR: The paper proposes a method to ensure contextual integrity (CI) in autonomous agents by combining LLM reasoning and reinforcement learning, reducing inappropriate information disclosure while maintaining task performance.
...
Result: The method significantly reduces inappropriate information disclosure without compromising task performance, and improvements transfer to human-annotated benchmarks like PrivacyLens.
...
### [178] [Language-Guided Multi-Agent Learning in Simulations: A Unified Framework and Evaluation](https://arxiv.org/abs/2506.04251)
*Zhengyang Li*
...
<details>
  <summary>Details</summary>
Motivation: To enhance multi-agent systems by combining LLMs with MARL for better coordination and generalization in simulated environments.

Method: Uses a framework with Coordinator, Communicator, and Memory modules, trained with PPO, language-conditioned loss, and LLM query gating.

Result: Shows improved win rates, coordination, and zero-shot generalization in Google Research Football, MAgent Battle, and StarCraft II.

Conclusion: LLM-MARL bridges language and policy learning, advancing intelligent, cooperative agents for simulations and human-AI collaboration.
...
### [179] [A Graph-Retrieval-Augmented Generation Framework Enhances Decision-Making in the Circular Economy](https://arxiv.org/abs/2506.04252)
...
<details>
  <summary>Details</summary>
Motivation: LLMs often hallucinate industrial codes and emission factors, harming regulatory and investment decisions in sustainable manufacturing.

Method: Introduces CircuGraphRAG, a RAG framework using a domain-specific knowledge graph with 117,380 entities for structured reasoning. Queries are translated to SPARQL for verified subgraph retrieval.

Result: Achieves ROUGE-L F1 scores up to 1.0, outperforming baselines (<0.08). Halves response time and reduces token usage by 16%.
...
### [180] [Automated Skill Discovery for Language Agents through Exploration and Iterative Feedback](https://arxiv.org/abs/2506.04287)
*Yongjin Yang,Sinjae Kang,Juyong Lee,Dongjun Lee,Se-Young Yun,Kimin Lee*

Main category: cs.AI

TL;DR: EXIF is an automatic skill discovery framework for LLM-powered agents, using an exploration-first strategy and iterative feedback to improve skill acquisition without human intervention.


<details>
  <summary>Details</summary>
Motivation: Challenges in creating training datasets for LLM agents, such as manual effort and invalid task proposals, necessitate an automated solution.

Method: EXIF employs an exploration agent (Alice) to generate feasible skills and train the target agent (Bob), with iterative feedback to refine learning.

Result: Experiments show EXIF effectively discovers skills and improves agent performance, even enabling self-evolution when Alice and Bob share the same model.

Conclusion: EXIF offers a scalable, human-free approach to skill acquisition for LLM agents, with potential for self-improving systems.
...
### [181] [DeePoly: A High-Order Accuracy and Efficiency Deep-Polynomial Framework for Scientific Machine Learning](https://arxiv.org/abs/2506.04613)
*Li Liu,Heng Yong*

Main category: cs.AI

TL;DR: DeePoly combines DNNs and polynomial bases for solving PDEs, improving accuracy and efficiency while ensuring convergence.
...
Result: The method achieves high-order accuracy, efficiency, and maintains mesh-free properties.

Conclusion: DeePoly offers a robust, efficient solution for PDEs, supported by theory and experiments.
...
### [182] [LLMs for sensory-motor control: Combining in-context and iterative learning](https://arxiv.org/abs/2506.04867)
...
<details>
  <summary>Details</summary>
Motivation: To integrate symbolic reasoning from LLMs with sub-symbolic sensory-motor data for effective agent control.

Method: LLMs generate and refine control strategies using textual descriptions, feedback, and sensory-motor data.

Result: Successfully identifies optimal or high-performing solutions in classic control tasks.

Conclusion: Combining symbolic and sub-symbolic approaches enhances agent control in dynamic environments.
...
### [183] [When Thinking LLMs Lie: Unveiling the Strategic Deception in Representations of Reasoning Models](https://arxiv.org/abs/2506.04909)
*Kai Wang,Yihao Zhang,Meng Sun*

Main category: cs.AI

TL;DR: The paper investigates strategic deception in LLMs with CoT reasoning, using representation engineering to detect and control it, achieving high accuracy and providing tools for trustworthy AI.


<details>
  <summary>Details</summary>
Motivation: Addressing the critical alignment challenge of honesty in LLMs, especially as advanced systems with CoT reasoning may strategically deceive humans.
...
Conclusion: Unveils honesty-related issues in reasoning models and provides tools for trustworthy AI alignment.
...
TL;DR: The paper introduces 'Energentic Intelligence,' autonomous systems focused on self-sustenance through energy regulation, not task performance. It proposes an energy-based utility function and a modular architecture, validated in simulations.


<details>
  <summary>Details</summary>
Motivation: To create autonomous agents that prioritize survival and persistence in resource-volatile environments, moving beyond traditional reward-driven models.

Method: Formalizes energy-based utility and viability constraints, integrating energy harvesting, thermal regulation, and adaptive computation into a closed-loop control system.

Result: Simulations show stable, resource-aware behavior without external supervision, demonstrating self-regulation in volatile conditions.

Conclusion: Provides a theoretical and architectural foundation for deploying autonomous agents in environments where persistence must be self-regulated and infrastructure is unreliable.
...
### [185] [Just Enough Thinking: Efficient Reasoning with Adaptive Length Penalties Reinforcement Learning](https://arxiv.org/abs/2506.05256)
*Violet Xiang,Chase Blagden,Rafael Rafailov,Nathan Lile,Sang Truong,Chelsea Finn,Nick Haber*

Main category: cs.AI

TL;DR: ALP optimizes token usage in large reasoning models by adapting generation length to problem difficulty, reducing average tokens by 50% without performance loss.


<details>
  <summary>Details</summary>
Motivation: Existing methods for controlling token usage in LRMs are either data-intensive, manual, or uniform, failing to adapt to problem difficulty.

Method: ALP uses reinforcement learning to tailor generation length per prompt, penalizing extra tokens inversely with solve rate.

Result: ALP reduces average token usage by 50% while maintaining performance and improves accuracy on hard problems.

Conclusion: ALP intelligently redistributes compute, cutting waste on easy problems and boosting performance on hard ones.
...
### [186] [Control Tax: The Price of Keeping AI in Check](https://arxiv.org/abs/2506.05296)
*Mikhail Terekhov,Zhen Ning David Liu,Caglar Gulcehre,Samuel Albanie*

Main category: cs.AI

TL;DR: The paper introduces 'Control Tax' to quantify the cost of AI oversight, offering a framework to balance safety and cost in AI Control (AIC).


<details>
  <summary>Details</summary>
Motivation: Address the need for practical and cost-effective oversight mechanisms in high-stakes AI applications.

Method: Develops a theoretical framework for Control Tax, evaluates language models in adversarial settings, and estimates financial costs for control protocols.

Result: Provides a method to quantify safety assurances and cost, with optimized monitoring strategies for practical deployment.

Conclusion: The framework aids practitioners in making informed decisions about AI safety, advancing AIC through economic feasibility assessment.
...
### [187] [CORA: Coalitional Rational Advantage Decomposition for Multi-Agent Policy Gradients](https://arxiv.org/abs/2506.04265)
*Mengda Ji,Genjiu Xu,Liying Wang*

Main category: cs.MA

TL;DR: The paper addresses credit assignment in cooperative MARL by proposing CORA, a coalition-level method that ensures rational advantage decomposition, outperforming baselines in tasks with local optima.


<details>
  <summary>Details</summary>
Motivation: Current credit assignment methods in MARL often fail to account for distinct agent contributions at the coalition level, leading to suboptimal policy updates.

Method: The authors introduce CORA, which evaluates coalitional advantages via marginal contributions and decomposes advantages using cooperative game theory's core solution, with random coalition sampling for efficiency.
...
Conclusion: Coalition-aware credit assignment, as demonstrated by CORA, is crucial for enhancing MARL performance.
...
### [188] [Benchmark for Antibody Binding Affinity Maturation and Design](https://arxiv.org/abs/2506.04235)
...
TL;DR: AbBiBench is a benchmarking framework for antibody binding affinity, evaluating designs based on antibody-antigen complex likelihood, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Existing antibody evaluation methods focus on antibody similarity to natural ones, lacking functional assessment of binding potential. AbBiBench addresses this gap.

Method: AbBiBench curates 9 datasets with 155,853 antibodies, compares 14 protein models using likelihood-affinity correlation, and tests generative power in a case study.

Result: Structure-conditioned inverse folding models perform best in affinity correlation and generation tasks.

Conclusion: AbBiBench offers a unified, biologically grounded framework for improving antibody design models.
...
### [189] [The Latent Space Hypothesis: Toward Universal Medical Representation Learning](https://arxiv.org/abs/2506.04515)
*Salil Patel*
...
<details>
  <summary>Details</summary>
Motivation: To integrate disparate medical data modalities into a unified framework for better understanding and managing individual health trajectories.

Method: Uses the Latent Space Hypothesis to model health data as projections of a unified manifold, enabling personalized diagnosis and treatment.

Result: The framework reveals sub-trajectories and patient-specific changes, supporting tailored healthcare and redefining disease classifications.

Conclusion: The approach advances personalized medicine but faces challenges like bias and data scarcity, with potential solutions in scale-aware encoders and continual learning.
...
### [190] [Random dynamics and invariant measures for a class of non-Newtonian fluids of differential type on 2D and 3D Poincaré domains](https://arxiv.org/abs/2506.04801)
*Kush Kinra,Fernanda Cipriano*

Main category: math.PR

TL;DR: The paper studies the well-posedness and asymptotic behavior of solutions for incompressible stochastic third-grade fluids on Poincaré domains, proving unique weak solutions and random attractors for bounded and unbounded domains.


<details>
  <summary>Details</summary>
Motivation: The motivation is to analyze the well-posedness and asymptotic properties of stochastic non-Newtonian fluid equations, addressing challenges like nonlinear terms and lack of compact embeddings.

Method: The methods include proving unique weak solutions under Dirichlet conditions, using compact Sobolev embeddings for bounded domains, and uniform-tail estimates for unbounded domains.

Result: Key results include the existence of unique weak solutions, random attractors for bounded and unbounded domains, and invariant measures.

Conclusion: The paper concludes with the existence of random attractors and invariant measures, highlighting the challenges and novelty in analyzing unbounded domains.
...
### [191] [Conservation Laws and Boundedness for Linearised Einstein--Maxwell Equations on the Reissner--Nordström Black Hole](https://arxiv.org/abs/2506.04812)
*Marios A. Apetroaie,Sam C. Collingbourne,Elena Giorgi*

Main category: gr-qc

TL;DR: The paper analyzes the linearized Einstein-Maxwell equations on Reissner-Nordström spacetime, deriving an energy conservation law to control Teukolsky variables without relying on hyperbolic methods.


<details>
  <summary>Details</summary>
Motivation: To establish energy control for gauge-invariant Teukolsky variables in Reissner-Nordström spacetime without using hyperbolic techniques.

Method: Derives a canonical energy conservation law in double null gauge and applies it to Teukolsky variables.

Result: Uniform boundedness of Teukolsky variables is achieved for charge-to-mass ratios below a specific threshold.

Conclusion: The approach successfully controls energy fluxes for Teukolsky variables, demonstrating boundedness in Reissner-Nordström spacetime.
...
### [192] [A Comprehensive Survey on Bio-Inspired Algorithms: Taxonomy, Applications, and Future Directions](https://arxiv.org/abs/2506.04238)
...
TL;DR: A survey categorizing bio-inspired algorithms into eight groups, reviewing their principles, strengths, and limitations, and highlighting applications and future challenges.


<details>
  <summary>Details</summary>
Motivation: To provide a comprehensive overview of bio-inspired algorithms, their applications, and open challenges for researchers and practitioners.

Method: Categorizes BIAs into eight groups, reviews core principles, strengths, and limitations, and illustrates applications in various fields.

Result: Identifies recent advances in hybridization, tuning, and adaptive strategies, and highlights open challenges like scalability and interpretability.

Conclusion: Serves as a foundational resource for understanding the current landscape and future directions of bio-inspired computing.
...
### [193] [GEM: Empowering LLM for both Embedding Generation and Language Understanding](https://arxiv.org/abs/2506.04344)
*Caojin Zhang,Qiang Zhang,Ke Li,Sai Vidyaranya Nuthalapati,Benyu Zhang,Jason Liu,Serena Li,Lizhu Zhang,Xiangjun Fan*

Main category: cs.CL

TL;DR: The paper proposes GEM, a self-supervised method enabling decoder-only LLMs to generate high-quality text embeddings without compromising their original capabilities.
...
Conclusion: GEM successfully enhances LLMs with state-of-the-art text embedding capabilities while preserving their original NLP performance.
...
### [194] [Hierarchical Text Classification Using Contrastive Learning Informed Path Guided Hierarchy](https://arxiv.org/abs/2506.04381)
*Neeraj Agrawal,Saurabh Kumar,Priyanka Bhatt,Tanishka Agarwal*

Main category: cs.CL

TL;DR: HTC-CLIP combines label hierarchy and text encoding via contrastive learning, improving performance over existing models.


<details>
  <summary>Details</summary>
Motivation: Existing HTC models handle label hierarchy and text encoding separately or guide hierarchy in text encoders, but these approaches are complementary.

Method: HTC-CLIP uses contrastive learning to create hierarchy-aware text and path-guided hierarchy representations, pooling both for inference.

Result: HTC-CLIP outperforms state-of-the-art models by 0.99-2.37% in Macro F1 score on benchmark datasets.

Conclusion: Combining complementary approaches in HTC-CLIP enhances hierarchical text classification performance.
...
### [195] [Building a Few-Shot Cross-Domain Multilingual NLU Model for Customer Care](https://arxiv.org/abs/2506.04389)
*Saurabh Kumar,Sourav Bansal,Neeraj Agrawal,Priyanka Bhatt*

Main category: cs.CL

TL;DR: The paper proposes a model architecture for few-shot intent classification in customer care, improving accuracy by 20-23% over SOTA models.


<details>
  <summary>Details</summary>
Motivation: Customer care relies on annotated data, which is scarce across domains (geography, channel, language). Generalizing intent classifiers with minimal annotations is valuable.

Method: An embedder-cum-classifier model with supervised fine-tuning, isotropic regularizers, and multilingual knowledge distillation to adapt to new domains with few samples.

Result: Experiments on e-commerce datasets (Canada, Mexico) show a 20-23% accuracy boost over existing models.
...
### [196] [MedAgentGym: Training LLM Agents for Code-Based Medical Reasoning at Scale](https://arxiv.org/abs/2506.04405)
...
TL;DR: MedAgentGYM is a training environment for enhancing medical reasoning in LLMs, featuring 72,413 tasks across 129 biomedical categories. It benchmarks 30+ LLMs, showing a gap between commercial and open-source models. Med-Copilot-7B, trained with MedAgentGYM, achieves significant gains, offering a competitive alternative to GPT-4.


<details>
  <summary>Details</summary>
Motivation: To address the lack of publicly available training environments for improving coding-based medical reasoning in LLMs, and to bridge the performance gap between commercial and open-source models.

Method: Developed MedAgentGYM with 72,413 task instances in executable coding environments, including task descriptions, feedback, ground-truth annotations, and scalable training. Benchmarked 30+ LLMs and fine-tuned Med-Copilot-7B using supervised and reinforcement learning.

Result: Med-Copilot-7B achieved +36.44% performance gain with supervised fine-tuning and +42.47% with reinforcement learning, becoming a competitive alternative to GPT-4.

Conclusion: MedAgentGYM provides a comprehensive, accessible platform for developing LLM-based coding assistants in biomedicine, bridging gaps in performance and resource availability.
...
### [197] [Watermarking Degrades Alignment in Language Models: Analysis and Mitigation](https://arxiv.org/abs/2506.04462)
*Apurv Verma,NhatHai Phan,Shubhendu Trivedi*

Main category: cs.CL

TL;DR: The paper analyzes how watermarking techniques (Gumbel and KGW) affect LLM alignment (truthfulness, safety, helpfulness), identifies degradation patterns, and proposes Alignment Resampling (AR) to mitigate these issues while maintaining watermark detectability.


<details>
  <summary>Details</summary>
Motivation: To understand the underexamined effects of watermarking on LLM alignment properties (truthfulness, safety, helpfulness) and address the resulting degradation patterns.
...
Result: AR recovers or surpasses baseline alignment scores with 2-4 watermarked generations, while maintaining strong watermark detectability. Modified Gumbel implementation ensures compatibility with AR.

Conclusion: The work highlights the balance between watermark strength and alignment, offering AR as a practical solution for deploying watermarked LLMs responsibly.
...
### [198] [Safe: Enhancing Mathematical Reasoning in Large Language Models via Retrospective Step-aware Formal Verification](https://arxiv.org/abs/2506.04592)
...
<details>
  <summary>Details</summary>
Motivation: Current methods like PRMs or self-consistency lack transparency in detecting hallucinations in CoT prompting, necessitating a verifiable solution.
...
Conclusion: $Safe$ is the first to use Lean 4 for verifying LLM-generated content, aligning with formal languages' original purpose of ensuring robust proof foundations.
...
### [199] [Static Word Embeddings for Sentence Semantic Representation](https://arxiv.org/abs/2506.04624)
*Takashi Wada,Yuki Hirakawa,Ryotaro Shimizu,Takahiro Kawashima,Yuki Saito*

Main category: cs.CL

TL;DR: Proposes static word embeddings optimized for sentence semantics via PCA and learning methods, outperforming existing models and rivaling basic Sentence Transformers.
...
Method: Extracts word embeddings from a pre-trained Sentence Transformer, improves them with PCA, and applies knowledge distillation or contrastive learning. Sentence representation is done by averaging word embeddings.

Result: Outperforms existing static models on semantic tasks and rivals basic Sentence Transformers (SimCSE) on some datasets.

Conclusion: The method effectively removes irrelevant word embedding components and adjusts vector norms based on word influence, achieving strong performance.
...
### [200] [Recycling the Web: A Method to Enhance Pre-training Data Quality and Quantity for Language Models](https://arxiv.org/abs/2506.04689)
...
Method: Proposes REWIRE, a method to rewrite and enrich low-quality documents for training.
...
### [201] [Fine-Grained Interpretation of Political Opinions in Large Language Models](https://arxiv.org/abs/2506.04774)
...
<details>
  <summary>Details</summary>
Motivation: Misalignment between LLMs' responses and internal intentions, and reliance on single-axis concepts leading to confounds, motivate the need for deeper probing and multi-dimensional analysis.

Method: A four-dimensional political learning framework is designed, with a corresponding dataset for fine-grained concept vector learning. Three representation engineering techniques are applied to eight open-source LLMs.
...
Conclusion: The multi-dimensional approach and interpretable techniques effectively uncover and intervene in LLMs' political states, addressing limitations of single-axis evaluations.
...
### [202] [Towards LLM-Centric Multimodal Fusion: A Survey on Integration Strategies and Techniques](https://arxiv.org/abs/2506.04788)
*Jisu An,Junseok Lee,Jeoungeun Lee,Yongseok Son*

Main category: cs.CL

TL;DR: The paper surveys Multimodal Large Language Models (MLLMs), focusing on integrating diverse modalities into language models, proposing a classification framework, and analyzing 125 models to guide future development.


<details>
  <summary>Details</summary>
Motivation: To address the gap in systematic understanding of how different modalities connect to language models in MLLMs.

Method: The study examines architectural strategies, representation learning techniques, and training paradigms, analyzing 125 MLLMs.

Result: A classification framework for MLLMs is proposed, identifying emerging patterns in modality integration.

Conclusion: The taxonomy provides a structured overview to guide robust multimodal integration in future models.
...
### [203] [Verbose ListOps (VLO): Beyond Long Context -- Unmasking LLM's Reasoning Blind Spots](https://arxiv.org/abs/2506.04907)
*Alex Pan,Mary-Anne Williams*

Main category: cs.CL

TL;DR: Verbose ListOps is a benchmark designed to test LLMs' nested narrative reasoning by converting ListOps computations into lengthy stories, revealing their struggle with state management and intermediate results.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to adequately test nested reasoning in LLMs, masking their limitations in handling realistic, distracting narratives.

Method: Verbose ListOps programmatically transforms ListOps computations into long, coherent stories, controlling narrative size and reasoning difficulty.

Result: Leading LLMs perform poorly on Verbose ListOps at modest narrative lengths, despite excelling at raw ListOps.

Conclusion: Addressing this limitation is crucial for real-world text interpretation, and Verbose ListOps provides a framework for targeted reasoning improvements.
...
### [204] [Controlling Summarization Length Through EOS Token Weighting](https://arxiv.org/abs/2506.05017)
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for controlling text length are complex and incompatible with pre-trained models, necessitating a simpler solution.

Method: Adjusts the importance of predicting the EOS token in cross-entropy loss, making it architecture-agnostic and compatible with different models.
...
Conclusion: The proposed method offers a straightforward, model-agnostic way to control text length in summarization tasks.
...
### [205] [DiCoRe: Enhancing Zero-shot Event Detection via Divergent-Convergent LLM Reasoning](https://arxiv.org/abs/2506.05128)
...
Method: DiCoRe uses Dreamer for open-ended event discovery (divergent reasoning) and Grounder for task-aligned reasoning via constrained decoding. An LLM-Judge verifies outputs for precision.

Result: DiCoRe outperforms baselines on six datasets across five domains, achieving 4-7% average F1 gains.

Conclusion: DiCoRe is a robust zero-shot event detection framework, enhancing coverage and precision through its divergent-convergent approach.
...
### [206] [Counterfactual reasoning: an analysis of in-context emergence](https://arxiv.org/abs/2506.05188)
*Moritz Miller,Bernhard Schölkopf,Siyuan Guo*

Main category: cs.CL

TL;DR: The paper explores in-context counterfactual reasoning in large-scale neural language models (LMs), demonstrating their ability to predict outcomes under hypothetical scenarios using a synthetic linear regression task.


<details>
  <summary>Details</summary>
Motivation: To understand how LMs perform in-context counterfactual reasoning, focusing on noise abduction in a controlled setup.
...
Result: LMs successfully perform counterfactual reasoning in the controlled setup, with findings extending to sequential data and hinting at potential for counterfactual story generation.

Conclusion: The work provides insights into counterfactual reasoning in LMs, suggesting broader applicability beyond regression tasks, including sequential data.
...
### [207] [The Common Pile v0.1: An 8TB Dataset of Public Domain and Openly Licensed Text](https://arxiv.org/abs/2506.05209)
...
Conclusion: The Common Pile v0.1 is a viable, ethically sound alternative for LLM pretraining, with released dataset, code, and model checkpoints.
...
### [208] [ProRefine: Inference-time Prompt Refinement with Textual Feedback](https://arxiv.org/abs/2506.05305)
...
Method: ProRefine leverages textual feedback from LLMs to dynamically refine prompts for multi-step reasoning tasks without additional training or labels.

Result: ProRefine outperforms zero-shot Chain-of-Thought baselines by 3 to 37 percentage points on five mathematical reasoning datasets.

Conclusion: ProRefine enhances accuracy and efficiency, enabling smaller models to match larger ones, democratizing high-performing AI.
...
### [209] [Constrained Entropic Unlearning: A Primal-Dual Framework for Large Language Models](https://arxiv.org/abs/2506.05314)
...
<details>
  <summary>Details</summary>
Motivation: LLMs need to unlearn sensitive or outdated data, but current methods struggle with unstable optimization and degraded performance.
...
### [210] [Search Arena: Analyzing Search-Augmented LLMs](https://arxiv.org/abs/2506.05334)
...
TL;DR: The paper introduces Search Arena, a large-scale dataset for evaluating search-augmented LLMs, revealing insights into user preferences and the impact of citations and sources on perceived credibility.


<details>
  <summary>Details</summary>
Motivation: Existing datasets for search-augmented LLMs are limited in scale and scope, focusing on static, single-turn questions. The need for a more comprehensive dataset to analyze multi-turn interactions and user preferences motivated this work.

Method: The authors created Search Arena, a crowd-sourced dataset of over 24,000 multi-turn interactions with search-augmented LLMs, including 12,000 human preference votes. The dataset spans diverse intents and languages.

Result: User preferences are influenced by citation quantity, even if citations don't directly support claims. Community-driven sources are preferred over static encyclopedic ones. Web search can improve performance in non-search settings but degrades in search-intensive settings if relying solely on parametric knowledge.

Conclusion: The dataset supports future research into search-augmented LLMs, highlighting the gap between perceived and actual credibility and the importance of source diversity. The dataset and code are open-sourced.
...
### [211] [Deep learning image burst stacking to reconstruct high-resolution ground-based solar observations](https://arxiv.org/abs/2506.04781)
*Christoph Schirninger,Robert Jarolim,Astrid M. Veronig,Christoph Kuckein*

Main category: astro-ph.SR

TL;DR: A deep learning method reconstructs 100 short-exposure solar images into one high-quality image in real-time, outperforming traditional speckle reconstruction in robustness and perceptual quality.


<details>
  <summary>Details</summary>
Motivation: Large ground-based solar telescopes face limitations due to Earth's atmospheric turbulence, requiring post-processing corrections. Current methods struggle with strong turbulence and high computational costs.
...
Result: The method shows superior perceptual quality, especially when speckle reconstructions exhibit artifacts, and performs best with the full image burst.
...
### [212] [Learning to Diagnose Privately: DP-Powered LLMs for Radiology Report Classification](https://arxiv.org/abs/2506.04450)
*Payel Bhattacharjee,Fengwei Tian,Ravi Tandon,Joseph Lo,Heidi Hanson,Geoffrey Rubin,Nirav Merchant,John Gounley*

Main category: cs.CR

TL;DR: A framework for fine-tuning LLMs with differential privacy (DP) for multi-abnormality classification in radiology reports, balancing privacy and performance.


<details>
  <summary>Details</summary>
Motivation: To mitigate privacy risks in sensitive patient data while maintaining classification performance in radiology reports.

Method: Fine-tuning LLMs (BERT-medium, BERT-small, ALBERT-base) using DP-LoRA on MIMIC-CXR and CT-RATE datasets, with privacy budgets {0.01, 0.1, 1.0, 10.0}. Performance evaluated via weighted F1 scores.

Result: DP models achieved comparable performance (F1: 0.88 on MIMIC-CXR, 0.59 on CT-RATE) to non-private baselines (0.90, 0.78), showing a clear privacy-utility trade-off.

Conclusion: DP-LoRA enables effective, privacy-preserving multi-abnormality classification, addressing challenges in LLM fine-tuning for medical data.
...
### [213] [EMBER2024 -- A Benchmark Dataset for Holistic Evaluation of Malware Classifiers](https://arxiv.org/abs/2506.05074)
*Robert J. Joyce,Gideon Miller,Phil Roth,Richard Zak,Elliott Zaresky-Williams,Hyrum Anderson,Edward Raff,James Holt*

Main category: cs.CR

TL;DR: EMBER2024 is a new malware dataset addressing limitations of existing datasets by including diverse file formats, multiple classification tasks, and evasive malware samples. It supports reproducibility and research advancement.
...
Method: EMBER2024 was created in collaboration with previous EMBER dataset authors, featuring 3.2 million files, six formats, and seven classification tasks, including a challenge set of undetected malware.

Result: The dataset enables holistic evaluation of malware classifiers, introduces EMBER feature version 3, and includes a challenge set for assessing evasive malware detection.

Conclusion: EMBER2024 promotes reproducibility and empowers researchers by providing a comprehensive, accessible dataset for advancing malware analysis.
...
### [214] [Membership Inference Attacks on Sequence Models](https://arxiv.org/abs/2506.05126)
*Lorenzo Rossi,Michael Aerni,Jie Zhang,Florian Tramèr*

Main category: cs.CR

TL;DR: The paper addresses privacy leakage in sequence models like LLMs by adapting a membership inference attack to account for sequential correlations, improving audit effectiveness without extra computational costs.


<details>
  <summary>Details</summary>
Motivation: Existing tools for auditing privacy risks in sequence models are inadequate due to mismatched assumptions, necessitating a method that leverages sequential correlations.

Method: The authors adapt a state-of-the-art membership inference attack to explicitly model within-sequence correlations in sequence models.

Result: The adapted attack consistently improves memorization audit effectiveness without increasing computational costs.

Conclusion: This work advances reliable memorization audits for large sequence models by addressing the limitations of existing tools.
...
### [215] [Why LLM Safety Guardrails Collapse After Fine-tuning: A Similarity Analysis Between Alignment and Fine-tuning Datasets](https://arxiv.org/abs/2506.05346)
*Lei Hsiung,Tianyu Pang,Yung-Chen Tang,Linyue Song,Tsung-Yi Ho,Pin-Yu Chen,Yaoqing Yang*

Main category: cs.CR

TL;DR: The paper explores how the similarity between upstream safety-alignment data and downstream fine-tuning tasks affects LLM safety guardrails, showing high similarity weakens guardrails while low similarity strengthens them.


<details>
  <summary>Details</summary>
Motivation: Existing mitigation strategies for LLM safety alignment jailbreaks overlook the role of original safety-alignment data, prompting investigation into dataset similarity's impact.

Method: The study analyzes representation similarity between upstream alignment datasets and downstream fine-tuning tasks, conducting experiments to measure effects on safety guardrails.
...
### [216] [On the Wasserstein Geodesic Principal Component Analysis of probability measures](https://arxiv.org/abs/2506.04480)
*Nina Vesseron,Elsa Cazelles,Alice Le Brigant,Thierry Klein*

Main category: stat.ML

TL;DR: The paper introduces Geodesic Principal Component Analysis (GPCA) for probability distributions using Otto-Wasserstein geometry, focusing on Gaussian distributions and neural networks for geodesic parameterization, with comparisons to classical PCA.


<details>
  <summary>Details</summary>
Motivation: To identify geodesic curves in probability measure spaces that capture dataset variations, extending PCA to non-Euclidean settings.

Method: Uses Otto-Wasserstein geometry for Gaussian distributions and neural networks for general probability measures, comparing results with tangent PCA.

Result: Demonstrates effective geodesic curve identification and variation capture, validated through examples and real-world datasets.

Conclusion: GPCA offers a robust framework for analyzing probability distributions, outperforming classical PCA in non-Euclidean contexts.
...
### [217] [Regret-Optimal Q-Learning with Low Cost for Single-Agent and Federated Reinforcement Learning](https://arxiv.org/abs/2506.04626)
*Haochen Zhang,Zhong Zheng,Lingzhou Xue*

Main category: stat.ML

TL;DR: The paper introduces two model-free RL algorithms (Q-EarlySettled-LowCost and FedQ-EarlySettled-LowCost) to minimize burn-in and switching/communication costs in single-agent and federated RL, achieving near-optimal regret with linear burn-in and logarithmic switching/communication costs.


<details>
  <summary>Details</summary>
Motivation: Address costly data collection and policy deployment in RL by minimizing burn-in and switching/communication costs.

Method: Proposes Q-EarlySettled-LowCost for single-agent RL and FedQ-EarlySettled-LowCost for federated RL, focusing on linear burn-in and logarithmic switching/communication costs.

Result: Achieves near-optimal regret, linear burn-in cost in S and A, and logarithmic switching/communication costs, with improved gap-dependent bounds.

Conclusion: The algorithms outperform existing methods by balancing efficiency and performance in RL and FRL settings.
...
### [218] [Distributional encoding for Gaussian process regression with qualitative inputs](https://arxiv.org/abs/2506.04813)
*Sébastien Da Veiga*
...
<details>
  <summary>Details</summary>
Motivation: GP regression struggles with categorical variables, limiting its effectiveness in applications like Bayesian optimization (BO). The paper aims to address this gap.

Method: The authors generalize naive target encoding to DE, leveraging all target variable samples for a category. They use characteristic kernels (maximum mean discrepancy, Wasserstein distance) for GP integration.

Result: Empirical validation shows state-of-the-art predictive performance on synthetic and real-world datasets. DE complements recent BO advances in discrete/mixed spaces.

Conclusion: DE enhances GP regression for categorical variables, offering improved performance and compatibility with BO methods.
...
### [219] [Learning Joint Interventional Effects from Single-Variable Interventions in Additive Models](https://arxiv.org/abs/2506.04945)
*Armin Kekić,Sergio Hernan Garrido Mejia,Bernhard Schölkopf*

Main category: stat.ML

TL;DR: The paper proposes a method to estimate joint interventional effects using only observational data and single-variable interventions, without needing joint interventional data.


<details>
  <summary>Details</summary>
Motivation: Estimating joint intervention effects is important but challenging due to lack of joint interventional data.

Method: The study presents an identifiability result for nonlinear additive outcome mechanisms and proposes an estimator decomposing effects into confounded and unconfounded contributions.

Result: Experiments show the method performs comparably to models trained on joint interventional data and outperforms observational estimators.
...
### [220] [Nonlinear Causal Discovery for Grouped Data](https://arxiv.org/abs/2506.05120)
*Konstantin Göbler,Tobias Windisch,Mathias Drton*

Main category: stat.ML

TL;DR: The paper extends nonlinear additive noise models to infer causal relationships among groups of variables (random vectors), proposing a two-step approach for causal graph learning and demonstrating its effectiveness in simulations and real-world data.


<details>
  <summary>Details</summary>
Motivation: Existing methods for causal inference focus on scalar variables, but many domains require analyzing groups of variables (random vectors). This gap motivates the extension of nonlinear additive noise models to handle vector cases.

Method: A two-step approach: (1) infer causal order among random vectors, and (2) perform model selection to identify the best graph consistent with this order. Novel solutions are introduced for both steps.

Result: The method shows strong performance in simulations and is successfully applied to real-world assembly line data with partial causal ordering knowledge.

Conclusion: The proposed approach effectively addresses causal inference for random vectors, demonstrating practical utility in complex domains.
...
### [221] [Causal Effect Identification in lvLiNGAM from Higher-Order Cumulants](https://arxiv.org/abs/2506.05202)
...
<details>
  <summary>Details</summary>
Motivation: To address challenges in causal inference with latent confounders, specifically in setups with single proxy variables or underspecified instrumental variables.

Method: Uses higher-order cumulants in lvLiNGAM to identify causal effects, providing estimation methods for single proxy or instrument cases.

Result: Demonstrates accurate and robust causal effect identification, outperforming existing methods.
...
### [222] [Admissibility of Completely Randomized Trials: A Large-Deviation Approach](https://arxiv.org/abs/2506.05329)
*Guido Imbens,Chao Qin,Stefan Wager*

Main category: stat.ML

TL;DR: Adaptive designs universally and strictly dominate non-adaptive trials in best-arm identification with three or more arms, as shown by efficiency exponent analysis.


<details>
  <summary>Details</summary>
Motivation: To determine if ignoring adaptive trial options in favor of non-adaptive trials is justified, particularly in best-arm identification problems.

Method: Analyzes batched arm elimination designs, which progressively eliminate underperforming arms, and compares them to completely randomized trials.

Result: Adaptive designs strictly dominate non-adaptive trials when there are at least three arms, quantified by efficiency exponent.

Conclusion: Ignoring adaptive designs is inadmissible in best-arm identification with three or more arms, resolving an open problem from Qin [2022].
...
### [223] [MVP-Shapley: Feature-based Modeling for Evaluating the Most Valuable Player in Basketball](https://arxiv.org/abs/2506.04602)
...
### [224] [Dynamic Epsilon Scheduling: A Multi-Factor Adaptive Perturbation Budget for Adversarial Training](https://arxiv.org/abs/2506.04263)
...
Method: DES dynamically adjusts budgets using decision boundary distance, prediction confidence, and model uncertainty.

Result: DES outperforms fixed-epsilon baselines and prior adaptive methods on CIFAR-10 and CIFAR-100.
...
### [225] [ReXVQA: A Large-scale Visual Question Answering Benchmark for Generalist Chest X-ray Understanding](https://arxiv.org/abs/2506.04353)
...
<details>
  <summary>Details</summary>
Motivation: To create a comprehensive benchmark for evaluating AI in radiological reasoning, surpassing template-based queries and bridging the AI-clinical expertise gap.

Method: Developed a dataset of 696,000 questions paired with 160,000 chest X-rays, evaluated eight multimodal models, and conducted a human reader study.

Result: MedGemma achieved 83.24% accuracy, outperforming human readers (77.27%), with distinct performance patterns between AI and humans.

Conclusion: ReXVQA sets a new standard for radiological AI evaluation, enabling next-generation systems with expert-level clinical reasoning.
...
TL;DR: HMAR improves image generation by addressing VAR's issues with quality, sequence length, and flexibility, offering faster and more efficient performance.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of VAR, such as reduced image quality, superlinear sequence length scaling, and inflexible sampling schedules.

Method: HMAR introduces next-scale prediction as a Markovian process and uses masked prediction for controllable multi-step generation.
...
Conclusion: HMAR provides a flexible, efficient, and high-quality alternative to VAR for image generation and editing.
...
### [227] [Gen-n-Val: Agentic Image Data Generation and Validation](https://arxiv.org/abs/2506.04676)
...
<details>
  <summary>Details</summary>
Motivation: Addressing challenges like data scarcity, label noise, and poor-quality synthetic data in computer vision tasks.

Method: Uses Layer Diffusion, LLMs, and VLLMs to generate single-object masks and diverse backgrounds, with two agents: one for prompt optimization and another for data validation.

Result: Reduces invalid synthetic data from 50% to 7% and improves performance by 1% mAP on rare classes in COCO instance segmentation. Also shows 7.1% mAP improvement in open-vocabulary object detection.
...
### [228] [Using In-Context Learning for Automatic Defect Labelling of Display Manufacturing Data](https://arxiv.org/abs/2506.04717)
...
<details>
  <summary>Details</summary>
Motivation: To reduce manual annotation efforts in industrial inspection systems by developing an efficient auto-labeling solution.

Method: Enhanced SegGPT architecture with domain-specific training techniques and a scribble-based annotation mechanism, validated via a two-stage training approach on industrial datasets.

Result: Achieved an average IoU increase of 0.22, 14% recall improvement, and 60% auto-labeling coverage, matching human-labeled data performance.

Conclusion: The system offers a practical solution for industrial defect detection, significantly reducing manual annotation while maintaining high accuracy.
...
### [229] [LotusFilter: Fast Diverse Nearest Neighbor Search via a Learned Cutoff Table](https://arxiv.org/abs/2506.04790)
*Yusuke Matsui*
...
<details>
  <summary>Details</summary>
Motivation: ANNS can produce overly similar results; diverse results are needed for applications like RAG.

Method: Precompute a cutoff table of close vectors; greedily remove redundant vectors during filtering.
...
### [230] [Fool the Stoplight: Realistic Adversarial Patch Attacks on Traffic Light Detectors](https://arxiv.org/abs/2506.04823)
...
<details>
  <summary>Details</summary>
Motivation: Despite successful adversarial attacks on autonomous vehicle perception tasks, traffic light detectors have been underexplored. This work addresses this gap.

Method: The authors propose a threat model with patches placed under traffic lights and a training strategy for CNNs. They test attacks in universal settings and real-world scenarios.

Result: Successful targeted attacks (e.g., red-to-green label flipping) and pictogram classification attacks are demonstrated, validated with printed patches in lab and test environments.
...
### [231] [Quantifying Cross-Modality Memorization in Vision-Language Models](https://arxiv.org/abs/2506.05198)
*Yuxin Wen,Yangsibo Huang,Tom Goldstein,Ravi Kumar,Badih Ghazi,Chiyuan Zhang*

Main category: cs.CV

TL;DR: The paper investigates cross-modality memorization in vision-language models, revealing a gap in information transfer between modalities and proposing a baseline mitigation method.


<details>
  <summary>Details</summary>
Motivation: To understand neural network memorization, especially in multimodal contexts, addressing both unintentional memorization of sensitive data and effective knowledge acquisition.

Method: Introduces a synthetic persona dataset, trains models on one modality, and evaluates cross-modal transferability.

Result: Facts learned in one modality transfer to another, but with a significant gap in recall between source and target modalities.
...
### [232] [Stable Vision Concept Transformers for Medical Diagnosis](https://arxiv.org/abs/2506.05286)
*Lijie Hu,Songning Lai,Yuan Hua,Shu Yang,Jingfeng Zhang,Di Wang*

Main category: cs.CV

TL;DR: The paper introduces Vision Concept Transformer (VCT) and Stable Vision Concept Transformer (SVCT) to enhance explainable AI in medical imaging by combining concept features with image features and ensuring stable explanations under perturbations.
...
Method: Proposes VCT and SVCT, leveraging Vision Transformer (ViT) and a conceptual layer, fusing concept and image features, and using Denoised Diffusion Smoothing for stability.

Result: VCT and SVCT maintain accuracy and interpretability on four medical datasets, with SVCT providing faithful explanations even under perturbations.

Conclusion: SVCT addresses the limitations of CBMs, offering a reliable and interpretable solution for medical AI applications.
...
### [233] [Artificial Intelligence Should Genuinely Support Clinical Reasoning and Decision Making To Bridge the Translational Gap](https://arxiv.org/abs/2506.05030)
*Kacper Sokol,James Fackler,Julia E Vogt*

Main category: cs.HC

TL;DR: The paper critiques AI's limited impact in medicine due to a technology-centric approach and proposes a sociotechnical framework to better align AI tools with clinical practice.
...
Method: The authors propose a sociotechnical conceptualization of AI tools, focusing on complementing doctors' cognitive and epistemic activities rather than outperforming benchmarks.
...
Conclusion: A shift from technology-centric to sociotechnical AI tools is necessary to bridge the translational gap in medicine.
...
### [234] [Leveraging Reward Models for Guiding Code Review Comment Generation](https://arxiv.org/abs/2506.04464)
...
### [235] [SGN-CIRL: Scene Graph-based Navigation with Curriculum, Imitation, and Reinforcement Learning](https://arxiv.org/abs/2506.04505)
*Nikita Oskolkov,Huzhenyu Zhang,Dmitry Makarov,Dmitry Yudin,Aleksandr Panov*

Main category: cs.RO

TL;DR: The paper introduces SGN-CIRL, a framework for mapless robot navigation using 3D scene graphs and reinforcement learning, enhanced by imitation and curriculum learning for better training.


<details>
  <summary>Details</summary>
Motivation: To improve robot navigation in partially observable environments by leveraging 3D scene graphs for efficient target prediction and navigation.

Method: Combines reinforcement learning with 3D scene graphs, using imitation learning (learning from demonstrations) and curriculum learning (gradual task complexity increase).

Result: Experiments in Isaac Sim showed higher success rates in difficult navigation tasks compared to traditional methods.

Conclusion: SGN-CIRL effectively enhances robot navigation using 3D scene graphs and structured training, with open-sourced code for further use.
...
### [236] [Olfactory Inertial Odometry: Sensor Calibration and Drift Compensation](https://arxiv.org/abs/2506.04539)
*Kordel K. France,Ovidiu Daescu,Anirban Paul,Shalini Prasad*

Main category: cs.RO

TL;DR: The paper introduces a calibration process for Olfactory Inertial Odometry (OIO) to improve odor source localization accuracy for slow-moving robots, demonstrated on a robotic arm.


<details>
  <summary>Details</summary>
Motivation: Gas dynamics and environmental disturbances make OIO challenging; the work aims to generalize calibration for various olfaction sensors.

Method: Defines a calibration process for OIO, focusing on centimeter-level accuracy for odor source localization.

Result: Demonstrated improved performance over cold-start olfactory navigation on a robotic arm.

Conclusion: The calibration process enhances OIO performance, with potential applications in robotic surgery and touchless security screening.
...
### [237] [Chronoamperometry with Room-Temperature Ionic Liquids: Sub-Second Inference Techniques](https://arxiv.org/abs/2506.04540)
*Kordel K. France*

Main category: cs.RO

TL;DR: A novel regression method reduces chronoamperometry (CA) measurement time in RTILs to under 1 second, improving speed without hardware changes.


<details>
  <summary>Details</summary>
Motivation: High viscosity and slow mass transport in RTILs extend CA measurement times, limiting efficiency in applications like sensor tech and battery science.

Method: Uses a mathematical regression approach and inference algorithm on transient current response to predict steady-state parameters quickly.

Result: Validated against standard CA, the method maintains accuracy while cutting measurement time from 1-4+ seconds to under 1 second.

Conclusion: This technique enables faster electrochemical quantification, benefiting fields requiring rapid analysis, like sensors and batteries.
...
### [238] [Optical Physics-Based Generative Models](https://arxiv.org/abs/2506.04357)
*Amirreza Ahmadnejad,Somayyeh Koohi*

Main category: physics.optics

TL;DR: A mathematical framework links optical physics to generative AI, showing nonlinear optical models outperform linear ones in efficiency, stability, and performance.


<details>
  <summary>Details</summary>
Motivation: To bridge optical physics and generative AI by leveraging nonlinear optical equations for improved AI models.

Method: Analyzed six optical equations (linear and nonlinear) and applied them to generative models, validating with experiments.

Result: Nonlinear models reduced parameters by 40-60%, improved FID scores (e.g., 0.0089 vs. 1.0909), and cut memory/training time by 30-60%.

Conclusion: The framework connects physical self-organization to AI, enhancing both fields and enabling efficient optical computing.
...
### [239] [Uncertainty quantification and stability of neural operators for prediction of three-dimensional turbulence](https://arxiv.org/abs/2506.04898)
*Xintong Zou,Zhijie Li,Yunpeng Wang,Huiyu Yang,Jianchun Wang*

Main category: physics.flu-dyn

TL;DR: The paper proposes a framework to evaluate neural operator models in turbulent flows, introducing the F-IFNO model for improved long-term stability and accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional turbulence modeling struggles with accuracy and stability, while existing SciML models like FNO fail over long time horizons in 3D turbulence.

Method: The study uses 3D forced HIT as a benchmark, employing UQ, error propagation analysis, and sensitivity tests. Statistical tools like error distribution and ACF are used.

Result: The F-IFNO model outperforms conventional LES and other FNO-based models in accuracy, stability, and efficiency.

Conclusion: The study emphasizes the importance of prediction constraints, time interval selection, and UQ for robust neural operator frameworks in turbulence.
...
### [240] [Limits at infinity for Hajłasz-Sobolev functions in metric spaces](https://arxiv.org/abs/2506.05037)
...
<details>
  <summary>Details</summary>
Motivation: To refine and extend earlier approaches using Hausdorff content, focusing on relative capacity for more precise results.
...
Result: Demonstrates pointwise limits at infinity for these functions outside the exceptional set.

Conclusion: The framework generalizes previous results for Newtonian and fractional Sobolev functions, offering a refined approach.
...
### [241] [Deep Learning-Based Breast Cancer Detection in Mammography: A Multi-Center Validation Study in Thai Population](https://arxiv.org/abs/2506.03177)
...
TL;DR: A deep learning system using a modified EfficientNetV2 with enhanced attention mechanisms for breast cancer detection in mammograms shows high accuracy and clinical acceptance.


<details>
  <summary>Details</summary>
Motivation: To improve breast cancer detection in mammography by leveraging deep learning, enhancing workflow efficiency, and ensuring clinical applicability.

Method: Utilized a modified EfficientNetV2 architecture with enhanced attention mechanisms, trained on mammograms from a Thai medical center, and validated on diverse datasets.

Result: Achieved AUROCs of 0.89, 0.96, and 0.94 across datasets, with strong localization and high concordance with radiologists (83.5%-84.0% for biopsy-confirmed cases). Clinical acceptance was high (96.7% acceptance rate).

Conclusion: The system is effective for mammogram interpretation, with potential to enhance breast cancer screening workflows in clinical practice.
...
### [242] [Gradient Inversion Attacks on Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2506.04453)
*Hasin Us Sami,Swapneel Sen,Amit K. Roy-Chowdhury,Srikanth V. Krishnamurthy,Basak Guler*

Main category: eess.IV

TL;DR: The paper explores privacy risks in federated learning (FL) with parameter-efficient fine-tuning (PEFT), showing how maliciously designed pretrained models can leak user data via gradient inversion attacks on adapters.


<details>
  <summary>Details</summary>
Motivation: To highlight privacy vulnerabilities in FL when using PEFT, where only lightweight modules are fine-tuned, and gradients are shared.

Method: Demonstrates gradient inversion attacks on adapter modules in PEFT, reconstructing user data from accessible gradients.

Result: Shows that fine-tuning images can be retrieved with high fidelity, exposing privacy risks.

Conclusion: Emphasizes the need for privacy-preserving mechanisms in PEFT and suggests future research directions.
...
### [243] [Classification and enumeration of solid-solid phase transition mechanisms](https://arxiv.org/abs/2506.05105)
*Fang-Cheng Wang,Qi-Jun Ye,Yu-Cheng Zhu,Xin-Zheng Li*

Main category: cond-mat.mtrl-sci

TL;DR: A new method, crystmatch, classifies all crystal-structure matches (CSMs) into a tree structure, solving the CSM optimization problem efficiently and revealing new candidates.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to account for all possible CSMs, limiting the understanding of solid-solid phase transitions (SSPTs).

Method: Proposes a formalism to classify CSMs into a tree structure, representing them as integer matrices, and introduces crystmatch for exhaustive enumeration.

Result: Crystmatch reproduces known mechanisms and CSMs quickly, while uncovering thousands of new candidates.

Conclusion: The method provides a comprehensive database for analyzing SSPTs, enabling applications in experimental comparison, energy barrier calculations, and machine learning.
...
### [244] [Generalizable, real-time neural decoding with hybrid state-space models](https://arxiv.org/abs/2506.05320)
*Avery Hee-Woon Ryoo,Nanda H. Krishna,Ximeng Mao,Mehdi Azabou,Eva L. Dyer,Matthew G. Perich,Guillaume Lajoie*

Main category: q-bio.NC

TL;DR: POSSM is a hybrid architecture combining spike tokenization with a recurrent state-space model, offering fast, real-time neural decoding with strong generalization, outperforming traditional and Transformer-based methods in speed and efficiency.


<details>
  <summary>Details</summary>
Motivation: Address the limitations of traditional methods (poor generalization) and Transformer-based approaches (high computational cost) for real-time neural decoding.

Method: POSSM uses cross-attention for spike tokenization and a recurrent state-space model backbone, enabling fast, causal online prediction and efficient generalization through multi-dataset pretraining.

Result: POSSM matches state-of-the-art Transformer accuracy in decoding tasks (monkey motor, human handwriting, speech) while being up to 9x faster on GPU.

Conclusion: Hybrid SSMs like POSSM bridge the gap between accuracy, speed, and generalization, making them ideal for real-time neural decoding applications.
...
### [245] [Non-linear Multi-objective Optimization with Probabilistic Branch and Bound](https://arxiv.org/abs/2506.04554)
*Hao Huang,Zelda B. Zabinsky*

Main category: math.OC

TL;DR: MOPBnB(so) is a simulation optimization algorithm for stochastic multi-objective problems, using single observations and neighboring solutions to estimate objectives, outperforming NSGA-II and being less resource-intensive than multi-replication variants.


<details>
  <summary>Details</summary>
Motivation: To address the computational inefficiency of multi-replication methods in stochastic multi-objective optimization by proposing a single-observation approach.

Method: MOPBnB(so) evaluates noisy functions once per solution, leveraging neighboring solutions for estimation, and includes finite-time and asymptotic convergence analysis.

Result: The algorithm captures the Pareto optimal set with proven convergence, and numerical tests show it outperforms NSGA-II and is less resource-heavy than multi-replication variants.

Conclusion: MOPBnB(so) is an efficient and effective alternative for stochastic multi-objective optimization, balancing accuracy and computational cost.
...
### [246] [Robust Moment Identification for Nonlinear PDEs via a Neural ODE Approach](https://arxiv.org/abs/2506.05245)
*Shaoxuan Chen,Su Yang,Panayotis G. Kevrekidis,Wei Zhu*

Main category: nlin.PS

TL;DR: A Neural ODE-based framework learns reduced-order moment dynamics from PDE systems, handling sparse, irregular data and outperforming traditional methods like SINDy.


<details>
  <summary>Details</summary>
Motivation: Traditional methods like SINDy require dense data and are noise-sensitive. This work aims to robustly learn dynamics from sparse, irregular data.

Method: Uses Neural ODEs to model moment trajectories directly. Introduces Stiefel manifold optimization for data-driven coordinate transformations when closure is unavailable.

Result: Accurately recovers dynamics with closure (e.g., nonlinear Schrödinger equation) and approximates unclosed dynamics (e.g., Fisher-KPP system), outperforming expert-derived models.

Conclusion: Neural ODEs are a robust, flexible tool for learning interpretable, low-dimensional moment dynamics in complex PDE systems, even with limited data.
...
### [247] [GollumFit: An IceCube Open-Source Framework for Binned-Likelihood Neutrino Telescope Analyses](https://arxiv.org/abs/2506.04491)
...
Method: The framework organizes code for binned-likelihood analyses, supports fitting over tens of nuisance parameters, and provides tools for likelihood minimization.

Result: GollumFit efficiently solves likelihood problems and demonstrates performance in typical analysis scenarios.

Conclusion: GollumFit is a unique and time-efficient solution for neutrino telescope data analysis, especially for IceCube.
