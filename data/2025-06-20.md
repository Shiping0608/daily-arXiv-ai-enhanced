...
Method: Proposes a spectral warm-start strategy based on Hessian behavior insights.

Result: Demonstrates faster convergence compared to the reference method in numerical experiments.

Conclusion: The spectral warm-start strategy effectively mitigates slow convergence issues in the Sinkhorn algorithm.
...
<details>
  <summary>Details</summary>
Motivation: Existing shallow water models fail to accurately represent vertical velocity profiles under non-slip boundary conditions due to stiff source terms.

Method: Propose modified shallow water equations and moment-enhanced models with revised source term treatment, ensuring compatibility with hyperbolicity analysis.

Result: Modified models perform well under both non-slip and slip boundary conditions, validated via numerical comparison with Navier-Stokes equations.

Conclusion: The modified models offer improved accuracy and generalization for vertical velocity profiles, addressing limitations of standard approaches.
...
<details>
  <summary>Details</summary>
Motivation: To address complex crack topologies and fault rupture under high pressure using a phase field approach.

Method: Formulate DF-PFM based on elastodynamics, extend with unilateral contact, derive energy dissipation identities, and validate via numerical experiments.

Result: The unilateral contact condition is crucial for accurate shear crack propagation and preventing non-physical interpenetration.

Conclusion: The refined DF-PFM effectively simulates fault rupture under high pressure, ensuring energy consistency and physical accuracy.
...
Result: Efficient computation of gradients for time-dependent and nonlinear systems, supporting diverse equations, geometries, and boundary conditions.

Conclusion: The approach simplifies gradient-based optimization and sensitivity analyses in spectral simulations, demonstrating strong performance and utility.
...
Result: The proposed method outperforms established approaches in accurately approximating solutions in 1D and 2D settings.

Conclusion: The novel method effectively stabilizes and improves accuracy for convection-diffusion problems.
...
<details>
  <summary>Details</summary>
Motivation: To develop an efficient and accurate numerical scheme for solving complex 3D nonlinear elastodynamic sine-Gordon equations under given conditions.
...
Result: The method is temporally second-order and spatially third-order accurate, with validated stability and computational efficiency through numerical examples.

Conclusion: The proposed technique is effective, stable, and practically applicable for solving the target equations.
...
<details>
  <summary>Details</summary>
Motivation: Deep learning struggles with PDEs involving low regularity or singularities, prompting the need for a more robust method.
...
Result: Numerical experiments show WTN effectively handles multiscale features and sharp variations, mitigating non-convexity and ill-conditioning issues.

Conclusion: WTN is a robust and efficient method for solving challenging elliptic PDEs, with potential applicability to other PDE classes.
...
Method: The paper outlines a conceptually simple method to find the optimal Lorentz transformation aligning 4-vectors between two reference frames, with potential extensions to other matrix Lie groups.

Result: The proposed solutions effectively address the alignment problem in Minkowski space, leveraging the properties of Lorentz transformations.

Conclusion: The method provides a practical and extensible solution for alignment problems in Minkowski space and other matrix Lie groups.
...
<details>
  <summary>Details</summary>
Motivation: To provide an effective numerical solution for nonlinear singular BVPs.

Method: Development of a semi-orthogonal Tribonacci wavelet collocation method.

Result: The method offers an effective numerical approach for the specified problem class.

Conclusion: The proposed method is a viable solution for nonlinear singular BVPs.
...
<details>
  <summary>Details</summary>
Motivation: RKPM's integration challenges in FEM software due to Gauss-quadrature limitations motivated the development of interpolation-based methods for easier implementation.

Method: Interpolation-based RKPM represents background basis functions as linear combinations of Lagrange polynomial foreground basis, enabling FEM integration.

Result: The method matches classic RKPM's error convergence rates, solves higher-order PDEs, and reduces computational costs compared to Gauss-quadrature methods.

Conclusion: Interpolation-based RKPM offers a practical, efficient solution for integrating RKPM into existing FEM software while maintaining accuracy.
...
<details>
  <summary>Details</summary>
Motivation: To address elliptic interface problems with a robust and accurate method, avoiding common quasi-regularity constraints.

Method: Constructed on locally anisotropic mixed meshes, fitting interfaces via intersection points on background meshes, with interpolation and consistency error analysis.

Result: Numerical results confirm theoretical convergence rates, showing robustness and accuracy.

Conclusion: The method is effective, with proven convergence and practical performance.
...
<details>
  <summary>Details</summary>
Motivation: To address the need for accurate and flexible numerical methods for solving the 2D advection equation, especially in complex domains and with varying boundary conditions.

Method: Develops fourth-, sixth-, and eighth-order SLFV methods applicable to regular/irregular domains, handling zero/nonzero source terms, and accommodating periodic/incoming penetration conditions uniformly.

Result: Test results confirm high accuracy, flexibility, robustness, and excellent conditioning of the proposed SLFV methods.

Conclusion: The proposed SLFV methods are effective for solving the 2D advection equation, offering high-order convergence and adaptability to diverse conditions.
...
<details>
  <summary>Details</summary>
Motivation: To extend hybrid time-frequency methods to domains with trapping regions, where solutions exhibit both highly oscillatory behavior and slow decay in time.
...
Result: The method successfully extends hybrid schemes to challenging regimes, improving accuracy and efficiency.

Conclusion: The proposed Fourier transform method effectively addresses oscillatory and decay challenges in acoustic scattering problems, particularly in trapping regions.
...
<details>
  <summary>Details</summary>
Motivation: To efficiently identify elastic parameters from a single full-field measurement, addressing challenges like irregularity, anisotropy, and heterogeneity.

Method: Proposes a semi-analytical forward solver (direct method of lines) and formulates the inverse problem as an energy functional minimization with TV regularization, solved using Adam algorithm.
...
Conclusion: The method is effective for elastic parameter identification, offering computational efficiency and robustness.
...
Method: A two-step projection framework: linear projection via Proper Symplectic Decomposition, followed by a nonlinear autoencoder. Reduced dynamics are modeled with a Hamiltonian neural network, trained in two offline stages.

Result: Validation on benchmarks (Landau damping, two-stream instability) shows superior reduction properties compared to standard linear Hamiltonian methods.

Conclusion: The proposed method effectively reduces computational costs while maintaining accuracy and stability, outperforming traditional linear approaches.
...
Method: Decompose the equation into stiff linear and nonstiff nonlinear parts, then use exponential integrators and dynamic low-rank approaches. Supports rank-adaptation.
...
Conclusion: The proposed method is effective for low-rank approximations in stiff matrix differential equations, with demonstrated theoretical and numerical validity.
...
<details>
  <summary>Details</summary>
Motivation: The problem involves estimating the diagonal of a matrix when only a quadratic form oracle is available, a common scenario in large-scale computations.

Method: Proposes a stochastic diagonal estimation method using random Gaussian vectors to query the matrix quadratic form.

Result: Provides element-wise and norm-wise sample complexities, with numerical experiments confirming method effectiveness and theoretical tightness.

Conclusion: The method is effective for diagonal estimation in implicitly given matrices, supported by theoretical and experimental results.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for solving parabolic PDEs via FBSDE formulations require simulating multiple trajectories with small time steps, limiting performance, especially over large time intervals.

Method: The proposed 'shotgun method' avoids simulating full trajectories and instead uses the data distribution of trajectories.

Result: Numerical results show the method's competitiveness in performance and accuracy, even in dimensions up to 10000.

Conclusion: The shotgun method offers a promising alternative to traditional approaches for solving high-dimensional parabolic PDEs.
...
<details>
  <summary>Details</summary>
Motivation: The challenge lies in the singularity of the matrix in SDAEs, making numerical integration difficult. The goal is to develop an efficient method for high-dimensional SDAEs.

Method: A semi-implicit scheme splits the drift term into linear (implicit) and nonlinear (explicit) parts, avoiding nonlinear algebraic equations in constraints.

Result: The scheme achieves pathwise convergence with rate 1/2−ϵ. Numerical simulations confirm its efficiency in high dimensions.

Conclusion: The proposed method effectively handles singularity and scales well for high-dimensional SDAEs, validated by theory and experiments.
...
<details>
  <summary>Details</summary>
Motivation: To improve efficiency in solving the high-frequency Helmholtz equation by relaxing mesh constraints while maintaining accuracy, especially in non-trapping and PML regions.

Method: Uses non-uniform meshes and analyzes how varying meshwidth affects errors, leveraging properties of billiard trajectories and duality arguments.

Result: Demonstrates QO and BRE can be achieved with coarser meshes in certain regions, eliminating pollution in the PML.

Conclusion: Non-uniform meshes offer a practical way to reduce computational cost without sacrificing accuracy, with implications for scattering problems.
...
Result: Produces significantly tighter upper bounds compared to standard methods like the power method, validated in synthetic and real-world settings.

Conclusion: The Counterbalance estimator is effective for spectral norm estimation, especially for matrices with fast-decaying spectra.
...
<details>
  <summary>Details</summary>
Motivation: To analyze and improve stability results for viscoelastic plate problems with boundary damping and memory effects.

Method: Investigates solutions under conditions where the relaxation function decays nonlinearly, using a nonincreasing positive function.

Result: Extends and improves earlier stability results for such plate problems.

Conclusion: The work advances understanding of stability in viscoelastic plates with complex damping and source terms.
...
<details>
  <summary>Details</summary>
Motivation: To generalize and refine existing regularity results for solutions of Poisson and elliptic equations, particularly in Sobolev-Lorentz and Besov spaces, for bounded domains.

Method: The authors analyze weak and very weak solutions of Poisson and elliptic equations, using Sobolev-Lorentz and Besov spaces to derive optimal regularity properties.

Result: Solutions exhibit optimal regularity in Sobolev-Lorentz and Besov spaces, with specific bounds for different orders of regularity.

Conclusion: The results extend and refine the understanding of regularity for solutions of elliptic equations, providing optimal bounds in various function spaces.
...
<details>
  <summary>Details</summary>
Motivation: To address optimal transport problems where bulk transport interacts with dynamic transport on embedded metric graphs, ensuring solutions exist even for varying graph topologies.

Method: Proves existence for fixed graphs, then extends to star-shaped graphs with a penalty to prevent edge overlap, preserving topology.

Result: Existence of minimizers is demonstrated for both fixed and star-shaped graphs with topology constraints.

Conclusion: The approach successfully ensures solution existence while maintaining graph topology, leveraging variational techniques.
...
<details>
  <summary>Details</summary>
Motivation: To address the existence of solutions for a specific system of quadratic integral equations in higher-dimensional spaces.

Method: Fixed point technique to demonstrate the existence of perturbed solutions.

Result: Proves the existence of solutions in H^2(R^d,R^N) for d = 2, 3.

Conclusion: The fixed point method successfully establishes solution existence for the given system.
...
<details>
  <summary>Details</summary>
Motivation: To explain observed spatial segregation of mosquito species in tropical regions, where Aedes aegypti dominates urban areas and Aedes albopictus thrives in forests.

Method: A mathematical model using reaction-diffusion equations for aquatic and aerial phases in a heterogeneous environment, with strong competition in the larval stage.

Result: Established conditions preventing species invasion in homogeneous environments and demonstrated spatial segregation in heterogeneous environments, supported by numerical simulations.

Conclusion: The model successfully explains spatial segregation of mosquito species due to competition and environmental heterogeneity.
...
<details>
  <summary>Details</summary>
Motivation: To understand frequency cascades in forced nonlinear Schrödinger equations with a perturbed Gaussian well, simplifying the problem by discarding derivatives initially.

Method: Algebraic computations are used to derive explicit frequency cascade solutions when derivatives are ignored. Stability is then analyzed when derivatives are included.

Result: The initial algebraic solution remains stable when derivatives are incorporated, even over long time intervals. Numerical simulations validate the findings.

Conclusion: The approach successfully demonstrates frequency cascades and stability, with numerical evidence supporting the theoretical analysis.
...
Conclusion: Boundary vorticity significantly impacts enstrophy dynamics, with implications for both Stokes and Navier-Stokes systems.
...
<details>
  <summary>Details</summary>
Motivation: To generalize existing results for evolutionary PDEs in measure-theoretic settings to cases involving singular flux measures and bounded variation curves.

Method: Analyzes the relation between BV curves and solutions to continuity equations, introduces an auxiliary continuity equation in an augmented phase space, and derives probabilistic representations.

Result: A probabilistic representation of solutions is derived, involving Lipschitz trajectories and a superposition principle for BV curves.

Conclusion: The work provides a framework for studying measure-valued solutions with singular fluxes and extends the superposition principle to BV curves.
...
<details>
  <summary>Details</summary>
Motivation: To extend Krasnoschok's results for the time-fractional diffusion equation to the more complex multi-term case, which lacks standard scaling properties.
...
Result: Key findings include the jump relation and continuity of the double-layer potential, essential for boundary integral equations in time-dependent domains.

Conclusion: The results provide a foundation for analyzing boundary integral equations related to the MTFDE in dynamic settings.
...
<details>
  <summary>Details</summary>
Motivation: To improve understanding of energy dissipation in weak solutions of Euler equations and extend results below the Onsager critical exponent.
...
Result: Existence of dissipative weak solutions with dense initial data in $B^{\gamma}_{\infty,r<\infty}$ and applicability in dimensions $d \geq 2$.

Conclusion: The framework advances the construction of dissipative solutions and energy modulation techniques in fluid dynamics.
...
TL;DR: Existence and uniqueness of solutions for linear X-elliptic equations with L¹ data and zero Dirichlet boundary conditions, with continuous dependence on data and improved summability for solutions.


<details>
  <summary>Details</summary>
Motivation: To extend results for uniformly elliptic equations to the X-elliptic case, ensuring solutions behave similarly under improved data conditions.

Method: Proving existence, uniqueness, and continuous dependence for solutions, and analyzing summability improvements.

Result: Solutions exist, are unique, depend continuously on data, and show improved summability with better data.

Conclusion: The X-elliptic case mirrors uniformly elliptic behavior, with solutions robustly responding to data quality.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the unresolved problem of global existence and uniqueness of smooth solutions for the CNS system under large initial data.
...
Result: Established global existence and uniqueness of strong, classical, and smooth solutions.

Conclusion: The intrinsic entropy structure and parabolic nature of the CNS system enable rigorous control of regularity and solution behavior.
...
<details>
  <summary>Details</summary>
Motivation: To understand the behavior of solutions when the deformation term dominates collisions for large times, particularly for collision kernels with γ < 0.

Method: Formal analysis of long-time asymptotics for velocity distribution and explicit asymptotic profile derivation, with a focus on γ ∈ (-1,0).

Result: Explicit asymptotic profile for velocity distribution and discussion of behavior for γ < -1. Probabilistic interpretation of the process as a Markov combination of shear flows and collisions.

Conclusion: The study provides insights into the long-term behavior of solutions in hyperbolic-dominated regimes and offers a stochastic process framework for understanding the dynamics.
...
Result: Local well-posedness in $H^s(\mathbb{R} \times \mathbb{T})$ for all $s > \frac{3}{8}$ is achieved.

Conclusion: The modified Zakharov-Kuznetsov equation is well-posed for a broader range of $s$ values.
...
<details>
  <summary>Details</summary>
Motivation: To understand and classify positive solutions, including those with infinite energy, for these critical equations.

Method: Rigidity analysis of solutions, leveraging the Euler-Lagrange framework of Caffarelli-Kohn-Nirenberg inequalities.

Result: Classification of positive solutions, particularly for intrinsic dimensions 2<n<4, even with infinite energy.

Conclusion: The study provides a comprehensive classification of solutions, advancing understanding of these critical equations.
...
<details>
  <summary>Details</summary>
Motivation: Traditional methods like first-principles calculations are computationally expensive, and empirical potentials lack accuracy, making MLIPs a promising solution for studying twisted multilayer graphene.

Method: An Atomic Cluster Expansion (ACE) potential is developed, with datasets incorporating all twist angles and local stacking. Configurations are generated with periodic boundary conditions, refined via active learning and Bayesian uncertainty.

Result: The model achieves high accuracy and robustness, validated through numerical tests.

Conclusion: The developed MLIP is a viable tool for simulating twisted multilayer graphene, addressing computational and accuracy challenges.
...
<details>
  <summary>Details</summary>
Motivation: The study aims to explore hBN's suitability for plasma-facing components in centrifugal mirror fusion devices, addressing gaps in understanding its performance under intense plasma conditions.

Method: Computational modeling (RustBCA and OpenMC) was used to predict hBN's performance. Material coupons were tested in PISCES-A and CMFX, with a load-locked sample feedthrough for exposure.

Result: hBN showed better performance than silicon carbide, with less erosion from sputtering and grain ejection.

Conclusion: hBN is a promising material for plasma-facing components in centrifugal mirror fusion devices due to its resilience under plasma exposure.
...
<details>
  <summary>Details</summary>
Motivation: To determine the appropriate reference frame for axisymmetry in tokamak models with small 3D fields, as predictions vary based on the chosen frame (e.g., TF coil vs. PF coil centroid).

Method: Uses fully 3D equilibria from VMEC to analyze the correct reference frame under n=1 coil asymmetries, examining cases for SPARC and NSTX-U by studying the shifted magnetic axis and magnetic field line displacement.

Result: For SPARC, the TF coil set centroid approximates the correct frame; for NSTX-U, it's the radial location of the TF coil inboard legs. Linear MHD theory is valid for current tolerances but may falter for future devices with tolerances >1% of the minor radius.
...
<details>
  <summary>Details</summary>
Motivation: To accurately model frequency-dependent photo-absorption in plasmas, both real and imaginary parts of AC conductivity are needed. The imaginary part is often derived using complex integration, but alternate forms of Kramers-Kronig relations are explored for efficiency.

Method: Two methods are proposed: decomposition into simple elements and complex integration in a quarter of the complex plane, using an alternate Kramers-Kronig relation with ω'²−ω² in the denominator.

Result: The paper provides two derivations for the imaginary part of conductivity, validating the use of alternate Kramers-Kronig relations.

Conclusion: The alternate Kramers-Kronig relations are viable for determining the imaginary part of conductivity, offering flexibility in computational approaches.
...
<details>
  <summary>Details</summary>
Motivation: To improve the Super Transition Arrays approach for opacity calculations by extending the Hazak-Kurzweil method.

Method: Uses Fourier transform representation, linearity of transition energy, and factorization of configuration probabilities.

Result: Alternative expressions for bound-bound opacity, suitable for numerical implementation.

Conclusion: The extended method offers a practical alternative for opacity calculations.
...
<details>
  <summary>Details</summary>
Motivation: To understand and compare the accuracy and applicability of SPA and RPT in modeling nonlinear ion-acoustic waves in dusty plasmas.

Method: Two theoretical methods are applied: SPA (incorporating all nonlinearities) and RPT (yielding the Gardner equation at second order). Soliton profiles are derived numerically (SPA) and analytically (RPT).

Result: SPA provides more accurate soliton profiles but requires numerical integration, while RPT offers an analytic solution via the Gardner equation. The methods are compared for consistency.

Conclusion: SPA is more accurate for nonlinear analysis, while RPT provides a simpler analytic solution under specific conditions. The choice depends on the trade-off between accuracy and computational simplicity.
...
<details>
  <summary>Details</summary>
Motivation: To explore efficient electron acceleration and beam control using high-power Laguerre-Gaussian lasers in micro-plasma waveguides for applications in fundamental science and technology.

Method: 3D particle-in-cell simulations of electron acceleration by a Laguerre-Gaussian laser in a micro-plasma waveguide, focusing on the effects of laser polarization and waveguide modes.

Result: Demonstrated production of high-energy, high-charge, ultrashort electron beams with helical micro-bunching, controllable via laser spin and orbital angular momentum.

Conclusion: This method enables generation of relativistic electron beams with controlled helicity, promising for advancements in science and applications.
...
<details>
  <summary>Details</summary>
Motivation: To explore experimental analogues for cosmological phenomena, providing a new way to study universe dynamics in a controlled lab environment.

Method: Experimental evolution of annular, laser-driven plasma shock waves, analyzing their self-interaction and propagation trajectories.

Result: Shock waves exhibit behaviors analogous to cosmological models, including Hubble-like dynamics and perturbations resembling gravitational effects.

Conclusion: This approach offers a novel experimental method for simulating cosmological scenarios and gravitational waves at macroscopic scales.
...
<details>
  <summary>Details</summary>
Motivation: Traditional KD methods fine-tune pre-trained NNPs, increasing energy barriers and limiting high-energy structure exploration, which is crucial for stable MD simulations.

Method: Proposes a two-stage training: first, train the student NNP with data from an off-the-shelf teacher; then fine-tune with a smaller DFT dataset.

Result: Achieves comparable or superior accuracy in reproducing physical properties for organic and inorganic materials, reducing DFT calculations by 10x.

Conclusion: The framework efficiently generates accurate NNPs for MD simulations while minimizing costly DFT computations.
...
<details>
  <summary>Details</summary>
Motivation: Training highly accurate neural networks is difficult due to non-convex optimization and overlooked $L^{\infty}$ error in traditional MSE-focused approaches.

Method: Uses a staged training technique to refine neural networks by sequentially learning prediction residuals, optimizing loss functions, parameters, and adaptive data sampling.
...
<details>
  <summary>Details</summary>
Motivation: Existing MOF databases have high error rates, hindering computational screening. Rule-based methods are limited and misclassify structures.

Method: MOFClassifier uses a PU-CGCNN model to learn from perfect structures and predict a crystal-likeness score (CLscore).

Result: Achieves ROC value of 0.979 (previous best 0.912), identifies subtle errors, and recovers misclassified structures.

Conclusion: MOFClassifier enhances MOF database quality, aiding large-scale computational screening and material discovery.
...
<details>
  <summary>Details</summary>
Motivation: To enhance the applicability of MLFFs across diverse chemical systems by effectively leveraging data from multiple quantum-chemical methods.

Method: Systematic investigation of pre-training/fine-tuning and multi-headed training strategies, analyzing their mechanisms and effectiveness.

Result: Pre-training requires backbone adaptation for fine-tuning, while multi-headed models learn method-agnostic representations, enabling accurate predictions across multiple label sources with slight accuracy trade-offs.

Conclusion: Multi-headed training offers a cost-efficient, extensible alternative to sequential fine-tuning, advancing the development of universal MLFFs.
...
<details>
  <summary>Details</summary>
Motivation: Understanding CO's influence on Titan's atmospheric chemistry and organic matter synthesis, given its abundance and potential impact on haze layers.

Method: Simulated Titan's atmosphere with N2/CH4 mixtures and varying CO ratios (0.2%-9%), exposed to ECR plasma. Analyzed gas-phase and solid products using spectroscopy and mass spectrometry.

Result: CO enriches the chemical system, supplying oxygen and enhancing nitrogen reactivity, leading to more diverse and abundant organic products.

Conclusion: CO significantly impacts Titan's atmospheric chemistry, enhancing organic synthesis and haze formation complexity.
...
<details>
  <summary>Details</summary>
Motivation: The open magnetic field of the solar corona influences solar wind and coronal mass ejections, but analyzing it is challenging due to low signal-to-noise ratios in coronagraph images.

Method: QRaFT is a segmentation methodology tested on synthetic coronagraph images from a 3D magnetohydrodynamic model and applied to real-life coronal images.

Result: The extracted features align within ~4-7 degrees with the local magnetic field, demonstrating the method's accuracy.

Conclusion: QRaFT provides valuable empirical data on the corona's morphology, enhancing coronal and solar wind models and space weather forecasts.
...
Method: Proposes κ as a measure, analyzes its saturation under optimal conditions, and develops a reinforcement learning framework for circuit design.

Result: κ can saturate its theoretical upper bound, and reinforcement learning effectively optimizes circuit expressivity.

Conclusion: κ is a robust tool for QNN expressivity, and reinforcement learning aids in designing high-performance quantum circuits.
...
<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of randomised LCU algorithms, which are restricted to estimating single Pauli operator expectations due to unphysical state forms, and to enable broader applications like Hamiltonian simulation and eigenstate preparation.

Method: Analyzes randomised LCU, introduces a quantum instrument for non-completely-positive maps, constructs unbiased estimators for unphysical states, and demonstrates efficient realisation of composite LCU operators.

Result: Establishes a connection between randomised LCU and shadow tomography, enabling simultaneous estimation of many observables, and provides estimators and simulation complexity for Hamiltonian simulation and eigenstate tasks.

Conclusion: The work advances randomised LCU techniques by addressing key limitations and linking them to shadow tomography, broadening their applicability in quantum simulations.
...
<details>
  <summary>Details</summary>
Motivation: To address the computational challenges of analyzing low-frequency vibrations in large biomolecules using FRESEAN mode analysis.

Method: Combines coarse-graining of all-atom simulation trajectories with FRESEAN mode analysis to extract low-frequency vibrational information efficiently.

Result: Demonstrates that coarse-graining reduces computational costs while retaining essential vibrational information.

Conclusion: Coarse-graining enhances the practicality of FRESEAN mode analysis for studying large biomolecules.
...
<details>
  <summary>Details</summary>
Motivation: Understand the emergence of spatial inhomogeneities (clusters) in dense granular gases under microgravity, driven by energy influx and dissipation.

Method: Use DEM simulations for frictional spheres in a cuboid container, apply Kolmogorov-Smirnov test and caging criterion to detect clusters. Test machine learning models to predict clustering.
...
Conclusion: Machine learning is a promising tool for studying and predicting gas-cluster transitions in granular gases under specific conditions.
...
<details>
  <summary>Details</summary>
Motivation: To improve the accuracy and efficiency of predicting electrophysiological activity in the myocardium using PINNs by determining optimal training hyperparameters.

Method: Employed a fully-connected neural network (FCNN) with governing PDEs in strong form, scaled inputs, and finite element-generated training data. Studied loss weighting effects.

Result: Trained models predicted action potential and recovery variable fields, compared with FE simulations, and analyzed loss control during training.

Conclusion: The study provides insights into optimizing PINN training for cardiac electrophysiology, highlighting the importance of loss weighting and consistent scaling.
...
<details>
  <summary>Details</summary>
Motivation: To improve approximation accuracy of density and distribution functions while minimizing computational costs.

Method: Proposes algorithms based on Legendre polynomials, solving conditional optimization problems.

Result: Tested on examples with different smoothness levels, demonstrating effectiveness.

Conclusion: The algorithms efficiently balance accuracy and computational cost for density and distribution estimation.
...
<details>
  <summary>Details</summary>
Motivation: To extend kernel-based density estimation to non-periodic distributions on unbounded domains, overcoming limitations of previous methods.

Method: Wraps target densities into periodic versions via modulo operation and applies kernel ridge regression in scaled Korobov spaces.

Result: Achieves MISE convergence rate of O(M^(-1/(1+1/(2α)+ε))) for smooth, exponentially decaying densities, matching prior rates but for broader distributions.

Conclusion: The PSKK method effectively estimates non-periodic densities on unbounded domains, with theoretical and empirical validation.
...
<details>
  <summary>Details</summary>
Motivation: To address stricter derivative bounds in Finsler geometry compared to Riemannian cases, aiming to generalize gradient estimates.

Method: Uses Shi-type and Hamilton-type gradient estimates to relax curvature and derivative constraints.

Result: Demonstrates feasibility of removing strict curvature conditions in Finsler geometric flow.

Conclusion: The proposed estimates generalize gradient bounds, reducing dependency on strict geometric constraints.
...
<details>
  <summary>Details</summary>
Motivation: To extend functional inequalities to a broader class of manifolds by leveraging a Polya-Szego type inequality on noncollapsed RCD(k,n)-spaces.

Method: Rearrangement of functions on noncollapsed RCD(k,n)-spaces and application of Polya-Szego type inequality.

Result: Functional inequalities are proven for a wide class of manifolds, and a characterization of manifolds admitting a Moser-Trudinger type inequality is provided.

Conclusion: The work successfully generalizes functional inequalities and provides a characterization for specific manifolds, advancing the understanding of geometric analysis in this context.
