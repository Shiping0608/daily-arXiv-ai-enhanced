...
Result: The proposed method achieves faster convergence than the reference Sinkhorn algorithm, validated by numerical experiments.
...
Result: Modified models perform well under both non-slip and slip boundary conditions, validated through numerical comparison with incompressible Navier--Stokes equations.
...
<details>
  <summary>Details</summary>
Motivation: To address the need for accurate simulation of complex crack topologies and fault rupture under high pressure in fracture mechanics.

Method: Proposes DF-PFM based on the elastodynamic wave equation, extends it with a unilateral contact condition, and validates it using linear implicit time discretization and finite element methods.
...
Conclusion: The refined DF-PFM framework is effective for simulating fault rupture and crack propagation, ensuring energy consistency and accuracy in high-pressure scenarios.
...
Result: Efficient adjoint solvers for time-dependent and nonlinear systems, supporting diverse equations, geometries, and boundary conditions.

Conclusion: The approach simplifies gradient-based optimization and sensitivity analyses in spectral simulations, demonstrating strong performance and practical utility.
...
<details>
  <summary>Details</summary>
Motivation: Existing point cloud alignment methods rely on Euclidean metrics and don't generalize to the indefinite Minkowski metric.

Method: The paper outlines a conceptually simple method to find the optimal Lorentz transformation aligning 4-vectors between reference frames.

Result: The proposed method works for noisy measurements and extends to alignment problems in other matrix Lie groups.

Conclusion: The approach provides a practical solution for aligning 4-vectors in Minkowski space and has broader applicability.
...
Method: Introduces an auxiliary variable linked to the gradient of the field, forming a coupled problem, with well-posedness conditions established.

Result: The method outperforms established approaches in accurately approximating solutions in 1D and 2D settings.

Conclusion: The proposed method effectively stabilizes and improves accuracy for challenging convection-diffusion problems.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of solving complex 3D nonlinear elastodynamic sine-Gordon equations efficiently and accurately under initial and boundary conditions.
...
Result: The method is temporally second-order and spatially third-order accurate, with confirmed stability and computational efficiency through numerical examples.
...
Method: WTN uses TransNet's neural feature space as the trial space and radial basis functions as the test space, minimizing weak PDE residuals via least squares.

Result: WTN effectively addresses non-convexity and ill-conditioning in neural network training and handles multiscale or sharp-variation solutions.
...
Result: The method is presented as effective for solving the specified class of problems.

Conclusion: The semi-orthogonal tribonacci wavelet collocation method is a viable solution for non-linear singular BVPs.
...
<details>
  <summary>Details</summary>
Motivation: RKPM's integration challenges in FEM software due to inaccurate Gauss-quadrature methods and the need for sophisticated nodal techniques.

Method: Uses interpolation-based methods to represent RKPM basis functions via Lagrange polynomials on a foreground mesh, enabling FEM integration.

Result: Achieves error convergence rates similar to classic RKPM, solves higher-order PDEs, and handles multi-material problems with high accuracy.

Conclusion: Interpolation-based RKPM offers computational savings and easy FEM integration while maintaining accuracy.
...
<details>
  <summary>Details</summary>
Motivation: To address elliptic interface problems with a simpler and more flexible approach, avoiding the quasi-regularity assumption.

Method: Constructed on locally anisotropic mixed meshes by fitting the interface via intersection points on an unfitted background mesh. Includes interpolation error estimates and novel consistency error analysis.

Result: Numerical results confirm theoretical convergence rates, demonstrating robustness and accuracy.

Conclusion: The proposed method is effective for elliptic interface problems, offering improved flexibility and theoretical guarantees.
...
<details>
  <summary>Details</summary>
Motivation: To address the need for efficient and accurate numerical solutions for the 2D advection equation, especially in complex domains and with varying boundary conditions.

Method: Proposes fourth-, sixth-, and eighth-order SLFV methods applicable to regular/irregular domains, handling zero/nonzero source terms, and various boundary conditions uniformly.

Result: Test results confirm high-order convergence, accuracy, flexibility, robustness, and excellent conditioning of the SLFV methods.

Conclusion: The proposed SLFV methods are effective for solving the 2D advection equation, offering versatility and high performance in diverse scenarios.
...
<details>
  <summary>Details</summary>
Motivation: To extend hybrid time-frequency methods to domains with trapping regions, where solutions exhibit both highly oscillatory behavior and slow decay.
...
Result: The method effectively handles highly oscillatory solutions and slow decay in trapping regions.

Conclusion: The proposed Fourier transform method successfully broadens the applicability of hybrid time-frequency schemes in challenging acoustic scattering scenarios.
...
<details>
  <summary>Details</summary>
Motivation: To efficiently identify elastic parameters from a single full-field measurement, addressing challenges like irregularity, anisotropy, and singularities.

Method: Proposes a semi-analytical forward solver (direct method of lines) and formulates the inverse problem as energy minimization with TV regularization, solved using Adam algorithm.
...
Conclusion: The method is effective for elastic parameter identification, leveraging a novel solver and optimization approach.
...
Method: A two-step projection framework: initial linear projection via Proper Symplectic Decomposition, followed by a nonlinear autoencoder neural network. Reduced dynamics are modeled using a Hamiltonian neural network.
...
Method: Decompose the equation into stiff linear and nonstiff nonlinear parts, using exponential integrators and dynamic low-rank approaches, with rank-adaptation.
...
Conclusion: The method is effective for low-rank solutions in stiff matrix differential equations, with theoretical and numerical validation.
...
<details>
  <summary>Details</summary>
Motivation: The study aims to improve the efficiency of the $h$-FEM for high-frequency Helmholtz problems by leveraging non-uniform meshes and understanding the role of billiard trajectories in error behavior.

Method: The analysis uses duality arguments and accounts for the influence of mesh structure and solution behavior on local errors, focusing on non-uniform meshes and their impact on QO and BRE.

Result: The main theorem shows that QO and BRE can be achieved with coarser meshes away from trapping regions and PML, with no pollution in the PML.

Conclusion: Non-uniform meshes can optimize computational efficiency for Helmholtz problems while maintaining accuracy, particularly in the PML.
...
<details>
  <summary>Details</summary>
Motivation: The problem arises when direct access to the matrix is unavailable, but quadratic forms can be queried. Efficient diagonal estimation is needed.

Method: Proposes a stochastic estimation method using random Gaussian vectors to query the matrix quadratic form.

Result: Provides element-wise and norm-wise sample complexities, validated by numerical experiments on various matrices.

Conclusion: The method is effective and theoretical results are tight, as demonstrated by experiments.
...
Method: The proposed deep "shotgun method" avoids simulating full trajectories and instead uses the data distribution of these trajectories.

Result: Numerical results, including examples up to 10000 dimensions, show the method's competitiveness in performance and accuracy.

Conclusion: The shotgun method offers a more efficient and accurate alternative for solving high-dimensional parabolic PDEs compared to traditional trajectory-based approaches.
...
TL;DR: The paper analyzes the intrinsic and extrinsic structure of self-attention in transformers, showing invariance to softmax activation and proposing hierarchical tensor organization for signal processing tasks.


<details>
  <summary>Details</summary>
Motivation: To understand the structure of self-attention mechanisms and leverage it for interpretability and practical applications like model pruning.
...
Conclusion: The findings advance interpretability and enable practical applications like pruning and architecture comparison.
...
<details>
  <summary>Details</summary>
Motivation: The challenge lies in the singularity of the matrix in SDAEs, making numerical integration difficult. The goal is to develop an efficient scheme for high-dimensional problems.

Method: A semi-implicit method splits the drift term into linear (implicit) and nonlinear (explicit) parts, avoiding nonlinear algebraic equations in constraints. A dual scheme is derived for analysis.

Result: The scheme converges pathwise to the exact solution with rate $\frac{1}{2}-\epsilon$. Numerical simulations confirm efficiency in high dimensions.

Conclusion: The proposed scheme effectively handles singularity and is efficient for high-dimensional SDAEs, validated by theory and experiments.
...
Method: Proposes the Counterbalance estimator, leveraging matrix-vector products to derive upper bounds with probabilistic guarantees.
...
Conclusion: The work advances understanding of stability in viscoelastic plate models with complex damping and source terms.
...
<details>
  <summary>Details</summary>
Motivation: To generalize and refine existing regularity results for solutions of elliptic equations, particularly in Sobolev-Lorentz and Besov spaces, for bounded domains.

Method: The authors analyze solutions of Poisson and more general elliptic equations, proving their regularity in Sobolev-Lorentz spaces and embedding these into classical Besov spaces.

Result: For solutions of the Poisson equation, optimal regularity is shown in Sobolev-Lorentz and Besov spaces. Similar results are extended to more general elliptic equations with nonhomogeneous boundary data.

Conclusion: The paper successfully generalizes and refines regularity results for elliptic equations, providing optimal regularity in Sobolev-Lorentz and Besov spaces for bounded domains.
...
<details>
  <summary>Details</summary>
Motivation: To address optimal transport problems in a compact convex set coupled with dynamic transport on embedded metric graphs, ensuring solutions exist for fixed and varying (star-shaped) graphs.

Method: For fixed graphs, standard techniques are used. For varying star-shaped graphs, an additional penalty prevents edge overlap, preserving topology and enabling variational methods.

Result: Existence of solutions is proven for fixed graphs and star-shaped graphs with topology-preserving penalties.

Conclusion: The approach successfully extends optimal transport theory to coupled systems with metric graphs, ensuring solution existence under specified conditions.
...
<details>
  <summary>Details</summary>
Motivation: To address the solvability of quadratic integral equations in specific function spaces.

Method: Fixed point technique applied to the system of equations.

Result: Demonstrates the existence of a perturbed solution.

Conclusion: The fixed point approach successfully proves solution existence for the given system.
...
<details>
  <summary>Details</summary>
Motivation: To understand and model the observed spatial segregation of mosquito species in tropical regions, where Aedes aegypti dominates urban areas and Aedes albopictus thrives in forests.

Method: A reaction-diffusion system is introduced, accounting for aquatic and aerial phases of both species in a heterogeneous environment, with strong competition in the larval stage.

Result: A sufficient condition for preventing species invasion in homogeneous environments is derived, and spatial segregation is demonstrated in heterogeneous environments.

Conclusion: The model successfully explains spatial segregation between mosquito species, supported by theoretical and numerical results.
...
<details>
  <summary>Details</summary>
Motivation: To study frequency cascade phenomena in forced nonlinear Schrödinger equations using a simplified approach.

Method: Algebraic computations discard time and space derivatives to derive explicit frequency cascade; stability analysis includes derivatives.

Result: Initial algebraic solution remains stable when derivatives are included, supported by numerical simulations.

Conclusion: The approach provides insights into frequency cascade stability, validated numerically.
...
Result: Proved enstrophy dissipativity for Stokes system; derived new enstrophy dynamics equation for Navier-Stokes system.

Conclusion: Boundary vorticity significantly influences enstrophy dynamics, with distinct behaviors in Stokes and Navier-Stokes systems.
...
<details>
  <summary>Details</summary>
Motivation: To address the complex structure of the fundamental solution in the multi-term case, which lacks standard scaling properties, and extend prior work by Krasnoschok.

Method: Establishes jump relations for the double-layer potential in inhomogeneous MTFDE and proves continuity for the homogeneous case.

Result: Key findings include the jump relation and continuity proof, crucial for boundary integral equations in time-dependent domains.

Conclusion: The results advance the analysis of MTFDE in dynamic settings, providing foundational tools for boundary integral methods.
...
<details>
  <summary>Details</summary>
Motivation: To address the existence of dissipative weak solutions below the Onsager critical exponent and refine previous results by Giri and Radu.
...
Result: Existence of dissipative weak solutions in any dimension $d \geq 2$ and density of initial data in $B^{\gamma}_{\infty,r<\infty}$.

Conclusion: The work advances understanding of energy dissipation in Euler equations and provides a robust framework for constructing solutions below critical exponents.
...
<details>
  <summary>Details</summary>
Motivation: To generalize results for uniformly elliptic equations to X-elliptic equations, ensuring robust solution properties under weaker data conditions.

Method: Analyze linear X-elliptic equations with L¹ data and zero Dirichlet boundary conditions, focusing on solution dependence and summability improvements.

Result: Solutions exist, are unique, and depend continuously on data. Improved data summability leads to better solution summability.

Conclusion: The study extends classical elliptic results to X-elliptic cases, demonstrating analogous behavior under weaker assumptions.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the unresolved issue of global existence and uniqueness of smooth solutions for the CNS system under large initial data.
...
Conclusion: The intrinsic entropy structure and parabolic nature of the CNS system enable rigorous control of regularity and solution smoothness.
...
Method: Formal analysis of long-time asymptotics for velocity distributions, focusing on collision kernels with γ ∈ (-1,0). A probabilistic interpretation is also provided, modeling the process as a combination of shear flows and collisions.

Result: Explicit form of the asymptotic profile for velocity distributions is derived. Different asymptotic behaviors are identified for γ < -1. A Markov process model for tagged particle velocity is introduced.

Conclusion: The study clarifies the long-time asymptotics in the hyperbolic-dominated regime and provides a stochastic framework for understanding the combined effects of shear flows and collisions.
...
Conclusion: The modified Zakharov-Kuznetsov equation is locally well-posed under the improved conditions.
...
TL;DR: The paper analyzes singular/degenerate semilinear critical equations from Caffarelli-Kohn-Nirenberg inequalities in ℝᵈ (d≥2), proving rigidity results for positive solutions, including classification for infinite-energy solutions when 2<n<4.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the behavior of solutions to these critical equations, particularly focusing on rigidity and classification of positive solutions, even with infinite energy.

Method: The paper uses mathematical analysis techniques to classify and prove rigidity results for positive solutions of the equations.

Result: Key findings include the classification of solutions, especially those with infinite energy, when the intrinsic dimension n is between 2 and 4.

Conclusion: The work provides a comprehensive classification of solutions and establishes rigidity results, contributing to the understanding of these critical equations.
...
<details>
  <summary>Details</summary>
Motivation: Twisted multilayer graphene is a promising platform for quantum phenomena, but accurate simulations are challenging. MLIPs offer a balance between accuracy and computational cost, yet their application in twisted systems is limited.

Method: The authors develop an ACE potential, create datasets incorporating all twist angles and stacking configurations, and refine them using active learning with Bayesian uncertainty.

Result: The model is validated for accuracy and robustness across various numerical tests.

Conclusion: The ACE potential provides a reliable tool for simulating twisted multilayer graphene, bridging the gap in MLIP applications for such systems.
...
<details>
  <summary>Details</summary>
Motivation: To investigate hBN's suitability for plasma-facing components in centrifugal mirrors, given its thermal and electrical properties, and its unexplored performance under fusion-relevant conditions.

Method: Computational modeling (RustBCA, OpenMC) and experimental plasma exposure tests (PISCES-A, CMFX) were conducted to evaluate hBN's performance.

Result: hBN showed better resistance to erosion (sputtering and grain ejection) than silicon carbide under plasma exposure.

Conclusion: hBN is a promising material for plasma-facing components in centrifugal mirror fusion devices due to its durability and insulating properties.
...
<details>
  <summary>Details</summary>
Motivation: To determine the appropriate reference frame for axisymmetry in tokamak models with small 3D fields, as assumptions (e.g., TF vs. PF coil centroid) significantly impact predictions.

Method: Uses fully 3D equilibria from VMEC to analyze the correct frame under n=1 coil asymmetries, examining SPARC and NSTX-U cases via shifted magnetic axis and field line displacement.

Result: For SPARC, the TF coil centroid approximates the correct frame; for NSTX-U, it's the TF coil inboard legs' radial location. Linear MHD theory holds for current tolerances but may falter for future devices with larger tolerances.

Conclusion: The findings enable confident use of 3D perturbative models for assembly tolerances, clarifying theory application while highlighting caution for future devices.
...
<details>
  <summary>Details</summary>
Motivation: To accurately model frequency-dependent photo-absorption in plasmas, both real and imaginary parts of AC conductivity are needed. Existing methods rely on standard Kramers-Kronig relations, but alternate forms are also useful.

Method: Two approaches are proposed: decomposition into simple elements and complex integration in a quarter of the complex plane, using an alternate Kramers-Kronig relation with ω'²−ω² in the denominator.

Result: The paper provides two distinct methods to derive the imaginary part of conductivity, expanding the toolkit for plasma physics calculations.

Conclusion: The alternate Kramers-Kronig relations offer practical alternatives for determining the imaginary part of conductivity, enhancing flexibility in plasma modeling.
...
<details>
  <summary>Details</summary>
Motivation: To improve the Super Transition Arrays approach for bound-bound opacity calculations by extending the Hazak-Kurzweil method.

Method: Based on Fourier transform representation, linearity of transition energy, and factorization of configuration probabilities. Extended for Detailed Configuration Accounting.

Result: Alternative expressions for bound-bound opacity, convenient for numerical implementation.

Conclusion: The extended method provides a practical and efficient approach for opacity calculations.
...
<details>
  <summary>Details</summary>
Motivation: To understand and compare the behavior of ion-acoustic waves in dusty plasmas using two theoretical methods, SPA and RPT, for soliton solutions.

Method: SPA (incorporating all nonlinearities) and RPT (yielding the Gardner equation at second order) are applied. SPA requires numerical integration, while RPT provides an analytic soliton profile.

Result: Soliton solutions from both methods are analyzed and compared, with RPT requiring specific conditions for consistency.

Conclusion: The study highlights the trade-offs between accuracy (SPA) and analytical tractability (RPT) in modeling ion-acoustic waves in dusty plasmas.
...
Method: 3D particle-in-cell simulations were used to study electron acceleration by a Laguerre-Gaussian pulse in a micro-plasma waveguide, focusing on the effects of polarization and angular momentum.

Result: Simulations show production of ~100 MeV electron beams with ~10 nC charge, ~30 fs duration, and ~1 deg divergence, with helical micro-bunching controlled by laser properties.

Conclusion: This method enables generation of high-charge, relativistic electron beams with controlled helicity, offering significant potential for scientific and technological advancements.
...
<details>
  <summary>Details</summary>
Motivation: To explore analogies between plasma shock waves and cosmological universe dynamics for simulating cosmological scenarios in the lab.

Method: Studying the evolution of a laser-driven annular plasma shock wave and its self-interaction, comparing it to $w$CDM cosmologies and Hubble-like dynamics.

Result: Shock wave behavior mirrors cosmological models, including dark energy scenarios, and perturbations resemble gravitational effects in a matter-dominated universe.

Conclusion: This approach provides a novel experimental method for simulating cosmological models and gravitational waves at macroscopic scales.
...
<details>
  <summary>Details</summary>
Motivation: Existing kernel methods for density estimation require periodicity, limiting their applicability. The PSKK method addresses this by wrapping the density into a periodic version, enabling broader use.

Method: The PSKK method wraps the target density into a periodic form via modulo operation and applies kernel ridge regression in scaled Korobov spaces.

Result: The method achieves a MISE convergence rate of O(M^(-1/(1+1/(2α)+ε))) for smooth, exponentially decaying densities, matching prior work but for non-periodic distributions. Numerical experiments show superior performance.

Conclusion: The PSKK method effectively extends kernel-based density estimation to non-periodic densities on unbounded domains, with theoretical and empirical validation.
...
<details>
  <summary>Details</summary>
Motivation: To address stricter derivative bounds in Finsler geometric flows compared to Riemannian cases, aiming to relax these constraints.

Method: Uses Shi-type and Hamilton-type gradient estimates to analyze solutions to the parabolic equation under Finsler $CD(-K,N)$ flow.

Result: Demonstrates the feasibility of removing strict curvature conditions on derivative bounds in the geometric flow.

Conclusion: The proposed gradient estimates offer a way to relax restrictive conditions in Finsler geometric flows, expanding applicability.
...
<details>
  <summary>Details</summary>
Motivation: To extend functional inequalities, particularly Polya-Szego type inequalities, to a broader class of manifolds and characterize those with specific curvature bounds.

Method: Rearrangement of functions on noncollapsed RCD(k,n)-spaces and application of Polya-Szego type inequalities.

Result: Functional inequalities are proven, and a characterization of manifolds with lower bounded Ricci curvature admitting Moser-Trudinger type inequalities is provided.

Conclusion: The work successfully extends functional inequalities to a wider class of manifolds and characterizes specific curvature conditions for Moser-Trudinger type inequalities.
...
<details>
  <summary>Details</summary>
Motivation: To improve approximation accuracy of density and distribution functions while minimizing computational effort.

Method: Joint estimation using Legendre polynomials and conditional optimization.

Result: Tested on examples with varying smoothness, showing effectiveness.
...
<details>
  <summary>Details</summary>
Motivation: To study intrinsically anharmonic low-frequency vibrations in proteins, which provide insights into slow conformational transitions, without relying on harmonic approximations.

Method: FRESEAN mode analysis uses time correlation functions to isolate low-frequency vibrations. Coarse-graining of all-atom trajectories is introduced to reduce computational demands.

Result: Low-frequency vibrations from FRESEAN are effective collective variables for enhanced sampling. Coarse-graining enables efficient extraction of these vibrations.

Conclusion: Combining FRESEAN with coarse-graining offers a computationally efficient way to study low-frequency protein vibrations and conformational transitions.
...
<details>
  <summary>Details</summary>
Motivation: To understand and predict the gas-cluster transition in dense granular gases under microgravity, influenced by particle density, container geometry, and excitation.

Method: Discrete Element Method (DEM) simulations for frictional spheres, using Kolmogorov-Smirnov test and caging criterion. Machine learning models are also tested.
...
Conclusion: Machine learning provides a promising tool for predicting gas-cluster transitions, reducing reliance on time-consuming simulations.
...
<details>
  <summary>Details</summary>
Motivation: Understanding the solar corona's magnetic geometry is crucial for solar wind dynamics and space weather forecasting, but low signal-to-noise ratios in coronagraph images make analysis challenging.

Method: The Quasi-Radial Field-line Tracing (QRaFT) method segments coronagraph images to detect field-aligned features, tested on synthetic images from a 3D magnetohydrodynamic model and real-life coronal images.

Result: QRaFT-detected features align within ~4-7 degrees with the local magnetic field in synthetic tests and perform well on real-life images.

Conclusion: QRaFT provides valuable empirical data on the corona's morphology, potentially enhancing coronal, solar wind models, and space weather forecasts.
...
<details>
  <summary>Details</summary>
Motivation: Traditional KD methods increase energy barriers, limiting high-energy structure data. A gentler teacher model is needed for stable MD simulations.

Method: Two-stage training: student NNP trained with off-the-shelf teacher data, then fine-tuned with a small DFT dataset.

Result: Achieves comparable/superior accuracy for organic and inorganic materials, reducing DFT calculations by 10x.

Conclusion: The framework efficiently generates accurate NNPs with fewer DFT calculations, benefiting material property simulations.
...
<details>
  <summary>Details</summary>
Motivation: Training highly accurate neural networks is difficult due to non-convex optimization, hyperparameter tuning, and the neglect of $L^{\infty}$ error in favor of MSE.

Method: HiPreNets refine a staged training technique by sequentially learning prediction residuals with additional networks, optimizing loss functions, parameters, and adaptive data sampling.

Result: The framework is validated on benchmark problems, demonstrating improved accuracy.

Conclusion: HiPreNets effectively address training challenges and enhance neural network precision.
...
### [55] [Learning to Maximize Quantum Neural Network Expressivity via Effective Rank](https://arxiv.org/abs/2506.15375)
*Juan Yao*
...
<details>
  <summary>Details</summary>
Motivation: Accurately characterizing QNN expressivity is crucial for optimal quantum circuit design, but it remains an open challenge.

Method: The authors propose the effective rank ($\kappa$) as a measure, analyze its saturation under optimal conditions, and develop a reinforcement learning framework for automated circuit design.

Result: $\kappa$ can saturate its theoretical upper bound ($d_n=4^n-1$) for an $n$-qubit system under optimal conditions, and reinforcement learning effectively designs expressive circuits.

Conclusion: The study establishes $\kappa$ as a robust tool for QNN expressivity and demonstrates the potential of reinforcement learning in quantum circuit design, advancing quantum machine learning.
...
<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of randomised LCU algorithms, particularly the inability to deterministically prepare states and restricted applicability to single Pauli operator estimation.

Method: Analyzes randomised LCU, introduces a quantum instrument for non-completely-positive maps, constructs unbiased estimators for unphysical states, and demonstrates composite LCU realisation.

Result: Establishes a connection between randomised LCU and shadow tomography, enabling efficient multi-observable estimation. Provides estimators and simulation complexity for Hamiltonian simulation and eigenstate preparation.

Conclusion: Randomised LCU algorithms, enhanced by the proposed quantum instrument and estimators, offer broader applicability and efficiency in quantum simulations, linking to shadow tomography for practical use cases.
...
Method: Uses a fully-connected neural network (FCNN) with scaled governing PDEs, finite element-generated training data, and weighted losses to study training control.

Result: Trained models predict action potential and recovery variables, with results compared to finite element simulations.

Conclusion: Optimal hyperparameters and loss-weighting methods enhance PINN training and prediction accuracy for cardiac electrophysiology.
...
Method: MOFClassifier uses a PU-CGCNN model to learn from perfect crystal structures and predict CLscores, classifying MOFs as computation-ready.

Result: Achieves a ROC value of 0.979, identifies subtle errors, and recovers misclassified structures, reducing false negatives.

Conclusion: MOFClassifier enhances MOF database quality, aiding high-throughput screening and accelerating material discovery.
...
<details>
  <summary>Details</summary>
Motivation: To improve MLFFs by effectively leveraging data from multiple quantum-chemical methods for broader applicability.

Method: Systematically investigates pre-training/fine-tuning and multi-headed training, analyzing their mechanisms and performance.

Result: Pre-training requires backbone adaptation, while multi-headed models learn method-agnostic representations, enabling cost-efficient data generation.

Conclusion: Multi-headed training, though slightly less accurate, offers extensibility and paves the way for universal MLFFs.
...
<details>
  <summary>Details</summary>
Motivation: Understanding CO's role in Titan's atmospheric chemistry and organic matter synthesis.

Method: Lab simulations with CO in N2/CH4 mixtures, exposed to ECR plasma, analyzed via spectroscopy and mass spectrometry.

Result: CO increases organic product complexity, oxygen supply, and nitrogen reactivity.

Conclusion: CO significantly impacts Titan's atmospheric chemistry, enhancing organic synthesis.
