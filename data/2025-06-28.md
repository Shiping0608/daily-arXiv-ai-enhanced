...
Motivation: To recover the initial electric field in an inhomogeneous, anisotropic medium without needing initial magnetic field or charge density data, addressing practical constraints.

Method: A time-dimension reduction approach projects the electric field onto a Legendre polynomial-exponential basis, reformulating the problem into spatial systems. The quasi-reversibility method is used for reconstruction.

Result: Numerical experiments show accurate reconstruction of the initial electric field even with 10% noise, proving the method's robustness.

Conclusion: The proposed approach effectively solves the inverse problem, demonstrating applicability to realistic scenarios with noisy data.
...
<details>
  <summary>Details</summary>
Motivation: To efficiently and accurately evaluate BIOs for dense, polydisperse suspensions of spheroids, addressing challenges in particulate suspension flows.

Method: Derives formulas for Laplace equation layer potentials using spheroidal harmonics, combines analytical near-field evaluations with quadrature for far-field, and accelerates with the fast multipole method.

Result: Validated through numerical tests, the method efficiently handles hundreds of particles on a single processor for both Laplace and Stokes problems.

Conclusion: The framework is accurate, efficient, and adaptable to particulate suspension flows, demonstrating scalability for large systems.
...
Method: Uses multicontinuum homogenization, formulates coupled cell problems in oversampled regions, and derives generalized equations.

Result: Proposed multicontinuum models show high accuracy in numerical experiments for heterogeneous media.
...
<details>
  <summary>Details</summary>
Motivation: To address the inefficiencies in solving large-scale linear systems, the authors propose improved Kaczmarz methods that leverage randomized and greedy strategies for faster convergence.

Method: The paper presents two-dimensional randomized and greedy Kaczmarz methods, including semi-randomized versions with simple random sampling, designed for big data problems.

Result: Theoretical proofs confirm convergence to the least-norm solution, and numerical results show the methods outperform existing ones in computing time.

Conclusion: The proposed methods are effective for large-scale linear systems, offering faster convergence and practical advantages for big data applications.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of maintaining energy stability and mesh quality in numerical simulations of curvature-dependent geometric gradient flows, particularly for Willmore flow and Gauss curvature flow.

Method: Uses two novel geometric identities to derive a new variational formulation, leading to an implicit fully discrete scheme with parametric finite element method (PFEM). Incorporates tangential velocity control for mesh quality.

Result: The method preserves energy dissipation and maintains good mesh quality, demonstrated through extensive numerical experiments.

Conclusion: ES-PFEM provides a robust framework for curvature-dependent flows, with potential applications in surface evolution and mesh improvement.
...
<details>
  <summary>Details</summary>
Motivation: To develop boundary conditions for compressible Navier-Stokes equations that ensure stability and accuracy in entropy, mass, and energy.

Method: Proposed boundary conditions, proved a priori estimates, and integrated with entropy-stable finite-volume schemes. Numerical computations validated robustness.

Result: Successful derivation of boundary conditions and their stable approximation with finite-volume schemes. Numerical tests confirmed robustness.

Conclusion: The method provides reliable boundary conditions for compressible flows, compatible with entropy-stable schemes and validated numerically.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of identifying unknown sources in fractional diffusion equations from noisy measurements.

Method: Reformulates the problem as an optimization task with Tikhonov regularization and uses the Levenberg-Marquardt method for numerical reconstruction.

Result: Uniqueness is proven, and numerical examples demonstrate the algorithm's efficiency and accuracy.

Conclusion: The proposed method effectively solves the inverse source problem for fractional diffusion equations.
...
Method: Multicontinuum homogenization distinguishes continua by physical characteristics, coupling them via space-time local cell problems on representative volume elements.

Result: Upscaled system provides computable macroscopic coefficients, validated for accuracy and efficiency in numerical experiments.
...
Method: Pipeline includes resolution-adaptive point cloud generation, signed distance field creation, hierarchical winding number segmentation, and SPH-inspired particle relaxation.

Result: Robust, memory-efficient, and converges to exact geometry with higher resolution.

Conclusion: The method is meshless, integrates easily into existing frameworks, and handles imperfect geometries effectively.
...
Method: Derives conditions for non-invariance of modified Hamiltonians and non-preservation of first integrals, and explores order-compensating transformations.

Result: Demonstrates computational significance of coordinate choices and potential for improving method accuracy.

Conclusion: Systematic understanding of coordinate effects can enhance the practical use of symplectic methods.
...
<details>
  <summary>Details</summary>
Motivation: To improve the suboptimal stability and convergence results of Runge--Kutta based gCQ for sectorial problems, matching the performance of uniform-step CQ.

Method: Uses Runge--Kutta based gCQ for sectorial problems, with theoretical analysis and numerical experiments.

Result: Achieves same convergence order as original CQ under same regularity, with efficient implementation and optimal graded meshes for singular data.

Conclusion: gCQ is effective for sectorial problems, offering high-order convergence and computational efficiency, supported by numerical results.
...
<details>
  <summary>Details</summary>
Motivation: Classical methods like rational functions achieve root-exponential convergence for non-smooth functions, but polynomial approximations lag. Recent work suggests composite polynomials can recover exponential rates, motivating the need for tailored solutions for asymmetric functions.

Method: A class of weighted deep polynomial approximants is introduced, combining learnable deep polynomials with one-sided weights to capture local non-smoothness and global growth. A stable graph-based parameterization strategy is proposed for optimization.

Result: Numerical results demonstrate superior performance over Taylor, Chebyshev, and standard deep polynomial approximants, even with equal parameters.

Conclusion: The proposed weighted deep polynomial approximants effectively address asymmetric function behavior, outperforming existing methods and offering practical optimization via graph-based parameterization.
...
<details>
  <summary>Details</summary>
Motivation: The study aims to improve simulations of stopping power for α particles at NIF, where electrons are in a warm dense state requiring quantum and kinetic considerations.

Method: A second-order Strang splitting method is used, with a full-rank solver and adaptive-rank solver (SLAR scheme) for advection and Wigner integral terms.

Result: The solver achieves O(N) complexity, preserves mass, and maintains momentum accuracy, with adaptive-rank results matching full-rank simulations.

Conclusion: The adaptive-rank method shows promise for high-dimensional Wigner-Poisson simulations, enabling kinetic studies of stopping power in warm dense plasmas.
...
Method: Uses Streamline Diffusion method for discretization, Virtual Element Method for velocity and species concentrations, and Discontinuous Galerkin scheme for time. Error estimates derived via Gauss-Radau interpolation and numerical integration.

Result: Achieves arbitrary-order accuracy in space and time, supported by numerical experiments.

Conclusion: The proposed methods effectively model reactive transport with high accuracy, validated by theoretical and experimental results.
...
<details>
  <summary>Details</summary>
Motivation: Poroelasticity problems are crucial in engineering and geophysics but face challenges like singularity in locking cases, necessitating effective preconditioners for iterative solutions.

Method: Developed nonsingular preconditioners with eigenvalue clusters around 1 and outliers. Used block triangular Schur complement preconditioners and a locking-free weak Galerkin finite element method.

Result: Preconditioners are robust, with eigenvalue cluster bounds independent of mesh size, time step, and locking parameters. Numerical results confirm effectiveness in 2D and 3D.

Conclusion: The proposed preconditioners are efficient, parameter-robust, and avoid exact Schur complement computations, making them suitable for large-scale poroelasticity problems.
...
<details>
  <summary>Details</summary>
Motivation: To generalize the VMS framework for nonlinear problems while preserving accuracy and conservation properties.
...
Result: The method closely approximates optimal projections and maintains high-order accuracy, confirmed by numerical results.

Conclusion: The framework is robust, accurate, and applicable to a wide range of nonlinear multiscale problems.
...
<details>
  <summary>Details</summary>
Motivation: High computational and memory costs in numerical solutions of parameter identification inverse problems for kinetic equations.

Method: PDE constrained optimization with adjoint equations, B-spline approximation for scattering coefficient, dynamical low-rank approximation (DLRA) with rank-adaptive basis update & Galerkin integrator, and adaptive gradient descent step size.

Result: Significant reduction in memory and computational costs, validated by numerical results with different initial conditions.

Conclusion: The DLRA scheme is accurate and efficient compared to full solver solutions.
...
Method: Uses polar decomposition and iterative optimization to approximate unitary matrices, proving equivalence classes reduce search space.

Result: Optimal solutions form equivalence classes, and the algorithm identifies critical points (local minima) of the objective function.

Conclusion: The method effectively approximates quantum channels with limited data and rigorous guarantees of convergence to local minima.
...
<details>
  <summary>Details</summary>
Motivation: To extend previous results on nonlinear Schrödinger equations by analyzing mixed fractional Laplacians and combined nonlinearities, focusing on fractional Sobolev subcritical or critical cases.

Method: The authors use variational methods to analyze the problem, proving the existence of ground states and mountain pass solutions under specific conditions on the exponents.
...
Conclusion: The results generalize prior work by Chergui et al. and Luo et al., providing new insights into the existence of solutions for such nonlinear equations.
...
<details>
  <summary>Details</summary>
Motivation: To address the analytical challenges posed by nonlocal nonlinearity in charge-conserving PB equations and understand how domain geometry influences electrostatic interactions.

Method: Uses the principal coordinate system, exponential-type estimates, and moving plane arguments to prove asymptotic expansions in three characteristic regions of the domain.

Result: Derives second-order asymptotic formulas in Region I, exponential decay estimates in Regions II and III, and expansions for electric potential, field, and ionic charge density.

Conclusion: The study provides a comprehensive framework for understanding boundary layer solutions in singular perturbation problems, highlighting the role of domain geometry in electrostatic interactions.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of modeling immiscible two-phase flow with diffusive interfaces, particularly the phase transitions driven by non-monotonic pressure-density relationships in the van der Waals equation.

Method: A refined $L^2$-energy framework is developed to analyze the one-dimensional Cauchy problem, ensuring existence and uniqueness of global strong solutions without smallness constraints on initial data.

Result: Global strong solutions exist and are unique for non-vacuum, finite-temperature initial conditions, with all physical quantities remaining bounded despite density fluctuations and phase transitions.

Conclusion: The study confirms the robustness of the Navier-Stokes/Allen-Cahn system in handling phase transitions and density fluctuations, providing a theoretical foundation for further research in two-phase flow dynamics.
...
<details>
  <summary>Details</summary>
Motivation: Motivated by an inverse problem, the study aims to understand how the solution and its normal derivative depend on the shape of the interior boundary.

Method: The authors analyze the maps from the shape parameter (a diffeomorphism) to the solution and its normal derivative, proving their smoothness. They also compute the differential of the normal derivative with respect to the shape parameter.

Result: The solution and its normal derivative are shown to be smooth functions of the shape parameter. The differential of the normal derivative on the exterior boundary is computed.

Conclusion: The results provide insights into the dependence of solutions on domain geometry, with potential applications to inverse problems.
...
Method: Cast MFG system as a variational inequality, use low-order $p$-Laplacian term for coercivity, and regularize Hamiltonian for weak solutions.

Result: Existence of strong solutions for power-growth and singular congestion models, and weak solutions for minimal growth Hamiltonians.

Conclusion: The Banach-space framework unifies earlier results, avoids high-order smoothing, and is suitable for numerical algorithms.
...
<details>
  <summary>Details</summary>
Motivation: To establish uniqueness in determining a potential from far field data at a fixed angle, addressing inverse problems.

Method: Uses a new Carleman estimate and techniques from Bukhgeim and Klibanov for inverse problems.

Result: The potential is uniquely determined by the far field pattern at a fixed angle.

Conclusion: The approach successfully resolves the uniqueness question for such inverse problems.
...
<details>
  <summary>Details</summary>
Motivation: To establish stability conditions for solutions near equilibria in semilinear parabolic systems and apply these to fluid-solid dynamics.

Method: Uses linearization around equilibria, assuming analytic semigroup generation, and proves exponential convergence for globally existing solutions.

Result: Solutions near equilibria converge exponentially to steady states. Applied to fluid-filled solids, weak solutions stabilize exponentially in specified norms.

Conclusion: The abstract stability principle is validated and applied successfully to fluid-solid systems, demonstrating exponential convergence to equilibrium.
...
<details>
  <summary>Details</summary>
Motivation: To improve computational efficiency and scaling in relativistic CCSD for large systems with heavy atoms.

Method: Tucker decomposition of amplitude tensors in RCCSD, tested on (AuCl)$_n$, Au$_n$, and YbCl$_2$ models, with compression via singular value rejection.

Result: Achieved 1 kJ/mol accuracy with high compression; only ~3% of doubles amplitudes were significant for YbCl$_7$.

Conclusion: Rank reduction in relativistic CCSD is feasible, and the Goldstone diagrammatic technique is preferred for high-precision modeling of heavy-atom systems.
...
<details>
  <summary>Details</summary>
Motivation: Accurate inclusion of anharmonic and nuclear quantum effects (NQEs) in phonon simulations is computationally expensive, necessitating a more efficient approach.

Method: Leverages quantum correlators for anharmonic phonon frequencies and QTB for NQEs, tested on 1D systems and solid neon.

Result: Demonstrates efficiency and accuracy in calculating phonon dispersion relations, with analysis of strengths and limitations.

Conclusion: The proposed scheme offers a promising solution for efficient phonon calculations, balancing computational cost and accuracy.
...
<details>
  <summary>Details</summary>
Motivation: High computational costs in 2D/3D PIC simulations, especially due to the CD bottleneck, necessitate scalable parallelization methods beyond conventional private-grid approaches.
...
Conclusion: The method enhances CD performance and scalability in PIC simulations while requiring minimal code changes.
...
Method: Uses a parametrized psi_N grid and interferometer data for density reconstruction, accelerated by a multi-layer perceptron, and employs a proportional-integration controller with gains from system identification.

Result: Achieves median and average errors of 1.5% and 2.5%, respectively, and demonstrates dynamic target following and density reduction via pump-out under RMP.

Conclusion: The developed controller enhances experimental flexibility by allowing dynamic density target setting or constant density maintenance during discharges.
...
<details>
  <summary>Details</summary>
Motivation: To validate the analytically derived transonic plasma acceleration solution in a magnetic nozzle and compare it with 2D MHD simulations.

Method: Axisymmetric two-dimensional (rz) magnetohydrodynamics (MHD) simulations were used to compare with the analytically derived solution.

Result: The analytical solution matches well near the axis and applies to arbitrary magnetic surfaces. Simulations confirm a shock-like transition to the unique transonic profile, with the magnetic field adjusting to fit the analytic predictions.

Conclusion: The study confirms the robustness of the analytical solution for transonic plasma acceleration in magnetic nozzles, even with non-vacuum initial conditions.
...
<details>
  <summary>Details</summary>
Motivation: Understanding the physics behind impurity seeding's effectiveness in mitigating ELM damage in fusion plasma.

Method: Nonlinear magnetohydrodynamic (MHD) simulations to analyze ELM crash triggers and impurity seeding effects.

Result: Impurity seeding modifies pedestal pressure, triggering high-n ballooning instabilities, with impurity density and seeding location critical for ELM control.

Conclusion: Impurity seeding's role in ELM mitigation is clarified, highlighting key parameters for effective control.
...
<details>
  <summary>Details</summary>
Motivation: To explore the effects of continuous lithium granule injection on plasma confinement and impurity transport in stellarators.

Method: Experimental observation and simulations using the drift-kinetic transport code SFINCS.
...
<details>
  <summary>Details</summary>
Motivation: To understand the interplay between spin and orbital angular momentum in nonlinear optics and explore the generation of controllable high-intensity UV/X-ray pulses with orbital angular momentum.

Method: Analyzed the 2D peripheral electron dynamics on the rim of a small aperture diffracting an intense LG laser beam, derived a theory, and validated it with simulations.

Result: Discovered that harmonic beams from linearly polarized LG drivers exhibit multiple LG modes with topological charges equal to the harmonic order, breaking simple conservation rules.

Conclusion: Provides fundamental insights into nonlinear optics and enables the generation of versatile high-intensity UV/X-ray pulses with controllable orbital angular momentum.
...
<details>
  <summary>Details</summary>
Motivation: To understand and optimize the enhancement of wakefield acceleration using an all-optical plasma waveguide, while addressing the unintended generation of strong RF radiation.

Method: Combination of laboratory measurements, particle-in-cell simulations, and analytic approximations to study the phenomenon.

Result: A 20 J laser pulse excites a 1 Joule, 400 GW broadband THz surface wave with a peak electric field of 35 GV/m.

Conclusion: The study highlights the dual outcomes of wakefield acceleration: enhanced electron bunches and unintended RF radiation, providing insights for future optimization.
...
<details>
  <summary>Details</summary>
Motivation: To understand how anisotropy in the Fermi surface of 2D materials leads to electromagnetic instabilities and their implications.

Method: Used a kinetic model incorporating temperature, chemical potential, and anisotropy ratio, with linear and quadratic band structures. Analyzed growth rates and performed simulations.
...
Conclusion: The study reveals unique electromagnetic instabilities in anisotropic 2D materials, with potential applications in controlled magnetic field generation.
...
<details>
  <summary>Details</summary>
Motivation: To address coupled structural integrity problems (e.g., thermo-mechanical fracture, hydraulic fracture) with a versatile, easily implementable approach.

Method: Uses phase field and multi-physics modeling, leveraging the heat transfer equation for implementation in Abaqus via UMAT/UMATHT subroutines.

Result: Demonstrates good agreement with experimental and analytical solutions across 2D/3D problems.

Conclusion: The framework is effective for diverse engineering problems, with freely available subroutines for broader adoption.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the computational intractability of bifurcation and stability analysis in high-dimensional systems by developing reduced-order models (ROMs) that capture emergent dynamics.

Method: The method involves four steps: manifold learning to identify latent dimensions, GPR-based ROM construction, numerical bifurcation analysis in latent space, and reconstruction of results in the original space.

Result: The framework successfully identifies latent dimensionality and constructs surrogate models for bifurcation analysis, demonstrated on wake flow and sudden-expansion channel flow.

Conclusion: The approach enables efficient and accurate bifurcation and stability analysis in high-dimensional systems, overcoming computational limitations.
...
<details>
  <summary>Details</summary>
Motivation: The challenge lies in modeling complex multiscale dynamical systems where resolving all scales is computationally expensive, especially in scenarios like turbulent flows. Traditional deterministic and local closure models are restrictive in such regimes.

Method: The framework jointly trains convolutional autoencoders with conditional diffusion models in latent spaces, reducing sampling dimensionality while preserving physical characteristics.

Result: The joint training approach discovers a latent space with small reconstruction errors and good diffusion model performance. Integrated simulations show computational acceleration with comparable accuracy to standard diffusion models.

Conclusion: The proposed stochastic modeling framework via latent conditional diffusion models offers a computationally efficient solution for closure modeling in multiscale systems without sacrificing predictive accuracy.
...
<details>
  <summary>Details</summary>
Motivation: To improve global optimization of multivariate functions by leveraging quantum computing for efficiency and accuracy.

Method: Combines quantum state encoding for solution quality estimation with classical local optimization.

Result: QAGS outperforms classical methods in accuracy, time, and space complexity.

Conclusion: QAGS effectively contracts search space toward global optima with controlled computational complexity.
...
<details>
  <summary>Details</summary>
Motivation: To establish unconditional linear stability for the Kerr family across the entire subextremal range, building on prior results.

Method: Uses mode stability results and zero energy behavior computations of the linearized gauge-fixed Einstein equation.

Result: Successfully proves linear stability for the Kerr family in the full subextremal range.

Conclusion: The study confirms the stability of the Kerr family, leveraging prior analytical and computational work.
...
<details>
  <summary>Details</summary>
Motivation: To understand plasma dynamics free of fast Langmuir oscillations and preserve quasineutrality, requiring incompressible bulk plasma flow.

Method: Uses slow manifold reduction and Poisson-Dirac submanifolds to deduce a Hamiltonian formulation for the quasineutral limit of the Vlasov-Poisson system.

Result: The Hamiltonian structure synthesizes known Poisson brackets for incompressible fluids and collisionless kinetic equations.

Conclusion: The derived Hamiltonian formulation effectively models quasineutral plasma dynamics while preserving incompressibility and balancing electric fields.
...
<details>
  <summary>Details</summary>
Motivation: To challenge the assumption that symmetric domains must have symmetric eigenfields for the lowest curl eigenvalue.

Method: Constructing families of rotationally symmetric toroidal domains in ℝ³ and analyzing their eigenfields.

Result: Some domains have symmetric eigenfields, while others do not, disproving the folk wisdom.
...
<details>
  <summary>Details</summary>
Motivation: To understand why donor-acceptor structures with two acceptors exhibit higher ET rates than predicted by classical or simple quantum models.

Method: Analysis of coupling interactions in multi-acceptor systems, focusing on acceptor-acceptor effects on free energy, donor-acceptor couplings, and reaction-coordinate motion.

Result: Observed 4-5x ET rate increase explained by acceptor-acceptor interactions, surpassing simple additive predictions.

Conclusion: The findings clarify ET rate enhancements in multi-acceptor systems and offer insights for designing tailored energy and electron transfer kinetics.
