...
TL;DR: Novel numerical method for 1D1V Boltzmann-BGK equation combining operator splitting, third-order transport scheme, second-order collision scheme, and conservation-enforcing polynomial modification.
...
Method: Operator splitting separates transport and collision steps; third-order Lax-Wendroff scheme for transport; second-order L-stable TR-BDF method for collision; quadratic Hermite polynomial modification of Maxwell-Boltzmann distribution to enforce conservation.

Result: Verified on piecewise constant initial data with periodic boundaries; demonstrated exact conservation of mass, momentum, and energy up to machine precision.

Conclusion: Successfully developed a comprehensive numerical framework for Boltzmann-BGK equation that maintains conservation properties while being accurate across flow regimes, implemented in publicly available software.
...
Method: Uses specially designed 1D multi-channel convolutional neural networks with circular padding in a divide-and-conquer framework. First classifies the impedance cylinder shape, then reconstructs the boundary curve and impedance function using far-field measurements from direct problem solutions.

Result: Extensive numerical experiments, including noisy scenarios, demonstrate the approach's efficiency and robustness in accurately reconstructing the unknown boundary curve and impedance function.

Conclusion: The proposed neural network-based divide-and-conquer framework provides an effective and robust solution for inverse electromagnetic scattering problems involving complex magneto-dielectric and impedance cylinder configurations.
...
TL;DR: A dual-domain deep learning framework accelerates multiscale basis function computation in MGMsFEM for Darcy flow problems, achieving significant speedup while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Darcy flow in heterogeneous porous media has multiscale characteristics that challenge conventional numerical methods. MGMsFEM addresses these but constructing multiscale basis functions is computationally expensive.
...
Conclusion: The method offers potential for future applications in real-world reservoir engineering by efficiently solving multiscale Darcy flow problems.
...
TL;DR: A hybrid approach coupling local non-intrusive Operator Inference ROMs with high-fidelity FOMs using overlapping Schwarz alternating method for efficient multiscale simulations.


<details>
  <summary>Details</summary>
Motivation: Address long runtime and complex mesh generation challenges in traditional high-fidelity multiscale modeling and simulation.

Method: Hybrid approach combining subdomain-local OpInf ROMs with FOMs using overlapping Schwarz alternating method, enabling integration of disparate models, meshes, and time schemes.

Result: Demonstrated on 3D solid dynamics problems with up to 106x speedup compared to conventional FOM-FOM couplings.

Conclusion: Paves way for more efficient simulation workflows in engineering with potential extensions to various PDEs.
...
TL;DR: The paper applies model order reduction methods to accelerate wave propagation simulations in nonlinear materials, comparing intrusive (POD-based) and non-intrusive (DMD, OpInf) approaches.


<details>
  <summary>Details</summary>
Motivation: To address the computational challenges of simulating wave propagation in nonlinear materials or damaged structures, which result in high-dimensional problems that require acceleration.

Method: Uses both intrusive methods (projection-based POD) and non-intrusive methods (data-driven DMD and operator inference) depending on equation knowledge and operator availability.

Result: Evaluates the performance of reduction techniques in three different numerical examples of wave propagation problems.

Conclusion: Model order reduction methods provide effective acceleration for wave propagation simulations in nonlinear materials, with both intrusive and non-intrusive approaches showing promising results.
...
TL;DR: A fast algorithm for solving the scalar wave equation with point sources using spectral methods and Fourier projection to reduce computational complexity from O(M²N_t) to O((M + N³logN)N_t).


<details>
  <summary>Details</summary>
Motivation: To efficiently evaluate wave equation solutions for large collections of point sources, overcoming the computational burden of naive O(M²N_t) approaches.
...
Conclusion: The algorithm provides an efficient solution for time-domain wave equation scattering problems with high accuracy and reduced computational cost.
...
TL;DR: Provides concentration and variance bounds for stochastic iterative methods, extending beyond expected error analysis to worst-case behavior.


<details>
  <summary>Details</summary>
Motivation: Understanding worst-case behavior of stochastic iterative methods is important for practical applications, as existing theoretical results mainly focus on expected error bounds.

Method: Analyzes a general class of linear stochastic iterative methods (including randomized Kaczmarz and Gauss-Seidel) and nonlinear methods (including randomized Kaczmarz for linear inequalities).

Result: Derives upper bounds for the concentration and variance of error in these stochastic iterative methods.

Conclusion: The analysis provides theoretical tools for generating confidence intervals around expected error bounds, enhancing understanding of stochastic methods' worst-case performance.
...
<details>
  <summary>Details</summary>
Motivation: To solve the Poisson problem when boundary data is unknown, using compensating interior point measurements instead of boundary measurements to lower regularity requirements.
...
Result: Improved pointwise error estimates for Riesz representers and enhanced performance measures for the recovery algorithm in various norms.

Conclusion: Interior point measurements provide a viable alternative to boundary measurements for Poisson problem recovery, with better regularity conditions and improved error estimates.
...
TL;DR: Sharp stability results for ascent and descent spectra under strong resolvent convergence, with computable finite-element diagnostic γ_h as practical surrogate for reduced minimum modulus γ(T-λ).


<details>
  <summary>Details</summary>
Motivation: To establish spectral stability for non-selfadjoint and singularly perturbed operators in finite element approximations, where norm resolvent convergence fails for rough or singular limits.

Method: Uses strong resolvent convergence (SRS) framework with reduced minimum modulus γ(T-λ)>0 as key condition, transfers Kaashoek-Taylor criteria via gap convergence of operator graphs, extends to powers using B-Fredholm theory, and introduces computable diagnostic γ_h = σ_min(M^{-1/2}(A_h-λM)M^{-1/2}).

Result: Numerical experiments confirm that liminf_{h→0}γ_h>0 is necessary and sufficient for spectral stability, while Volterra-type counterexample shows closed-range condition is indispensable for powers.

Conclusion: SRS combined with quantitative control of γ_h rescues ascent-descent stability in realistic computational settings, providing practical framework for finite element approximations of non-selfadjoint operators.
...
TL;DR: A fully implicit Crank-Nicolson discontinuous Galerkin method is developed for the Ginzburg-Landau equation with rigorous proofs of unique solvability and unconditionally optimal error estimates.


<details>
  <summary>Details</summary>
Motivation: To develop a robust numerical scheme for the Ginzburg-Landau equation with guaranteed solvability and optimal convergence properties.

Method: Fully implicit Crank-Nicolson discontinuous Galerkin method with novel analytical techniques for proving unique solvability and error estimates.

Result: Successfully established unique solvability and unconditionally optimal error estimates under L²-norm and energy norm, validated by numerical examples.

Conclusion: The proposed method provides a reliable numerical framework for solving the Ginzburg-Landau equation with rigorous theoretical guarantees and practical validation.
...
TL;DR: Preconditioned Krylov subspace methods with sine transform-based preconditioners are developed to efficiently solve large dense linear systems from 3D space-fractional diffusion equations, achieving linear convergence rates independent of discretization sizes.


<details>
  <summary>Details</summary>
Motivation: Three-dimensional conservative space-fractional diffusion equations result in large, dense three-level Toeplitz linear systems that are computationally challenging to solve efficiently.

Method: Developed preconditioned Krylov subspace methods (PCG for symmetric cases, PGMRES for non-symmetric cases) with sine transform-based preconditioners, and provided detailed convergence analysis.

Result: For symmetric cases, preconditioned matrices have spectra bounded in (1/2, 3/2), ensuring linear convergence. For non-symmetric cases, PGMRES also achieves linear convergence independent of discretization stepsizes. Iteration counts are uniformly bounded regardless of matrix sizes.

Conclusion: The proposed preconditioned Krylov subspace methods demonstrate optimal performance with iteration counts independent of problem size, confirmed by numerical experiments in 2D and 3D for both symmetric and non-symmetric cases.
...
TL;DR: A p-adaptive high-order mesh-free framework for fluid flow simulation using dynamic polynomial refinement based on local error estimates, achieving up to 50% computational cost savings while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: To enable accurate and efficient simulation of fluid flows in complex geometries by developing an adaptive mesh-free method that can dynamically adjust polynomial order to capture non-linear regions while reducing computational costs.

Method: High-order differential operators constructed using anisotropic basis functions with polynomial reproduction up to order p. Dynamic p-refinement strategy with new refinement indicator based on local Laplacian error estimates, incorporated into solution procedure with minimal computational overhead.

Result: Successfully applied to canonical PDEs and 2D compressible reacting flow in porous media. Achieved up to 50% computational cost savings while maintaining specified accuracy levels. Effectively captured highly non-linear regions requiring high-order approximation.

Conclusion: The p-adaptive high-order mesh-free method effectively captures non-linear regions where high-order approximation is necessary, reduces computational costs compared to non-adaptive methods, and preserves high accuracy and solution stability.
...
TL;DR: Two new Rosenbrock methods for solving index-1 differential algebraic equations: Rodas6P (sixth-order) and Tsit5DA (fifth-order, improved from existing fourth-order methods).


<details>
  <summary>Details</summary>
Motivation: To develop higher-order Rosenbrock methods for solving index-1 differential algebraic equations, improving upon existing methods like Rodas3P-5P and the 1989 fourth-order approach.

Method: Rodas6P follows the same design principles as previous Rodas methods. Tsit5DA uses explicit solution for differential equations and linear-implicit approach for algebraic equations, building on the Tsit5 method.

Result: Theoretical properties are verified through order tests and benchmarks, confirming the methods achieve their claimed orders (sixth-order for Rodas6P, fifth-order for Tsit5DA).

Conclusion: Two new higher-order Rosenbrock methods (Rodas6P and Tsit5DA) are successfully developed and validated for solving index-1 differential algebraic equations, providing improved numerical performance over existing methods.
...
TL;DR: This paper investigates algebraic multigrid (AMG) preconditioners for isogeometric analysis (IgA) systems, demonstrating their robust and scalable performance in HPC environments using PSCToolkit.


<details>
  <summary>Details</summary>
Motivation: Isogeometric analysis produces large, ill-conditioned linear systems that scale poorly with polynomial degree and spatial dimension, requiring efficient preconditioners for practical computation.

Method: The study uses AMG preconditioners tailored for IgA discretizations, implemented with distributed-memory and GPU-accelerated strategies via PSCToolkit.

Result: AMG preconditioners achieve robust and scalable performance across benchmark tests, making them practical solvers for large IgA systems.

Conclusion: AMG preconditioners are effective and scalable solutions for large-scale IgA systems in engineering and scientific applications.
...
<details>
  <summary>Details</summary>
Motivation: To develop a finite element method that can handle star-shaped polygons while maintaining high-order accuracy and conformity across elements.

Method: Constructs high-order shape functions as linear combinations of standard finite element basis functions defined on local trivial sub-triangulations of each element. The refinement is only used for shape function construction and doesn't increase degrees of freedom.

Result: The method creates a finite element space that includes polynomials of desired order, preserves conformity across elements, and inherits convergence properties from standard finite element frameworks. Numerical experiments confirm expected convergence rates.

Conclusion: The Zipped Finite Element Method successfully provides a high-order conforming finite element approach for star-shaped polygons with proven convergence properties.
...
<details>
  <summary>Details</summary>
Motivation: To reduce high computational costs of complex arithmetic in solving complex symmetric indefinite linear systems by developing efficient iterative methods.

Method: LHSS uses alternating iterative scheme with symmetric real coefficient matrices. PLHSS extends this with preconditioning, deriving optimal parameters and convergence rates.

Result: Theoretical analysis shows convergence depends on eigenvalues of real symmetric matrices. PLHSS has superior convergence, well-clustered eigenvalues, and orthogonal eigenvectors.

Conclusion: LHSS and PLHSS methods are efficient, showing mesh size independent and parameter-insensitive convergence in numerical experiments with GMRES and COCG methods.
...
TL;DR: A novel vertex-based graph Fourier neural solver for linear elasticity equations that combines block-Jacobi smoothing with learnable Fourier-domain correction, outperforming classical multigrid methods.


<details>
  <summary>Details</summary>
Motivation: Numerical solution of linear elasticity equations on unstructured grids is challenging, especially with heterogeneous and anisotropic materials.

Method: Integrates block-Jacobi smoothing (for high-frequency errors) with learnable Fourier-domain module (for remaining frequency spectrum errors) in a vertex-based graph Fourier neural solver.

Result: Extensive experiments on 2D/3D problems show superior robustness and efficiency over classical smoothed aggregation algebraic multigrid method. Frequency analysis confirms complementarity between smoother and neural corrector.
...
TL;DR: Energy-conserving Hamiltonian Boundary Value Methods (HBVMs) for Hamiltonian systems with efficient solvers exploiting low-rank structure


<details>
  <summary>Details</summary>
Motivation: Long-term preservation of energy and symplecticity is essential in Hamiltonian systems applications

Method: HBVMs reformulate as matrix equations with low-rank right-hand side; use Krylov projection for linear systems and simplified Newton iterations/Newton-Krylov framework with adaptive time-stepping for nonlinear systems

Result: Numerical experiments on semi-discretized wave equations demonstrate efficiency and robustness

Conclusion: Proposed approach provides efficient and robust solution for Hamiltonian systems requiring long-term energy conservation
...
TL;DR: The paper establishes criteria for mean-square exponential stability of neutral stochastic differential delay equations with Markovian switching and their Euler-Maruyama numerical approximations, proving the numerical scheme can reproduce the true solution's exponential decay rate.


<details>
  <summary>Details</summary>
Motivation: To address the complexities in stability analysis of NSDDEs with Markovian switching, particularly the interaction between neutral terms, time-varying delays, and structural changes governed by Markov chains.

Method: Developed novel stability criteria using analytical methods and analyzed the Euler-Maruyama numerical scheme for approximating the system.

Result: Established practical stability criteria for both the original system and its numerical approximations, proving the numerical scheme can achieve arbitrary accuracy in reproducing the exponential decay rate with sufficiently small step sizes.

Conclusion: The theoretical framework provides effective stability analysis tools for NSDDEs with Markovian switching, with numerical validation demonstrating the practical applicability of the results.
...
<details>
  <summary>Details</summary>
Motivation: To extend solvability results from scalar nonlocal equations to coupled vector-valued systems, addressing the challenges of strong coupling in nonlocal operators.
...
Result: Proves existence of unique strong solution u in [H^{2s,p}(R^d)]^d for any f in [L^p(R^d)]^d with 1<p<∞, for nonlocal operators with kernels comparable to |y|^{-(d+2s)}.

Conclusion: Successfully extends recent scalar nonlocal equation techniques to strongly-coupled vector systems, establishing comprehensive L^p-solvability theory for these nonlocal coupled systems.
...
TL;DR: Extends higher integrability results for gradient of local quasiminimizers to weighted Orlicz spaces, including weighted variable exponent and double phase cases, and proves existence of minimizers.


<details>
  <summary>Details</summary>
Motivation: To generalize previous higher integrability results to weighted frameworks with generalized Orlicz growth conditions, covering important special cases like weighted variable exponent and double phase problems.

Method: Uses variational integrals in weighted Orlicz spaces, assumes weights belong to Muckenhoupt class and growth functions satisfy appropriate structural conditions, then proves local higher integrability of gradients for local quasiminimizers.

Result: Proved that gradient of any local quasiminimizer has local higher integrability in weighted Orlicz framework, and established existence of minimizers for the associated functional.

Conclusion: Successfully extended higher integrability theory to weighted generalized Orlicz spaces, providing unified framework that includes weighted variable exponent and double phase cases as special instances.
...
TL;DR: Establishes best constants for singular anisotropic Adams' type inequality with exact growth in R^n and on bounded domains using anisotropic rearrangement techniques.


<details>
  <summary>Details</summary>
Motivation: To prove optimal constants for singular anisotropic Adams' type inequalities with precise growth conditions in both unbounded and bounded domains.

Method: Uses anisotropic rearrangement techniques to derive the inequalities.

Result: Obtained best constants for singular anisotropic Adams' type inequality with exact growth in R^n and extended the result to bounded domains.

Conclusion: Successfully established optimal constants for singular anisotropic Adams' inequalities in both R^n and bounded domains using anisotropic rearrangement methods.
...
TL;DR: The paper proves that for the Robin Laplacian problem, the ratio of the first two eigenvalues μ₂/μ₁ is maximized by a disk for all dimensions N≥2 and all boundary parameters σ≥σₛ, and that this maximum ratio strictly decreases with σ.


<details>
  <summary>Details</summary>
Motivation: To resolve the Payne-Schaefer conjecture (2001) and Henrot's restatement (2003) about maximizing the eigenvalue ratio μ₂/μ₁ for the Robin Laplacian, which is analogous to the classical Payne-Pólya-Weinberger conjecture for the fixed membrane problem.

Method: Mathematical analysis of the Robin Laplacian eigenvalue problem, establishing properties of the eigenvalue ratio μ₂/μ₁ as a function of the boundary parameter σ and the domain shape.

Result: Proved that μ₂/μ₁ is maximized by a disk for all dimensions N≥2 and all σ≥σₛ, and that the maximum value strictly decreases over (0,∞).

Conclusion: The result confirms that measuring the ratio of first two eigenfrequencies can determine whether an elastically supported drum is circular, answering a variant of Yau's Problem 77.
...
TL;DR: This paper presents an inverse boundary value problem for the Helmholtz equation, reconstructing two constant coefficients and a Dirichlet polygonal obstacle from single Cauchy data using a modified factorization method.


<details>
  <summary>Details</summary>
Motivation: To solve the inverse problem of reconstructing both material coefficients and obstacle geometry from limited boundary measurements, which has applications in non-destructive testing and medical imaging.

Method: Adapted the one-wave factorization method to recover polygonal obstacles and coefficients, using a modified factorization with Dirichlet-to-Neumann operator to handle eigenvalue issues.

Result: Verified uniqueness results under a priori assumptions and demonstrated efficient reconstruction through intensive numerical examples.

Conclusion: The proposed modified factorization method effectively recovers both obstacle geometry and material coefficients from single Cauchy data, with numerical validation showing efficiency.
...
<details>
  <summary>Details</summary>
Motivation: To develop and analyze a mathematical model for understanding pattern formation in biomembranes driven by curvature effects, which is important for biological processes like cell signaling and membrane organization.

Method: Derived a phase-field model as gradient flow of approximated Canham-Helfrich energy, leading to coupled Cahn-Hilliard and fourth-order reaction-diffusion equations. Used minimizing movement scheme for existence proofs, Moreau-Yosida regularization for singular potentials, and finite element discretization for numerical simulations.

Result: Proved existence of weak solutions for regular and singular potentials, established higher regularity and uniqueness, and demonstrated through numerical experiments that different physical parameters produce striped, dotted, or snake-like membrane patterns.

Conclusion: The phase-field model provides a rigorous mathematical framework for studying curvature-driven pattern formation in biomembranes, with analytical guarantees and numerical evidence showing diverse pattern formation depending on physical parameters.
...
TL;DR: The paper proves that solutions to Δu=Vu with nonnegative bounded potential V must vanish identically if their L² norm decays to zero over annular regions. This establishes the Landis conjecture for algebraically decaying solutions and extends to exterior domains and nonlinear Schrödinger equations.


<details>
  <summary>Details</summary>
Motivation: To prove the Landis conjecture, which concerns the unique continuation property for solutions of elliptic equations with decaying potentials, particularly establishing that sufficiently fast decay implies trivial solutions.

Method: Uses analysis of solutions to Δu=Vu with continuous, nonnegative bounded potential V, examining the behavior of L² norms over annular regions |x| between r_j and r_j+1 as r_j→∞.

Result: Shows that if the L² norm of u over these annular regions decays to zero along any sequence r_j→∞, then u must be identically zero. This validates the Landis conjecture for algebraically decaying solutions.

Conclusion: The Landis conjecture holds for solutions with sufficiently fast algebraic decay, and the results extend to exterior domains and certain nonlinear Schrödinger equations with appropriate conditions on the potential's zero set.
...
TL;DR: Improved stability threshold for Couette flow in rotating Navier-Stokes-Coriolis system, achieving Re^{-α} scaling with α > 2/3 for 3D and α ≥ 5/6 for 2.5D domains.


<details>
  <summary>Details</summary>
Motivation: To understand the transition from laminar to turbulent flow by quantifying the perturbation magnitude needed to trigger instability and its scaling with Reynolds number.

Method: Used anisotropic Sobolev spaces tailored to zero modes, introduced new dispersive structure for zero modes, derived Strichartz-type estimates, and exploited combined effects of rotation (dispersion) and mixing mechanisms.

Result: Proved that if initial perturbation satisfies ||v_in - (y,0,0)|| ≤ ε₀Re^{-α} with α > 2/3 (3D) or α ≥ 5/6 (2.5D), the solution exists globally and remains close to Couette flow.

Conclusion: The improved stability threshold scaling is achieved by leveraging rotation-induced dispersion and mixing effects, with anisotropic analysis addressing the main analytical challenges of zero-nonzero mode interactions.
...
TL;DR: Existence of normalized standing wave solutions for nonlinear Schrödinger-Bopp-Podolsky system with mass constraint, using perturbation methods under Navier/Neumann boundary conditions.


<details>
  <summary>Details</summary>
Motivation: Study normalized solutions (with fixed L^2 mass) for coupled Schrödinger-Bopp-Podolsky system, which combines quantum mechanics with Bopp-Podolsky electrodynamics in bounded domains.

Method: Adapt perturbation method to analyze the system with Lagrange multiplier ω enforcing mass constraint, under different boundary conditions (Navier/Neumann) for the Bopp-Podolsky field φ.
...
Conclusion: Perturbation method successfully establishes existence and multiplicity results for normalized standing waves in coupled Schrödinger-Bopp-Podolsky systems with mass constraints.
...
<details>
  <summary>Details</summary>
Motivation: To systematically study degenerate/singular eigenvalue equations with distance functions near boundaries of convex domains, addressing all possible exponent scenarios.

Method: Adapted Perron method, comparison principle, and construction of classical sub-solutions and super-solutions.

Result: Complete classification of solution existence/nonexistence; derivation of global estimates based on distance function when solutions exist.

Conclusion: Successfully classified all cases and established comprehensive existence theory with global estimates for this class of degenerate/singular eigenvalue equations.
...
TL;DR: Multiplicity results for time-independent Gross-Pitaevskii equations on closed Riemannian manifolds, relating solution count to topology of maximum velocity set in small momentum regime.


<details>
  <summary>Details</summary>
Motivation: To study critical points of Ginzburg-Landau energy with prescribed momentum according to tangent velocity fields, particularly non-minimizing solutions.

Method: Uses critical point theory, Γ-convergence for Ginzburg-Landau functionals, and develops new results for codimension 2 isoperimetric-type problems in small flux regime.

Result: Obtains lower bounds on multiplicity of solutions in terms of topology of maximum velocity set, valid in small momentum and vorticity core size regime.

Conclusion: The multiplicity of solutions to Gross-Pitaevskii equations is governed by topological properties of the velocity field, with proofs relying on advanced analytical techniques.
...
TL;DR: Analysis of qualitative properties of single blow-up solutions to nonlocal equations with slightly subcritical exponents, focusing on eigenvalue estimates and Morse index derivation for single-bubble solutions.


<details>
  <summary>Details</summary>
Motivation: To understand the qualitative behavior of eigenpairs in the linearized problem of nonlocal equations with slightly subcritical exponents, particularly in bounded domains for dimensions 3, 4, and 5.

Method: Using local Pohozaev identities and blow-up analysis to estimate the first (n+2)-eigenvalues and their corresponding eigenfunctions in the linearized problem.

Result: Provides estimates on eigenvalues and eigenfunctions, and examines qualitative behavior of eigenpairs for i=1,...,n+2 in the linearized nonlocal equations.

Conclusion: Derives the Morse index of a single-bubble solution in a nondegenerate setting as a corollary of the eigenvalue analysis.
...
TL;DR: Existence and regularity analysis for Dirichlet problem in radiation hydrodynamics with L^1 data, extending previous bounded solution results to unbounded solutions.


<details>
  <summary>Details</summary>
Motivation: To prove existence of solutions to radiation hydrodynamics equations with L^1 data in bounded domains, where previous work only handled regular data and bounded solutions.

Method: Develop new solution framework since Anzellotti theory doesn't apply to unbounded solutions; study regularity for L^p data with 1<p<N.

Result: Proved existence of solutions for L^1 data and established regularity results for L^p data consistent with previous findings.

Conclusion: Successfully extended existence theory to L^1 data cases and confirmed regularity patterns for broader data classes in radiation hydrodynamics problems.
...
<details>
  <summary>Details</summary>
Motivation: To prove existence of positive solutions for a coupled Laplacian system with nonlinear terms involving parameters λ and β

Method: Using Nehari manifold and fibering maps associated with the Euler functional for the system

Result: Proved existence of at least one positive solution

Conclusion: The Nehari manifold and fibering maps approach successfully establishes existence of positive solutions for the Laplacian system
...
TL;DR: Existence of polychromatic breather solutions for cubically nonlinear Maxwell equations in dispersive media with complex frequencies, constructed as Fourier series via iterative linear ODEs.


<details>
  <summary>Details</summary>
Motivation: To study nonlinear polychromatic surface plasmons in waveguide geometries with dispersive media, where complex frequencies lead to temporal decay.

Method: Construct solutions as Fourier series in y and t via iterative linear ODEs, with leading frequency as eigenvalue of operator pencil on R. Verify assumptions on spectrum and resolvent estimates.

Result: Proved existence of polychromatic breather solutions under spectral assumptions, with concrete example of waveguide interface between two homogeneous media satisfying conditions.

Conclusion: Established existence theory for nonlinear polychromatic surface plasmons in dispersive waveguides, providing constructive method via Fourier series and linear ODE iterations.
...
TL;DR: Develops Hamilton-Jacobi framework for propagation dynamics in field-road systems with unidirectional advection and Wentzell boundary conditions, revealing critical transition between rectilinear and road-assisted regimes.


<details>
  <summary>Details</summary>
Motivation: To analyze asymptotic propagation dynamics in complex field-road systems where classical comparison methods fail, particularly for non-order-preserving systems with enhanced diffusion and tangential drift.
...
Result: Established existence, uniqueness, and explicit variational representation of viscosity solution; revealed critical transition governed by geometrically derived curve separating propagation regimes; extended framework to conical domains with intersecting roads.

Conclusion: The variational methodology provides robust framework for analyzing propagation patterns shaped by advection and diffusion parameters, demonstrating interplay between field and road dynamics in determining invasion regions.
...
Method: Employ monotone iteration techniques for systems satisfying monotonicity conditions to find minimal and maximal weak solutions between sub- and supersolution pairs; use growth conditions for non-monotone cases.

Result: Proved existence of weak solutions for quasimonotone elliptic PDE systems, with specific results for both monotone and non-monotone scenarios, supported by concrete examples.

Conclusion: The paper successfully establishes existence theorems for weak solutions of coupled elliptic systems with quasimonotone nonlinearities, providing both theoretical framework and practical applications through examples.
...
TL;DR: Analysis of a hyperbolic Cahn-Hilliard tumor growth model with well-posedness, continuous dependence on therapy functions, and asymptotic convergence to viscous model.


<details>
  <summary>Details</summary>
Motivation: To study tumor growth models where the chemical potential relaxation is hyperbolic rather than parabolic, providing more realistic modeling and analyzing therapy effects.

Method: Mathematical analysis of initial-boundary value problem for hyperbolic Cahn-Hilliard tumor model, including well-posedness proofs, continuous dependence on therapy functions, and asymptotic analysis as inertial coefficient vanishes.

Result: Proved well-posedness, continuous dependence on drug and antiangiogenic therapy functions, established solution regularity, and showed convergence to viscous Cahn-Hilliard model when proliferation is constant.

Conclusion: The hyperbolic tumor growth model is mathematically sound, responds continuously to therapy inputs, and connects naturally to the classical viscous model through asymptotic analysis.
...
Conclusion: The Lagrangian formulation can be systematically extended to various hydrodynamic models through stochastic variational principles, providing unified mathematical framework and rigorous existence results.
...
TL;DR: The paper proves global well-posedness and scattering for the modified Novikov-Veselov (mNV) system with large L² data using inverse scattering methods, and establishes related results for the Novikov-Veselov system.


<details>
  <summary>Details</summary>
Motivation: Previous work by Schottdorf established global well-posedness for small L² data, but the large data problem remained open. The mNV system is L² critical and completely integrable, belonging to the same hierarchy as the defocusing Davey-Stewartson II system.

Method: Uses inverse scattering methods and proves a new nonlinear Gagliardo-Nirenberg inequality for the associated scattering transform. Also employs the Miura map to connect mNV and NV flows, and proves a sharp, scale invariant form of the Agmon-Allegretto-Piepenbrink principle.

Result: Main result: mNV system is globally well-posed for large L² data with solutions scattering as time goes to ±∞. As a byproduct, proves global well-posedness for the Novikov-Veselov problem at critical H⁻¹ + L¹ level for soliton-free data.

Conclusion: The paper successfully extends the well-posedness theory for the mNV system from small to large data using inverse scattering techniques, with important auxiliary results including new inequalities and principles that are of independent interest.
...
TL;DR: Elementary proof of long-time inviscid damping for Sobolev perturbations near Couette flow in 2D Euler equations, with velocity damping estimates up to time scales O(ε^{-δ_s}) for different Sobolev exponents.


<details>
  <summary>Details</summary>
Motivation: To provide a simple, elementary proof of inviscid damping phenomena for perturbations near the Couette flow in 2D Euler equations, establishing long-time stability results.

Method: Elementary proof approach for analyzing Sobolev perturbations of size O(ε) in H^s spaces near the Couette flow (y,0) on 𝕋 × ℝ domain.

Result: Velocity damping estimates valid up to time scale t = O(ε^{-δ_s}), where δ_s = 1/3 when s→1+ and δ_s = 1/2 for s > 2.

Conclusion: The paper successfully establishes long-time inviscid damping for Sobolev perturbations near Couette flow with explicit time scales depending on the Sobolev exponent.
...
TL;DR: Systematic review of 22 studies (2024-2026) showing ML/DL models outperform traditional methods in financial forecasting, with Random Forest, XGBoost, LSTM, and hybrid approaches being most effective.


<details>
  <summary>Details</summary>
Motivation: To examine how machine learning and deep learning have transformed financial forecasting, decision-making, and modeling by promoting innovation and efficiency in financial systems.

Method: Systematic review following PRISMA 2020 guidelines, analyzing 22 peer-reviewed articles from Scopus (2024-2026) across credit risk prediction, cryptocurrency, asset pricing, and macroeconomic policy modeling.

Result: ML/DL techniques outperform traditional models by capturing nonlinear dependencies and enhancing predictive accuracy. Explainable AI methods (SHAP, feature importance) improve transparency. Most used models include Random Forest, XGBoost, SVM, LSTM, BiLSTM, CNN, and hybrid approaches.

Conclusion: AI-driven computational finance shows significant progress but faces challenges in interpretability, generalizability, and data quality. Future directions include cross-domain applications and responsible AI integration in finance.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of constructing solutions to the 3D Euler equations with stochastic perturbations that maintain key physical properties like energy conservation.

Method: Using probabilistic methods to construct strong solutions for the three-dimensional Euler equations with additive noise, ensuring continuity in time and Hölder regularity in space.

Result: Successfully constructed solutions that are almost surely continuous in time, Hölder in space, and satisfy the local energy inequality up to arbitrarily large stopping times.

Conclusion: The approach provides probabilistically strong solutions to stochastic 3D Euler equations with desirable regularity properties and physical consistency.
...
TL;DR: Analysis of generalized solitary waves in Boussinesq abcd system with variable bottom topography, focusing on collision problems and long-range interactions.


<details>
  <summary>Details</summary>
Motivation: To investigate the existence of generalized solitary waves and collision dynamics in the physically relevant variable bottom regime of the Boussinesq abcd system, which models shallow water waves.

Method: Construct new approximate solutions capturing interactions between solitary waves and slowly varying bottom topography represented by h=εh₀(εt,εx), using perturbation theory with small parameter ε.

Result: Established existence of generalized solitary waves and described their evolution without destruction when interacting with variable bottom topography.

Conclusion: The paper successfully demonstrates that solitary waves can persist and evolve in variable bottom regimes through careful construction of approximate solutions that capture long-range interactions.
...
TL;DR: Rigorous derivation of incompressible Navier-Stokes-Fourier system with time-dependent forcing from inelastic hard-spheres Boltzmann equation for viscoelastic granular gases, capturing nontrivial hydrodynamic effects and determining Haff's law.


<details>
  <summary>Details</summary>
Motivation: To establish the first rigorous hydrodynamic limit from the Boltzmann equation for viscoelastic granular gases, where inelasticity depends on particle velocities, overcoming limitations of constant restitution coefficient models.

Method: Used a self-similar change of variables that creates energy balance between inflow and outflow, leading to a non-autonomous rescaled Boltzmann equation. The scaling explicitly depends on Knudsen number and restitution coefficient.

Result: Proved convergence of solutions to the rescaled Boltzmann equation towards an incompressible Navier-Stokes-Fourier system with self-consistent time-dependent forcing terms. Determined exact dissipation rate of granular temperature (Haff's law).

Conclusion: Successfully derived a new hydrodynamic model for viscoelastic granular gases that naturally captures energy balance dynamics, providing rigorous foundation for granular flow modeling and exact determination of temperature dissipation.
...
TL;DR: A hybrid physics-neural model that predicts scalar transport in complex domains orders of magnitude faster than 3D simulations, using differentiable framework to learn physical parameters and neural closure models.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D numerical simulations are computationally expensive (hours), but most analysis only requires reduced metrics, creating need for faster surrogate models.

Method: End-to-end differentiable framework that jointly learns physical model parameterization (orthotropic diffusivity) and non-Markovian neural closure model to capture unresolved coarse-grained effects, enabling stable long time horizon rollouts.

Result: Achieves 0.96 Spearman correlation coefficient at final simulation time, runs orders of magnitude faster (from hours to <1 min), data-efficient (learns with only 26 training data), and generalizes to out-of-distribution scenarios like moving sources.

Conclusion: The differentiable physics-neural framework enables fast, accurate, and generalizable coarse-grained surrogates for physical phenomena, bridging computational efficiency with physical accuracy.
...
TL;DR: Muon-catalyzed fusion (μCF) can produce valuable isotopes using high-energy neutrons without breaching material heat flux limits, making it viable for isotope production before achieving energy breakeven.


<details>
  <summary>Details</summary>
Motivation: To improve economic prospects for muon-catalyzed fusion by using it for isotope production, which requires lower performance thresholds than energy generation.

Method: Employ μCF systems as intense neutron sources for transmuting valuable isotopes, leveraging the relaxed heat flux constraints since no external heating is required.

Result: A μCF system with 10^8 muons/second could produce 0.5 mg of ²²⁵Ac per year (10x current global supply) using 10g of ²²⁶Ra feedstock.

Conclusion: μCF systems should be accelerated for neutron-driven isotope production as they can be viable well before net energy generation is possible, opening up various radioisotope transmutation pathways.
...
### [47] [Physics Steering: Causal Control of Cross-Domain Concepts in a Physics Foundation Model](https://arxiv.org/abs/2511.20798)
...
Result: Successfully demonstrated causal control over physical behaviors by inducing or removing specific physical features from simulations through concept direction injection.

Conclusion: Scientific foundation models learn generalized representations of physical principles rather than superficial correlations, opening new avenues for understanding and controlling AI models in scientific discovery.
...
### [48] [SUPN: Shallow Universal Polynomial Networks](https://arxiv.org/abs/2511.21414)
*Zachary Morrow,Michael Penwarden,Brian Chen,Aurya Javeed,Akil Narayan,John D. Jakeman*

Main category: cs.LG

TL;DR: SUPNs (shallow universal polynomial networks) achieve better function approximation with fewer parameters than DNNs and KANs by using polynomial layers, reducing overparameterization issues.


<details>
  <summary>Details</summary>
Motivation: DNNs and KANs require many parameters leading to overparameterization, which reduces transparency and creates optimization challenges with local minima affecting generalization.

Method: Replace all but the last hidden layer with a single polynomial layer with learnable coefficients, combining DNN and polynomial strengths for expressivity with fewer parameters.

Result: SUPNs converge at same rate as best polynomial approximation, show lower approximation error and variability than DNNs/KANs by order of magnitude, even outperform polynomial projection on non-smooth functions.

Conclusion: SUPNs provide superior function approximation with fewer parameters, better optimization stability, and improved performance compared to existing methods.
...
### [49] [Direct numerical simulation of thermo-diffusively unstable premixed hydrogen-air flames in a fully-developed turbulent channel flow at $Re_τ=530$](https://arxiv.org/abs/2511.20930)
*Felix Rong,Max Schneider,Hendrik Nicolai,Christian Hasse,Andrea Gruber*

Main category: physics.flu-dyn

TL;DR: DNS study of premixed hydrogen-air flames in turbulent channel flow shows intrinsic thermo-diffusive phenomena enhance flame reactivity, with different behaviors for lean (φ=0.25) and richer (φ=0.35) mixtures interacting with wall turbulence.


<details>
  <summary>Details</summary>
Motivation: To understand how intrinsic thermo-diffusive phenomena interact with realistic near-wall shear turbulence in premixed hydrogen-air flames, particularly how varying turbulence intensity and wall-distance affect flame propagation characteristics.

Method: Direct Numerical Simulations (DNS) of premixed hydrogen-air flames in fully-developed turbulent channel flow at Re_τ=530, analyzing two flame conditions (φ=0.25 and φ=0.35) using local stretch factor I_0 to quantify reactivity enhancements.

Result: At φ=0.25, flame shows augmented I_0 due to thermo-diffusive interaction with weak core turbulence. At φ=0.35, flame experiences increasing turbulence intensity toward walls, with peak I_0 co-located with peak Reynolds stresses at y+~10, indicating strong synergistic interaction between thermo-diffusive phenomena and wall turbulence.

Conclusion: Thermo-diffusive phenomena significantly enhance flame reactivity in turbulent conditions, with particularly strong synergistic effects occurring in near-wall regions where peak reactivity coincides with peak turbulent stresses, leading to significantly enhanced flame speeds.
...
### [50] [$\texttt{CRLS}$: Convolutional Regularized Least Squares Framework for Reduced Order Modeling of Transonic Flows](https://arxiv.org/abs/2511.21425)
*Muhammad Bilal,Ashwin Renganathan*

Main category: physics.flu-dyn

TL;DR: CRLS framework for reduced-order modeling of transonic flows with shocks using convolutional smoothing and deconvolution to handle parameter-dependent discontinuities.


<details>
  <summary>Details</summary>
Motivation: Standard POD models perform poorly with parameter-dependent discontinuities in snapshots, leading to smeared shocks and non-physical oscillations in transonic flows.

Method: Apply 1D Gaussian convolution to smooth snapshots, extract POD bases from smoothed data, learn parametric dependence via RBF interpolation, and recover sharp shocks through regularized least squares deconvolution.

Result: CRLS shows improved shock location/strength, lower pressure/field errors, and 42% reduction in POD modes needed compared to standard POD and smoothed-POD baselines on RAE2822 airfoil transonic flow.

Conclusion: CRLS provides accurate, data-efficient, automated shock-aware reduced order models for high-speed aerodynamic design.
...
### [51] [Flows of conformally coclosed $G_2$-structures with dilaton](https://arxiv.org/abs/2511.21055)
*Spiro Karigiannis,Sébastien Picard,Caleb Suan*

Main category: math.DG

TL;DR: The paper studies G2-structure flows via dimensional reduction, showing how geometric flows in G2-geometry reduce to complex geometry flows. Key examples include the G2-Laplacian coflow (lifting Kähler-Ricci flow) and a 7D lift of anomaly flow.


<details>
  <summary>Details</summary>
Motivation: To understand natural geometric flows in G2-geometry through the principle of dimensional reduction, connecting them to established flows in complex geometry.

Method: Using dimensional reduction principle to lift flows from complex geometry to G2-geometry, specifically studying G2-Laplacian coflow and G2-anomaly flow on conformally coclosed G2-structures.

Result: Established connections between G2-geometry flows and complex geometry flows, with the G2-Laplacian coflow lifting Kähler-Ricci flow and a 7D anomaly flow lifting the complex anomaly flow.

Conclusion: The dimensional reduction principle provides a powerful framework for understanding G2-geometry flows by relating them to well-studied complex geometry flows, with potential applications to short-time existence and fixed point analysis.
...
### [52] [The Critical LYZ equation in Kähler Geometry](https://arxiv.org/abs/2511.21492)
...
<details>
  <summary>Details</summary>
Motivation: To solve the critical case of the solvability problem for phase θ≤(n-2)π/2 posed by Collins-Jacob-Yau and Li, and extend applications to Hessian equations with reduced assumptions.

Method: Establish existence of smooth solutions for the LYZ equation at the critical phase θ=(n-2)π/2.

Result: Proved existence of smooth solutions at critical phase, enabling solutions to 3D Hessian equation σ₂=1 and 4D Hessian quotient equation σ₃=σ₁ under weaker conditions.

Conclusion: The critical phase problem for LYZ equation is solved, providing new results for Hessian equations with reduced assumptions.
...
TL;DR: Pseudospectral optimal control theory combines pseudospectral theory with optimal control theory, with applications in aerospace and autonomous systems.


<details>
  <summary>Details</summary>
Motivation: Both optimal control and pseudospectral theory share Sobolev spaces as their home space, making their combination natural for solving challenging control problems.

Method: Combining pseudospectral theory with optimal control theory to construct pseudospectral optimal control theory, with implementation on NASA spacecraft and embedded platforms.

Result: Successful flight demonstrations onboard NASA spacecraft and the 2011 launch of pseudospectral optimal control in embedded platforms.

Conclusion: Pseudospectral optimal control is changing how we solve challenging control problems in aerospace and autonomous systems through emerging trends in both theory and practice.
...
### [54] [Sublinear Time Low-Rank Approximation of Hankel Matrices](https://arxiv.org/abs/2511.21418)
*Michael Kapralov,Cameron Musco,Kshiteej Sheth*

Main category: cs.DS

TL;DR: First sublinear-time algorithm for low-rank approximation of PSD Hankel matrices with polylog(n,1/ε) runtime and robust error guarantees.


<details>
  <summary>Details</summary>
Motivation: PSD Hankel matrices are known to be approximately low-rank (Beckermann-Townsend bound), making them ideal targets for efficient low-rank approximation algorithms, but no sublinear-time methods existed.

Method: Structure-preserving existence proof followed by sampling-based algorithm using Vandermonde structure and universal ridge leverage score bounds.

Result: Achieves rank-O(log n log(1/ε)) approximation in polylog(n,1/ε) time with error matching Beckermann-Townsend bound, plus robustness to non-Hankel noise.

Conclusion: Provides first sublinear-time algorithm for PSD Hankel matrix approximation with theoretical guarantees, establishing finite-dimensional analog of AAK theorem.
...
### [55] [Opportunities and Challenges of Computational Electromagnetics Methods for Superconducting Circuit Quantum Device Modeling: A Practical Review](https://arxiv.org/abs/2511.20774)
*Samuel T. Elkin,Ghazi Khan,Ebrahim Forati,Brandon W. Langley,Dogan Timucin,Reza Molavi,Sara Sussman,Thomas E. Roth*

Main category: quant-ph

TL;DR: This review provides a practical introduction to computational electromagnetics (CEM) methods, focusing on challenges in modeling multiscale superconducting circuit quantum devices and offering guidance for tool selection and problem mitigation.


<details>
  <summary>Details</summary>
Motivation: Emerging superconducting circuit quantum devices present unique challenges for CEM methods due to unconventional material properties and multiscale features (nanometer to centimeter scales), which can stress existing tools and lead to simulation failures, accuracy loss, or increased computation times.

Method: The paper reviews fundamental aspects of major CEM techniques, highlighting main concepts without rigorous derivations, and discusses advanced topics related to multiscale modeling challenges with specific examples from superconducting circuit quantum devices.

Result: The review serves as a practical guide for researchers to choose appropriate CEM techniques and provides insights into steps to alleviate modeling challenges in multiscale device simulation.

Conclusion: Future research directions are needed to improve the design capabilities for increasingly sophisticated superconducting circuit quantum devices, and while focused on this area, the insights benefit researchers from other fields facing similar multiscale electromagnetic modeling challenges.
...
### [56] [Rapid ground state energy estimation with a Sparse Pauli Dynamics-enabled Variational Double Bracket Flow](https://arxiv.org/abs/2511.21651)
*Chinmay Shrikhande,Arnab Bachhar,Aaron Rodriguez Jimenez,Nicholas J. Mayhall*

Main category: quant-ph

TL;DR: A variational double bracket flow algorithm using Sparse Pauli Dynamics achieves efficient ground state energy estimation for strongly correlated quantum systems with <1% error compared to DMRG, showing significant speedups (10 min vs 50+ hours) for large systems.


<details>
  <summary>Details</summary>
Motivation: Ground state energy estimation for strongly correlated quantum systems is computationally challenging, especially for higher-dimensional problems where tensor network methods like DMRG become inefficient.

Method: Variational double bracket flow (vDBF) algorithm that leverages Sparse Pauli Dynamics with greedy operator selection, coefficient truncation, and energy-variance extrapolation.

Result: Achieves <1% error relative to DMRG benchmarks for Heisenberg and Hubbard models in 1D and 2D. For 100-qubit Heisenberg lattice: 10 min on single CPU vs 50+ hours on 64 threads for DMRG. Even larger speedup for 128-qubit Hubbard model.

Conclusion: Classical simulation techniques developed for quantum advantage benchmarking can provide practical tools for many-body physics, offering efficient alternatives to traditional methods like DMRG.
...
### [57] [AI4X Roadmap: Artificial Intelligence for the advancement of scientific pursuit and its future directions](https://arxiv.org/abs/2511.20976)
...
TL;DR: AI is transforming scientific discovery by extending research capabilities across multiple domains, with key themes including trustworthy data, transferable models, integrated workflows, and generative systems grounded in practical synthesis.
...
Method: Analysis across biology, chemistry, climate science, mathematics, materials science, physics, self-driving laboratories and unconventional computing, identifying common themes and requirements.

Result: Identified the need for diverse trustworthy data, transferable models, integrated AI systems in scientific workflows, and generative approaches focused on synthesisability rather than idealized phases.

Conclusion: AI systems must be more transparent and capable of accelerating discovery in complex real-world environments by closing loops between prediction and validation while maintaining reproducibility and physical interpretability.
...
TL;DR: A unified bond dipole theory explains interface dipoles and Fermi level pinning at metal-semiconductor interfaces through localized bonding between semiconductor dangling bonds and metal orbitals.


<details>
  <summary>Details</summary>
Motivation: To provide a microscopic understanding of interface dipoles and Fermi level pinning by unifying various interface states (MIGS, DBSS, bonding states) under a single bonding mechanism framework.
...
Result: Localized bonding between semiconductor dangling bonds and metal orbitals generates large interface dipoles and strong Fermi level pinning, even with just a single metal monolayer. The key parameter is the density of surface dangling bonds that can form chemical bonds with the metal.

Conclusion: The unified bond dipole theory explains weaker pinning in ionic semiconductors versus covalent ones and provides practical guidance for engineering metal-semiconductor interfaces and tuning Schottky barrier heights.
...
### [59] [Stochastic Optimal Control of Interacting Particle Systems in Hilbert Spaces and Applications](https://arxiv.org/abs/2511.21646)
*Filippo de Feo,Fausto Gozzi,Andrzej Święch,Lukas Wessels*

Main category: math.PR

TL;DR: This paper develops the theoretical foundation for optimal control of interacting particle systems in Hilbert spaces, proving convergence of finite particle value functions to mean-field Hamilton-Jacobi-Bellman equations and establishing connections between particle system controls and their limiting counterparts.


<details>
  <summary>Details</summary>
Motivation: Optimal control of interacting particles governed by stochastic evolution equations in Hilbert spaces is an open research area, with applications in systems modeled by stochastic PDEs, delay differential equations, and partially observed stochastic systems.

Method: The authors prove convergence of finite particle system value functions to mean-field Hamilton-Jacobi-Bellman equations, establish connections between particle system controls and lifted limit control problems, and demonstrate C^{1,1}-regularity under additional assumptions.

Result: The paper shows that value functions converge to unique L-viscosity solutions of mean-field HJB equations, identifies optimal controls correspondence between particle systems and lifted problems, and applies the theory to economic problems with stochastic delay and partial differential equations.

Conclusion: These are the first results for stochastic optimal control of interacting particle systems of stochastic evolution equations in Hilbert spaces, providing foundational theory with applications in economics.
...
TL;DR: Study shows twisted bismuth bilayers exhibit edge-enhanced electronic properties, with electronic density of states at Fermi level increasing up to 10x, potentially enabling higher superconducting critical temperatures.


<details>
  <summary>Details</summary>
Motivation: Address boundary heterogeneity concerns in functional twistronic devices, as standard studies focus mainly on twist-angles while ignoring edge effects in real devices.
...
Result: Found dramatic enhancement of electronic density of states at Fermi level (up to 10x increase) due to structural disorder at flake edges, with continuous property variations away from bulk.

Conclusion: Structural disorder at edges provides powerful mechanism for increasing superconducting Tc, and twist-angle is identified as critical parameter for designing devices with topologically enhanced properties.
...
### [61] [Approximate Bayesian Computation Made Easy: A Practical Guide to ABC-SMC for Dynamical Systems with \texttt{pymc}](https://arxiv.org/abs/2511.21587)
*Mario Castro*
...
<details>
  <summary>Details</summary>
Motivation: To bridge the gap between the potential of ABC-SMC methods and their adoption by researchers who perceive them as complex, by providing an accessible, example-driven introduction.

Method: Uses Approximate Bayesian Computation with Sequential Monte Carlo (ABC-SMC) implemented in Python with PyMC's probabilistic programming interface, demonstrated through predator-prey dynamics and hierarchical epidemic models.

Result: Provides working implementations and diagnostic tools for ABC-SMC analyses, illustrating how to handle partial observability, parameter identifiability, and hierarchical structures in Bayesian frameworks.

Conclusion: ABC-SMC offers a powerful likelihood-free alternative for parameter inference in mechanistic models, and this tutorial makes it accessible through practical Python examples that ensure reproducibility and easy adaptation.
...
TL;DR: Quantum hard-sphere fluid analyzed using affine-quantization and path integral Monte Carlo methods under Bose-Einstein statistics


<details>
  <summary>Details</summary>
Motivation: To study the thermodynamic properties of quantum hard-sphere fluids using modern quantum mechanical approaches

Method: Applied affine-quantization and path integral Monte Carlo method to solve for thermodynamic properties under Bose-Einstein statistics

Result: The paper presents solutions for the thermodynamic properties of quantum hard-sphere fluids

Conclusion: Affine-quantization combined with path integral Monte Carlo provides an effective approach for analyzing quantum hard-sphere fluids
...
### [63] [Impact of Cosmic Ray Distribution on the Growth and Saturation of Bell Instability](https://arxiv.org/abs/2511.21154)
*Saikat Das,Siddhartha Gupta,Prateek Sharma*

Main category: astro-ph.HE

TL;DR: The paper studies how cosmic ray distributions affect non-resonant streaming instability growth and saturation using kinetic simulations, finding that low-energy CRs dominate saturation while high-energy CRs remain weakly scattered.
...
Method: One-dimensional kinetic simulations investigating mono-energetic and power-law cosmic ray momentum distributions.

Result: Linear growth depends only on CR current, but saturation strongly depends on CR distribution. Low-energy CRs dominate current relaxation and magnetic growth, while high-energy CRs remain weakly scattered. Mono-energetic CRs effectively amplify magnetic fields and isotropize.

Conclusion: A modified saturation prescription is provided and a layered CR-confinement scenario is proposed upstream of astrophysical shocks, relevant to high-energy particle acceleration.
