...
TL;DR: A novel numerical method for solving the 1D1V Boltzmann-BGK equation using operator splitting, third-order transport scheme, and collision treatment with exact conservation properties.


<details>
  <summary>Details</summary>
Motivation: To develop an accurate, efficient, and robust numerical method for simulating gas dynamics using the Boltzmann equation that works across all flow regimes while maintaining exact conservation of mass, momentum, and energy.

Method: Operator splitting separates transport and collision steps. Uses third-order Lax-Wendroff-type scheme for transport and second-order L-stable TR-BDF method for collision. Introduces novel collision treatment using quadratic Hermite polynomial to enforce exact conservation despite truncated velocity range and quadrature errors.

Result: The scheme demonstrates exact conservation of mass, momentum, and energy up to machine precision when tested on piecewise constant initial data with periodic boundary conditions. Method is implemented in freely available Java code.

Conclusion: The developed numerical method successfully combines multiple techniques to create an accurate and robust solver for the Boltzmann-BGK equation that maintains exact conservation properties across all flow regimes.
...
TL;DR: A divide-and-conquer framework using 1D multi-channel CNNs with circular padding solves inverse electromagnetic scattering for magneto-dielectric cylinders covering impedance cylinders.
...
Method: Uses specially designed 1D multi-channel Convolutional Neural Networks with circular padding in a divide-and-conquer framework. The direct problem solution provides far-field measurements as input data. The approach first classifies the impedance cylinder shape, then reconstructs the boundary curve and impedance function.

Result: Extensive numerical experiments, including noisy scenarios, demonstrate the efficiency and robustness of the proposed approach in accurately reconstructing boundary curves and impedance functions.

Conclusion: The divide-and-conquer framework with 1D multi-channel CNNs provides an effective and robust solution for inverse electromagnetic scattering problems involving complex magneto-dielectric and impedance cylinder configurations.
...
TL;DR: A dual-domain deep learning framework accelerates multiscale basis function computation in MGMsFEM for Darcy flow in heterogeneous porous media, achieving significant speedup while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Darcy flow in heterogeneous porous media has multiscale characteristics that challenge conventional numerical methods. While MGMsFEM addresses these challenges, constructing multiscale basis functions remains computationally expensive.

Method: Proposes a dual-domain deep learning framework that extracts and decodes permeability field features in both frequency and spatial domains to rapidly generate numerical matrices of multiscale basis functions.

Result: Numerical experiments show the framework achieves significant computational acceleration while maintaining high approximation accuracy.

Conclusion: The method offers potential for future applications in real-world reservoir engineering by enabling faster computation of multiscale basis functions in MGMsFEM.
...
TL;DR: A hybrid approach combining local OpInf ROMs and FOMs using overlapping Schwarz alternating method for efficient multiscale simulations with 106x speedup.


<details>
  <summary>Details</summary>
Motivation: Address long runtime and complex mesh generation in traditional high-fidelity multiscale simulations.

Method: Couple subdomain-local non-intrusive OpInf ROMs with FOMs using overlapping Schwarz alternating method, enabling integration of disparate models, meshes, and time schemes.

Result: Demonstrated on 3D solid dynamics problems with up to 106x speedup compared to conventional FOM-FOM couplings.

Conclusion: Enables more efficient simulation workflows for engineering applications with potential extensions to various PDEs.
...
Method: Applied both intrusive (projection-based POD) and non-intrusive (data-driven DMD and operator inference) model order reduction methods to wave propagation problems, with theoretical foundations recalled and three numerical examples used for evaluation.

Result: Evaluated the performance of different reduction techniques across three numerical examples, demonstrating the effectiveness of both intrusive and non-intrusive approaches for accelerating wave propagation simulations.

Conclusion: Model order reduction methods, whether intrusive or non-intrusive, provide effective means to accelerate wave propagation simulations in nonlinear and damaged materials, with the choice depending on equation knowledge and operator availability.
...
TL;DR: A fast algorithm for evaluating scalar wave equation solutions from multiple point sources, reducing computational cost from O(M²N_t) to O((M + N³logN)N_t) using windowed Fourier projection and avoiding absorbing boundary conditions.


<details>
  <summary>Details</summary>
Motivation: To efficiently solve the scalar wave equation driven by many point sources, as naive evaluation requires O(M²N_t) work which becomes computationally prohibitive for large M.

Method: Uses windowed Fourier projection (WFP) method to split potential into local part (evaluated directly) and smooth history part approximated by N³-point Fourier transform with simple recursion relations for Fourier coefficients, controlling oscillations via spatial truncation of hyperbolic Green's function.

Result: Achieves 6-digit accuracy with up to 1 million sources and targets, demonstrating significant computational efficiency improvement over naive methods.

Conclusion: The method provides an efficient solution for time-domain wave equation scattering problems and can serve as a key component in addressing such problems.
...
TL;DR: This paper provides theoretical bounds on the variance and concentration of error for stochastic iterative methods, going beyond expected error analysis to understand near-worst-case behavior.


<details>
  <summary>Details</summary>
Motivation: Stochastic iterative methods are widely used in large-scale problems but most theoretical results only provide expected error bounds. Understanding their behavior in near-worst-case scenarios is important for practical applications.

Method: The authors develop upper bounds for concentration and variance of error for both linear stochastic iterative methods (including randomized Kaczmarz and Gauss-Seidel) and nonlinear stochastic iterative methods (including randomized Kaczmarz for linear inequalities).

Result: The paper provides theoretical upper bounds on the variance and concentration of error for these stochastic methods, enabling generation of confidence intervals around expected error bounds.

Conclusion: This work enhances the theoretical understanding of stochastic iterative methods by providing variance and concentration bounds, which is crucial for practical applications where worst-case behavior matters.
...
TL;DR: Optimal recovery for Poisson problem with unknown boundary data using interior point measurements, with improved error estimates for Riesz representers and recovery algorithm performance.


<details>
  <summary>Details</summary>
Motivation: To address the Poisson problem when boundary data is unknown, using compensating interior point measurements to lower regularity requirements and improve recovery accuracy.

Method: Finite element algorithm with interior point measurements, focusing on finite element approximation of Riesz representers associated with measurements, with pointwise error analysis.

Result: Main result is a pointwise error estimate for Riesz representers, leading to improved performance estimates for the recovery algorithm in various norms.

Conclusion: The approach provides enhanced recovery performance for the Poisson problem with unknown boundary data by using interior point measurements and improved Riesz representer approximations.
...
TL;DR: Sharp stability results for ascent and descent spectra under strong resolvent convergence, with computable finite-element diagnostic γ_h serving as practical surrogate for reduced minimum modulus γ(T-λ).


<details>
  <summary>Details</summary>
Motivation: To establish spectral stability for non-selfadjoint operators and singularly perturbed problems in finite element approximations, where norm resolvent convergence fails for rough or singular limits.

Method: Uses strong resolvent convergence (SRS) framework with reduced minimum modulus γ(T-λ)>0 as key quantitative hypothesis, transfers Kaashoek-Taylor criteria via gap convergence of operator graphs, and extends to B-Fredholm theory for powers.

Result: Numerical experiments confirm lim inf γ_h>0 is necessary and sufficient for spectral stability; finite-element diagnostic γ_h remains uniformly positive in convection-dominated regimes with stabilized schemes.

Conclusion: SRS combined with quantitative control of γ_h rescues ascent-descent stability in computational settings, while closed-range condition is indispensable for powers as demonstrated by Volterra-type counterexample.
...
TL;DR: A fully implicit Crank-Nicolson discontinuous Galerkin method is developed for solving the Ginzburg-Landau equation, with proven unique solvability and unconditionally optimal error estimates.
...
Method: A fully implicit Crank-Nicolson discontinuous Galerkin method with novel analytical techniques to handle the cubic nonlinear term and establish rigorous mathematical properties.

Result: The method achieves unique solvability and unconditionally optimal error estimates in both L²-norm and energy norm, validated through two numerical examples.

Conclusion: The proposed method provides a reliable and accurate numerical framework for solving the Ginzburg-Landau equation with rigorous mathematical guarantees.
...
TL;DR: Developed preconditioned Krylov subspace methods with sine transform-based preconditioners to efficiently solve large dense Toeplitz systems from 3D space-fractional diffusion equations, proving linear convergence rates independent of matrix sizes.


<details>
  <summary>Details</summary>
Motivation: Three-dimensional conservative space-fractional diffusion equations discretized using Crank-Nicolson finite volume methods result in large, dense three-level Toeplitz linear systems that are computationally challenging to solve efficiently.

Method: Used preconditioned Krylov subspace methods: PCG for symmetric cases and PGMRES for non-symmetric cases, with sine transform-based preconditioners. Provided detailed convergence analysis showing spectra bounds and linear convergence rates.

Result: Proved preconditioned matrices have spectra uniformly bounded in (1/2, 3/2) for symmetric case, leading to linear convergence. Demonstrated PGMRES also achieves linear convergence independent of discretization stepsizes. Numerical experiments in 2D/3D confirmed optimal performance.

Conclusion: The proposed preconditioned Krylov subspace methods with sine transform-based preconditioners provide efficient, scalable solvers for 3D space-fractional diffusion equations with iteration counts independent of matrix sizes.
...
TL;DR: Novel p-adaptive high-order mesh-free framework for fluid flow simulation with dynamic polynomial order refinement based on local error estimates, achieving up to 50% computational cost savings while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: To enable accurate and efficient simulation of fluid flows in complex geometries by developing an adaptive framework that dynamically adjusts polynomial approximation order where needed, reducing computational costs without sacrificing accuracy.

Method: Constructs high-order differential operators using anisotropic basis functions for arbitrary node distributions, implements dynamic p-refinement strategy with a novel error indicator based on local Laplacian operator estimates, and locally adjusts polynomial order p during solution procedure.

Result: The method successfully captures highly non-linear regions requiring high-order approximation, maintains specified accuracy levels, and demonstrates up to 50% computational cost savings compared to non-adaptive methods in test cases including compressible reacting flow in porous media.

Conclusion: The p-adaptive high-order mesh-free method effectively balances computational efficiency and accuracy by dynamically adjusting polynomial order, proving particularly beneficial for complex fluid flow simulations in porous media while preserving solution stability.
...
TL;DR: Two new Rosenbrock methods for solving index-1 differential algebraic equations: Rodas6P (6th-order) and Tsit5DA (5th-order) based on Tsit5 method.


<details>
  <summary>Details</summary>
Motivation: To develop improved higher-order methods for solving index-1 differential algebraic equations, building on existing RodasP series and explicit-implicit approaches.

Method: Rodas6P follows same design principles as Rodas3P-5P methods; Tsit5DA uses explicit solution for differential equations and linear-implicit approach for algebraic equations.

Result: Theoretical properties verified through order tests and benchmarks, showing successful development of 6th-order Rodas6P and improved 5th-order Tsit5DA methods.

Conclusion: Two new effective Rosenbrock methods (Rodas6P and Tsit5DA) have been developed for solving index-1 differential algebraic equations, with verified theoretical properties.
...
TL;DR: This paper investigates algebraic multigrid (AMG) preconditioners for isogeometric analysis (IgA) systems, showing they achieve robust and scalable performance in HPC environments using PSCToolkit.


<details>
  <summary>Details</summary>
Motivation: IgA produces large, ill-conditioned linear systems with unfavorable scaling of bandwidth and condition number with degree p and dimension d, requiring specialized preconditioners for efficient solving.

Method: Uses AMG preconditioners tailored for IgA discretizations, implemented with distributed-memory and GPU-accelerated strategies via Parallel Sparse Computation Toolkit (PSCToolkit).

Result: AMG preconditioners demonstrate robust and scalable performance across benchmark tests, effectively handling large-scale IgA systems.

Conclusion: AMG preconditioners are practical and effective solvers for large IgA systems in engineering and scientific applications, confirming their potential in HPC environments.
...
TL;DR: A new polygonal finite element method called Zipped Finite Element Method for star-shaped polygons that constructs high-order shape functions using local sub-triangulations without increasing degrees of freedom.


<details>
  <summary>Details</summary>
Motivation: To develop an efficient finite element method for star-shaped polygons that maintains high-order accuracy while preserving conformity and avoiding increased computational complexity.

Method: Constructs high-order shape functions as linear combinations of standard finite element basis functions defined on local trivial sub-triangulations of each polygon element. The refinement is only used for shape function construction, not for the final degrees of freedom.
...
Conclusion: Numerical experiments confirm expected convergence rates, demonstrating the effectiveness of the Zipped Finite Element Method for star-shaped polygons while maintaining computational efficiency.
...
TL;DR: Proposes LHSS iteration method for complex symmetric indefinite linear systems, reducing computational costs by using real symmetric matrices. Extends to PLHSS with better convergence and optimal parameters.


<details>
  <summary>Details</summary>
Motivation: To reduce high computational costs of complex arithmetic in solving complex symmetric indefinite linear systems through efficient iterative methods.

Method: LHSS uses alternating iterative scheme with real symmetric matrices. PLHSS extends this with preconditioning, deriving optimal parameters and convergence rates.

Result: Theoretical analysis shows convergence depends on eigenvalues and parameters. PLHSS has superior convergence with well-clustered eigenvalues and orthogonal eigenvectors.

Conclusion: LHSS and PLHSS methods demonstrate mesh size independent and parameter-insensitive convergence, with PLHSS preconditioners showing efficiency in GMRES and COCG methods.
...
TL;DR: A vertex-based graph Fourier neural solver for linear elasticity on unstructured grids that combines block-Jacobi smoothing with learnable Fourier-domain correction, outperforming classical multigrid methods.


<details>
  <summary>Details</summary>
Motivation: Numerical solution of linear elasticity equations on unstructured grids is challenging with heterogeneous/anisotropic materials, requiring robust and efficient solvers.

Method: Vertex-based graph Fourier neural solver integrating block-Jacobi smoothing (for high-frequency errors) with learnable Fourier-domain module (for remaining frequency spectrum errors).

Result: Extensive 2D/3D experiments show superior robustness and efficiency over classical smoothed aggregation algebraic multigrid method; frequency analysis confirms complementarity between smoother and neural corrector.

Conclusion: The proposed solver effectively addresses linear elasticity challenges on unstructured grids through complementary smoothing and neural correction, achieving state-of-the-art performance.
...
TL;DR: HBVMs preserve energy and symplecticity for Hamiltonian systems. For linear systems, Krylov projection solvers are used. For nonlinear systems, simplified Newton iterations and Newton-Krylov methods with preconditioning are applied, along with adaptive time-stepping.


<details>
  <summary>Details</summary>
Motivation: Long-term preservation of energy and symplecticity is essential for Hamiltonian systems in various applications.

Method: Energy-conserving Hamiltonian Boundary Value Methods (HBVMs) with multi-stage schemes. For linear systems: Krylov projection solvers. For nonlinear systems: simplified Newton iterations and Newton-Krylov framework with preconditioning, combined with adaptive time-stepping.

Result: Numerical experiments on semi-discretized wave equations demonstrate the efficiency and robustness of the proposed approach.

Conclusion: The proposed HBVMs with specialized solvers and adaptive time-stepping provide an efficient and robust method for preserving energy and symplecticity in Hamiltonian systems.
...
TL;DR: This paper establishes stability criteria for neutral stochastic differential delay equations with Markovian switching and proves that the Euler-Maruyama method can reproduce the exponential decay rate of the true solution.


<details>
  <summary>Details</summary>
Motivation: To address the complex stability analysis of NSDDEs with Markovian switching, which involves interactions between neutral terms, time-varying delays, and structural changes governed by Markov chains.

Method: Theoretical analysis using novel stability criteria and numerical approximation via the Euler-Maruyama method.

Result: Established practical criteria for mean-square exponential stability and proved that numerical schemes can reproduce the exponential decay rate with arbitrary accuracy when step size is sufficiently small.

Conclusion: The theoretical framework provides effective stability analysis tools for NSDDEs with Markovian switching, with numerical validation demonstrating the approach's effectiveness.
...
TL;DR: This paper studies L^p-solvability of strongly-coupled nonlocal systems with vector-valued operators, proving existence and uniqueness of strong solutions for f in L^p spaces.


<details>
  <summary>Details</summary>
Motivation: To extend solvability results from scalar nonlocal equations to coupled vector-valued systems, addressing the mathematical challenges of strongly-coupled nonlocal operators.

Method: Uses method of continuity, establishes operator continuity and a priori estimates through parabolic system analysis, combining commutator estimates, Sobolev embeddings, level set estimates, and bootstrap arguments.

Result: Proves existence of unique strong solution u in [H^{2s,p}(R^d)]^d for any f in [L^p(R^d)]^d with 1<p<∞, for nonlocal operators with kernel comparable to |y|^{-(d+2s)}.

Conclusion: Successfully extends recent scalar nonlocal equation techniques to coupled vector-valued systems, providing comprehensive L^p-solvability theory for strongly-coupled nonlocal operators.
...
TL;DR: Higher integrability of gradients for local quasiminimizers in weighted Orlicz spaces, with existence of minimizers for associated functionals.


<details>
  <summary>Details</summary>
Motivation: Extend previous higher integrability results to weighted settings under generalized Orlicz growth conditions, covering weighted variable exponent and double phase frameworks.

Method: Assume weights belong to Muckenhoupt class and growth functions satisfy appropriate structural conditions, then prove local higher integrability of gradients for local quasiminimizers.

Result: Proved gradient higher integrability in weighted Orlicz spaces and established existence of minimizers for associated functionals.

Conclusion: Successfully extended higher integrability theory to weighted generalized Orlicz frameworks, providing unified approach for weighted variable exponent and double phase cases.
...
<details>
  <summary>Details</summary>
Motivation: To develop optimal constants for singular anisotropic Adams' type inequalities with precise growth conditions in both unbounded and bounded domains.

Method: Uses anisotropic rearrangement techniques to derive the inequalities.

Result: Obtained best constants for the singular anisotropic Adams' inequality in ℝⁿ and proved the inequality also holds on bounded domains Ω⊂ℝⁿ.

Conclusion: Successfully established optimal constants for singular anisotropic Adams' type inequalities with exact growth conditions in both ℝⁿ and bounded domains using anisotropic rearrangement methods.
...
TL;DR: This paper proves that for the Robin Laplacian problem, the ratio of the first two eigenvalues μ₂/μ₁ is maximized by a disk for all dimensions N≥2 and boundary parameters σ≥σₛ, and that this maximum ratio strictly decreases with σ.


<details>
  <summary>Details</summary>
Motivation: To resolve the Payne-Schaefer conjecture (2001) and Henrot's restatement (2003) about maximizing the ratio of the first two Robin eigenvalues, and to provide a positive answer to a variant of Yau's Problem 77 about determining circular drums from eigenfrequency ratios.

Method: Mathematical analysis of the Robin Laplacian eigenvalue problem, establishing properties of the eigenvalue ratio μ₂/μ₁ as a function of the boundary parameter σ and domain geometry.

Result: Proved that the disk maximizes μ₂/μ₁ for all dimensions N≥2 and all σ≥σₛ (where σₛ is a critical positive value), and showed that the maximum value of μ₂/μ₁ strictly decreases with σ over (0,∞).

Conclusion: The work confirms the Payne-Schaefer conjecture for the Robin Laplacian and demonstrates that measuring the ratio of first two eigenfrequencies can determine whether an elastically supported drum is circular.
...
TL;DR: This paper presents an inverse boundary value problem for the Helmholtz equation, aiming to reconstruct a Dirichlet polygonal obstacle and two constant coefficients from a single pair of Cauchy data using adapted factorization methods.


<details>
  <summary>Details</summary>
Motivation: To solve the challenging inverse problem of simultaneously recovering both geometric (polygonal obstacle location/shape) and physical (two constant coefficients) parameters from limited boundary data in Helmholtz equation applications.

Method: Adapted the one-wave factorization method to recover the polygonal obstacle and coefficients, and employed a modified factorization using the Dirichlet-to-Neumann operator to handle eigenvalue-related difficulties.

Result: Verified uniqueness results under certain a priori assumptions and demonstrated through intensive numerical examples that the proposed method is efficient for solving this inverse problem.

Conclusion: The developed factorization approach successfully addresses the inverse boundary value problem, enabling simultaneous reconstruction of geometric obstacles and physical coefficients from minimal boundary data.
...
TL;DR: Analysis of a phase-field model for curvature-driven pattern formation in biomembranes, including existence, uniqueness, and numerical simulations of various membrane patterns.


<details>
  <summary>Details</summary>
Motivation: To study pattern formation in biomembranes using a phase-field approach that approximates the two-phase Canham-Helfrich energy, enabling mathematical analysis and numerical simulation of curvature-driven structures.

Method: Derived as a gradient flow of an energy functional, leading to coupled Cahn-Hilliard and fourth-order reaction-diffusion equations. Used minimizing movement scheme for existence proofs, Moreau-Yosida regularization for singular potentials, and finite element discretization for numerical simulations.

Result: Proved existence of weak solutions for regular and singular potentials, established higher regularity and uniqueness. Numerical experiments revealed different membrane patterns (striped, dotted, snake-like) depending on physical parameters.

Conclusion: The developed phase-field model successfully captures curvature-driven pattern formation in biomembranes, with rigorous mathematical analysis and numerical validation showing diverse structural patterns emerging from different parameter regimes.
...
TL;DR: The paper proves that solutions to Δu=Vu with nonnegative bounded potential V must vanish identically if their L² norm decays to zero over expanding annular regions, establishing the Landis conjecture for algebraically decaying solutions.


<details>
  <summary>Details</summary>
Motivation: To establish rigorous conditions under which solutions to Schrödinger-type equations with nonnegative potentials must vanish identically, particularly addressing the Landis conjecture about decay properties of solutions.

Method: Analysis of solutions to Δu=Vu using integral estimates over expanding annular domains, with extension to exterior domains and nonlinear Schrödinger equations under conditions on the zero set of the potential.

Result: Proved that if the L² norm of u over sequences of annular regions r_j≤|x|≤r_j+1 decays to zero, then u≡0 on ℝ^d, establishing Landis conjecture for algebraically decaying solutions.

Conclusion: The decay condition on annular regions provides a sufficient criterion for unique continuation and vanishing of solutions, with generalizations to exterior domains and nonlinear equations.
...
TL;DR: Improved stability threshold for Couette flow in rotating Navier-Stokes system, achieving Re^{-α} scaling with α > 2/3 or α ≥ 5/6 depending on domain.


<details>
  <summary>Details</summary>
Motivation: To understand the transition from laminar to turbulent flow by quantifying perturbation thresholds that trigger instability in Couette flow under rotation.

Method: Used anisotropic Sobolev spaces tailored to zero modes, introduced new dispersive structure for zero modes with Strichartz-type estimates, and exploited combined effects of rotation (dispersion) and mixing mechanisms.

Result: Proved global existence and asymptotic stability of Couette flow for initial perturbations bounded by ε₀Re^{-α} with α > 2/3 (2D domain) or α ≥ 5/6 (1D×torus domain).

Conclusion: The improved stability threshold demonstrates the crucial role of dispersive effects from rotation and careful treatment of zero/non-zero mode interactions in achieving better scaling with Reynolds number.
...
TL;DR: Existence of normalized standing wave solutions for nonlinear Schrödinger-Bopp-Podolsky system with mass constraint, using perturbation methods.


<details>
  <summary>Details</summary>
Motivation: Study normalized solutions for coupled Schrödinger-Bopp-Podolsky system with L² mass constraint, addressing existence and multiplicity under different boundary conditions.

Method: Adapted perturbation method to analyze the system with Navier or Neumann boundary conditions for phi, considering technical conditions on nonlinearity f.

Result: Proved existence of normalized solutions for all masses μ in interval (0, μ₀), multiplicity when f is odd, and normalized ground state solutions for star-shaped domains.

Conclusion: The perturbation method successfully establishes existence and multiplicity results for normalized standing wave solutions in the Schrödinger-Bopp-Podolsky system under various conditions.
...
TL;DR: Complete classification of existence/nonexistence of viscosity solutions for eigenvalue-type Dirichlet problems with distance function terms on convex domains, including all exponent cases and global estimates.


<details>
  <summary>Details</summary>
Motivation: To systematically study degenerate/singular eigenvalue equations with distance functions in the right-hand side, which can cause boundary degeneracy/singularity in convex domains.

Method: Adaptations of Perron method and comparison principle, construction of classical sub-solutions and super-solutions, investigation of all distance function exponent cases.

Result: Complete classification of existence and nonexistence results, derivation of global estimates based on distance function when solutions exist.

Conclusion: Provides comprehensive understanding of viscosity solutions for this class of degenerate/singular eigenvalue equations with distance function terms through systematic analysis and construction methods.
...
TL;DR: Multiplicity results for Gross-Pitaevskii equation solutions on Riemannian manifolds, with lower bounds based on topology of velocity field.


<details>
  <summary>Details</summary>
Motivation: Study critical points of Ginzburg-Landau energy with prescribed momentum from tangent velocity fields, beyond just minimizers.

Method: Critical point theory, Γ-convergence for Ginzburg-Landau functionals, and new results for codimension 2 isoperimetric problems.

Result: Lower bounds on solution multiplicity in terms of topology of maximum velocity set, valid for small momentum and vorticity core size.

Conclusion: Topological structure of velocity field determines minimum number of solutions, with applications in small flux regime.
...
TL;DR: Analysis of qualitative properties of single blow-up solutions to nonlocal equations with slightly subcritical exponents in bounded domains, focusing on eigenvalue estimates and Morse index calculations.


<details>
  <summary>Details</summary>
Motivation: To understand the qualitative behavior of blow-up solutions in nonlocal equations with critical exponents, particularly examining eigenpairs and Morse indices in a nondegenerate setting.

Method: Using local Pohozaev identities and blow-up analysis to estimate first (n+2)-eigenvalues and their eigenfunctions for the linearized problem of nonlocal equations.

Result: Provides estimates on eigenvalues and eigenfunctions, and derives the Morse index of single-bubble solutions in nondegenerate cases for dimensions n=3,4,5.

Conclusion: The study successfully characterizes the qualitative properties of blow-up solutions and establishes the Morse index for single-bubble solutions in nonlocal critical equations.
...
TL;DR: Existence proof for Dirichlet problem in radiation hydrodynamics with L¹ data, extending previous bounded solution results to unbounded solutions using new theoretical developments.


<details>
  <summary>Details</summary>
Motivation: To address the Dirichlet problem for radiation hydrodynamics equations with L¹ data, where previous work only handled regular data (Lᴺ) and bounded solutions, requiring new approaches for unbounded solutions.

Method: Used new theoretical developments beyond Anzellotti pairing theory to define solution meaning for unbounded solutions, and studied regularity for Lᵖ data with 1<p<N.

Result: Proved existence of solutions for L¹ data where solutions cannot be bounded, and established regularity results consistent with previous findings for Lᵖ data.

Conclusion: Successfully extended existence theory to L¹ data cases requiring unbounded solutions, with coherent regularity results that align with previous bounded solution frameworks.
...
TL;DR: Existence of positive solution for Laplacian system using Nehari manifold and fibering maps


<details>
  <summary>Details</summary>
Motivation: To prove the existence of at least one positive solution for a specific Laplacian system with nonlinear terms involving parameters λ, β, and functions a(x), b(x)

Method: Using the Nehari manifold and fibering maps associated with the Euler functional for the system

Result: Proved the existence of at least one positive solution for the Laplacian system on a bounded region Ω

Conclusion: The Nehari manifold and fibering maps approach successfully establishes the existence of positive solutions for this class of Laplacian systems
...
TL;DR: Existence of polychromatic breather solutions for cubically nonlinear Maxwell equations in dispersive media with complex frequencies causing temporal decay, constructed as Fourier series via iterative linear ODEs.


<details>
  <summary>Details</summary>
Motivation: Study nonlinear electromagnetic wave propagation in dispersive media, particularly polychromatic surface plasmons at waveguide interfaces, where complex frequencies lead to temporal decay.

Method: Construct solutions as Fourier series in propagation direction and time, with leading frequency as eigenvalue of operator pencil. Use iterative sequence of linear ordinary differential equations and resolvent estimates.

Result: Proved existence of polychromatic breather solutions under spectral assumptions and resolvent estimates, with concrete example of interface between two homogeneous media satisfying conditions.

Conclusion: Successfully established existence theory for nonlinear polychromatic surface plasmons in waveguide geometries with dispersive media, providing mathematical framework for such localized wave phenomena.
...
TL;DR: Develops Hamilton-Jacobi framework for analyzing propagation dynamics in field-road systems with unidirectional advection and Wentzell boundary conditions, revealing critical transitions between rectilinear and road-assisted propagation regimes.


<details>
  <summary>Details</summary>
Motivation: To understand asymptotic propagation dynamics in complex field-road systems where classical comparison methods fail, particularly for non-order-preserving systems with degenerate media and boundary effects.

Method: Uses Hamilton-Jacobi variational inequality as singular limit of reaction-diffusion system, synthesizing viscosity solution theory, optimal control formulation, and variational analysis to derive existence, uniqueness, and explicit variational representation.

Result: Establishes critical transition in propagation behavior governed by geometrically derived curve separating rectilinear and road-assisted regimes, with solution characterized by fundamental solution via optimal paths.

Conclusion: The framework successfully extends to non-order-preserving systems and conical domains, demonstrating robustness of variational methodology in capturing complex propagation patterns shaped by advection-diffusion interplay.
...
TL;DR: Existence of weak solutions for elliptic PDE systems with quasimonotone nonlinearities, using monotone iteration for ordered solutions and growth conditions for non-monotone cases.


<details>
  <summary>Details</summary>
Motivation: To establish existence results for coupled elliptic PDE systems with quasimonotone nonlinearities both in the domain interior and on the boundary, addressing cases with and without monotonicity.

Method: Employ monotone iteration techniques for systems satisfying monotonicity conditions to find minimal and maximal solutions between sub- and supersolutions; use growth conditions for non-monotone cases.

Result: Proved existence of weak solutions in both monotone and non-monotone scenarios, with concrete examples demonstrating applicability.

Conclusion: The paper successfully establishes existence theorems for weak solutions of elliptic PDE systems with quasimonotone nonlinearities, providing both theoretical results and practical examples.
...
TL;DR: The paper analyzes a hyperbolic Cahn-Hilliard tumor growth model with inertial term, proving well-posedness, continuous dependence on therapy parameters, and convergence to viscous model as inertia vanishes.


<details>
  <summary>Details</summary>
Motivation: To study tumor growth models where the chemical potential relaxation is hyperbolic rather than parabolic, allowing for inertial effects and broader applicability to various therapy scenarios.

Method: Mathematical analysis of initial-boundary value problems for hyperbolic Cahn-Hilliard tumor growth models, including well-posedness proofs, continuous dependence on therapy functions, and asymptotic analysis as inertial coefficient approaches zero.

Result: The model is well-posed with continuous dependence on drug and antiangiogenic therapy functions. Solutions converge to viscous Cahn-Hilliard tumor growth model when inertial coefficient tends to zero, applicable to broad class of double-well potentials including nonsmooth ones.

Conclusion: The hyperbolic relaxation approach provides a mathematically rigorous framework for tumor growth modeling that connects to classical viscous models while accommodating inertial effects and various therapeutic interventions.
...
TL;DR: Extension of Lagrangian formulation for Navier-Stokes to broader hydrodynamic models, derived from stochastic Hamilton-Pontryagin variational principle, with generalized Kelvin circulation theorem and local well-posedness results.


<details>
  <summary>Details</summary>
Motivation: To extend the Lagrangian formulation beyond Navier-Stokes to a wider class of hydrodynamic models and establish connections with stochastic variational principles.

Method: Stochastic Hamilton-Pontryagin variational principle, Lagrangian-Eulerian formulation, fixed point arguments for well-posedness analysis.

Result: Successfully extended Lagrangian formulation to broader hydrodynamic models, derived from stochastic variational principle, established generalized Kelvin circulation theorem, and proved local well-posedness for certain fluid equations.

Conclusion: The Lagrangian formulation can be systematically extended to various hydrodynamic models through stochastic variational principles, providing unified framework and mathematical foundations for fluid dynamics analysis.
...
<details>
  <summary>Details</summary>
Motivation: Previous work by Schottdorf established global well-posedness for small L² data in the mNV system, but the large data problem remained open. This paper addresses this gap.

Method: Uses inverse scattering methods and proves a new nonlinear Gagliardo-Nirenberg inequality for the scattering transform. Also employs the Miura map to connect mNV and NV flows.

Result: Proves mNV system is globally well-posed for large L² data with scattering behavior. Also establishes global well-posedness for NV problem at critical H⁻¹ + L¹ level for soliton-free data.

Conclusion: The mNV system exhibits global well-posedness and scattering for large initial data, with key technical contributions including new inequalities and the application of inverse scattering techniques.
...
TL;DR: Elementary proof of long-time inviscid damping for Sobolev perturbations near Couette flow in 2D Euler equations, with velocity damping estimates up to time O(ε^{-δ_s}) for different Sobolev exponents.


<details>
  <summary>Details</summary>
Motivation: To establish rigorous mathematical understanding of inviscid damping phenomena in 2D Euler equations near Couette flow, providing explicit time scales for velocity decay without viscosity.

Method: Elementary proof approach using Sobolev space analysis for vorticity perturbations in H^s spaces, with careful treatment of different Sobolev exponents s>1.

Result: Velocity damping estimates valid up to time t = O(ε^{-δ_s}), where δ_s = 1/3 as s→1+ and δ_s = 1/2 for s>2, for initial vorticity perturbations of size O(ε) in H^s.

Conclusion: The elementary proof successfully establishes long-time inviscid damping for 2D Euler equations near Couette flow, with explicit time scales depending on Sobolev regularity of initial perturbations.
...
TL;DR: Systematic review of ML/DL applications in finance (2024-2026) showing superior performance over traditional models in credit risk, crypto, asset pricing, and macroeconomics using models like Random Forest, XGBoost, LSTM, and CNN.


<details>
  <summary>Details</summary>
Motivation: To examine how machine learning and deep learning have transformed forecasting, decision-making, and financial modeling to promote innovation and efficiency in financial systems.

Method: Systematic review following PRISMA 2020 guidelines analyzing 22 peer-reviewed articles from Scopus (2024-2026), focusing on ML/DL models including Random Forest, XGBoost, SVM, LSTM, BiLSTM, CNN, and hybrid approaches.
...
TL;DR: Construction of probabilistically strong solutions to 3D Euler equations with additive noise, continuous in time and Hölder in space, satisfying local energy inequality up to large stopping times.


<details>
  <summary>Details</summary>
Motivation: To develop rigorous mathematical solutions for the challenging 3D Euler equations with stochastic perturbations, ensuring probabilistic strength and key physical properties.

Method: Construct probabilistically strong solutions using additive noise perturbation, ensuring continuity in time and Hölder regularity in space.

Result: Successfully obtained solutions that are almost surely continuous in time, Hölder in space, and satisfy the local energy inequality up to arbitrarily large stopping times.

Conclusion: The approach provides a robust framework for handling 3D Euler equations with stochastic forcing, maintaining important physical constraints probabilistically.
...
TL;DR: Analysis of generalized solitary waves in Boussinesq abcd system with variable bottom topography, focusing on existence and collision problems in physically relevant regimes.


<details>
  <summary>Details</summary>
Motivation: To investigate how solitary waves interact with slowly varying bottom topography in the Boussinesq abcd system, which models shallow water waves, without being destroyed by these interactions.

Method: Constructed new approximate solutions that capture interactions between solitary waves and slowly varying bottoms represented by h=εh₀(εt,εx), where ε is a small parameter and h₀ is a fixed smooth profile.

Result: Established existence of generalized solitary waves and described their collision behavior with variable bottom topography, enabling detailed analysis of weak long-range interactions.

Conclusion: The approach successfully describes solitary wave evolution in variable bottom regimes without destruction, providing insights into long-range interactions in shallow water wave systems.
...
TL;DR: Rigorous derivation of incompressible Navier-Stokes-Fourier system with time-dependent forcing from inelastic hard-spheres Boltzmann equation for viscoelastic granular gases, capturing nontrivial hydrodynamic effects.


<details>
  <summary>Details</summary>
Motivation: To derive hydrodynamic limits from the Boltzmann equation for granular gases with velocity-dependent restitution coefficients, overcoming limitations of constant restitution models where balanced dynamics must be imposed artificially.

Method: Used self-similar change of variables that explicitly depends on Knudsen number and restitution coefficient, leading to a non-autonomous rescaled Boltzmann equation whose solutions converge weakly to the hydrodynamic limit.

Result: Successfully derived a new incompressible Navier-Stokes-Fourier system with self-consistent time-dependent forcing terms, and determined the exact dissipation rate of granular temperature (Haff's law).

Conclusion: The approach provides a rigorous framework for deriving hydrodynamic equations from granular gas models with velocity-dependent inelasticity, capturing essential physical effects that were previously inaccessible.
...
TL;DR: A hybrid physics-neural model that predicts scalar transport in complex domains orders of magnitude faster than 3D simulations, achieving high accuracy with minimal training data.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D numerical simulations are computationally expensive (hours) while most analysis only requires reduced metrics, creating a need for faster surrogate models.

Method: End-to-end differentiable framework that jointly learns physical model parameterization (orthotropic diffusivity) and a non-Markovian neural closure model to capture unresolved coarse-grained effects for stable long-term predictions.

Result: Achieves 0.96 Spearman correlation coefficient with only 26 training data points, reduces simulation time from hours to less than 1 minute, and generalizes to out-of-distribution scenarios like moving sources.

Conclusion: The differentiable physics-neural framework enables fast, accurate, and generalizable coarse-grained surrogates for physical phenomena with high data efficiency.
...
### [46] [Isotope Production in Muon-Catalyzed-Fusion Systems](https://arxiv.org/abs/2511.20951)
*J. F. Parisi,A. Rutkowski*

Main category: physics.plasm-ph

TL;DR: Muon-catalyzed fusion can produce valuable isotopes using high-energy neutrons without external heating, enabling higher neutron flux than conventional fusion systems. This approach could make μCF economically viable for isotope production before achieving energy breakeven.


<details>
  <summary>Details</summary>
Motivation: To improve the economic prospects of muon-catalyzed fusion by using it as an intense neutron source for producing valuable isotopes, particularly medical radioisotopes like ²²⁵Ac, which has limited global supply.

Method: Using muon-catalyzed fusion reactions to generate high-flux high-energy neutrons without external heating, allowing higher neutron flux without material heat flux constraints. The system transmutes isotopes using these neutrons, with a proposed setup using 10⁸ muons/second and 10g of ²²⁶Ra feedstock.

Result: A μCF system with 10⁸ muons/second could produce up to 0.5 mg of ²²⁵Ac per year - ten times current global supply. The required fusion events per muon and muon energy cost can be relaxed by orders of magnitude compared to energy production requirements.

Conclusion: Muon-catalyzed fusion systems show strong potential for neutron-driven isotope production before achieving net energy generation, motivating accelerated development of μCF technology for medical and industrial isotope production.
...
### [47] [Direct numerical simulation of thermo-diffusively unstable premixed hydrogen-air flames in a fully-developed turbulent channel flow at $Re_τ=530$](https://arxiv.org/abs/2511.20930)
*Felix Rong,Max Schneider,Hendrik Nicolai,Christian Hasse,Andrea Gruber*

Main category: physics.flu-dyn

TL;DR: DNS study of premixed hydrogen-air flames in turbulent channel flow shows how thermo-diffusive phenomena interact with wall turbulence to enhance flame reactivity and speed.


<details>
  <summary>Details</summary>
Motivation: To understand how intrinsic thermo-diffusive phenomena affect premixed flame propagation in realistic near-wall turbulent flows, particularly how turbulence intensity and wall proximity influence flame characteristics.

Method: Direct Numerical Simulations (DNS) of premixed hydrogen-air flames in turbulent channel flow at Re_τ=530, analyzing two flame conditions (φ=0.25 and φ=0.35) with different propagation behaviors and proximity to walls.

Result: At φ=0.25, flame shows reactivity enhancement in core flow due to TD phenomena interacting with weak turbulence. At φ=0.35, flame exhibits strong synergistic interaction between TD phenomena and wall turbulence near y+~10, resulting in significantly enhanced flame speed.

Conclusion: Thermo-diffusive phenomena interact synergistically with wall turbulence, particularly in near-wall regions, leading to enhanced flame reactivity and speed, with peak enhancement occurring where Reynolds stresses peak.
...
### [48] [$\texttt{CRLS}$: Convolutional Regularized Least Squares Framework for Reduced Order Modeling of Transonic Flows](https://arxiv.org/abs/2511.21425)
*Muhammad Bilal,Ashwin Renganathan*

Main category: physics.flu-dyn

TL;DR: CRLS framework combines Gaussian convolution, POD, and deconvolution to create accurate reduced-order models for transonic flows with shocks, overcoming limitations of standard POD methods.


<details>
  <summary>Details</summary>
Motivation: Standard POD methods perform poorly with parameter-dependent discontinuities like shocks, causing smeared shocks, stair-stepping, or non-physical oscillations in transonic flow modeling.

Method: Apply 1D Gaussian convolution to smooth snapshots, extract POD bases, learn parametric dependence via RBF interpolation, then use regularized least squares deconvolution to recover sharp shock structures.

Result: CRLS shows improved shock location/strength, lower surface-pressure/field errors, and 42% reduction in POD modes needed compared to standard POD and smoothed-POD baselines on RAE2822 airfoil transonic flow.

Conclusion: CRLS provides an accurate, data-efficient, and automated approach for shock-aware reduced order models in high-speed aerodynamic design.
...
### [49] [Quantum Hard Spheres with Affine Quantization](https://arxiv.org/abs/2511.21119)
*Riccardo Fantoni*

Main category: cond-mat.stat-mech

TL;DR: Quantum hard-sphere fluid analyzed using affine-quantization and path integral Monte Carlo methods for Bose-Einstein statistics systems.


<details>
  <summary>Details</summary>
Motivation: To study the thermodynamic properties of quantum hard-sphere fluids using affine-quantization approach for systems obeying Bose-Einstein statistics.

Method: Applied affine-quantization to quantum hard-spheres and used path integral Monte Carlo method to solve for thermodynamic properties.

Result: The paper presents solutions for thermodynamic properties of quantum hard-sphere fluids under Bose-Einstein statistics.

Conclusion: Affine-quantization combined with path integral Monte Carlo provides an effective approach for studying quantum hard-sphere fluids with Bose-Einstein statistics.
...
TL;DR: AI and machine learning are extending scientific discovery capabilities across multiple domains by enabling researchers to probe, predict, and design more effectively, with emphasis on trustworthy data, transferable models, and integrated workflows.


<details>
  <summary>Details</summary>
Motivation: To provide a forward-looking view of how AI can accelerate scientific discovery across various scientific domains while maintaining reproducibility and physical interpretability.

Method: The roadmap examines AI-enabled science across biology, chemistry, climate science, mathematics, materials science, physics, self-driving laboratories and unconventional computing, identifying shared themes and requirements.

Result: Identified key requirements including diverse and trustworthy data, transferable electronic-structure models, AI systems integrated into end-to-end workflows, and generative systems grounded in synthesisability rather than idealized phases.

Conclusion: The perspectives outline current state of AI-enabled science, identify bottlenecks in data, methods and infrastructure, and chart directions for building more transparent and capable AI systems that can accelerate discovery in complex real-world environments.
...
### [51] [Impact of Cosmic Ray Distribution on the Growth and Saturation of Bell Instability](https://arxiv.org/abs/2511.21154)
*Saikat Das,Siddhartha Gupta,Prateek Sharma*

Main category: astro-ph.HE

TL;DR: The paper examines how cosmic ray momentum distributions affect the nonresonant streaming instability, finding that linear growth depends only on CR current while saturation depends on distribution shape, with low-energy CRs dominating saturation.


<details>
  <summary>Details</summary>
Motivation: To understand how different cosmic ray momentum distributions influence the growth and saturation of nonresonant streaming instability in weakly magnetized plasmas.

Method: Used one-dimensional kinetic simulations to investigate mono-energetic and power-law CR momentum distributions and their effects on NRSI growth and saturation.

Result: Linear growth is governed solely by CR current and insensitive to distribution, but saturation depends strongly on CR distribution through CR isotropization. Low-energy CRs dominate current relaxation and magnetic growth in power-law distributions.

Conclusion: Proposed a modified saturation prescription accounting for CR distribution effects and a layered CR-confinement scenario upstream of astrophysical shocks relevant to high-energy particle acceleration.
...
### [52] [Opportunities and Challenges of Computational Electromagnetics Methods for Superconducting Circuit Quantum Device Modeling: A Practical Review](https://arxiv.org/abs/2511.20774)
*Samuel T. Elkin,Ghazi Khan,Ebrahim Forati,Brandon W. Langley,Dogan Timucin,Reza Molavi,Sara Sussman,Thomas E. Roth*

Main category: quant-ph

TL;DR: This review provides a practical introduction to computational electromagnetics (CEM) methods for modeling superconducting circuit quantum devices, highlighting challenges in multiscale modeling and offering guidance for researchers.


<details>
  <summary>Details</summary>
Motivation: Emerging superconducting circuit quantum devices with unconventional materials and multiscale features (nanometer to centimeter) stress current CEM capabilities, causing simulation issues like increased time, accuracy loss, or unreliable solutions that need addressing.

Method: The paper reviews fundamental aspects of major CEM techniques, covers advanced topics related to multiscale modeling challenges with specific superconducting circuit examples, and discusses practical steps to alleviate these challenges.

Result: The review serves as an accessible introduction to CEM fundamentals and provides insights into addressing multiscale modeling difficulties, particularly for superconducting quantum devices.

Conclusion: Future research directions are needed to improve the design capabilities for increasingly sophisticated superconducting circuit quantum devices, and the insights provided benefit researchers beyond this specific field.
...
### [53] [Rapid ground state energy estimation with a Sparse Pauli Dynamics-enabled Variational Double Bracket Flow](https://arxiv.org/abs/2511.21651)
*Chinmay Shrikhande,Arnab Bachhar,Aaron Rodriguez Jimenez,Nicholas J. Mayhall*

Main category: quant-ph

TL;DR: A variational double bracket flow algorithm using Sparse Pauli Dynamics achieves accurate ground state energy estimation for strongly correlated quantum systems with significant speedups over DMRG.
...
Method: Variational double bracket flow algorithm leveraging Sparse Pauli Dynamics with greedy operator selection, coefficient truncation, and energy-variance extrapolation.

Result: Less than 1% error relative to DMRG benchmarks for Heisenberg and Hubbard models in 1D and 2D; 10 minutes for 100-qubit system vs 50+ hours for DMRG; even greater speedup for 128-qubit Hubbard model.
...
### [54] [Approximate Bayesian Computation Made Easy: A Practical Guide to ABC-SMC for Dynamical Systems with \texttt{pymc}](https://arxiv.org/abs/2511.21587)
*Mario Castro*

Main category: q-bio.PE

TL;DR: A practical tutorial introducing ABC-SMC for likelihood-free parameter inference in mechanistic models using Python and PyMC, with examples from ecology and epidemiology.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between the theoretical potential of ABC-SMC methods and practical adoption by researchers who find them complex, by providing an accessible, example-driven introduction.

Method: Uses Approximate Bayesian Computation with Sequential Monte Carlo (ABC-SMC) implemented in Python via PyMC's probabilistic programming interface, with practical examples including predator-prey dynamics and hierarchical epidemic models.

Result: Provides working code examples that demonstrate how to implement, diagnose, and interpret ABC-SMC analyses, showing when and why the method works and how to handle partial observability and hierarchical structures.

Conclusion: ABC-SMC is a powerful likelihood-free alternative for parameter inference in mechanistic models, and this tutorial makes it accessible through practical Python examples that ensure reproducibility and easy adaptation to new problems.
...
### [55] [Flows of conformally coclosed $G_2$-structures with dilaton](https://arxiv.org/abs/2511.21055)
*Spiro Karigiannis,Sébastien Picard,Caleb Suan*

Main category: math.DG

TL;DR: The paper studies G2-structure flows via dimensional reduction, showing how geometric flows in G2-geometry reduce to complex geometry flows. Main examples include the G2-Laplacian coflow (lifting Kähler-Ricci flow) and a 7D lift of anomaly flow on complex threefolds.


<details>
  <summary>Details</summary>
Motivation: To understand geometric flows in G2-geometry through the principle of dimensional reduction, connecting them to established flows in complex geometry.

Method: Using dimensional reduction principle to lift complex geometry flows to G2-geometry. Specifically studies G2-Laplacian coflow and G2-anomaly flow, comparing them and analyzing their properties.

Result: The G2-anomaly flow deforms conformally coclosed G2-structures. The paper establishes connections between these flows and investigates their short-time existence and fixed points.

Conclusion: Dimensional reduction provides a powerful framework for understanding geometric flows in G2-geometry by relating them to well-studied flows in complex geometry, with applications to both the G2-Laplacian coflow and G2-anomaly flow.
...
### [56] [The Critical LYZ equation in Kähler Geometry](https://arxiv.org/abs/2511.21492)
*Jixiang Fu,Shing-Tung Yau,Dekai Zhang*

Main category: math.DG

TL;DR: Existence of smooth solutions for LYZ equation at critical phase θ=(n-2)π/2, solving Collins-Jacob-Yau and Li's problem. Applications include 3D σ₂=1 and 4D σ₃=σ₁ equations under weaker assumptions.


<details>
  <summary>Details</summary>
Motivation: Solve the critical case of Collins-Jacob-Yau and Li's problem about solvability for phase θ ≤ (n-2)π/2 in the LYZ equation.

Method: Establish existence of smooth solutions for the LYZ equation at the critical phase θ=(n-2)π/2.

Result: Proved existence of smooth solutions at critical phase, enabling solutions to 3D Hessian equation σ₂=1 and 4D Hessian quotient equation σ₃=σ₁ under weaker assumptions.

Conclusion: Successfully solved the critical case of the posed problem and extended applications to Hessian equations with reduced requirements.
...
### [57] [Physics Steering: Causal Control of Cross-Domain Concepts in a Physics Foundation Model](https://arxiv.org/abs/2511.20798)
...
<details>
  <summary>Details</summary>
Motivation: To investigate whether the phenomenon of internal abstract concept representations found in language and vision models is a general property of foundation models, specifically in scientific/physics domains.
...
Conclusion: Scientific foundation models learn generalized representations of physical principles rather than superficial correlations, opening new avenues for understanding and controlling AI-enabled scientific discovery.
...
### [58] [SUPN: Shallow Universal Polynomial Networks](https://arxiv.org/abs/2511.21414)
*Zachary Morrow,Michael Penwarden,Brian Chen,Aurya Javeed,Akil Narayan,John D. Jakeman*

Main category: cs.LG

TL;DR: SUPNs (shallow universal polynomial networks) achieve better function approximation with fewer parameters than DNNs and KANs by replacing hidden layers with a single polynomial layer, reducing optimization complexity and improving generalization.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks and KANs require many parameters leading to overparameterization, complex optimization landscapes with local minima, and high sensitivity to initialization, which affects generalization performance.

Method: Replace all but the last hidden layer with a single layer of polynomials with learnable coefficients, combining DNN and polynomial strengths. Provide theoretical convergence guarantees and explicit quasi-optimal parameter formulas.

Result: SUPNs achieve approximation errors and variability that are often an order of magnitude lower than DNNs and KANs for the same number of parameters. They even outperform polynomial projection on non-smooth functions in experiments.

Conclusion: SUPNs offer a more efficient and stable alternative to DNNs and KANs for function approximation, providing better performance with fewer parameters while maintaining theoretical guarantees.
...
### [59] [Sublinear Time Low-Rank Approximation of Hankel Matrices](https://arxiv.org/abs/2511.21418)
*Michael Kapralov,Cameron Musco,Kshiteej Sheth*

Main category: cs.DS

TL;DR: First sublinear-time algorithm for low-rank approximation of PSD Hankel matrices with polylog(n,1/ε) runtime and O(log n log(1/ε)) rank, matching theoretical bounds and providing robustness to noise.


<details>
  <summary>Details</summary>
Motivation: PSD Hankel matrices are known to be approximately low-rank (Beckermann-Townsend theorem), making them natural targets for efficient low-rank approximation algorithms, but no sublinear-time algorithms existed previously.

Method: Structure-preserving existence proof for Hankel approximations, then sampling-based approach using Vandermonde structure and universal ridge leverage score bounds to achieve sublinear time.

Result: Algorithm runs in polylog(n,1/ε) time, computes rank-O(log n log(1/ε)) Hankel approximation matching theoretical error bounds, and is robust to non-Hankel noise.

Conclusion: First sublinear-time algorithm for PSD Hankel matrix approximation that preserves Hankel structure, achieves optimal rank bounds, and handles noise, establishing finite-dimensional analog of AAK theorem.
...
### [60] [Stochastic Optimal Control of Interacting Particle Systems in Hilbert Spaces and Applications](https://arxiv.org/abs/2511.21646)
*Filippo de Feo,Fausto Gozzi,Andrzej Święch,Lukas Wessels*

Main category: math.PR

TL;DR: This paper establishes the theoretical foundations for mean-field limits of optimal control problems involving interacting particle systems governed by stochastic evolution equations in Hilbert spaces, proving convergence of value functions and optimal controls.
...
Method: The authors prove convergence of finite particle system value functions to a limiting function that is the unique L-viscosity solution of the corresponding mean-field Hamilton-Jacobi-Bellman equation in probability measure space, and identify its lift with the value function of the 'lifted' limit optimal control problem.

Result: Under suitable assumptions, they show C^{1,1}-regularity of the lifted value function, prove that the limiting function projects onto the finite particle value functions, and establish correspondence between optimal controls of particle systems and the lifted control problem.

Conclusion: These are the first results of this kind for stochastic optimal control problems of interacting particle systems of stochastic evolution equations in Hilbert spaces, with applications to economic problems involving stochastic delay differential equations and stochastic partial differential equations.
...
### [61] [Unified interface dipole theory for Fermi level pinning effect at metal-semiconductor contacts](https://arxiv.org/abs/2511.21494)
*Ziying Xiang,Jun-Wei Luo,Shu-Shen Li*

Main category: cond-mat.mtrl-sci

TL;DR: A unified bond dipole theory explains Fermi level pinning at metal-semiconductor interfaces through localized bonding between semiconductor dangling bonds and metal orbitals, showing that even a single metal monolayer can generate strong pinning.


<details>
  <summary>Details</summary>
Motivation: To provide a microscopic understanding of interface dipoles and Fermi level pinning by unifying various interface states (MIGS, DBSS, bonding states) under a single bonding mechanism rather than treating them as independent causes.

Method: Combined first-principles calculations with tight-binding analysis using Harrison's bond-orbital model to study localized bonding between semiconductor surface dangling bonds and metal orbitals.

Result: Showed that localized bonding generates large interface dipoles and strong Fermi level pinning, with pinning strength governed by the density of surface dangling bonds available for chemical bonding with the metal.

Conclusion: The theory explains weaker pinning in ionic semiconductors versus covalent ones and provides practical guidance for engineering metal-semiconductor interfaces and tuning Schottky barrier heights.
...
### [62] [A Review of Pseudospectral Optimal Control: From Theory to Flight](https://arxiv.org/abs/2511.20843)
*I. M. Ross,M. Karpenko*

Main category: math.OC

TL;DR: Pseudospectral optimal control theory combines pseudospectral theory with optimal control theory, leveraging Sobolev spaces as the common mathematical foundation. The paper reviews key theoretical results and discusses NASA flight demonstrations, implementation details, and emerging trends in both theory and practice.


<details>
  <summary>Details</summary>
Motivation: Both optimal control theory and pseudospectral theory share Sobolev spaces as their mathematical foundation, making it natural to combine them into pseudospectral optimal control theory for solving challenging aerospace control problems.

Method: Combines pseudospectral theory with optimal control theory, utilizing Sobolev spaces as the common mathematical framework. The approach has been implemented in embedded platforms since 2011.

Result: Successful flight demonstrations onboard NASA spacecraft, with pseudospectral optimal control proving critical for successful flight operations. The embedded implementation is changing how challenging control problems in aerospace and autonomous systems are solved.

Conclusion: Pseudospectral optimal control theory represents a significant advancement in control theory, with practical applications demonstrated through NASA flight tests and embedded platform implementations that are transforming solutions to complex aerospace and autonomous system control problems.
...
### [63] [Edge-Dependent Superconductivity in Twisted Bismuth Bilayers](https://arxiv.org/abs/2511.21657)
...
TL;DR: Twisted bismuth bilayers show enhanced electronic density of states at flake edges due to structural disorder, potentially enabling dramatically increased superconducting critical temperatures.


<details>
  <summary>Details</summary>
Motivation: To address boundary heterogeneity in twisted bilayer materials and understand how structural disorder at flake edges affects electronic properties, particularly for functional device applications.

Method: Used ab initio density functional theory to systematically map electronic properties (band topology, density of states, superconductivity) and vibrational properties of twisted bismuth bilayers, focusing on edge effects.
...
Conclusion: Structural disorder at flake edges provides powerful mechanism for increasing superconducting critical temperature, establishing theoretical framework for next-generation twistronic materials and guiding search for metastable structures with enhanced superconductivity.
