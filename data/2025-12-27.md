...
TL;DR: Odd-degree finite elements achieve optimal convergence for nonlinear dispersive wave equations, while even-degree elements show reduced accuracy due to super-approximation properties of L²-projection of derivatives.


<details>
  <summary>Details</summary>
Motivation: To explain the unusual convergence behavior observed in energy-conservative Galerkin discretizations for nonlinear dispersive wave equations, where odd-degree finite elements perform optimally while even-degree elements show reduced accuracy.

Method: Analysis of finite element space structure, focusing on the standard L²-projection of derivatives and its super-approximation property for odd polynomial degrees. Application to energy-conservative Galerkin approximation of the regularized long-wave equation with cubic energy functional.

Result: The convergence behavior is intrinsic to finite element space structure. The L²-projection of derivatives has super-approximation property only for odd polynomial degrees. For the regularized long-wave equation, the scheme conserves mass and energy, approximates impulse with high accuracy, and has established a priori error bounds.

Conclusion: The odd-even convergence phenomenon in Galerkin discretizations for nonlinear dispersive wave equations is fundamentally linked to structural properties of finite element spaces, specifically the super-approximation behavior of L²-projections for odd polynomial degrees.
...
TL;DR: Theoretical study of greedy algorithm stability, focusing on how small perturbations (noisy data) affect algorithm outcomes.


<details>
  <summary>Details</summary>
Motivation: While greedy approximation theory typically focuses on convergence and rate of convergence, there's a need to study another important property: stability against perturbations like noisy data.

Method: Theoretical analysis of greedy algorithms' stability properties, specifically examining how algorithms respond to noisy data perturbations.

Result: The paper presents theoretical results demonstrating that greedy algorithms can maintain stability when dealing with noisy data perturbations.

Conclusion: Stability is a crucial property for greedy algorithms alongside convergence and rate of convergence, and these algorithms can be designed to be robust against noisy data perturbations.
...
<details>
  <summary>Details</summary>
Motivation: Traditional velocity-pressure formulations on surfaces require increased computational costs to guarantee tangential and incompressible velocity fields. There's a need for formulations that naturally preserve these structural properties while handling general surfaces including non-simply connected ones where harmonic velocity components are important.

Method: Develops a streamfunction-vorticity formulation that relies only on scalar and finite-dimensional quantities. The approach handles general surfaces (including non-simply connected) and properly accounts for harmonic components of velocity fields. The formulation is rigorously validated by proving equivalence to velocity-pressure formulations under reasonable regularity assumptions.

Result: The formulation ensures exactly tangential and incompressible velocity fields while being pressure robust. Numerical examples demonstrate applicability. Compared to traditional methods, this approach maintains structural properties without increasing computational costs.

Conclusion: Streamfunction-vorticity formulation provides a computationally efficient approach for surface fluid dynamics that naturally preserves essential structural properties (tangentiality, incompressibility, pressure robustness) on general surfaces including non-simply connected ones.
...
TL;DR: Hermite basis functions, originally used for Schrödinger equations with harmonic potential, are shown to be stable and effective for nonlinear dispersive equations on unbounded domains without potential.


<details>
  <summary>Details</summary>
Motivation: To extend the proven stability properties of Hermite basis functions from Schrödinger equations with harmonic potential to more general cases, particularly for nonlinear dispersive equations on unbounded domains.

Method: The paper demonstrates that Hermite basis functions maintain their stability properties when applied to Schrödinger equations without potential, making them suitable for spatial discretization of nonlinear dispersive equations on unbounded domains.

Result: Hermite basis functions are shown to be a natural and stable basis choice for computing nonlinear dispersive equations on unbounded domains, extending their applicability beyond harmonic potential cases.

Conclusion: Hermite basis functions provide a powerful and stable discretization method for nonlinear dispersive equations on unbounded domains, generalizing their successful application from harmonic potential Schrödinger equations.
...
TL;DR: Inexact block Schur complement preconditioning for linear poroelasticity with hybrid discretization achieves mesh- and locking-parameter-independent convergence for both MINRES and GMRES methods.


<details>
  <summary>Details</summary>
Motivation: Pure Dirichlet boundary conditions for displacement cause near-singularity in the leading block of the algebraic system in nearly incompressible regimes (locking), making iterative solution inefficient.

Method: Reformulate as a three-field problem with inherent regularization to maintain original solution while ensuring nonsingularity. Apply inexact block diagonal and triangular Schur complement preconditioners to MINRES and GMRES methods.

Result: Both MINRES and GMRES achieve convergence independent of mesh size and locking parameter for regularized system. Similar results for mixed boundary conditions without regularization. Numerical experiments in 2D/3D confirm benefits.

Conclusion: The proposed preconditioning approach is robust for poroelasticity problems, handling locking issues effectively, with practical validation through spinal cord simulation with discontinuous material parameters.
...
TL;DR: Mixed precision GADI method accelerates large sparse linear system solving by using low precision for subsystems and high precision for residuals/updates, achieving 1.7-3.1× speedup on GPU.


<details>
  <summary>Details</summary>
Motivation: To accelerate solution of large-scale sparse linear systems Ax=b by reducing computational cost while maintaining high accuracy, leveraging mixed precision computing capabilities of modern hardware.

Method: Three-precision GADI framework: solves subsystems in low precision (Bfloat16/FP32), computes residuals and solution updates in high precision. Includes rounding error analysis, convergence conditions, and GPR-based parameter selection strategy for regularization parameter α.

Result: Achieved speedups of 2.6×, 1.7×, and 3.1× over full double precision GADI on 2D/3D convection-diffusion and reaction-diffusion problems with up to 1.3×10^8 unknowns on NVIDIA A100 GPU.

Conclusion: Mixed precision GADI effectively balances speed and accuracy, with theoretical analysis providing convergence guarantees and practical parameter selection enabling robust performance on large-scale problems.
...
<details>
  <summary>Details</summary>
Motivation: To develop and analyze numerical methods for stochastic Boussinesq systems driven by multiplicative noise, which are important for modeling fluid dynamics with thermal effects under random perturbations.

Method: Combines standard mixed finite element method for spatial discretization with semi-implicit Euler-Maruyama scheme for temporal discretization. Uses localization technique with high-moment stability estimates for analysis.

Result: Establishes error bounds for velocity, pressure, and temperature approximations. Proves convergence in probability for fully discrete method in both L² and H¹-type norms. Numerical experiments validate theoretical estimates.
...
TL;DR: FORCE-α numerical fluxes are systematically evaluated in high-order finite volume schemes for hyperbolic PDEs, showing they're competitive alternatives to upwind fluxes.


<details>
  <summary>Details</summary>
Motivation: Centered fluxes like FORCE-α offer flexibility for complicated hyperbolic systems where Riemann solvers are difficult to construct or computationally expensive, unlike upwind fluxes that require Riemann problem analysis.

Method: Uses a semidiscrete finite volume framework with WENO spatial reconstruction and Deferred Correction time discretization up to order 7, applied to ideal Euler equations in 1D and 2D.

Result: FORCE-α numerical fluxes perform competitively compared to classical upwind fluxes within high-order accuracy frameworks, with differences decreasing as order increases.

Conclusion: FORCE-α fluxes are viable alternatives to upwind fluxes in high-order finite volume schemes for hyperbolic systems, offering flexibility without sacrificing performance.
...
TL;DR: A variationally correct operator learning framework using FOSLS objectives with provable norm equivalence to solution errors, implemented via Reduced Basis Neural Operators for stability and efficiency.


<details>
  <summary>Details</summary>
Motivation: Standard PDE-residual losses for neural operators lack variational correctness - small residuals don't guarantee small solution errors due to non-compliant norms and inconsistent boundary condition penalties.

Method: Develops first-order system least-squares (FOSLS) objectives with provable norm equivalence, incorporates boundary conditions via variational lifts, and proposes Reduced Basis Neural Operator (RBNO) that predicts coefficients for pre-computed conforming reduced basis to ensure variational stability.
...
Conclusion: The proposed variationally correct operator learning framework provides mathematically rigorous error control, superior accuracy in PDE-compliant norms, and reliable error estimation, addressing fundamental limitations of standard PDE-residual loss formulations.
...
<details>
  <summary>Details</summary>
Motivation: To extend previous results on long-time behavior of NLS solutions by obtaining uniform decay estimates for the critical case where initial data lies in the homogeneous Sobolev space $\dot{H}^{s_c}(\mathbb{R}^3)$ with $s_c = \frac{5}{6}$.

Method: Investigates the Cauchy problem for nonlinear Schrödinger equation in 3D, focusing on mass-supercritical and energy-subcritical regime. Uses analysis in critical homogeneous Sobolev spaces to derive decay estimates for long-time dynamics.

Result: Obtains a uniform decay estimate for long-time dynamics of solutions with initial data in $\dot{H}^{s_c}(\mathbb{R}^3)$, extending previous results in this critical regime.

Conclusion: The paper successfully establishes decay estimates for NLS solutions in the critical Sobolev space, advancing understanding of long-time behavior in the mass-supercritical, energy-subcritical regime in three dimensions.
...
TL;DR: Multi-scale convergence theory for Hamilton-Jacobi PDEs in probability measure spaces, arising from hydrodynamic limits of N-particle Lagrangian dynamics.


<details>
  <summary>Details</summary>
Motivation: To establish rigorous convergence results for Hamilton-Jacobi equations in probability measure spaces that arise from hydrodynamic limits of deterministic particle systems, addressing infinite-dimensional singular averaging structures.

Method: Develops indirect variational approach using finite-dimensional weak K.A.M. theory extended to infinite-dimensional settings; employs viscosity solution techniques, projection methods for submetry structures, multi-scale convergence in metric spaces, and comparison principles for probability measure spaces.

Result: Successfully identifies limiting Hamiltonian and proves rigorous convergence of solutions for nonlinear PDEs in probability measure spaces under weakly interacting particle assumptions.

Conclusion: Establishes multi-scale convergence theory for Hamilton-Jacobi PDEs in probability measure spaces, developing new mathematical techniques for infinite-dimensional singular averaging problems with applications to hydrodynamic limits.
...
TL;DR: The paper proves existence of infinitely many solutions for oscillatory resonant PDE problems on balls/rectangles, with asymptotic formulas and numerical validation.
...
Method: Analytical approach for Dirichlet problems on balls and rectangles in R^n, proving existence of infinitely many solutions, deriving asymptotic formulas in terms of first harmonic, and implementing numerical computations to validate results.
...
Conclusion: The paper successfully extends results for oscillatory resonant problems, providing both theoretical existence proofs and practical computational validation with accurate asymptotic formulas.
...
<details>
  <summary>Details</summary>
Motivation: To establish uniqueness of rough solutions to nonlinear kinetic equations, particularly completing the global well-posedness theory for the homogeneous Landau-Coulomb equation in the critical space L^{3/2}(ℝ³).

Method: Uses the ℳ-operator technique (developed in previous works) applied to the space-homogeneous case, where the 𝕄-operator can be taken as a Bessel potential operator.

Result: Proves uniqueness of H-solutions satisfying specific regularity conditions: ⟨v⟩^{k₀}f ∈ C([0,T]; L^{3/2}(ℝ³)) and ⟨v⟩^{-3/2}∇_v((⟨v⟩^{k₀}f)^{3/4}) ∈ L²((0,T)×ℝ³) for any k₀ ≥ 5.

Conclusion: The solutions constructed in previous work (GGL25) are unique, thus completing the global well-posedness theory in the critical space L^{3/2}(ℝ³).
...
TL;DR: Quantitative bounds for Hölder exponents in Krylov-Safonov and Evans-Krylov theories when ellipticity ratio is near 1.


<details>
  <summary>Details</summary>
Motivation: To establish precise quantitative bounds for Hölder regularity exponents in two important PDE theories (Krylov-Safonov and Evans-Krylov) when the ellipticity ratio approaches unity, which is a critical regime in elliptic and parabolic PDE analysis.
...
Result: Establishes quantitative bounds for Hölder exponents when ellipticity ratio is close to one, providing precise regularity estimates in this important parameter regime.

Conclusion: The paper provides quantitative regularity estimates for solutions to elliptic/parabolic equations when the ellipticity ratio is near 1, combining Ishii-Lions and Schauder perturbation techniques to cover both Krylov-Safonov and Evans-Krylov theories.
...
TL;DR: Global gradient bounds for degenerate elliptic systems with complex coefficients having small BMO (not VMO) coefficients, extending regularity theory in complex-valued settings.


<details>
  <summary>Details</summary>
Motivation: To establish global gradient bounds for divergence-form degenerate elliptic systems with complex-valued coefficients under minimal regularity assumptions. The coefficients only need to be sufficiently small in BMO, which is weaker than the VMO condition typically required. This extends previous work that established existence and uniqueness under less restrictive assumptions.

Method: The authors prove a global Calderón-Zygmund-type estimate for weak solutions to these degenerate elliptic systems. The approach builds on recent work that established existence and uniqueness under natural assumptions, and extends it to obtain gradient bounds and Morrey-space regularity.

Result: The main result is a global Calderón-Zygmund-type estimate for weak solutions, from which Morrey-space regularity follows as a consequence. This provides gradient bounds for solutions under the minimal assumption that coefficients are sufficiently small in BMO (not requiring VMO).

Conclusion: This work contributes to better understanding solution behavior for complex-valued elliptic systems and extends regularity theory in the complex setting. It demonstrates that global gradient bounds can be obtained under weaker coefficient regularity than previously known, specifically with BMO rather than VMO coefficients.
...
<details>
  <summary>Details</summary>
Motivation: To address the long-standing problem of proving existence of infinitely many solutions for nonlinear problems that lack symmetry, particularly for nonvariational elliptic PDEs with gradient dependence where traditional variational methods fail, and to overcome the collapse issue in periodic Hamiltonian systems when extending solutions from finite intervals to the whole real line.

Method: Develops a refined variational truncation method that systematically separates solutions. For semilinear elliptic PDEs, uses this truncation method to obtain infinite sequences of positive and negative solutions. For nonvariational elliptic PDEs with gradient dependence, combines the truncation method with an iterative scheme. For periodic Hamiltonian systems, shows that multiplicity constructed on finite intervals survives in the limit to the whole real line.

Result: 1) Proves existence of infinitely many positive and negative solutions for semilinear elliptic PDEs. 2) First proof of existence of infinitely many solutions for nonvariational elliptic PDEs with gradient dependence. 3) Shows multiplicity of solutions for periodic Hamiltonian systems on the whole real line without collapse.

Conclusion: The paper presents a unified truncation methodology that works across variational and non-variational PDEs and infinite dimensional dynamical systems, providing a robust tool for multiplicity problems without symmetry assumptions. The core novelty is a carefully designed truncation method that systematically separates solutions.
...
TL;DR: Existence and uniqueness results for Navier-Stokes fluid interacting with a spring-restrained rigid body under far-field velocity conditions.


<details>
  <summary>Details</summary>
Motivation: To study equilibrium configurations in fluid-structure interaction problems where a rigid body with spring forces and restoring moments interacts with a Navier-Stokes fluid under far-field velocity conditions.

Method: Mathematical analysis of the coupled Navier-Stokes system with rigid body dynamics, addressing the nonlinearity introduced by body rotation around a given axis.

Result: Demonstrates existence of equilibrium configurations in the "large" and uniqueness in the "small" for the coupled fluid-body system.

Conclusion: The paper establishes rigorous mathematical existence and uniqueness results for a challenging nonlinear fluid-structure interaction problem involving rotational dynamics.
...
TL;DR: The paper studies existence and non-existence of solutions for singular elliptic boundary value problems with p-Laplacian and singular term a(x)/u^γ, showing solvability for large μ > μ₀ and no finite energy solutions for small μ < μ₀*.
...
Method: Study the singular elliptic boundary value problem with p-Laplacian operator, singular term a(x)/u^γ, and positive forcing term μf(x). Analyze existence and non-existence of finite energy solutions using variational methods and comparison principles for p-Laplacian equations with singular nonlinearities.

Result: For any positive f ∈ L¹(Ω), the problem is solvable for any μ > μ₀ (with μ₀ large enough), while no finite energy solution exists for 0 < μ < μ₀* (with μ₀* small enough). This extends previous results to p≠2 and provides new insights for p=2 with critical singular growth.

Conclusion: The paper successfully extends the classical results on singular elliptic problems to the p-Laplacian framework, establishing threshold behavior for existence/non-existence of solutions depending on the parameter μ, with applications to both subcritical and critical singular growth cases.
...
TL;DR: Global well-posedness and asymptotic convergence of strong solutions to a norm-preserving nonlinear heat equation on Poincaré domains.


<details>
  <summary>Details</summary>
Motivation: To establish global existence and analyze long-term behavior of solutions to an L²-norm preserving nonlinear heat equation, which maintains unit norm constraint throughout evolution.

Method: Modified nonlinearity, m-accretive evolution equation theory, resolvent and Yosida approximation methods, Łojasiewicz-Simon gradient inequality on Hilbert submanifolds.

Result: Proved existence of global strong solution for p in specified ranges; showed convergence to stationary state in W^{2,q} ∩ W^{1,q}_0 spaces for bounded domains with even p and d≤3.

Conclusion: Provides alternative method for establishing global existence and analyzing asymptotic behavior of norm-preserving nonlinear heat equations, demonstrating convergence to equilibrium.
...
TL;DR: The paper constructs optimal Hardy-weights for Finsler p-Dirichlet integrals with and without potentials, using local Morrey space conditions.


<details>
  <summary>Details</summary>
Motivation: To extend Hardy-type inequalities to Finsler p-Dirichlet integrals in domains with singular points, providing optimal weights for both the basic operator and its perturbation with potentials.

Method: Construct optimal Hardy-weights for the Finsler p-Dirichlet integral Q₀ on domains with removed singular points, and for Q_V with potentials in a subspace of local Morrey spaces, under certain conditions on the domain and parameters.

Result: Successfully constructs explicit optimal Hardy-weights for both Q₀ and Q_V, establishing sharp Hardy-type inequalities for Finsler p-Laplacian operators in singular domains.

Conclusion: The work provides optimal Hardy-weights for Finsler p-Dirichlet integrals, extending classical Hardy inequalities to anisotropic settings with singularities and potential perturbations.
...
TL;DR: Analysis of a diffuse-interface model for binary mixtures in porous media using Navier-Stokes-Cahn-Hilliard system with periodic homogenization, yielding different effective models depending on capillarity strength scaling.


<details>
  <summary>Details</summary>
Motivation: To understand the behavior of binary incompressible mixtures in periodically perforated porous media at microscopic and macroscopic scales, particularly how capillarity effects influence the effective macroscopic equations.

Method: Two-part analysis: (1) existence of weak solutions with uniform ε-estimates for fixed ε, (2) periodic homogenization (ε→0) with two regimes based on capillarity strength λ^ε scaling.

Result: Two distinct effective models emerge: (i) Stokes-Cahn-Hilliard system without macroscopic convection for vanishing capillarity (λ=0), (ii) full Navier-Stokes-Cahn-Hilliard system with nonlinear convection for balanced capillarity (λ∈(0,∞)). Also establishes convergence of microscopic free energy to homogenized energy functional.

Conclusion: The scaling of capillarity strength relative to pore scale critically determines the macroscopic behavior of binary mixtures in porous media, with different effective equations emerging for different capillarity regimes.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to understand the interplay between spatial transport and degeneracies in collision operators for nonlinear transport equations, specifically how dispersion effects can significantly extend solution lifespan beyond what conservation laws alone predict.

Method: Develops a functional framework capturing spatial transport and collision operator degeneracies, uses dispersive estimates for transport flow to quantify spatial propagation decay effects, and applies these to obtain improved bounds for the nonlinear collision operator.

Result: Shows that small solutions near vacuum can be propagated on significantly longer time scales, extending classical quadratic lifespan to quartic time scale through dispersion effects.

Conclusion: Dispersion generated by spatial propagation provides a mechanism to overcome structural degeneracies in collision operators, allowing for extended solution existence beyond conservation law predictions in nonlinear transport equations.
...
<details>
  <summary>Details</summary>
Motivation: To extend classical Green-function techniques to fractional differential equations involving Prabhakar fractional derivatives, which represent a wider class of fractional operators beyond traditional Caputo/Riemann-Liouville derivatives.

Method: Uses structural properties of Prabhakar kernel and generalized Mittag-Leffler functions to reduce the boundary value problem to a Volterra type integral equation, enabling explicit construction of the Green's function.

Result: Successfully constructs the corresponding Green's function, derives a closed-form integral representation of the solution, and proves existence and uniqueness of solutions for the boundary value problem.

Conclusion: The work extends analytical tools to Prabhakar-type fractional differential equations, providing foundations for studying boundary and inverse problems in this more general fractional calculus framework.
...
<details>
  <summary>Details</summary>
Motivation: The Prabhakar fractional derivative is a generalization that encompasses several well-known fractional derivatives. Understanding its properties and developing operational calculus for it enables solving complex fractional differential equations that use this operator.

Method: Establish fundamental properties of the nth-level Prabhakar fractional derivative, particularly its relationship with the corresponding Prabhakar fractional integral. Develop Mikusinski-type operational calculus framework for this derivative to solve differential equations involving this operator.

Result: Successfully established the properties of the nth-level Prabhakar fractional derivative and developed operational calculus for it. Applied this framework to obtain analytical solutions for two problems: a fractional order ordinary differential equation and the time fractional heat equation, both containing the nth-level Prabhakar derivative.

Conclusion: The study provides a comprehensive framework for working with nth-level Prabhakar fractional derivatives, including operational calculus and solution methods, making it applicable to various fractional differential equation problems in mathematical physics and engineering.
...
TL;DR: This paper addresses a gap in Wiegner's theorem about Navier-Stokes solution decay rates in 2D for non-algebraic decay cases.


<details>
  <summary>Details</summary>
Motivation: Wiegner's seminal work 40 years ago established sharp algebraic decay rates for Navier-Stokes solutions, showing they behave asymptotically like heat equation solutions in the L²-norm. However, there's a gap in the conclusion for 2D solutions with non-algebraic decay rates that needs to be addressed.

Method: The paper presumably uses mathematical analysis techniques to examine the asymptotic behavior of Navier-Stokes solutions in 2D, focusing on cases with non-algebraic decay rates where Wiegner's original theorem has limitations.

Result: The paper closes the identified gap in Wiegner's theorem for 2D Navier-Stokes solutions with non-algebraic decay rates, providing a more complete understanding of their asymptotic behavior.

Conclusion: The research completes Wiegner's foundational work by addressing the missing piece for 2D Navier-Stokes solutions with non-algebraic decay, contributing to a more comprehensive theory of asymptotic behavior for fluid equations.
...
TL;DR: Analysis of Cauchy problem for nonstationary discrete p-Laplacian on infinite graphs with inhomogeneous density, proving precise stabilization rates and universal bounds for p>2.


<details>
  <summary>Details</summary>
Motivation: To understand the long-time behavior of solutions to the nonstationary discrete p-Laplacian equation with inhomogeneous density on infinite graphs, particularly when the density is non-power function and decays rapidly.

Method: Uses energy inequalities and develops a new embedding result to analyze the Cauchy problem on infinite graphs that support Sobolev inequality.

Result: Proves precise rate of stabilization in time for nonnegative solutions when p>2 with non-power density functions, and establishes universal bounds when density decays sufficiently fast.

Conclusion: The paper provides rigorous analysis of stabilization behavior for discrete p-Laplacian equations on graphs, with results depending on the properties of the inhomogeneous density function and the parameter p.
...
TL;DR: The paper provides a first-principles derivation of classical background electromagnetic fields in QED from coherent states, establishing a unified framework that connects operator-based and functional approaches while enabling consistent treatment of effects beyond fixed background approximations.


<details>
  <summary>Details</summary>
Motivation: Classical background fields are widely used in QED for various physical situations (laser-matter interactions, strong-field phenomena), but their quantum origin needs clarification. The paper aims to systematically derive these classical fields from coherent states to provide a rigorous foundation.

Method: Starting from the operator formulation of QED, the authors show how scattering amplitudes between coherent states lead to effective background field descriptions. They maintain separation between coherent laser modes and other quantized photon degrees of freedom, and derive how conventional generating functionals with prescribed background fields emerge as limiting cases.

Result: The framework allows consistent incorporation of effects beyond fixed background approximations (depletion, backreaction) without assuming particular field strengths. The path integral representation is obtained as a reformulation of underlying Heisenberg picture amplitudes, providing unified view of operator-based and functional approaches.

Conclusion: The results establish a general, intensity-independent foundation for QED with coherent background fields, within which standard formulations of strong-field QED arise as well-defined special cases, clarifying the quantum origin of classical background fields.
...
TL;DR: Researchers develop predictive scaling laws for laser-driven ion acceleration using a unified multiphysics model, enabling optimization of proton and ion beams for medical and materials applications.


<details>
  <summary>Details</summary>
Motivation: Despite active research on laser-driven ion beams for applications like proton therapy and materials modification, there's a lack of predictive correlations between laser/target parameters and ion-beam properties due to the complex, multiphysics nature of laser-plasma interactions.
...
Result: Generated scaling relations, contour maps, and box plots that correlate proton, carbon, and oxygen ion properties (cutoff energies, divergences, ionization states) to laser/target parameters including pulse duration, power, spot size, target thickness, prepulse interval, contrast, wavelength, and polarization.

Conclusion: The framework provides predictive and physically interpretable tools for understanding and optimizing laser-driven ion sources across a wide parameter space, elucidating the coupled roles of laser pulse and target geometry in TNSA ion acceleration.
...
TL;DR: SPARC tokamak impurity transport simulations show turbulent transport dominates neoclassical, leading to low tungsten accumulation in H-mode plasmas with minimal impact from rotation and pedestal impurity variations.


<details>
  <summary>Details</summary>
Motivation: To predict impurity transport behavior in the upcoming SPARC tokamak, particularly for tungsten (W) accumulation which can degrade plasma performance, and to understand how transport mechanisms compare to ITER predictions.

Method: Used ASTRA+STRAHL framework with FACIT for neoclassical transport, TGLF-SAT2 for turbulent transport, and neural network trained on EPED simulations for self-consistent pedestal calculations. Benchmarked against previous simulations and performed sensitivity studies on impurity concentrations, rotation, and DT fuel composition.

Result: Turbulent impurity transport dominates neoclassical transport; predictions are insensitive to W pedestal concentrations; Ar variations have small effects; rotation has minimal impact; optimal DT composition is 55-45%; all results suggest low W accumulation in SPARC.

Conclusion: SPARC, operating at low collisionality, will experience low tungsten accumulation similar to ITER predictions, with turbulent transport prevailing over neoclassical transport, making it robust against various modeling uncertainties.
...
### [30] [Prediction Air Temperature in Geothermal Heat Exchangers Using Pseudorandom Numbers: The New DARL Model](https://arxiv.org/abs/2512.19976)
*C. Ramírez-Dolores,J. C. Zamora-Luria,J. A. Altamirano-Acosta,L. Sarao-Cruz,P. Jiménez-Palma,J. Moreno-Falconi*

Main category: cs.CY

TL;DR: DARL model uses Fermat's prime seeds and pseudo-random numbers to predict air temperature distribution in Earth-Air-Water Heat Exchangers with <6.2% error, reducing sensor dependency.
...
Method: Developed DARL model that integrates experimental boundary condition data with simulations using pseudo-random numbers (PRNs). PRNs are generated using Fermat's prime numbers as seeds. The model uses ordinary linear regressions and robust statistical validation including Shapiro-Wilk test and root mean square error.

Result: The model can estimate thermal distribution of air at different lengths with relative error less than 6.2%. Demonstrates efficiency, predictive capacity, and potential to reduce dependence on sensors.

Conclusion: DARL represents a significant methodological advance for EAWHE characterization, offering an accurate alternative to dense sensor networks while maintaining predictive capability with minimal instrumentation requirements.
...
### [31] [Propagation Estimates for the Boson Star Equation](https://arxiv.org/abs/2512.20718)
*Sébastien Breteaux,Jérémy Faupin,Viviana Grasselli*

Main category: math-ph

TL;DR: The paper analyzes propagation properties of boson star equations with general two-body potentials, proving finite speed of propagation bounds and asymptotic phase-space estimates.


<details>
  <summary>Details</summary>
Motivation: To understand the propagation properties of solutions to boson star equations with general interaction potentials, particularly how fast information can travel and asymptotic behavior of solutions.

Method: The authors work with boson star equations in Sobolev spaces, assuming the two-body potential decomposes into a finite signed measure plus a bounded function. They use mathematical analysis techniques to prove propagation bounds and asymptotic estimates.

Result: 1) Local solutions cannot propagate faster than light speed (up to exponentially small remainder). 2) For short-range potentials with regular, small initial data, they prove asymptotic phase-space propagation estimates and minimal velocity estimates depending on scattering state momentum.

Conclusion: The boson star equation exhibits relativistic-like propagation constraints, with solutions bounded by light speed propagation and specific asymptotic behaviors determined by scattering states for appropriate potentials and initial conditions.
...
### [32] [On the Relationship Between Nanoflare Energy and Delay in the Closed Solar Corona](https://arxiv.org/abs/2512.20875)
*Shanwlee Sow Mondal,James A. Klimchuk,Craig D. Johnston,Lars K. S. Daldorff*

Main category: astro-ph.SR

TL;DR: Study finds no correlation between nanoflare energies and their delays, suggesting nanoflare onset is not determined solely by magnetic stress thresholds but may involve triggering by other events.


<details>
  <summary>Details</summary>
Motivation: Understanding the relationship between nanoflare energies and their delays is crucial for revealing the physical mechanisms behind these events and plasma response in solar active regions.

Method: Used 3D multi-strand simulation with prescribed photospheric motions to generate nanoflares self-consistently. Quantified energies and durations using three distinct methods, then investigated correlations using two non-parametric, rank-based statistical tests.

Result: Consistently found little to no correlation between nanoflare energies and delays across all methods. The exponent α in the assumed relation E ∝ τ_D^α peaks near zero, and delay distributions within fixed energy bins are broad. Results hold for both preceding and subsequent event correlations, and for high-energy nanoflares.
...
### [33] [Flow Gym](https://arxiv.org/abs/2512.20642)
*Francesco Banelli,Antonio Terpin,Alan Bonomi,Raffaello D'Andrea*

Main category: physics.flu-dyn

TL;DR: Flow Gym is a toolkit for flow-field quantification research and deployment, inspired by OpenAI Gym, with synthetic image generation and unified algorithm interface.


<details>
  <summary>Details</summary>
Motivation: To create a standardized toolkit for flow-field quantification research that provides a unified interface for testing, deploying, and training algorithms, similar to how OpenAI Gym standardized reinforcement learning environments.

Method: Built as a toolkit using SynthPix for synthetic image generation, provides unified interface for flow-field quantification algorithms, includes integrations of existing algorithms and JAX implementations.

Result: A comprehensive toolkit that enables standardized evaluation and deployment of flow-field quantification methods from consecutive tracer particle images, with growing library of algorithm integrations.

Conclusion: Flow Gym successfully creates a standardized platform for flow-field quantification research, facilitating algorithm comparison, deployment, and training through its unified interface and synthetic data generation capabilities.
...
### [34] [Velocity dip in turbulent mixed convection of an open Poiseuille-Rayleigh-Bénard channel](https://arxiv.org/abs/2512.20977)
*Ben-Rui Xu,Ao Xu,Heng-Dong Xi*

Main category: physics.flu-dyn

TL;DR: Velocity-dip phenomenon emerges in turbulent mixed convection in open PRB channels with free-slip upper boundary due to large-scale roll structures transporting low-speed fluid upward, creating near-surface low-velocity regions.


<details>
  <summary>Details</summary>
Motivation: To understand the emergence of velocity-dip phenomenon in turbulent mixed convection where maximum streamwise velocity occurs below the upper boundary rather than at the surface, which has implications for environmental flows and heat transfer applications.

Method: 3D direct numerical simulations (DNS) for Rayleigh numbers 10^5 ≤ Ra ≤ 10^8 at fixed Pr = 0.71 and Re_b = 2850, analyzing flow regimes from shear-dominated to buoyancy-dominated, and developing analytical model based on momentum budget with Reynolds shear stress and buoyancy production terms.

Result: Three flow regimes identified: shear-dominated with small-scale streaks, mixed regime with large-scale rolls spanning full channel height, and buoyancy-dominated with fragmented rolls. Velocity dip occurs when large-scale rolls transport low-speed fluid upward, creating near-surface low-velocity regions. Model accurately reproduces DNS velocity profiles across Ra range.

Conclusion: Velocity-dip phenomenon in turbulent mixed convection arises from large-scale roll structures that transport low-speed fluid upward, creating laterally extended low-speed regions near the upper boundary. The proposed model successfully captures this behavior through balance of buoyancy and shear production with dissipation.
...
TL;DR: Scientific ML models (NODEs and UDEs) predict n-body trajectories more efficiently than traditional black-box ML, with UDEs needing only 20% of data vs NODEs' 90% for accurate forecasting.


<details>
  <summary>Details</summary>
Motivation: Traditional ML models for n-body trajectory prediction are data-intensive black boxes that ignore physical laws, lacking interpretability. Scientific ML embeds known physics directly into ML frameworks for more efficient and interpretable predictions.

Method: Used Scientific ML frameworks in Julia: Neural ODEs (NODEs) and Universal Differential Equations (UDEs) to predict n-body system dynamics. Employed synthetically created noisy data to simulate real-world observational limitations. Determined forecasting breakdown point - the minimum training data needed for accurate predictions.

Result: UDE model is significantly more data-efficient, requiring only 20% of data for correct forecasting, while Neural ODE requires 90% of data. Both models successfully predict n-body trajectories while incorporating physical laws.

Conclusion: Scientific ML approaches, particularly UDEs, offer efficient and interpretable alternatives to traditional black-box ML for n-body trajectory prediction, requiring substantially less training data while maintaining physical consistency.
...
### [36] [Improving Matrix Exponential for Generative AI Flows: A Taylor-Based Approach Beyond Paterson--Stockmeyer](https://arxiv.org/abs/2512.20777)
...
<details>
  <summary>Details</summary>
Motivation: Matrix exponential is fundamental for scientific computing and system simulation, with applications in control theory, quantum mechanics, and generative machine learning. While Padé approximants with scaling/squaring have been standard, recent Taylor-based methods offer superior accuracy and reduced computational complexity, especially for high-throughput generative AI flows.
...
Result: Extensive numerical experiments show significant acceleration and high numerical stability compared to existing state-of-the-art implementations, establishing the method as highly efficient for large-scale generative modeling.

Conclusion: The proposed Taylor-based algorithm provides an efficient, high-performance solution for matrix exponential computation in generative AI applications, outperforming traditional Padé-based approaches.
...
### [37] [The space spinor formalism and estimates for spinor fields](https://arxiv.org/abs/2512.20768)
*Mariem Magdy,Juan A. Valiente Kroon*

Main category: gr-qc

TL;DR: The paper adapts the space spinor formalism to construct estimates for spinor fields satisfying first-order equations, connecting this approach to existing estimation strategies and recasting hyperbolicity concepts in spinor contexts.


<details>
  <summary>Details</summary>
Motivation: To develop systematic methods for constructing estimates for spinor fields satisfying first-order equations, bridging the gap between existing estimation techniques for second-order hyperbolic equations and spinor field analysis.

Method: Uses the space spinor formalism for 2-component spinors to construct estimates, adapts the method of positive commutators (originally for second-order hyperbolic equations) to first-order spinor equations, and reformulates hyperbolicity concepts in spinor language.

Result: Develops a framework for constructing estimates for spinor fields, establishes connections with other estimation strategies, and provides spinor formulations of hyperbolicity concepts applicable to first-order equations.

Conclusion: The space spinor formalism provides an effective approach for constructing estimates for first-order spinor equations, successfully adapting techniques from second-order hyperbolic equations and offering new perspectives on hyperbolicity in spinor contexts.
...
### [38] [Minijets and Broken Stationarity in a Blazar : Novel Insights into the Origin of $γ$-ray Variability in CTA 102](https://arxiv.org/abs/2512.21240)
*Agniva Roychowdhury*

Main category: astro-ph.HE

TL;DR: Analysis of 18-year Fermi-LAT data shows CTA 102's GeV light curves don't follow strict log-normal distributions, with skewness decreasing after the 2017 flare, indicating a state transition from frequent flaring to more plateaued behavior explained by magnetic relaxation and a modified minijets-in-a-jet model.


<details>
  <summary>Details</summary>
Motivation: To understand why high-energy blazar light curves (particularly CTA 102's GeV emission) deviate from the historically preferred log-normal flux distribution, and to investigate the physical mechanisms behind the 2017 flare and subsequent state transition.

Method: Analyzed 18-year archival Fermi-LAT light curves (0.1-100 GeV) of CTA 102 from 2008-2025; performed statistical analyses of flux distributions; used Monte Carlo simulations of a modified minijets-in-a-jet model where GeV flares occur when maximum minijets move toward the broad line region and line of sight in an external Compton framework.

Result: Neither pre- nor post-flare GeV light curves follow strict log-normal distributions; significant reduction in skewness from pre- to post-flare indicates transition from energetic state with frequent flaring to more plateaued state with occasional flaring; modified minijets model successfully reproduces both GeV flares and their flux distributions.

Conclusion: The state transition in CTA 102 can be explained by magnetic relaxation following the 2017 flare, where multiple reconnection events caused the flare and subsequent magnetic field ordering minimized energy; the modified minijets-in-a-jet model effectively explains both the flare production and observed flux distribution characteristics.
...
### [39] [Ab initio Approach to Collective Excitations in Excitonic Insulators](https://arxiv.org/abs/2512.20969)
*Fengyuan Xuan,Jiexi Song,Zhiyuan Sun*

Main category: cond-mat.mtrl-sci

TL;DR: An ab initio method for studying collective excitations in symmetry-broken electronic systems like excitonic insulators, density waves, and superconductors, demonstrated on biased WSe2-MoSe2 bilayer.
...
Method: Derived the Bethe-Salpeter-Equation for particle-hole excitations in quasiparticle representation, solving for collective excited states and computing order parameter fluctuations.

Result: Successfully applied to excitonic insulating phases of biased WSe2-MoSe2 bilayer, revealing gapless phase-mode, subgap Bardasis-Schrieffer modes, and above-gap scattering states.

Conclusion: This work establishes a pathway for quantitative first-principles predictions of excited state phenomena in symmetry-broken electronic systems.
...
### [40] [Implicit Numerical Scheme for the Hamilton-Jacobi-Bellman Quasi-Variational Inequality in the Optimal Market-Making Problem with Alpha Signal](https://arxiv.org/abs/2512.20850)
*Alexey Meteykin*
...
<details>
  <summary>Details</summary>
Motivation: Address the problem of combined stochastic and impulse control for market makers in limit order books, where explicit methods have time-step restrictions and stability issues.
...
Result: Removes time-step restrictions of explicit methods, ensures unconditional stability, and establishes convergence to unique viscosity solution via monotonicity, stability, consistency conditions and comparison principle.

Conclusion: The proposed implicit scheme with policy iteration provides a stable, convergent numerical method for market maker control problems without time-step limitations.
...
### [41] [Formal O(N3) scaling GW calculations by block tensor decomposition for large molecule systems](https://arxiv.org/abs/2512.21022)
...
<details>
  <summary>Details</summary>
Motivation: The GW approximation is crucial for quasiparticle energies but has high computational cost and poor scaling, limiting applications to large molecular systems.

Method: Extends block tensor decomposition (BTD) algorithm with imaginary-time GW formalism and real space screening for polarizability; optimizes parameters using JADE algorithm on S66 dataset.

Result: Achieves observed O(N²) scaling in test systems; BTD-based random phase approximation also shows O(N²) scaling; enables eigenvalue-self-consistent GW calculations for systems with >3000 basis functions.

Conclusion: BTD establishes an efficient and scalable approach for large-scale GW calculations in molecular systems, overcoming previous computational limitations.
...
### [42] [Quantum Homotopy Algorithm for Solving Nonlinear PDEs and Flow Problems](https://arxiv.org/abs/2512.21033)
...
<details>
  <summary>Details</summary>
Motivation: Quantum algorithms for nonlinear PDEs are challenging to develop but crucial for practical quantum computing applications, especially for flow problems.

Method: Embed PDEs in truncated high-dimensional linear space using quantum homotopy analysis, discretize with finite-difference methods using compact quantum algorithm, adapt to nonlinearity and physics.

Result: Improved complexity estimates in matrix operator norms, condition number, simulation time, accuracy; provides embedding strategy, stability bounds, gate counts, query complexity; connects nonlinearity measure to Reynolds-like parameter.

Conclusion: Demonstrates hybrid quantum algorithm's potential for simulating practical nonlinear phenomena on near-term and fault-tolerant quantum devices.
...
### [43] [The Dynamical Anatomy of Anderson Acceleration:From Adaptive Momentum to Variable-Mass ODEs](https://arxiv.org/abs/2512.21269)
*Kewang Chen,Yongqiu Jiang,Kees Vuik*

Main category: math.OC

TL;DR: This paper bridges Anderson Acceleration (AA) with continuous dynamical systems, revealing AA's instability mechanism and proposing Energy-Guarded AA (EG-AA) with improved convergence stability.


<details>
  <summary>Details</summary>
Motivation: The dynamics of Anderson Acceleration (introduced in 1965) remain poorly understood compared to classical Nesterov acceleration. Fundamental questions about AA's physical nature and stability mechanisms have remained unanswered, motivating a rigorous connection between discrete acceleration algorithms and continuous dynamical systems.

Method: The authors use High-Resolution ODE analysis to show AA can be rewritten as an adaptive momentum method converging to a second-order ODE with Variable Effective Mass. Through Lyapunov energy analysis, they identify AA's instability mechanism (unchecked effective mass growth acting as negative damping) and discover implicit Hessian-driven damping. They then propose Energy-Guarded Anderson Acceleration (EG-AA) that enforces thermodynamic consistency via inertial governing.

Result: Theoretical analysis proves EG-AA improves upon gradient descent by maximizing geometric contraction of linear subspace projection while suppressing nonlinear approximation errors. Theoretical bounds confirm EG-AA is no worse than standard AA, and numerical experiments demonstrate strictly improved convergence stability and rates in ill-conditioned convex composite problems.

Conclusion: The paper provides fundamental theoretical insights into Anderson Acceleration's dynamics, reveals its instability mechanism, and introduces EG-AA as a thermodynamically consistent algorithm with proven improved convergence properties over standard AA.
...
<details>
  <summary>Details</summary>
Motivation: To provide a comprehensive mathematical foundation for symmetry-protected BICs in waveguide arrays, moving beyond approximations like tight-binding or orthogonal coupled-mode equations to enable precise device design.

Method: Transforms wave propagation into Nonorthogonal Coupled-Mode Equations (NCME), derives exact expressions using Bessel function addition theorems, analyzes finite and infinite arrays, and proves BIC existence in symmetric waveguide configurations with numerical verification of symmetry-breaking effects.

Result: Successfully derived exact expressions for overlap integrals and coupling coefficients, characterized dispersion relations for infinite arrays, provided rigorous proof of BIC existence in symmetric waveguide systems, and demonstrated transition from perfect BIC to leaky modes with symmetry-breaking perturbations.

Conclusion: The work establishes a comprehensive mathematical framework for symmetry-protected BICs, providing an efficient and precise computational model for designing BIC-based optical devices with rigorous analytical foundations.
...
### [45] [Regularity of Einstein 5-manifolds via 4-dimensional gap theorems](https://arxiv.org/abs/2512.21317)
...
<details>
  <summary>Details</summary>
Motivation: To understand the fine structure of singularities in noncollapsed limits of 5-dimensional Einstein manifolds with bounded Ricci curvature, particularly improving regularity results beyond previous understanding.

Method: Uses geometric analysis techniques including tangent cone analysis, Gromov-Hausdorff convergence, and orbifold theory. Proves a key independent result that spherical and hyperbolic 4-orbifolds are isolated among Einstein 4-orbifolds in Gromov-Hausdorff sense.

Result: Five main results: (1) tangent cone uniqueness on top stratum, (2) singular set contained in countable union of Lipschitz curves/points, (3) Lipschitz curves are smooth geodesics away from nowhere dense set, (4) interior of geodesics removable leading to real-analytic orbifolds, (5) tangent cone uniqueness at infinity for asymptotically Ricci-flat manifolds with line splitting.

Conclusion: The results significantly improve regularity understanding of 5-dimensional Einstein manifold limits and suggest broader questions about orbifold regularity in higher dimensions. The isolation result for Einstein 4-orbifolds provides gap theorems that don't extend to higher dimensions.
