...
TL;DR: The paper proposes a new cluster-robust vector iteration method that replaces LOPCG with asymptotically equivalent two-term recurrences and uses timely correction with far previous iterates to improve convergence for clustered eigenvalues.


<details>
  <summary>Details</summary>
Motivation: Existing single-vector eigensolvers like LOPCG and its alternatives suffer from frequent delays and staircase-shaped convergence behavior when computing clustered eigenvalues, which cannot be explained by existing estimates. This inefficiency motivates the development of more robust methods.

Method: The authors construct a class of cluster robust vector iterations by: 1) replacing LOPCG with asymptotically equivalent two-term recurrences, and 2) timely correcting search directions by selecting a far previous iterate as augmentation.

Result: The new approach significantly reduces both the number of required steps and the total computational time compared to existing methods.

Conclusion: The proposed cluster-robust vector iteration method effectively addresses the convergence delays in existing CG-like eigensolvers for clustered eigenvalues, leading to improved efficiency in eigenvalue computations.
...
TL;DR: The paper provides tight error bounds for two extremal matrix approximation problems: approximating top eigenspaces of Hermitian matrices and approximating orthonormal polar factors of general matrices.


<details>
  <summary>Details</summary>
Motivation: The paper addresses fundamental problems in matrix analysis - approximating important matrix structures (eigenspaces and polar factors) that have applications in numerical linear algebra, optimization, and data analysis. These approximations are computationally important when exact computation is expensive or impractical.

Method: The paper uses matrix analysis techniques to derive theoretical error bounds. While the abstract doesn't specify particular algorithms, it focuses on establishing tight (optimal) bounds on approximation quality for these two extremal problems.

Result: The main result is obtaining tight error bounds on the quality of approximations for both problems. These bounds are optimal in the sense that they cannot be improved without additional assumptions.

Conclusion: The paper provides fundamental theoretical guarantees for approximating important matrix structures, establishing optimal error bounds that can guide algorithm design and analysis in numerical linear algebra applications.
...
TL;DR: STNP method solves fractional Burgers equations with nonlinear parametrization, avoiding Gibbs oscillations and outperforming spectral methods with fewer degrees of freedom.


<details>
  <summary>Details</summary>
Motivation: Classical numerical methods struggle with fractional Burgers equations due to combined nonlocality and shock-forming dynamics, suffering from Gibbs oscillations and requiring ad hoc stabilization that degrades in transport-dominated regimes.

Method: Sequential-in-time nonlinear parametrization (STNP) represents solution with nonlinear parametric ansatz, projects governing dynamics onto tangent space of parameter manifold using regularized least-squares at each time step, avoiding global optimization.

Result: STNP achieves oscillation-free shock resolution, accurately captures long-time dynamics, outperforms high-order spectral schemes with spectral vanishing viscosity, requires significantly fewer degrees of freedom, and avoids ad hoc stabilization.

Conclusion: STNP provides stable, well-posed time-marching scheme for fractional Burgers equations with theoretical guarantees, offering superior performance over classical methods while eliminating stabilization tuning issues.
...
TL;DR: MGDL effectively solves highly oscillatory Fredholm integral equations with rigorous error analysis and adaptive grade selection based on training performance.


<details>
  <summary>Details</summary>
Motivation: To develop an effective deep learning approach for solving challenging highly oscillatory Fredholm integral equations of the second kind, which are difficult to solve with traditional numerical methods.

Method: Multi-Grade Deep Learning (MGDL) with rigorous error analysis of continuous and discrete models, plus a novel adaptive algorithm that selects network grade based on training performance.

Result: Discrete MGDL retains convergence and stability of continuous model under small quadrature error; DNN training error identified as primary approximation source; adaptive MGDL shows accuracy, effectiveness and robustness in numerical experiments with highly oscillatory (wavenumber 500) and singular solutions.

Conclusion: MGDL provides a robust and effective approach for solving highly oscillatory Fredholm integral equations, with adaptive grade selection based on training performance ensuring optimal performance.
...
TL;DR: ECFM-based optimal experimental design tends to place measurements in stiff regions where responses are small, making it impractical for noisy real-world applications.


<details>
  <summary>Details</summary>
Motivation: To explore optimal experimental design (OED) using the explicit constraint force method (ECFM), which has been used for solution reconstruction and inverse problems but hasn't been investigated for experimental design.

Method: Developed an OED formulation based on constraint forces analogous to traditional Fisher information matrix approaches, then tested on several example problems to compare with standard methods.

Result: ECFM-based OED tends to position measurements in the stiffest regions of systems where responses are minimal, making this approach impractical in the presence of measurement noise or finite precision.

Conclusion: ECFM is not a viable approach to optimal experimental design due to its tendency to place measurements in regions with small responses, which is problematic for real-world applications with noise.
...
Result: Framework demonstrated optimal convergence rates in both space and time through rigorous convergence analyses. Parametric study revealed that magnitude parameter of stress-dependent variation in wave speed leads to wavefront steepening and shock discontinuities, while exponent parameter acts as nonlinearity filter - high values suppress nonlinear effects in small-strain regimes, low values allow significant dispersive behavior.

Conclusion: The work provides a validated computational tool for analyzing shock formation in advanced nonlinear materials, successfully capturing important nonlinear wave phenomena and offering insights into how constitutive parameters influence wave propagation behavior in nonlinear elastic materials.
...
TL;DR: Numerical computation of L² Marcinkiewicz-Zygmund constants for cubature rules and their role in polynomial approximation using orthogonal bases across various domains.


<details>
  <summary>Details</summary>
Motivation: To understand how Marcinkiewicz-Zygmund constants affect polynomial approximation quality and to compare different approximation methods (least squares projection, standard hyperinterpolation, and exactness-relaxed hyperinterpolation) across various geometric domains.

Method: Numerical computation of L² Marcinkiewicz-Zygmund constants for cubature rules on multiple domains (interval, square, disk, triangle, cube, sphere). Comparison of least squares projection approximation power with standard hyperinterpolation and its exactness-relaxed version.

Result: The paper provides numerical results of Marcinkiewicz-Zygmund constants for various cubature rules and compares the approximation performance of different polynomial approximation methods across tested domains. Matlab codes are made available as open-source.

Conclusion: The study offers practical computational tools and insights into how Marcinkiewicz-Zygmund constants influence polynomial approximation quality, with implications for choosing appropriate cubature rules and approximation methods for different domains.
...
TL;DR: A finite element method that preserves eigenvalue bounds for tensor-valued convection-diffusion equations by formulating the problem as a variational inequality on a convex set and using eigenvalue truncation at nodes.


<details>
  <summary>Details</summary>
Motivation: To develop a numerical method that preserves the eigenvalue range of solutions for tensor-valued time-dependent convection-diffusion equations at the degrees of freedom, ensuring physical constraints are maintained during discretization.

Method: Uses high-order CIP-stabilized finite element spatial discretization, formulates the problem as a variational inequality on a convex set of tensor-valued functions with eigenvalue bounds, implements nodal projection that diagonalizes and truncates eigenvalues, and employs implicit Euler temporal discretization.

Result: Proves unconditional stability and optimal-order error estimates for the scheme, with numerical experiments confirming theoretical results and demonstrating the method's ability to maintain eigenvalue constraints in convection-dominated regimes.

Conclusion: The proposed method successfully preserves eigenvalue bounds at degrees of freedom while maintaining accuracy, particularly important for convection-dominated problems where physical constraints must be respected.
...
TL;DR: Theoretical analysis of Virtual Element Method for incompressible non-Newtonian Carreau-Yasuda flows in shear-thickening regime (r>2), extending previous shear-thinning work with novel analytical tools.


<details>
  <summary>Details</summary>
Motivation: Extend Virtual Element Method analysis to shear-thickening non-Newtonian flows (r>2), addressing both degenerate and non-degenerate cases, which requires new theoretical tools beyond previous shear-thinning work.

Method: Virtual Element Method with exactly divergence-free discrete velocity field, compatible with polygonal meshes. Novel analytical tools include: inf-sup stability analysis in non-Hilbertian norms, specially designed stabilization for nonlinear structure (r>2), and discrete norm tailored to nonlinear constitutive relation.

Result: Comprehensive theoretical analysis for shear-thickening regime (r>2) of Carreau-Yasuda flows, with numerical results demonstrating practical performance of the proposed formulation.

Conclusion: Successfully extends Virtual Element Method to shear-thickening non-Newtonian flows with novel theoretical framework, maintaining advantages of divergence-free velocity and polygonal mesh compatibility.
...
TL;DR: GVD is a self-tuning image decomposition method that combines variational models with learned adaptive weights via statistics or neural networks, eliminating manual parameter tuning.


<details>
  <summary>Details</summary>
Motivation: Classical variational/optimization models for cartoon-texture decomposition are numerically intractable and require tedious manual tuning of global regularization parameters, creating a bottleneck in preprocessing.

Method: Proposes Guided Variational Decomposition (GVD) with spatially adaptive quadratic norms where pixel-wise weights are learned either through local probabilistic statistics or via a lightweight neural network within a bilevel optimization framework.

Result: GVD provides a unified, interpretable, and computationally efficient model that bridges classical variational methods with modern adaptive/data-driven approaches, delivering robust and superior decomposition performance with automatic parameter selection.

Conclusion: GVD offers a self-tuning, robust solution for reliable cartoon-texture image decomposition that eliminates manual parameter tuning while maintaining interpretability and computational efficiency.
...
TL;DR: A new method for rigorous integration of parabolic PDEs with guaranteed error bounds between numerical and exact solutions, using a posteriori fixed point reformulation and adaptive time-stepping.


<details>
  <summary>Details</summary>
Motivation: Current PDE simulations provide approximate numerical solutions without correctness guarantees. The paper aims to develop a method that produces rigorous, explicit error bounds between numerical approximations and exact solutions, ensuring correctness over entire time intervals.

Method: Uses a fixed point reformulation based on piece-wise constant approximations of linearization around numerical solutions. The approach yields simple-to-understand estimates enabling optimization of proofs and adaptive time-stepping. For stable hyperbolic equilibria, can prove convergence with an infinitely long final timestep.

Result: Demonstrates ability to rigorously integrate over relatively long time intervals and capture non-trivial dynamics through examples on Swift-Hohenberg, Ohta-Kawasaki, and Kuramoto-Sivashinsky equations.

Conclusion: The method provides rigorous error bounds for parabolic PDE integration with simplicity and efficiency that should enable generalization to other parabolic PDEs and applications to boundary value problems.
...
TL;DR: Extends variable projection method to handle differentiable regularization on nonlinear parameters and general-form Tikhonov on linear variables, with quasi-Newton solver and convergence analysis, plus inexact LSQR variant for large-scale problems.


<details>
  <summary>Details</summary>
Motivation: Separable nonlinear least squares problems are common in inverse problems like semi-blind image deblurring. Variable projection efficiently solves these by eliminating linear variables, but existing methods don't handle regularization on nonlinear parameters well. Need to extend VarPro to handle both nonlinear parameter regularization and general-form Tikhonov regularization on linear variables.

Method: Extends VarPro to minimize problems with differentiable regularization on nonlinear parameters and general-form Tikhonov on linear variables. Develops quasi-Newton method for reduced problem with local convergence analysis. For large-scale problems, introduces inexact LSQR-based variant that uses approximations while maintaining convergence.

Result: Provides local convergence analysis establishing conditions for superlinear/quadratic convergence. Inexact variant proves local convergence despite approximations. Numerical experiments on semi-blind deblurring show parameter regularization prevents degenerate no-blur solutions, methods achieve accurate reconstructions, and inexact variant offers favorable accuracy-cost tradeoff.

Conclusion: Extended VarPro framework successfully handles regularization on both linear and nonlinear parameters. Quasi-Newton method with convergence analysis provides theoretical guarantees. Inexact LSQR variant enables efficient large-scale computation while maintaining convergence. Methods are effective for semi-blind deblurring, preventing degenerate solutions and achieving accurate reconstructions.
...
<details>
  <summary>Details</summary>
Motivation: To study parabolic equations with coefficients that can be singular, degenerate, or both, which arise in various applications but lack established regularity theory due to the challenging nature of the coefficients.

Method: Introduces weighted parabolic cylinders suitable for the equations, uses parabolic ABP estimates and perturbation methods, performs careful regularization and truncation of weights, and establishes local quantitative lower estimates of solutions (mean sojourn times).

Result: Proves F.-H. Lin type weighted W^{2,ε} estimates under smallness assumptions on weighted mean oscillation of the weight, extending Evans' results from elliptic to parabolic settings with singular-degenerate coefficients.

Conclusion: Provides foundational tools and estimates for studying fully nonlinear parabolic equations with singular-degenerate coefficients, establishing a regularity theory framework for this challenging class of equations.
...
<details>
  <summary>Details</summary>
Motivation: To develop a regularized version of curvature flow for immersed plane curves with fixed boundary that avoids singularities and provides smooth approximations to the standard curvature flow.

Method: Uses an elastica-type functional with small positive parameter ε to create a fourth-order regularization of curvature flow, with Dirichlet boundary conditions for immersed plane curves.

Result: The approximating flow smoothly converges to the standard curvature flow as ε→0 for all times before the first singularity occurs in the limit flow.

Conclusion: The fourth-order regularization provides a well-behaved approximation to curvature flow that maintains smoothness and converges to the original flow before singularities develop.
...
TL;DR: The paper proves long-time bounds for solutions to Fokker-Planck equations with subelliptic diffusion, extending previous elliptic results to subelliptic cases with global L^p bounds.


<details>
  <summary>Details</summary>
Motivation: To extend recent work on semiclassical derivative estimates for Fokker-Planck equations from elliptic to subelliptic diffusion cases, addressing situations where the Hörmander condition holds but diffusion is not fully elliptic.

Method: Constructs a suitable parametrix for the Fokker-Planck equation with subelliptic diffusion, assuming linear jump operators in the Lindbladian and conditions implying the Hörmander condition holds.

Result: Shows that semiclassical derivative estimates established for elliptic diffusion in previous works also hold for subelliptic diffusion, with global bounds in L^p for all 1 ≤ p ≤ ∞.

Conclusion: Successfully extends long-time bounds and derivative estimates from elliptic to subelliptic Fokker-Planck equations, providing a more general framework for analyzing quantum systems with subelliptic diffusion.
...
TL;DR: The paper studies whether higher-order Lipschitz functions on domain boundaries can be decomposed into sums of boundary values of Lamé-Navier system solutions with jumps across the boundary, using Clifford algebra and Hardy projections.


<details>
  <summary>Details</summary>
Motivation: To investigate decomposition properties of boundary functions for solutions to the Lamé-Navier system (elasticity equations) using Clifford algebra framework, extending classical results to higher-order Lipschitz spaces.

Method: Uses Clifford algebra to reformulate Lamé-Navier system via Euclidean Dirac operator, employs Hardy projections related to singular integral operators from Clifford analysis, and leverages involution properties on first-order Lipschitz classes.

Result: Establishes decomposition results for higher-order Lipschitz boundary functions into sums of boundary values of Lamé-Navier system solutions with prescribed jumps across the boundary.

Conclusion: Clifford algebra and Hardy projection techniques provide powerful tools for analyzing boundary value problems for Lamé-Navier system, extending decomposition results to higher regularity spaces.
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses a free boundary problem representing bread baking in a hot oven, where the evaporation front growth depends on water content and boundary conditions involve temperature, creating mathematical difficulties that need to be resolved.

Method: By improving the regularity of solutions, the authors overcome the mathematical difficulties and establish local existence and uniqueness of solutions. They also analyze maximal intervals of existence under certain sign conditions on initial data.

Result: The paper proves existence of solutions locally in time and their uniqueness. Additionally, under specific sign conditions for initial data, the authors derive results on the maximal interval of existence for solutions.

Conclusion: The authors successfully resolve the mathematical challenges in the bread baking free boundary problem, establishing rigorous existence and uniqueness results while providing insights into solution behavior over time.
...
TL;DR: Liouville-type theorems for stationary shear thickening fluid equations in a slab: axisymmetric solutions trivial if L∞-norm grows mildly with radius; bounded general solutions trivial if ruʳ bounded.


<details>
  <summary>Details</summary>
Motivation: To establish Liouville-type theorems (showing solutions must be trivial under certain conditions) for stationary solutions of shear thickening fluid equations in slab domains, extending similar results from Navier-Stokes equations.

Method: Inspired by Bang et al.'s work on Navier-Stokes, establishes Saint-Venant type estimate characterizing growth of local Dirichlet integral. Key innovation: estimating constant in Korn's inequality over different domains.

Result: Two main results: 1) Axisymmetric solutions must be trivial if local L∞-norm grows mildly as radius R grows; 2) Bounded general solutions u must be trivial if ruʳ is bounded.

Conclusion: Successfully proves Liouville-type theorems for shear thickening fluid equations using Saint-Venant estimates and Korn's inequality analysis, extending Navier-Stokes techniques to non-Newtonian fluid models.
...
TL;DR: Proves Kato square root estimate for elliptic operators with Robin boundary conditions on bounded domains with accretive coefficients.


<details>
  <summary>Details</summary>
Motivation: Previous Kato square root estimates don't cover Robin boundary conditions with possibly unbounded conductivity, and first-order approaches fail for this case.

Method: Uses second-order divergence form elliptic operators with accretive coefficients, working on bounded locally uniform domains with Robin boundary conditions.

Result: Successfully proves the Kato square root estimate for operators with Robin boundary conditions, overcoming limitations of first-order approaches.

Conclusion: Extends Kato square root theory to include Robin boundary conditions with unbounded conductivity, providing new mathematical tools for boundary value problems.
...
TL;DR: This paper establishes regularity results for Lipschitz solutions to degenerate elliptic equations in 2D, showing singular sets are negligible and deriving sharp partial C¹ regularity when degeneracy occurs only on curves.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand regularity properties of solutions to highly degenerate elliptic equations in two dimensions, extending previous results that only handled zero-dimensional degeneracy sets to more complex cases where degeneracy occurs on curves.

Method: The authors use a differential inclusions viewpoint, establish a pointwise gradient localization theorem, and exploit the hyperbolic structure of the equation along degeneracy curves. They employ tools from Hamilton-Jacobi equations and scalar conservation laws to compensate for regularity loss.

Result: The main results show that the singular set of nondifferentiability points is ℋ¹-negligible, and they derive new sharp partial C¹ regularity results when the degeneracy set consists of curves, recovering and extending all previously known results.

Conclusion: The paper successfully extends regularity theory for degenerate elliptic equations from zero-dimensional degeneracy sets to curve degeneracy sets, establishing that singular sets are negligible and obtaining optimal partial C¹ regularity through novel techniques combining elliptic, hyperbolic, and conservation law theories.
...
TL;DR: The paper proves semiclassical hypocoercivity estimates for degenerate Kramers-Fokker-Planck operators and derives Eyring-Kramers formulas for their spectrum, quantifying spectral gaps using adapted WKB methods.


<details>
  <summary>Details</summary>
Motivation: To analyze Kramers-Fokker-Planck operators with general degenerate coefficients, which are important in statistical physics but challenging due to their degeneracy, requiring new analytical tools to understand their spectral properties.

Method: Uses semiclassical analysis techniques, proving hypocoercivity estimates for degenerate operators, then adapts the WKB method to construct sharp Gaussian quasimodes for specific degenerate operators in the semiclassical regime.

Result: Successfully proves semiclassical hypocoercivity estimates for a large class of degenerate Kramers-Fokker-Planck operators, derives Eyring-Kramers formulas for the bottom of their spectrum, and quantifies the spectral gap separating these eigenvalues from the rest of the spectrum.

Conclusion: The adapted WKB method provides an effective approach for analyzing degenerate Kramers-Fokker-Planck operators, yielding precise spectral information including Eyring-Kramers formulas and spectral gap quantification, advancing the understanding of such operators in semiclassical regimes.
...
TL;DR: The paper studies the 2D semigeostrophic system in small-amplitude scaling, showing it converges to incompressible Euler equations with improved lifespan (ε⁻¹|log log ε|) and strong velocity stability (O(ε) in L²).


<details>
  <summary>Details</summary>
Motivation: To establish a quantitative bridge between the semigeostrophic system and incompressible Euler equations, improving upon existing convergence rates and existence times.

Method: Combines incompressible transport structure with sharp elliptic control of velocity gradient and flow-based stability arguments within a natural bootstrap regime for Poisson/Monge-Ampère coupling.

Result: Two main results: 1) Lifespan lower bound with log-log gain (persistence at least ε⁻¹|log log ε|), 2) Strong velocity-stability estimate with O(ε) rate in L² on any bootstrap window.

Conclusion: The results provide a clean quantitative bridge from SG^ε to Euler that is both longer-lived (by log-log factor) and quantitatively stable in velocity, complementing and improving previous convergence estimates.
...
TL;DR: Overview of magnetic Laplacian results on hyperbolic surfaces from arXiv:2505.08584 and ongoing work with Charles and Chabert


<details>
  <summary>Details</summary>
Motivation: The magnetic Laplacian on hyperbolic surfaces provides a rich analytic framework for studying quantum phenomena, offering insights into spectral properties and geometric analysis in non-Euclidean settings.

Method: Mathematical analysis of the magnetic Laplacian operator on hyperbolic surfaces, combining spectral theory, geometric analysis, and quantum mechanics approaches.

Result: Main results from arXiv:2505.08584 plus additional findings from ongoing collaborative work, establishing properties of the magnetic Laplacian on hyperbolic surfaces.

Conclusion: The magnetic Laplacian on hyperbolic surfaces serves as a powerful framework for understanding quantum phenomena, with results from the referenced paper and ongoing work contributing to this analytic perspective.
...
TL;DR: The paper proves global existence and uniform boundedness of weak solutions for a doubly degenerate nutrient taxis system in 2D bounded domains with arbitrary smooth initial data.


<details>
  <summary>Details</summary>
Motivation: The system models Bacillus subtilis aggregation patterns on thin agar plates. Previous results required 1D, small initial data, or convex domains. The authors aim to establish global boundedness for arbitrary smooth initial data in 2D.
...
Result: Proves that for arbitrary smooth initial data in 2D bounded domains, the problem admits a global weak solution that remains uniformly bounded in time.

Conclusion: The quadratic degradation term provides sufficient dissipation to overcome degeneracy, enabling global boundedness results without restrictive assumptions on initial data or domain geometry.
...
TL;DR: Analysis of fast-reaction approximation to triangular SKT model on bounded domains in dimensions ≤3, providing explicit convergence rates in various function spaces.


<details>
  <summary>Details</summary>
Motivation: To understand the behavior of the triangular Shigesada-Kawasaki-Teramoto (SKT) model under fast-reaction approximation, which is important for studying pattern formation and cross-diffusion systems in mathematical biology and ecology.

Method: Using fast-reaction approximation approach on bounded domains in physical dimensions d≤3, analyzing convergence rates in different function spaces (L∞L²∩L²H¹ for whole domain, L∞Hˡ for interior).

Result: Provides explicit convergence rates: on whole domain in L∞L²∩L²H¹ spaces, and in interior in any L∞Hˡ space for all l>0.

Conclusion: The fast-reaction approximation to the triangular SKT model converges with explicit, quantifiable rates in various function spaces, offering rigorous mathematical justification for this approximation in cross-diffusion systems.
...
TL;DR: Existence and regularity of solutions to modified Navier-Stokes equations for fluid motion with a rotating rigid body in R3, using weighted Sobolev spaces to handle large-distance behavior.


<details>
  <summary>Details</summary>
Motivation: To mathematically analyze fluid-structure interaction problems involving a rotating rigid body in an unbounded domain (R3), where standard approaches may not adequately handle the behavior of solutions at large distances from the body.

Method: Uses modified Navier-Stokes equations to model the fluid motion with a rotating rigid body. Employs weighted Sobolev spaces to properly describe and control the asymptotic behavior of solutions at infinity, allowing for appropriate decay conditions.

Result: Proves existence and regularity of solutions under suitable assumptions, with solutions satisfying appropriate conditions at infinity through the weighted Sobolev space framework.

Conclusion: The weighted Sobolev space approach provides a rigorous mathematical framework for studying fluid-structure interaction problems in unbounded domains, establishing well-posedness results for modified Navier-Stokes equations with rotating bodies.
...
TL;DR: This paper studies a quasilinear two-species chemotaxis system with two chemicals, establishing critical parameter conditions that determine solution behavior: finite-time blow-up vs. global boundedness/existence.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand the dynamics of a two-species chemotaxis system with two chemicals, where each species produces a chemical that attracts the other species. This models biological phenomena where different cell populations interact through chemical signaling. The key question is to determine parameter regimes that lead to finite-time blow-up (cell aggregation) versus global boundedness or existence.

Method: The authors analyze a quasilinear PDE system with two species (u and w) and two chemicals (v and z). They study the asymptotic behavior of diffusion coefficient D(s) and sensitivity function S(s) with power-law forms. Using mathematical analysis techniques for chemotaxis systems, they establish critical parameter conditions involving exponents p and q relative to spatial dimension n.

Result: Three main results: 1) In a ball domain, when q-p > 2-n/2 and q > 1-n/2, there exist radially symmetric initial data leading to finite-time blow-up. 2) In any smooth bounded domain, when q-p < 2-n/2, all solutions are globally bounded. 3) In any smooth bounded domain, when q < 1-n/2, all solutions exist globally. These define two critical lines: q-p = 2-n/2 and q = 1-n/2.

Conclusion: The system exhibits a trichotomy of behaviors determined by parameter relationships: finite-time blow-up occurs above both critical lines, global boundedness occurs below the first critical line, and global existence occurs below the second critical line. This provides a complete classification of solution dynamics for the two-species chemotaxis system with two chemicals.
...
TL;DR: This paper studies a two-species chemotaxis system with flux-limitation and identifies a critical blow-up curve at p=q=(n-2)/(n-1) for n≥3, distinguishing between finite-time blow-up and global bounded existence.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand the blow-up behavior and global existence of solutions in a two-species chemotaxis system with flux-limitation terms, which generalizes classical Keller-Segel models and incorporates saturation effects in the chemotactic sensitivity.

Method: The authors analyze a two-species chemotaxis system with flux-limitation terms of the form (1+|∇v|²)^{-p/2} and (1+|∇z|²)^{-q/2}. They use mathematical analysis techniques including energy methods, comparison principles, and blow-up constructions to establish critical thresholds for solution behavior.

Result: The main results identify a critical blow-up curve: for n≥3, when p,q < (n-2)/(n-1), finite-time blow-up can occur; when p,q > (n-2)/(n-1), solutions exist globally and remain bounded. For n=1, solutions are always globally bounded regardless of p,q values.

Conclusion: The critical threshold p=q=(n-2)/(n-1) separates regimes of potential finite-time blow-up from guaranteed global bounded existence in the two-species chemotaxis system with flux-limitation, providing a complete characterization of solution behavior based on the flux-limitation parameters.
...
TL;DR: The paper establishes a comparison principle for entire solutions of the Allen-Cahn equation with nodal sets asymptotic to minimizing hypercones, showing that inclusion of positive phases enforces global ordering and uniquely determines solutions.


<details>
  <summary>Details</summary>
Motivation: To understand the global structure and ordering properties of entire solutions to the Allen-Cahn equation, particularly when nodal sets (including singular ones) approach minimizing hypercones at infinity, analogous to principles in minimal surface theory.

Method: Develops a maximum principle for the linearized Allen-Cahn operator on unbounded, possibly non-smooth domains, and establishes a comparison principle that relates inclusion of positive phases to global ordering of solutions.

Result: Shows that positive phase inclusion enforces global ordering of solutions, making the positive phase uniquely determine the solution, and that strict phase inclusion implies disjoint nodal sets.

Conclusion: Provides an Allen-Cahn analogue of the strong maximum principle for minimal hypersurfaces, establishing fundamental ordering properties for entire solutions with asymptotically conical nodal sets.
...
TL;DR: This paper proves finite-time blow-up solutions exist for a quasilinear two-species chemotaxis system with two chemicals when the diffusion exponents satisfy a specific inequality, complementing previous boundedness results.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate blow-up phenomena in a two-species chemotaxis system with nonlinear diffusion and two chemicals, complementing existing boundedness results by Zhong (2021) who showed globally bounded solutions under different parameter conditions.

Method: The authors study a quasilinear chemotaxis system with two species (u,w) and two chemicals (v,z) where v and z satisfy elliptic equations with nonlocal terms. They analyze the system with nonlinear diffusion functions D₁(s)≃s^{m₁-1} and D₂(s)≃s^{m₂-1} for m₁,m₂>1, focusing on the parameter regime where blow-up occurs.

Result: The main result proves that when m₁ + m₂ > max{m₁m₂ + 2m₁/n, m₁m₂ + 2m₂/n} with n≥3, the system admits solutions that blow up in finite time, complementing the boundedness result where the inequality is reversed.

Conclusion: The paper establishes a sharp threshold for blow-up vs. boundedness in this two-species chemotaxis system, showing that the inequality m₁ + m₂ relative to max{m₁m₂ + 2m₁/n, m₁m₂ + 2m₂/n} determines whether solutions remain bounded or blow up in finite time.
...
TL;DR: Spatial predator-prey model with prey refuge and predator directed movement shows complex coexistence patterns and bifurcation structures.


<details>
  <summary>Details</summary>
Motivation: To understand how spatial protection zones (refuges) for prey interact with predator movement behavior, particularly far-sighted directed movement rather than chemotaxis-type mechanisms.

Method: Study a spatial predator-prey model with prey refuge inaccessible to predators, using far-sighted population flux for predator movement. Analyze both nonstationary (local well-posedness) and stationary problems, focusing on bifurcation analysis and global continuation of coexistence states.

Result: Established local-in-time well-posedness despite refuge interface discontinuity. Identified bifurcation threshold for positive steady states from predator-only equilibria. Found that strong directed movement induces turning-point structures and multiplicity of coexistence steady states.

Conclusion: The interplay between spatial protection (refuge) and predator movement behavior creates complex coexistence patterns, with far-sighted directed movement leading to nontrivial bifurcation structures and multiple steady states.
...
TL;DR: The paper establishes well-posedness of weak solutions to nonlinear parabolic PDEs with rough coefficients and initial data in critical Besov spaces, without smallness conditions, using weighted Z-spaces and novel hypercontractive SIO theory.


<details>
  <summary>Details</summary>
Motivation: To develop a robust theory for nonlinear parabolic PDEs with rough coefficients and rough initial data, addressing the challenge of establishing well-posedness without requiring smallness conditions, which is particularly relevant for applications like rough reaction-diffusion equations.

Method: The authors use weighted Z-spaces to handle rough coefficients and initial data, develop a novel theory of hypercontractive singular integral operators on these weighted spaces, and employ a self-improving property for super-linear reverse Hölder inequalities as key tools in their analysis.

Result: The paper proves existence and uniqueness of maximal weak solutions for nonlinear parabolic PDEs with rough coefficients and initial data in critical homogeneous Besov spaces, without requiring smallness conditions, and demonstrates the theory with applications to rough reaction-diffusion equations.

Conclusion: The developed framework provides a powerful approach for studying nonlinear parabolic PDEs with rough data, with the methodology being extensible to other equation classes like Burgers-type equations and quasi-linear problems in subsequent work.
...
TL;DR: The paper proves interior regularity estimates for regularized optimal transport with subquadratic or entropic regularization, showing uniform convergence to unregularized solutions.
...
Method: The authors derive sharp local bounds on support size for regularized optimal transport with general convex, superlinear regularization terms, then use these to prove interior Lipschitz estimates for transport-like maps and gradient Lipschitz estimates for potentials.

Result: Proved interior Lipschitz estimates for transport maps and gradient Lipschitz estimates for potentials under bi-C¹,α regularity assumptions. For strictly subquadratic and entropic regularization, obtained improved C¹ and C² estimates. Results are uniform in regularization parameter, leading to convergence to unregularized solutions.

Conclusion: The paper establishes strong interior regularity results for regularized optimal transport with uniform estimates in regularization parameter, providing convergence guarantees to unregularized solutions and improving known global bias bounds.
...
TL;DR: A joint framework combining voxel-flow network with phase-field simulations enables long-term microstructure evolution prediction with high accuracy and physical consistency, achieving 1000x speedup over traditional methods.


<details>
  <summary>Details</summary>
Motivation: Current machine learning methods for phase-field microstructure prediction suffer from low flexibility, poor generalization, and short prediction time lengths, limiting their practical application for long-horizon forecasting.

Method: A joint framework coupling voxel-flow network (VFN) with phase-field simulations in alternating manner. VFN predicts pixel flow from past snapshots while preserving periodic boundaries, and periodic PF simulations suppress artifacts and reduce accumulated errors.

Result: VFN is ~1000x faster than PF simulation on GPU. For grain growth and spinodal decomposition, MSE and SSIM remain 6.76% and 0.911 when predicting 18 frames from 2 inputs. Ultra-long prediction (82 frames from 2 inputs) shows NMSE of average grain area at 1.64%.

Conclusion: The joint framework enables rapid, generalized, flexible, and physically consistent microstructure forecasting from image-based data for ultra-long time scales, overcoming limitations of current methods.
...
TL;DR: Cross-validation of rotational temperatures from multiple molecular species in microplasmas reveals N₂(C) is unreliable as a gas temperature probe due to energy transfer interference, while OH(A) provides more straightforward measurements.


<details>
  <summary>Details</summary>
Motivation: Accurate gas temperature determination in microplasmas is critical for applications, but isolated diagnostic approaches can yield misleading results under non-equilibrium conditions, necessitating cross-validation of multiple thermometric probes.

Method: High-resolution rotational spectra of N₂(C), OH(A), NH(A), and NO(A) were recorded from a DC Ar/N₂ microhollow cathode discharge plasma jet. Detailed experimental analysis and robust fitting of rotational spectra were performed to estimate rotational temperatures.

Result: N₂(C) rotational temperature is significantly influenced by energy transfers from argon metastables and spectral interference from NH(A), making it unreliable as a thermometric probe in Ar-rich MHCD. OH(A) rotational population distribution is less sensitive to Ar-induced perturbations across various discharge conditions. The study identifies conditions where measured rotational temperatures can be reliably considered in equilibrium with gas temperature.

Conclusion: Cross-validating multiple thermometric probes and investigating relevant excitation kinetics is essential when measuring rotational temperatures in reactive microplasmas, as different molecular species show varying sensitivities to plasma perturbations.
...
TL;DR: AI-based methods accelerate radiation hydrodynamics: MLP approximates M1-multigroup closure (3000× speedup), PINNs solve equations for temporal extrapolation.


<details>
  <summary>Details</summary>
Motivation: Radiation hydrodynamics simulations are computationally expensive due to numerical evaluation of M1-multigroup closure relation and restrictive CFL condition limiting time steps.

Method: Two AI strategies: 1) Multi-Layer Perceptron approximates M1-multigroup closure relation; 2) Physics-Informed Neural Networks directly solve radiation hydrodynamics equations for temporal extrapolation.
...
Conclusion: AI methods significantly accelerate radiation hydrodynamics simulations, with MLP providing dramatic speedup for closure relation and PINNs showing promise for equation solving and extrapolation.
...
### [37] [Multigroup Radiation Diffusion on a Moving Mesh: Implementation in RICH and Application to Tidal Disruption Events](https://arxiv.org/abs/2601.05120)
*Itamar Giron,Menahem Krief,Nicholas C. Stone,Elad Steinberg*

Main category: astro-ph.HE

TL;DR: Extended RICH radiation-hydrodynamics code with multigroup flux-limited diffusion solver for astrophysical simulations with extreme dynamic range, validated against benchmarks and applied to stellar tidal disruption events.


<details>
  <summary>Details</summary>
Motivation: Radiation-hydrodynamics (RHD) is crucial for understanding high-energy astrophysical phenomena, but existing codes often use simplified grey approximations that don't capture spectral effects. There's a need for more sophisticated multigroup RHD codes that can handle problems with extreme dynamic range and dynamically important radiation forces.

Method: Extended the publicly available RICH code from grey flux-limited diffusion to multigroup FLD solver. RICH is a semi-Lagrangian code that solves RHD equations on an unstructured moving mesh, making it the first multigroup RHD moving mesh code. Includes novel scheme to accelerate convergence in optically thick cells.

Result: Successfully validated multigroup module against multiple analytic benchmarks, including novel test of RHD Doppler term. Applied to stellar tidal disruption event simulation with intermediate-mass black hole, producing self-consistent early-time X-ray flash prior to peak optical/UV light, matching observations of TDE AT 2022dsb.

Conclusion: The multigroup RICH code represents a significant advancement in RHD simulation capabilities, enabling more accurate modeling of astrophysical phenomena with spectral dependence and extreme dynamic range, particularly valuable for studying tidal disruption events and other high-energy astrophysical processes.
...
### [38] [A Virtual Heat Flux Method for Simple and Accurate Neumann Thermal Boundary Imposition in the Material Point Method](https://arxiv.org/abs/2601.04570)
*Jidu Yu,Jidong Zhao*
...
Method: Constructs virtual flux field on auxiliary domain surrounding physical boundary to exactly satisfy prescribed boundary condition, transforming surface integral in weak form into easily computed volumetric integral.

Result: VHFM achieves accuracy comparable to conforming node-based imposition, significantly outperforms conventional particle-based approaches, validated through 1D transient, 2D/3D curved boundaries, and complex moving geometry benchmarks.

Conclusion: VHFM provides simple, computationally efficient, robust solution for accurate thermal boundary conditions in thermo-mechanical and multiphysics MPM frameworks without explicit boundary tracking.
...
### [39] [A high order accurate and provably stable fully discrete continuous Galerkin framework on summation-by-parts form for advection-diffusion equations](https://arxiv.org/abs/2601.05071)
*Mrityunjoy Mandal,Jan Nordström,Arnaud G Malan*

Main category: math-ph

TL;DR: High-order accurate fully discrete SBP-SAT scheme for IBVPs using CG finite elements achieves super-convergence O(p+2) for p≥2 and energy stability.


<details>
  <summary>Details</summary>
Motivation: To develop a fully discrete numerical scheme for Initial Boundary Value Problems that is both high-order accurate and energy-stable in both space and time dimensions.

Method: Continuous Galerkin finite elements with Summation-By-Parts (SBP) form for both spatial and temporal discretization, using Simultaneous Approximation Term (SAT) technique for weak imposition of initial and boundary conditions.

Result: The scheme achieves super-convergence O(p+2) for p≥2 (where p is Lagrange basis degree), provides energy stability, and efficiently captures space-time variations even on coarse meshes.

Conclusion: The SBP-SAT formulation yields an energy-stable, high-order accurate fully discrete method that is computationally effective for solving IBVPs with demonstrated super-convergence properties.
...
### [40] [Neumann series of Bessel functions for the solutions of the Sturm-Liouville equation in impedance form and related boundary value problems](https://arxiv.org/abs/2601.04513)
*Abigail G. Márquez-Hernández,Víctor A. Vicente-Benítez*

Main category: math.CA

TL;DR: Neumann series representation using spherical Bessel functions for Sturm-Liouville impedance equation solutions, with explicit recursive construction and uniform error bounds for spectral parameter in strip |Im ρ| ≤ C.


<details>
  <summary>Details</summary>
Motivation: To develop efficient numerical methods for spectral problems with non-deteriorating accuracy, particularly for computing eigenvalues of Sturm-Liouville equations in impedance form where κ ∈ W^{1,2} and has no zeros.

Method: Represent solutions as Neumann series of spherical Bessel functions with x-dependent coefficients constructed via recursive integration. Derive uniform truncation error bounds for spectral parameter ρ = √λ satisfying |Im ρ| ≤ C.

Result: Explicit construction procedure for series coefficients, uniform error bounds for the representation, and development of numerical method enabling eigenvalue computation with non-deteriorating accuracy.

Conclusion: The Neumann series representation provides an effective framework for solving spectral problems with guaranteed accuracy, particularly suitable for eigenvalue computations where traditional methods may deteriorate.
...
### [41] [Stochastic convergence of a class of greedy-type algorithms for Configuration Optimization Problems](https://arxiv.org/abs/2601.05029)
*Evie Nielen,Oliver Tse*

Main category: math.OC

TL;DR: Stochastic framework for greedy sampling methods as Markov processes enables convergence analysis under mild assumptions, with explicit rates and a new randomized algorithm.


<details>
  <summary>Details</summary>
Motivation: Greedy Sampling Methods (GSMs) are widely used for Configuration Optimization Problems but deterministic convergence analyses are restrictive and difficult to verify. A stochastic framework is needed for more practical convergence analysis.

Method: Formulate greedy-type methods as continuous-time Markov processes on configuration space. Analyze convergence in expectation and probability under mild structural assumptions. Derive explicit convergence rates (logarithmic, polynomial, exponential) based on abstract improvement condition. Introduce Randomized Polytope Division Method (R-PDM) as a randomized variant of classical Polytope Division Method.

Result: Proved exponential convergence of L^1-interpolation error for C^2-functions in one-dimensional piecewise linear interpolation. Demonstrated effectiveness and variance reduction of R-PDM in numerical experiments.

Conclusion: Stochastic framework provides rigorous convergence analysis for greedy sampling methods under practical assumptions. Randomized variants like R-PDM offer improved performance with variance reduction.
...
### [42] [Convergence Rates for Learning Pseudo-Differential Operators](https://arxiv.org/abs/2601.04473)
*Jiaheng Chen,Daniel Sanz-Alonso*

Main category: math.ST

TL;DR: This paper develops convergence rates for learning elliptic pseudo-differential operators using wavelet-Galerkin methods, proposing a sparse estimator with novel matrix compression and showing the learned operator enables efficient, stable Galerkin solvers.


<details>
  <summary>Details</summary>
Motivation: Elliptic pseudo-differential operators are fundamental in PDEs and mathematical physics, but learning them efficiently with statistical guarantees remains challenging. The paper aims to bridge operator learning, data-driven solvers, and wavelet methods in scientific computing.

Method: Uses a wavelet-Galerkin framework to formulate learning as structured infinite-dimensional regression with multiscale sparsity. Proposes a sparse estimator with novel matrix compression tailored to learning and nested-support strategy to balance approximation and estimation errors.

Result: Establishes convergence rates for the proposed estimator and shows that the learned operator induces an efficient and stable Galerkin solver whose numerical error matches its statistical accuracy.

Conclusion: The work successfully integrates operator learning, data-driven solvers, and wavelet methods, providing theoretical guarantees for learning elliptic pseudo-differential operators with practical computational efficiency and stability.
...
### [43] [Elimination Without Eliminating: Computing Complements of Real Hypersurfaces Using Pseudo-Witness Sets](https://arxiv.org/abs/2601.04383)
...
TL;DR: New method computes regions of real complement of hypersurfaces without requiring explicit equations, using pseudo-witness sets and line intersections instead of elimination.


<details>
  <summary>Details</summary>
Motivation: Many hypersurfaces in algebraic geometry arise from projections, and computing their real complement regions traditionally requires explicit equations. However, obtaining these equations through elimination can be computationally demanding or infeasible.

Method: Uses univariate interpolation by computing hypersurface-line intersections via pseudo-witness sets, avoiding explicit equation computation. This performs "elimination without actually eliminating" and is implemented in a forthcoming Julia package.

Result: The algorithm accurately recovers all regions of the real complement of hypersurfaces across several examples, demonstrating effectiveness without requiring explicit equations.

Conclusion: Proposes a practical alternative to elimination-based methods for computing real complement regions of hypersurfaces, enabling computations that were previously infeasible due to elimination complexity.
...
### [44] [Quantum Elastic Network Models and their Application to Graphene](https://arxiv.org/abs/2601.05161)
*Ioannis Kolotouros,Adithya Sireesh,Stuart Ferguson,Sean Thrasher,Petros Wallden,Julien Michel*

Main category: quant-ph

TL;DR: Quantum algorithm enables efficient simulation of macroscopic-scale elastic materials using quantum computers, demonstrating centimeter-scale graphene simulation with only ~160 logical qubits.


<details>
  <summary>Details</summary>
Motivation: Classical molecular dynamics simulations are infeasible for simulating materials with atomic-level resolution on macroscopic scales, even with simplified elastic network models, due to massive memory and computational requirements.

Method: Introduces Quantum Elastic Network Models (QENMs) using Babbush et al.'s quantum algorithm that offers exponential advantage for simulating coupled oscillators under specific conditions, applied to planar materials like graphene.

Result: Demonstrates efficient simulation of 2D graphene sheet with analysis of complexity for initial-state preparation, Hamiltonian simulation, and measurement. Estimates that centimeter-scale atomistic simulation requiring hundreds of petabytes classically could be encoded with ~160 logical qubits.

Conclusion: Quantum Elastic Network Models provide a promising quantum computational approach to overcome classical limitations in materials simulation, enabling macroscopic-scale atomistic modeling of materials like graphene with practical quantum resources.
...
### [45] [Differential syntactic and semantic encoding in LLMs](https://arxiv.org/abs/2601.04765)
*Santiago Acevedo,Alessandro Laio,Marco Baroni*

Main category: cs.CL

TL;DR: LLM representations encode syntax and semantics linearly; averaging sentence vectors creates syntactic/semantic centroids that capture significant linguistic information.


<details>
  <summary>Details</summary>
Motivation: To understand how syntactic and semantic information is encoded in the inner layer representations of large language models, specifically DeepSeek-V3.
...
Result: Syntactic and semantic centroids capture significant proportions of linguistic information; subtracting them strongly affects similarity with matched sentences, suggesting linear encoding. Cross-layer encoding profiles differ between syntax and semantics, and the two signals can be partially decoupled.

Conclusion: Syntax and semantics are at least partially linearly encoded in LLM representations, with differential encoding patterns across layers, allowing for some decoupling of these linguistic information types.
...
### [46] [PINN-Based Solution for a Diffusion Controlled Droplet Growth](https://arxiv.org/abs/2601.05042)
*Pavel Gol'din,Gennady Y. Gor*

Main category: physics.flu-dyn

TL;DR: PINN approach for modeling diffusion-controlled spherical droplet growth with moving boundary accurately captures self-similar growth law and concentration profiles.


<details>
  <summary>Details</summary>
Motivation: To develop a flexible and computationally efficient framework for solving moving-boundary diffusion problems, specifically for diffusion-controlled droplet growth where traditional methods can be complex.
...
Result: PINN accurately reproduces self-similar growth law and concentration profiles for wide range of initial droplet radii, demonstrating convergence toward asymptotic diffusive regime.
...
### [47] [Towards Spatio-Temporal Extrapolation of Phase-Field Simulations with Convolution-Only Neural Networks](https://arxiv.org/abs/2601.04510)
*Christophe Bonneville,Nathan Bieberdorf,Pieterjan Robbe,Mark Asta,Habib Najm,Laurent Capolungo,Cosmin Safta*

Main category: cs.CE

TL;DR: A U-Net surrogate model with diffusion-based initialization enables fast, accurate phase-field simulations of liquid metal dealloying, achieving 36,000× speedup while maintaining physics fidelity during extreme spatiotemporal extrapolation.
...
Method: Developed a fully convolutional, conditionally parameterized U-Net surrogate with convolutional self-attention and physically informed padding. Integrated a flood-fill corrector method for accuracy under extrapolation. Coupled with a conditional diffusion model to generate synthetic initial conditions, eliminating need for solver-based initialization. Trained on small-domain, short-time simulations but leverages U-Net's convolutional nature for large-scale extrapolation.

Result: Achieved relative errors below 5% in training regime and under 15% during large-scale extrapolation. Predicted key quantities and spatial statistics accurately across multiple alloy compositions. Delivered speed-ups up to 36,000×, reducing weeks-long simulations to seconds.

Conclusion: The framework successfully enables high-fidelity extrapolation in both space and time for LMD phase-field simulations, serving as a first step toward practical, efficient simulation tools for complex microstructural evolution studies.
...
### [48] [sidmkit: A Reproducible Toolkit for SIDM Phenomenology and Galaxy Rotation-Curve Modeling](https://arxiv.org/abs/2601.04735)
*Nalin Dhiman*

Main category: astro-ph.IM

TL;DR: sidmkit is a Python package that bridges SIDM microphysics with astrophysical constraints and provides rotation curve fitting tools for SPARC data, showing Burkert halos are often preferred over NFW.


<details>
  <summary>Details</summary>
Motivation: Self-interacting dark matter (SIDM) requires connecting microphysical scattering models to astrophysical observations, but existing tools lack integration between these layers. There's a need for reproducible, transparent tools to perform SIDM calculations and fit rotation curves consistently.

Method: Developed sidmkit package with: 1) SIDM microphysics implementation (velocity-dependent cross sections for Yukawa interactions using Born, classical, and Hulthén approximations with partial-wave checks), 2) velocity-moment averaging for Maxwellian speeds, 3) rotation curve fitting tools for SPARC data using bounded non-linear least squares with NFW/Burkert halo models, mass-to-light priors, and information criteria (AIC/BIC).

Result: Processed 191 rotmod galaxies (382 total fits). Burkert halo model is preferred (ΔBIC > 0) for 65.4% of galaxies, with strong preference (ΔBIC > 6) in 32.5% of galaxies.

Conclusion: sidmkit provides a unified framework for SIDM micro-to-macro calculations and rotation curve analysis, enabling reproducible research. The preference for Burkert over NFW profiles in many galaxies suggests SIDM effects or alternative halo shapes may be important.
...
TL;DR: Low-dose simulation algorithm for X-ray dark-field radiography creates stripe artifacts not present in actual low-dose images, with up to 10% signal difference between simulated and measured images.


<details>
  <summary>Details</summary>
Motivation: To validate a low-dose simulation algorithm for dark-field radiography and determine if stripe artifacts in simulated images are algorithm limitations rather than actual physical effects that would appear in real low-dose measurements.

Method: Acquired dark-field radiographs of an anthropomorphic chest phantom at different tube currents (different radiation doses). Simulated low-dose radiographs from high-dose measurements by virtually reducing irradiated area. Compared simulated vs. measured images using quantitative analysis of dark-field signal across different regions of interest.

Result: Actual low-dose radiographs at quarter standard dose were artifact-free. Simulated images showed up to 10% difference in dark-field signal compared to measured images. Algorithm-induced stripe artifacts degraded simulated image quality, especially at higher dose reductions.

Conclusion: Virtual irradiation area reduction is a simple method for generating low-dose radiographs in scanning-based dark-field radiography, but algorithm-induced stripe artifacts (particularly at high dose reductions) reduce image quality compared to actual measured low-dose images.
...
### [50] [Self-Organized Criticality from Protected Mean-Field Dynamics: Loop Stability and Internal Renormalization in Reflective Neural Systems](https://arxiv.org/abs/2601.04450)
*Byung Gyu Chae*

Main category: nlin.AO

TL;DR: Homeostatic dynamics enables self-organized criticality in neural systems through protected mean-field critical surfaces and intrinsic parameter flows.


<details>
  <summary>Details</summary>
Motivation: To understand how neural systems achieve and maintain criticality without external fine-tuning, and to unify loop renormalization with adaptive response mechanisms.

Method: Using MSRJD field-theoretic framework on reduced stochastic descriptions to analyze fluctuation effects, homeostatic curvature regularization, and response-driven structural adaptation.

Result: Fluctuations don't destabilize critical manifolds; homeostatic curvature dynamically regularizes loop corrections, creating protected mean-field critical surfaces. Response-driven adaptation generates intrinsic parameter flows that attract systems toward critical surfaces.

Conclusion: Reflective homeostatic dynamics provides a unified framework for autonomous criticality in neural systems, combining loop renormalization and adaptive response without external tuning.
