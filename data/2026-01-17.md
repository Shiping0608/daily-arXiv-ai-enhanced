...
TL;DR: The paper develops coupled epidemic-ecological models (LVSIS) combining SIR and Lotka-Volterra systems, and applies data-driven methods (Neural ODEs, KANODEs, SINDy) to learn dynamics from synthetic data, extending to spatio-temporal analysis.


<details>
  <summary>Details</summary>
Motivation: To understand the coupled dynamics between epidemic spread and ecological predator-prey interactions, which are typically studied separately, and to develop data-driven approaches for learning these complex coupled systems from observational data.

Method: 1. Developed LVSIS model combining SIR epidemic and Lotka-Volterra ecological models into four differential equations; 2. Applied Neural ODEs, KANODEs, and SINDy to learn dynamics from synthetic data; 3. Extended to spatio-temporal models to uncover hidden local couplings.

Result: Numerical experiments with synthetic data demonstrate the learning capabilities of data-driven methods in capturing coupled epidemic-ecological behavior, with successful extension to spatio-temporal analysis for identifying local interactions.

Conclusion: Data-driven approaches can effectively learn coupled epidemic-ecological dynamics, providing a framework for analyzing complex biological systems where disease spread and ecological interactions are interdependent, with potential applications in epidemiology and ecology.
...
TL;DR: A method for computing exit time probabilities for α-stable Lévy processes using PIDE framework with Feynman-Kac formula, achieving first-order convergence and computational efficiency over Monte Carlo methods.
...
Method: Approximates α-stable process by combining Brownian motion with compound Poisson process. Models exit time probability using partial integro-differential equations (PIDEs) framework. Uses Feynman-Kac formula for probabilistic representation, computed via tailored quadrature rules and interpolation techniques.

Result: Method achieves first-order convergence in time with significant computational advantages over standard Monte Carlo and deterministic approaches. Avoids assembling/solving large dense linear systems, improving efficiency. Validated through two numerical examples demonstrating accuracy and applicability to physical transport problems.

Conclusion: Proposed method provides efficient and accurate computation of exit time probabilities for α-stable Lévy processes, offering computational advantages for understanding anomalous diffusion in physical transport problems.
...
TL;DR: The paper introduces specular differentiation in 1D Euclidean space with fundamental analysis and applies it to develop numerical schemes for solving first-order ODE initial value problems.


<details>
  <summary>Details</summary>
Motivation: To develop a new differentiation concept (specular differentiation) in one-dimensional space and apply it to create novel numerical methods for solving ordinary differential equations.

Method: Proposes specular differentiation in 1D Euclidean space, provides fundamental analysis including quasi-Fermat's theorem and quasi-Mean Value Theorem, then develops multiple numerical schemes for first-order ODE initial value problems.

Result: Developed several numerical schemes, selected one based on numerical simulations, and proved it has first-order consistency and second-order local convergence.

Conclusion: Specular differentiation provides a useful framework for developing numerical methods for ODEs, with one scheme demonstrating promising theoretical properties of first-order consistency and second-order local convergence.
...
TL;DR: A constant-coefficient MSAV scheme for fast, energy-stable simulation of vesicle deformation using phase-field approach with O(N² log N) complexity and 6-15x speedup.


<details>
  <summary>Details</summary>
Motivation: Existing methods for simulating vesicle deformation under osmotic pressure (nonlinear multigrid and MSAV) require iterative solution of variable-coefficient systems at each time step, leading to high computational cost.

Method: Constant-coefficient MSAV (CC-MSAV) scheme that incorporates stabilization directly into the Cahn-Hilliard evolution equation rather than chemical potential, yielding fully decoupled constant-coefficient elliptic problems solvable via fast discrete cosine transform (DCT).

Result: Method achieves O(N² log N) complexity per time step while preserving unconditional energy stability and discrete mass conservation. Numerical experiments show second-order accuracy, mass conservation to relative errors below 5×10⁻¹¹, and 6-15x overall speedup on grids with N≥2048 compared to classical MSAV.

Conclusion: CC-MSAV provides significant efficiency gains without sacrificing accuracy, making it particularly suitable for large-scale simulations of vesicle dynamics.
...
TL;DR: Proposes new second-order symmetric algorithms for decoupled FBSDEs using ADI-inspired splitting methods, achieving second-order convergence with reduced computational cost.
...
Method: Inspired by alternating direction implicit (ADI) splitting methods for PDEs, the generator is split into two functions. For the value process Y, explicit and implicit schemes are alternately applied to these two generators, while using existing algorithms from Zhao & Li (2014) for the control process Z.

Result: The proposed schemes achieve second-order convergence rate, with rigorous theoretical proof. For equations with linear+nonlinear generator structure, the methods reduce the number of iterations needed for solving implicit schemes, decreasing computational cost while maintaining second-order accuracy.

Conclusion: The splitting methods offer clear advantages for FBSDEs with mixed linear-nonlinear generators, providing second-order convergence with reduced computational cost compared to existing methods, as verified by numerical examples including the backward stochastic Riccati equation from mean-variance hedging.
...
TL;DR: This paper provides a unified introduction to optimization methods in ML and SciML, highlighting how different problem structures in these domains shape algorithmic choices and effectiveness.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the fundamental differences in optimization problem structures between classical ML and SciML, where classical ML uses stochastic, sample-separable objectives while SciML involves physics-informed formulations with global coupling and stiffness, requiring different optimization approaches.

Method: The paper reviews first- and second-order optimization techniques in both deterministic and stochastic settings, discusses their adaptation to physics-constrained and data-driven SciML models, and illustrates practical strategies through tutorial examples.

Result: The analysis shows that optimization behavior in SciML is governed by spectral properties of underlying physical models rather than data statistics, limiting effectiveness of standard stochastic methods and motivating deterministic or curvature-aware approaches.

Conclusion: The paper concludes by highlighting open research directions at the interface of scientific computing and scientific machine learning, emphasizing the need for optimization methods tailored to the specific structural properties of SciML problems.
...
<details>
  <summary>Details</summary>
Motivation: Randomized Arnoldi is faster than standard Arnoldi for large-scale computing but produces different Hessenberg matrices that can cause convergence issues like delays or spike-like irregularities.

Method: Introduces a modification to randomized Arnoldi that enforces orthogonality between the last Arnoldi vector and previously generated subspace by solving one additional least-squares problem, restoring similarity with standard Arnoldi's Hessenberg matrix.

Result: The modified process produces approximations identical to standard Arnoldi for eigenvalue problems and matrix function evaluations, while being as fast as randomized Arnoldi and as robust as standard Arnoldi.

Conclusion: The proposed modification combines the speed advantages of randomized Arnoldi with the robustness and accuracy of standard Arnoldi, offering an improved method for large-scale scientific computing.
...
TL;DR: The Conjugate Gradient method's convergence is limited by graph locality - information propagates only one node per iteration, so graph diameter sets a minimum iteration count for accurate solutions.


<details>
  <summary>Details</summary>
Motivation: To understand and quantify the fundamental locality limitation of the Conjugate Gradient method, which arises from how information propagates through the graph structure induced by the system matrix's sparsity pattern.

Method: Analyzes the Conjugate Gradient method through the lens of graph theory, modeling the system matrix's sparsity pattern as a graph where nodes represent variables and edges represent non-zero matrix entries. Examines how information from the right-hand side propagates through this graph structure during iterations.

Result: Identifies that information can only propagate across directly connected graph nodes in each iteration, establishing that the graph diameter determines a minimum number of iterations required to achieve acceptable accuracy.

Conclusion: The Conjugate Gradient method has an inherent locality limitation rooted in graph connectivity, where the diameter of the induced graph imposes a fundamental lower bound on iteration count regardless of algorithmic optimizations.
...
<details>
  <summary>Details</summary>
Motivation: To investigate regularization properties of recent algorithms (aggregation method and RatCG method) for solving linear ill-posed problems in Hilbert spaces, and understand their connection to rational Krylov space methods.
...
<details>
  <summary>Details</summary>
Motivation: The Shifted Boundary Method (SBM) reduces body-fitted meshing complexity but creates algebraically challenging linear systems that are non-symmetric with non-local boundary coupling, making them resistant to standard AMG and simple smoothers for high-order discretizations.

Method: A geometric multigrid preconditioner with a novel Full-Residual Shy Patch smoother - a subspace correction strategy that filters out some patches while capturing full physics of the shifted boundary, overcoming limitations of previous cell-wise approaches.
...
Conclusion: The proposed approach proves that SBM can achieve both geometric flexibility (reducing meshing burden) and algebraic efficiency through effective preconditioning, making high-order SBM discretizations practically viable.
...
TL;DR: DG-ROD method restores optimal convergence for DG on polygonal approximations of curved domains via polynomial boundary data reconstruction.


<details>
  <summary>Details</summary>
Motivation: Discontinuous Galerkin methods lose convergence order when approximating curved boundaries with polygonal meshes due to geometric approximation errors. Existing higher-order strategies for finite elements and finite volumes needed extension and theoretical analysis for DG methods.

Method: DG-ROD method extends polynomial reconstruction of boundary data approach to discontinuous Galerkin methods. Provides rigorous theoretical analysis for 2D linear advection-diffusion-reaction problems with homogeneous Dirichlet BCs on convex/non-convex domains.

Result: Proves existence/uniqueness of discrete solution and derives error estimates. Shows DG-ROD achieves optimal convergence despite polygonal approximations under suitable regularity assumptions.

Conclusion: DG-ROD method successfully restores optimal convergence rates for DG on curved domains approximated by polygons. Theoretical results validated with numerical benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Studying optoelectronic materials requires computing thousands of eigenpairs of pseudo-hermitian Hamiltonians. Iterative eigensolvers are preferred for scalability on exascale systems, but existing methods like ChASE only handle hermitian matrices.

Method: Extended ChASE to pseudo-hermitian Hamiltonians with oblique Rayleigh-Ritz projection for quadratic convergence without dual basis construction. Introduced parallel implementation of recursive matrix-product operations in Chebyshev filter with minimal global communications.

Result: The pseudo-hermitian solver achieves similar convergence and performance as the original hermitian ChASE. Full numerical analysis and experimental tests support the development.

Conclusion: Successfully extended ChASE to handle pseudo-hermitian Hamiltonians for excitonic materials research, maintaining performance while adding new algorithmic improvements for efficient large-scale computations.
...
TL;DR: First numerically stable methods for derivative evaluation in barycentric rational approximation, plus extension of Thiele continued fraction derivative algorithms to higher orders.


<details>
  <summary>Details</summary>
Motivation: While fast approximation algorithms exist for barycentric and Thiele continued fraction representations, there's a need for numerically stable derivative evaluation methods, especially for higher-order derivatives.

Method: Developed numerically stable algorithms for derivative evaluation in barycentric representation (including O(n) algorithm for all derivatives), and extended existing O(n) algorithm for Thiele continued fraction first derivative to higher orders.

Result: Proposed methods are both robust and efficient, as confirmed by numerical experiments.

Conclusion: The paper provides the first numerically stable derivative evaluation methods for barycentric rational approximation and extends efficient derivative computation to higher orders for Thiele continued fractions.
...
<details>
  <summary>Details</summary>
Motivation: To establish sharp dispersive estimates for the Schrödinger equation on compact domains (square torus) at the critical scaling exponent, which is important for understanding nonlinear Schrödinger equations and their well-posedness theory.

Method: Frequency localization of initial data combined with analysis on small time windows whose length depends on the frequency parameter λ. The approach likely uses harmonic analysis techniques adapted to the torus geometry.

Result: Proves lossless Strichartz estimates at the critical exponent q_c = 2(n+1)/(n-1) for the Schrödinger equation on the square torus with frequency-localized data.

Conclusion: Achieves optimal dispersive estimates on compact domains by combining frequency localization with appropriately scaled time windows, providing sharp tools for studying nonlinear Schrödinger equations on tori.
...
<details>
  <summary>Details</summary>
Motivation: To generalize previous results by Bonorino and Klaser on non-existence of bounded viscosity solutions to semi-linear elliptic equations from hyperbolic spaces to more general Cartan-Hadamard manifolds, addressing the challenge of lacking totally geodesic foliations.

Method: Uses a novel comparison technique based on convex hypersurfaces inspired by Choi, Gálvez, and Lozano to overcome the absence of totally geodesic foliations that were available in hyperbolic spaces.

Result: Establishes non-existence results for bounded (viscosity) solutions to equations of the form Δu + f(u) = 0 in domains with prescribed asymptotic boundary on Cartan-Hadamard manifolds.

Conclusion: Demonstrates the interplay between curvature, Laplacian spectrum, and asymptotic boundary geometry in determining existence/non-existence of solutions, extending hyperbolic space results to broader geometric settings.
...
<details>
  <summary>Details</summary>
Motivation: To establish a simple criterion for determining linear stability/instability of small Bernstein-Green-Kruskal (BGK) waves in plasma physics.

Method: Mathematical analysis of BGK wave solutions to Vlasov-Poisson system, examining relationship between stability properties and derivative of energy distribution function at zero energy.

Result: Proves that linear stability or instability of small BGK waves is determined by sign of derivative of their energy distributions at zero energy.

Conclusion: Provides a clear mathematical criterion for stability analysis of BGK waves, connecting stability properties to fundamental characteristics of energy distribution functions.
...
<details>
  <summary>Details</summary>
Motivation: To develop a comprehensive theory for transport equations in Triebel-Lizorkin spaces that doesn't require the conventional divergence-free condition, enabling broader applications to various evolution equations in fluid dynamics and related fields.
...
Result: Establishes new a priori estimates and local well-posedness for transport equations in Triebel-Lizorkin spaces, with applications yielding complete local well-posedness for ideal MHD system covering both sub-critical and critical regularity regimes, plus blow-up criteria.

Conclusion: The developed theory provides a powerful framework for transport equations in Triebel-Lizorkin spaces, refining and substantially extending previous work, with demonstrated applicability to important fluid dynamics models like ideal MHD.
...
TL;DR: Comparison of two similar 2D diffusion equations in meteorology (quasi-geostrophic vs convection-diffusion) focusing on how their nonlinear effects differ in direction and impact on large-time asymptotic behavior.


<details>
  <summary>Details</summary>
Motivation: To understand the fundamental differences between two meteorologically important diffusion equations that appear similar in scale and order but differ in how nonlinear effects act directionally.

Method: Comparative analysis of the quasi-geostrophic equation and convection-diffusion equation, examining their large-time asymptotic behavior and nonlinear distortions in solution profiles.

Result: Quantifies the directional difference in nonlinear effects: one acts along isothermal surfaces while the other acts along temperature gradients. Shows how spatial symmetry of first approximation is crucial despite solutions requiring no symmetry.

Conclusion: While the two equations share mathematical similarities in scale and order, their fundamental difference lies in the directional nature of nonlinear effects, which significantly impacts their large-time asymptotic behavior and solution profiles.
...
TL;DR: The paper studies the Robin Laplacian in hyperbolic plane exteriors, showing essential spectrum is [1/4,∞), discrete eigenvalues exist below 1/4 under convexity when Robin parameter is below a critical constant, and proves the geodesic disk exterior maximizes the lowest Robin eigenvalue under area/perimeter constraints.


<details>
  <summary>Details</summary>
Motivation: To understand spectral properties of the Robin Laplacian in hyperbolic geometry, specifically in exterior domains, and establish extremal properties of geodesic disks for the lowest Robin eigenvalue under geometric constraints.

Method: Uses spectral analysis of the Robin Laplacian in hyperbolic plane, employs convexity assumptions on domains, compares eigenvalues between general convex domains and geodesic disks through curvature comparisons, and applies geometric constraints (area/perimeter).

Result: Essential spectrum is [1/4,∞); discrete eigenvalues below 1/4 exist under convexity when Robin parameter is below domain-dependent critical constant; geodesic disk exterior maximizes lowest Robin eigenvalue under fixed area/perimeter constraints among convex domains.

Conclusion: The geodesic disk serves as the extremal domain for maximizing the lowest Robin eigenvalue in hyperbolic plane exteriors under area or perimeter constraints, with corresponding inequalities for critical constants, establishing fundamental spectral-geometric relationships.
...
TL;DR: The paper analyzes scattering resonances in high-contrast elastic inclusions, identifying wavelength-scale and subwavelength resonance clusters with precise asymptotic scaling and lifetime dichotomies.


<details>
  <summary>Details</summary>
Motivation: To understand elastic wave scattering in high-contrast materials, particularly the behavior of resonances near Neumann eigenvalues and zero frequency, which is important for applications in metamaterials and wave manipulation.

Method: Mathematical analysis of the Lamé transmission problem using asymptotic methods as τ→0, studying the meromorphic continuation of the resolvent operator, and deriving explicit expansions with finite-rank leading terms and quantitative remainder bounds.

Result: Sharp asymptotic description of resonances: clusters near Neumann eigenvalues with O(τ) imaginary parts; subwavelength resonances with O(√τ) real parts and lifetime dichotomy (O(τ) vs O(τ²) imaginary parts). Classification of long-lived resonances and rigorous criteria for monopole/dipole behavior in microresonators.

Conclusion: The paper provides a complete classification of elastic resonances in high-contrast limit, establishing precise asymptotic formulas and lifetime properties, with applications to understanding wave phenomena in elastic metamaterials and microresonators.
...
TL;DR: The paper studies global minimizers of biharmonic Alt-Caffarelli functionals, showing half-space solutions are global minimizers for two-sided but not one-sided problems, identifying new classes with constant Laplacian, and proving two-sided minimizers don't satisfy PDEs.


<details>
  <summary>Details</summary>
Motivation: To understand the global minimization properties of biharmonic analogues of the Alt-Caffarelli functional, particularly comparing one-sided vs two-sided problems and characterizing the structure of minimizers across dimensions.

Method: Mathematical analysis of biharmonic Alt-Caffarelli functionals, building on recent work by Lamboley and Nahon that reduced potential minimizers in 2D to four categories. The authors extend this analysis to arbitrary dimensions.

Result: Half-space solutions are global minimizers for two-sided but not one-sided problems. Identified new classes of global minimizers with constant Laplacian. Three of four categories from 2D analysis persist in any dimension as global minimizers. Two-sided minimizers don't satisfy PDEs, unlike one-sided problems.

Conclusion: Biharmonic Alt-Caffarelli functionals exhibit fundamentally different behavior between one-sided and two-sided problems, with two-sided minimizers lacking PDE structure despite being global minimizers, and certain categories of minimizers generalize across dimensions.
...
TL;DR: The paper demonstrates how quasiconvexity helps determine optimal regularity and continuity conditions for time derivatives in nonlocal time-dependent obstacle problems.


<details>
  <summary>Details</summary>
Motivation: To understand and establish optimal regularity properties for temporal derivatives in nonlocal time-dependent obstacle problems, which are important in various applications involving free boundary problems and evolution equations with constraints.

Method: Utilizes the quasiconvexity property as a key analytical tool to study the regularity of temporal derivatives in nonlocal time-dependent obstacle problems, establishing conditions under which continuity can be guaranteed.

Result: Shows that quasiconvexity is effective in determining optimal regularity of temporal derivatives and provides specific conditions that ensure their continuity in these nonlocal time-dependent obstacle problems.

Conclusion: Quasiconvexity serves as a powerful property for analyzing regularity in nonlocal time-dependent obstacle problems, enabling the establishment of optimal regularity results and continuity conditions for temporal derivatives.
...
TL;DR: The paper introduces a Riemannian autocorrelation function for sets on manifolds, links it to BV functions and perimeter, and uses it to study Γ-convergence of non-local isoperimetric energies on spheres.


<details>
  <summary>Details</summary>
Motivation: To analyze non-local isoperimetric energies on spheres by developing a Riemannian analogue of Matheron's set covariogram from convex geometry, enabling the study of variational limits as the interaction range shrinks.

Method: Introduce a Riemannian autocorrelation function c_Ω for measurable sets on compact Riemannian manifolds. Characterize BV functions using geodesic difference quotients, linking Lipschitz continuity of c_Ω to finite perimeter. Reformulate non-local energies in terms of c_Ω to study Γ-convergence as ε→0.

Result: Ω has finite perimeter if and only if c_Ω is Lipschitz, with Lipschitz constant related to perimeter. The non-local energy E_{γ,ε} on spheres admits reformulation via c_Ω, enabling computation of Γ-limit as ε→0.

Conclusion: The Riemannian autocorrelation function provides a powerful tool connecting geometric measure theory on manifolds with variational analysis of non-local energies, establishing fundamental connections between Lipschitz properties, perimeter, and Γ-convergence limits.
...
TL;DR: Proof of soliton resolution conjecture for Benjamin-Ono equation: every regular decaying solution decomposes into finite sum of solitons plus radiative remainder in long-time asymptotics.


<details>
  <summary>Details</summary>
Motivation: To prove the soliton resolution conjecture for the Benjamin-Ono equation, which describes how solutions behave asymptotically as time goes to infinity. This is a fundamental question in nonlinear dispersive equations about the long-term behavior of solutions.
...
Result: Successfully proves soliton resolution conjecture: every sufficiently regular and decaying solution decomposes into finite sum of soliton solutions with different velocities plus radiative remainder term in long-time asymptotics.

Conclusion: The paper provides complete proof of soliton resolution for Benjamin-Ono equation, establishing rigorous connection between spectral data and asymptotic behavior of solutions through novel use of representation formula and Fourier analysis.
...
TL;DR: The paper proves Michael-Simon inequalities for anisotropic surface energies in 3D, showing they hold for convex integrands close to isotropic area and for ℓ^p norms, with connections to varifold compactness.


<details>
  <summary>Details</summary>
Motivation: Anisotropic surface energies generalize classical area by weighting integration over surfaces with integrands depending on tangent planes. While monotonicity formulas are unavailable for critical points, establishing Michael-Simon inequalities (Sobolev-type inequalities for surfaces) is important for understanding geometric properties and regularity of anisotropic minimal surfaces.

Method: The approach builds on unpublished work by Almgren but simplifies his proof significantly. A key innovation is a new functional inequality for vector fields on the plane, which serves as a quantitative version of Alberti's rank-one theorem. The method handles two cases: (1) convex integrands close to 1 in C¹ norm, and (2) integrands including ℓ^p norms for p∈(1,∞).

Result: For k=2 and n=3, the Michael-Simon inequality holds when F is convex and close to 1 in C¹ norm (so ℱ is close to usual area). The result also extends to another class of integrands including ℓ^p norms. Additionally, for general F satisfying the atomic condition, the validity of Michael-Simon inequality is shown to be equivalent to compactness of rectifiable varifolds.

Conclusion: The paper establishes Michael-Simon inequalities for anisotropic surface energies in 3D, providing important tools for studying anisotropic minimal surfaces. The simplified proof and new functional inequality represent significant technical advances, while the equivalence with varifold compactness reveals deeper connections between functional inequalities and geometric measure theory.
...
TL;DR: Tutorial on computing DC resistance, inductance, and capacitance of parallel wires using FEM with open boundary modeling, insulation effects, and defect analysis.


<details>
  <summary>Details</summary>
Motivation: To provide a practical tutorial for computing key electrical parameters (R, L, C) of parallel wire systems using finite element methods, including consideration of realistic factors like insulation and defects that analytical models often overlook.

Method: Uses 3D finite element method with infinite domain modeling for electrostatic and magnetostatic fields, electrokinetic formulation for current flow, and considers insulation effects and physical defects. Implemented with open-source ONELAB software with code provided.

Result: The paper presents simulation results validated against analytical models (when available) and commercial Altair Flux software, demonstrating the accuracy and practical application of the FEM approach for parallel wire analysis.

Conclusion: The tutorial successfully demonstrates how to compute R, L, C parameters for parallel wires using FEM with open boundary conditions, providing a validated approach that accounts for practical considerations like insulation and defects, with open-source implementation available.
...
TL;DR: PFM-LS method combines particle-based storage of level-set data near interfaces with grid-based representation elsewhere, using bidirectional flow maps and differential form interpretation for superior interface tracking with exceptional geometric fidelity and sub-grid feature preservation.


<details>
  <summary>Details</summary>
Motivation: Traditional level-set methods struggle with preserving geometric fidelity during complex deformations and capturing sub-grid features. There's a need for improved interface tracking that maintains shape accuracy and volume preservation while handling fine-scale details.

Method: Hybrid particle-grid approach storing level-set values, gradients, and Hessians on particles in a narrow band around the interface, using bidirectional flow maps for advection. Employs dual-timescale approach with long-range maps for values/gradients and frequent short-range map reinitialization for Hessians. Includes adaptive particle control and hybrid particle-grid quasi-Newton redistancing scheme.

Result: PFM-LS achieves state-of-the-art volume preservation and shape fidelity in 2D and 3D benchmarks, outperforming a broad range of existing level-set methods while preserving sub-grid features that traditional methods cannot capture.

Conclusion: The PFM-LS method represents a significant advancement in interface tracking, combining particle and grid representations with differential form interpretation to achieve exceptional geometric fidelity and feature preservation during complex deformations.
...
Method: Developed a novel interfacial treatment in volume penalization method (immersed boundary method) for general conjugate scalar transport with both flux and scalar jumps. First solved Neumann boundary condition case, then extended to general jumps with additional source term representing local jump conditions.

Result: Verified with 1D diffusion problem showing improved accuracy. Applied to fluid-solid coupled diffusion and advection-diffusion problems with scalar and flux jumps. Results show <3% average relative deviation compared to body-fitted mesh simulations.

Conclusion: The developed scheme successfully handles conjugate scalar transport with general interfacial boundary conditions on complex geometries using immersed boundary methods, achieving good accuracy comparable to body-fitted approaches.
...
<details>
  <summary>Details</summary>
Motivation: To support JT-60SA research plan by providing critical turbulence and flow measurements through Doppler reflectometry, which is relevant for multiple research fields in fusion plasma physics.

Method: 1) Identify scientific scope and program for DR in JT-60SA context; 2) Use ray tracing code for feasibility study with design hypotheses; 3) Identify geometric solution for probing core and edge; 4) Discuss conceptual design options (minimum viable vs baseline systems).

Result: Found viable geometric solution for DR installation that can probe both core and edge in required wave number range. Conceptual design could use small fraction of horizontal port, leaving space for other diagnostics.

Conclusion: DR system installation in JT-60SA is both viable and practical, with potential to significantly contribute to research objectives while using minimal port space.
...
TL;DR: 1X-2V Boltzmann-Poisson simulations reveal unique sheath formation near biased electrodes in Z-pinch fusion with parallel magnetic fields, showing non-monotonic potential profiles, magnetized electron-driven sheaths, and significant parallel currents despite minimal perpendicular current.


<details>
  <summary>Details</summary>
Motivation: Sheath formation near biased electrodes in magnetic fields parallel to the wall is poorly understood, especially in Z-pinch fusion experiments where this configuration occurs. Understanding these sheath dynamics is crucial for electrode behavior and plasma-wall interactions in fusion devices.

Method: 1X-2V Boltzmann-Poisson simulations of an axial cut at the pinch radius of a Z-pinch plasma between two biased electrodes with parallel magnetic field. Collision frequencies are artificially increased to enhance thermalization in the smaller simulation domain versus actual experiment size.

Result: Non-monotonic sheath profiles with potential increasing away from the wall to a peak before decaying. Classical sheath forms within electron gyroradius due to magnetized electrons gyrating into wall. Perpendicular current density is three orders of magnitude lower than unmagnetized predictions due to magnetic field acting as high resistivity. Significant parallel flows induce parallel current density three orders of magnitude larger than perpendicular current density.

Conclusion: Magnetic fields parallel to electrodes fundamentally alter sheath physics in Z-pinch configurations, creating unique non-monotonic profiles and shifting current mechanisms from electron-driven to ion-driven perpendicular currents, while enabling substantial parallel currents through force balance between pressure tensor and Lorentz force.
...
TL;DR: The paper describes upgrades to the Diagnostic Neutral Beam Injector (DNBI) for RFX-mod2 fusion experiment, which will enable core plasma measurements via CXRS and MSE diagnostics.


<details>
  <summary>Details</summary>
Motivation: The DNBI is crucial for obtaining novel information about Reversed Field Pinch plasma confinement by enabling Charge Exchange Recombination Spectroscopy (CXRS) and Motional Stark Effect (MSE) diagnostics to measure key plasma parameters at the core.

Method: The DNBI features an arc discharge H+ source with 4-grid 50 keV acceleration system, producing a 50 ms, 5 A ion beam that is neutralized via gas target. The system includes magnetic deflection of residual ions and can be modulated up to 250 Hz. The paper focuses on upgrades to electrical plants and control systems to improve reliability and safety.

Result: The paper presents the latest upgrades to the DNBI system, though specific performance metrics from the upgrades are not detailed in the abstract.

Conclusion: The upgraded DNBI will provide essential diagnostic capabilities for the RFX-mod2 experiment, enabling core plasma measurements that are critical for understanding Reversed Field Pinch confinement physics in fusion research.
...
TL;DR: Simulations capture both magnetic seed generation via electron Weibel instability and subsequent dynamo amplification in collisionless intracluster medium using 10-moment fluid solver with realistic mass ratio.


<details>
  <summary>Details</summary>
Motivation: Bridging spatiotemporal scales of magnetic seed field generation and dynamo amplification in weakly collisional intracluster medium presents extreme numerical challenge that existing studies haven't fully addressed.
...
Result: Electron heat-flux closure regulates pressure isotropization and effectively sets magnetic Reynolds number. Study shows how closure strength influences transition between kinetic pair-plasma regime and more MHD-like dynamo regime.

Conclusion: The approach enables simulation of both magnetic seed generation via electron Weibel instability and subsequent dynamo amplification in collisionless intracluster medium, revealing important transitions between kinetic and MHD regimes.
...
TL;DR: Follow-up response to comments on previous tokamak current ramp-up/down model paper


<details>
  <summary>Details</summary>
Motivation: Address and respond to critical comments made by Dr. A.H. Boozer on the author's previous paper about tokamak current ramp-up and ramp-down modeling

Method: Analysis and discussion of the comments received, likely involving theoretical examination, mathematical validation, and clarification of the original model's assumptions and limitations

Result: The report provides a detailed response to the comments, potentially defending the original model, clarifying misunderstandings, or acknowledging valid criticisms with appropriate adjustments

Conclusion: This follow-up paper serves to engage with the scientific community's feedback, improve the robustness of the original model, and contribute to ongoing discussions about tokamak current control in fusion research
...
Method: Formulates a canonical vorticity framework using canonical vorticity (weighted sum of fluid vorticity and magnetic field) as the canonical variable, unifying processes like Biermann battery and Weibel instability, and extending to relativistic regimes with identification of kineclinicity effect.

Result: The framework successfully unifies multiple magnetogenesis processes, predicts new pressure tensorial configurations as sources of self-generated magnetic fields and vorticity, identifies relativistic kineclinicity effects, and validates theoretical predictions using particle-in-cell simulations.

Conclusion: The canonical vorticity framework provides a comprehensive approach to understanding magnetogenesis in collisionless plasmas, with implications for both laboratory and astrophysical plasma environments, offering new insights into magnetic field generation across different regimes.
...
### [35] [Jordan-Segmentable Masks: A Topology-Aware definition for characterizing Binary Image Segmentation](https://arxiv.org/abs/2601.10577)
*Serena Grazia De Benedictis,Amedeo Altavilla,Nicoletta Del Buono*

Main category: cs.CV

TL;DR: The paper introduces a topology-aware segmentation evaluation framework based on the Jordan Curve Theorem for assessing structural coherence of segmentation masks, addressing limitations of conventional metrics.


<details>
  <summary>Details</summary>
Motivation: Current segmentation evaluation metrics (pixel-wise, region-based, boundary-focused) fail to capture structural and topological coherence. Small inaccuracies can yield high scores while masks lose global shape or connectivity, which is critical in applications like medical imaging where topological correctness matters.

Method: Proposes a topology-aware segmentation notion based on Jordan Curve Theorem adapted for digital planes. Defines "Jordan-segmentatable mask" - a binary segmentation ensuring topological separation into two connected components. Uses digital topology and homology theory to extract a 4-curve candidate from masks, verifying topological validity using Betti numbers (β₀ = β₁ = 1).

Result: Develops a mathematically rigorous, unsupervised criterion for assessing structural coherence of segmentation masks. The framework identifies when masks properly partition image domains into meaningful interior/exterior regions based on topological properties.

Conclusion: The approach combining digital Jordan theory and homological invariants provides a valuable alternative to standard evaluation metrics, especially for applications requiring topological correctness preservation.
...
### [36] [A universal Bochner formula for scalar curvature](https://arxiv.org/abs/2601.10618)
*Sven Hirsch*

Main category: math.DG

TL;DR: The paper introduces a universal Bochner formula for scalar curvature that unifies several existing geometric formulas and inequalities.


<details>
  <summary>Details</summary>
Motivation: To develop a comprehensive framework that connects various geometric formulas involving scalar curvature, providing a unified perspective on seemingly disparate results in differential geometry.

Method: Introduces a universal Bochner formula for scalar curvature that serves as a master equation, from which multiple known formulas can be derived as special cases.

Result: The universal formula successfully encompasses: (1) stability inequality for minimal slicings, (2) Schrödinger-Lichnerowicz-type formula, and (3) higher-dimensional version of Stern's level-set identity.

Conclusion: The universal Bochner formula provides a powerful unifying framework that reveals deep connections between different geometric results involving scalar curvature, potentially leading to new applications and insights in differential geometry.
...
### [37] [Design, Fabrication and Testing of a D-Shaped High Temperature Superconducting Magnet](https://arxiv.org/abs/2601.10295)
...
TL;DR: Researchers designed and fabricated a compact D-shaped superconducting magnet using REBCO HTS tapes for tokamak applications, achieving 0.23 T on-axis toroidal field.


<details>
  <summary>Details</summary>
Motivation: High-temperature superconductors enable compact, high-field magnets for tokamaks to achieve higher fusion power through increased on-axis magnetic fields.
...
Result: Successfully designed and fabricated a compact D-shaped coil system with winding pack terminations, joints, power supply interfacing, and performance testing capabilities.

Conclusion: Demonstrated feasibility of fabricating D-shaped coils using stacked HTS cable for compact tokamak magnet systems, advancing high-temperature superconductor applications in fusion research.
...
### [38] [Discrete versus continuous -- lattice models and their exact continuous counterparts](https://arxiv.org/abs/2601.10184)
*Lorenzo Fusi,Oliver Křenek,Vít Průša,Casey Rodriguez,Rebecca Tozzi,Martin Vejvoda*

Main category: physics.class-ph

TL;DR: The paper establishes systematic correspondences between discrete lattice models of interacting particles and continuous PDE models using Fourier analysis, focusing on dispersion relations across different boundary conditions.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between discrete lattice models (used in statistical mechanics and condensed matter physics) and continuous PDE models (used in continuum mechanics and field theory) by establishing rigorous mathematical correspondences.

Method: Systematic application of Fourier analysis tools to map between discrete and continuous settings, examining nearest-neighbor and multiple-neighbor interaction models, progressing from infinite to periodic to finite lattices with fixed-end boundary conditions.

Result: Established correspondences between discrete lattice models and continuous PDE counterparts through dispersion relation analysis, showing how boundary conditions affect the mapping between discrete and continuous descriptions.

Conclusion: Fourier analysis provides a powerful framework for systematically relating discrete and continuous models of interacting particles, with dispersion relations serving as a key bridge between the two descriptions across various boundary conditions.
...
### [39] [Shallow-KAN Based Solution of Moving Boundary PDEs](https://arxiv.org/abs/2601.09818)
...
<details>
  <summary>Details</summary>
Motivation: To develop a compact and efficient alternative to MLP-based approaches for solving moving boundary PDEs (Stefan problems) without requiring measurement data, leveraging the smaller architecture requirements of KANs while maintaining expressive power.

Method: Proposes a shallow KAN framework that directly approximates temperature distribution T(x,t) and moving interface Γ(t), enforcing governing PDEs, phase equilibrium, and Stefan condition through physics-informed residuals. Uses interface-focused collocation resampling to enhance accuracy. Validated with semi-infinite analytical solutions and extended to 2D using level-set based formulation for interface propagation within the KAN framework.

Result: Numerical experiments in 1D and 2D show accurate reconstructions of both temperature fields and interface dynamics. The framework demonstrates KANs' capability to solve complex moving boundary problems without measurement data, highlighting their potential as a compact and efficient alternative.

Conclusion: KANs are capable of solving complex moving boundary PDEs (Stefan problems) without measurement data, offering a compact and efficient alternative to traditional MLP-based approaches through physics-informed residuals and interface-focused techniques.
...
### [40] [Computing Statistical Properties of Velocity Fields on Current Quantum Hardware](https://arxiv.org/abs/2601.10166)
*Miriam Goldack,Yosi Atia,Ori Alberton,Karl Jansen*

Main category: quant-ph

TL;DR: Quantum methods for extracting statistical properties of velocity fields from quantum CFD simulations without full state tomography


<details>
  <summary>Details</summary>
Motivation: Quantum CFD offers favorable scaling but faces challenges in efficiently reading out simulation results, which has received limited attention in literature

Method: Develop methods to extract statistical properties (central moments, structure functions) directly from parameterized ansatz circuits, avoiding full quantum state tomography; implement for 1D velocity fields with 16 spatial points using 4 qubits

Result: Successfully analyzed sine wave signals and Burgers' equation snapshots with high accuracy on IBMQ's Heron2 system using QESEM error mitigation

Conclusion: Demonstrates practical quantum CFD computations on current quantum devices with efficient readout methods for statistical properties
...
### [41] [Electronic structure theory of H$_{3}$S: Plane-wave-like valence states, density-of-states peak and its guaranteed proximity to the Fermi level](https://arxiv.org/abs/2601.10016)
*Ryosuke Akashi*

Main category: cond-mat.supr-con

TL;DR: The paper explains the mechanism behind the DOS peak formation in H3S superconductors under pressure, showing it results from hybridization of specific plane waves due to Jones' large zone adjacency to the Fermi surface.


<details>
  <summary>Details</summary>
Motivation: While superconductivity in H3S under extreme pressure has been theoretically explained, the mechanism behind the peaked electronic density of states (DOS) that enables high transition temperatures has remained unclear and requires explanation.

Method: Detailed analysis of first-principles electronic wave functions, Fourier-mode analysis of self-consistent potential and atomic pseudopotentials, extraction of nearly uniform models, and investigation of plane-wave hybridization mechanisms.

Result: The DOS peak is shown to result from hybridization of specific plane waves, with adjacency of Jones' large zone to the plane-wave spherical Fermi surface identified as the root cause of multiple plane-wave hybridization and DOS peak formation near the Fermi level.

Conclusion: The theory resolves the minimal modeling problem of electronic states in H3S and establishes a mechanism that could help boost transition temperatures in pressure-induced superconductors.
...
### [42] [Remarks on the convex integration technique applied to singular stochastic partial differential equations](https://arxiv.org/abs/2601.09990)
...
<details>
  <summary>Details</summary>
Motivation: To explore convex integration as a potential method for constructing solutions to singular stochastic PDEs (where nonlinear terms become ill-defined due to rough random forces), and to assess its applicability to important models like the Φ⁴ quantum field theory equation.

Method: Review and analysis of convex integration technique as an alternative to established theories (regularity structures and paracontrolled distributions) for singular SPDEs. The paper examines whether convex integration can be applied to prove non-uniqueness for the Φ⁴ model.

Result: The review shows that while convex integration has emerged as a possible approach for singular SPDEs, its application to prove non-uniqueness for the Φ⁴ model appears unlikely based on current developments.

Conclusion: Convex integration represents a promising alternative method for singular SPDEs, but has limitations for certain important models like the Φ⁴ equation where proving non-uniqueness seems challenging with this approach.
...
### [43] [Malliavin Calculus for the stochastic Cahn-Hilliard equation driven by fractional noise](https://arxiv.org/abs/2601.10490)
...
<details>
  <summary>Details</summary>
Motivation: To investigate the existence of density for stochastic solutions to the Cahn-Hilliard equation perturbed by fractional white noise, which is important for understanding the probabilistic properties of such stochastic PDEs.

Method: Apply Malliavin calculus in one spatial dimension, construct localizing sequences to prove local existence of Malliavin derivative, derive sharp estimates for p-th power expectations of L∞-norm of stochastic integrals and associated operators.

Result: Proved that the stochastic solution admits continuous paths almost surely, has locally existing Malliavin derivative, and its law is absolutely continuous with respect to Lebesgue measure on ℝ, establishing existence of density.

Conclusion: The Cahn-Hilliard equation with additive fractional white noise in 1D has stochastic solutions that possess densities, with key contributions in analyzing stochastic integrals and deriving essential sharp estimates for the study.
...
### [44] [Smoothness of martingale observables and generalized Feynman-Kac formulas](https://arxiv.org/abs/2601.10539)
*Alex Karrila,Lauri Viitasaari*

Main category: math.PR

TL;DR: The paper proves that martingale observables of Itô processes satisfying Hörmander's criterion are smooth, leading to a generalized Feynman-Kac formula for degenerate diffusions with boundary stopping.


<details>
  <summary>Details</summary>
Motivation: To establish smoothness properties for martingale observables in stochastic processes and extend Feynman-Kac formulas to handle degenerate diffusions and boundary stopping problems.

Method: Uses Hörmander's criterion on Itô processes to prove smoothness of martingale observables, then applies this to derive a generalized Feynman-Kac formula through Itô calculus techniques.

Result: All martingale observables are smooth under Hörmander's criterion; a generalized Feynman-Kac formula is obtained that works for degenerate diffusions and boundary stopping with mild boundary regularity.

Conclusion: The smoothness results enable new applications in stochastic analysis, including solving PDE boundary-value problems and addressing questions in Schramm-Loewner evolutions via Girsanov transforms.
...
### [45] [Unbounded symbols, heat flow, and Toeplitz operators](https://arxiv.org/abs/2601.10711)
*Sam Looi*

Main category: math.FA

TL;DR: The paper disproves the Berger-Coburn heat-flow conjecture extension for Toeplitz operators on Bargmann space, showing that boundedness of the natural-domain operator U_g requires stronger quadratic averaging conditions than the linear averaging governing the form-defined operator T_g.


<details>
  <summary>Details</summary>
Motivation: To investigate the natural domain extension of the Berger-Coburn heat-flow conjecture for Toeplitz operators on Bargmann space, and understand the failure mechanism when extending from form-defined operators to natural-domain operators.

Method: Analyzes the gap between pointwise and uniform control of Gaussian averaging of |g|^2, constructs a counterexample with smooth nonnegative radial symbol satisfying coherent-state admissibility, and establishes equivalence between boundedness of U_g and Fock-Carleson measure conditions.

Result: Disproves the conjecture by showing T_g and U_g decouple for unbounded symbols: T_g is bounded while U_g is unbounded for constructed counterexample. Shows boundedness of U_g requires |g|^2 dμ to be a Fock-Carleson measure, strictly stronger than linear averaging condition for T_g.

Conclusion: The failure mechanism is a gap between pointwise and uniform control of Gaussian averaging of |g|^2. Under coherent-state hypothesis, only global geometry at infinity causes unboundedness, not local singularities. Heat-flow regularity is irreversible and bootstrapping cannot resolve the sufficiency-critical time gap.
...
### [46] [The transformation mechanisms among cuboctahedra, Ino's decahedra and icosahedra structures of magic-size gold nanoclusters](https://arxiv.org/abs/2601.10434)
*Ehsan Rahmatizad Khajehpasha,Mohammad Ismaeil Safa,Nasrin Eyvazi,Marco Krummenacher,Stefan Goedecker*

Main category: cond-mat.mtrl-sci

TL;DR: Using machine learning potentials, researchers discovered lower-energy amorphous icosahedral structures and asymmetric transformation pathways for gold nanoclusters that challenge previous models and better match experimental transformation times.


<details>
  <summary>Details</summary>
Motivation: Gold nanoclusters have multiple competing structural motifs with small energy differences, leading to structural coexistence and interconversion. Previous studies may have missed physically relevant transformation pathways and lower-energy structures.
...
Result: Found that high-symmetry transformations proceed through single barriers with soft-mode-driven motions. Identified lower-barrier asymmetric pathways leading to disordered, Jahn-Teller-stabilized amorphous icosahedra. Discovered new global minima for Au309 and Au561 that are up to 2.8 eV lower than previously reported. Transformation pathways give reasonable transformation times that roughly agree with experiments.

Conclusion: Both the shapes and transformation pathways studied in previous investigations are not physically relevant. The newly identified asymmetric pathways and amorphous structures better represent the actual behavior of gold nanoclusters, explaining experimental observations more accurately.
...
### [47] [Variable coherence model for free-electron laser pulses](https://arxiv.org/abs/2601.09885)
*Austin Bartunek,Nils H. Sommerfeld,Francois Mauger*

Main category: physics.optics

TL;DR: The Variable Coherence Model (VCM) simulates FEL pulses with adjustable noise characteristics while maintaining fixed average pulse parameters like bandwidth.


<details>
  <summary>Details</summary>
Motivation: To develop a more flexible model for simulating FEL pulses that allows continuous control over pulse noise characteristics while keeping average parameters constant, extending beyond existing partial coherence models.

Method: Builds on existing partial coherence model by implementing variable coherence width parameter, enabling continuous control over pulse noise. Performs systematic statistical analyses of intensity and sub-pulse counts in both time and frequency domains across three FEL parameter regimes.

Result: VCM successfully generates pulses ranging from maximally random to fully coherent by adjusting coherence width. Statistical analyses show how sub-pulse characteristics vary with coherence width, and absorption simulations demonstrate practical impact of variable coherence.

Conclusion: The VCM provides a versatile tool for simulating FEL pulses with tunable noise properties while maintaining consistent average parameters, offering improved flexibility for FEL pulse modeling and applications like absorption simulations.
...
### [48] [Near-Unity-Efficiency Gas Gratings for Ultraviolet, Visible, and Infrared High-Power Lasers](https://arxiv.org/abs/2601.09963)
...
TL;DR: Gas gratings created by interfering DUV lasers in ozone-doped gas flow achieve up to 99% diffraction efficiency across broad wavelength and pulse duration ranges, with high damage thresholds and stable operation.


<details>
  <summary>Details</summary>
Motivation: Conventional solid optics have limited damage thresholds and are vulnerable to debris, creating a need for robust, high-damage-threshold optical elements for manipulating high-energy lasers.

Method: Using interfering deep ultraviolet lasers to induce density modulations via photochemical reactions in ozone-doped gas flow, creating transient volume diffraction gratings. Performance characterized under various conditions including imprint fluence, gas composition, and grating geometries.

Result: Achieved 99% full beam diffraction efficiency across wavelengths from DUV to near-infrared and pulse durations from nanosecond to femtosecond, while preserving focusability and wavefront quality. Performance enhanced with carbon dioxide addition and demonstrated stable operation over hours.

Conclusion: Gas gratings provide a promising method for efficiently manipulating high-energy lasers with superior damage thresholds and debris immunity compared to solid optics, with validated theoretical models suggesting optimal scaling parameters for high-energy applications.
...
### [49] [On gradient stability in nonlinear PDE models and inference in interacting particle systems](https://arxiv.org/abs/2601.10326)
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of verifying gradient stability conditions for non-linear PDE inverse problems, which is crucial for statistical identifiability and computational efficiency in Bayesian inference settings.

Method: Uses a Banach space version of the implicit function theorem to analyze parameter-to-solution maps of non-linear PDEs, verifying gradient stability conditions, providing injectivity estimates, and establishing statistical identifiability results.

Result: Develops theoretical framework for gradient stability verification, applies it to non-linear reaction-diffusion systems and McKean-Vlasov interacting particle models, and proves polynomial-time convergence of Langevin-type algorithms for posterior sampling of interaction potentials.

Conclusion: The Banach space implicit function theorem approach successfully establishes gradient stability for non-linear PDE inverse problems, enabling rigorous statistical identifiability results and efficient computational algorithms for Bayesian inference in complex systems.
...
### [50] [In-Context Operator Learning on the Space of Probability Measures](https://arxiv.org/abs/2601.09979)
*Frank Cole,Dixi Wang,Yineng Chen,Yulong Lu,Rongjie Lai*

Main category: cs.LG

TL;DR: In-context operator learning for optimal transport that maps distribution pairs to OT maps using few-shot prompts without gradient updates at inference.


<details>
  <summary>Details</summary>
Motivation: To develop a solution operator that can compute optimal transport maps between distributions using only few-shot samples as context, eliminating the need for gradient-based optimization at inference time.

Method: Parameterize solution operator and develop scaling-law theory in two regimes: nonparametric setting with low-dimensional manifold tasks, and parametric setting (e.g., Gaussian families) with explicit architecture.

Result: Established generalization bounds for nonparametric setting showing how accuracy scales with prompt size, intrinsic dimension, and model capacity; provided explicit architecture for parametric setting with exact OT map recovery and finite-sample bounds.

Conclusion: The framework enables in-context learning of optimal transport operators from few-shot samples, validated on synthetic transports and generative-modeling benchmarks.
...
### [51] [Volume penalization method for simulating flows around a rotating solid with multiple reference frame and sliding mesh](https://arxiv.org/abs/2601.10230)
*Ming Liu,Yosuke Hasegawa*

Main category: physics.flu-dyn

TL;DR: The paper combines volume penalization method (VPM) with multiple reference frame (MRF) and sliding mesh (SLM) techniques to develop immersed-boundary approaches for simulating flows around rotating solids with complex geometry.


<details>
  <summary>Details</summary>
Motivation: Precise simulation of rotating solid objects with complex geometry in turbomachinery is challenging, requiring improved computational methods for fluid-based energy transfer applications.

Method: Combines VPM with MRF and SLM separately, uses level-set function for geometry representation on Cartesian grids, develops unified governing equations with VPM body-forcing terms for both fluid and solid regions.

Result: Simulations of flows around rotating cuboid show relative deviations of predicted pressure drop and torque between VPM and body-fitted method (BFM) are around 5%, validating the proposed VPM approaches.

Conclusion: The developed VPM with MRF and VPM with SLM schemes are valid for simulating flows around rotating solids, offering accurate alternatives to traditional body-fitted methods with approximately 5% deviation.
...
### [52] [Stable Differentiable Modal Synthesis for Learning Nonlinear Dynamics](https://arxiv.org/abs/2601.10453)
...
<details>
  <summary>Details</summary>
Motivation: Modal methods for physical modeling synthesis need extensions for nonlinear problems like high-amplitude string vibration. While scalar auxiliary variable techniques enable stable solvers for nonlinear systems, and neural ODEs can learn nonlinear dynamics from data, there's a need to combine these approaches for stable differentiable learning.

Method: Proposes combining scalar auxiliary variable techniques with neural ordinary differential equations to create stable differentiable models. Leverages analytical solutions for linear vibration of system's modes to keep physical parameters accessible without needing parameter encoders in architecture.

Result: The model can be trained to reproduce nonlinear dynamics of a string vibration system using synthetic data. Sound examples demonstrate the approach works as a proof of concept.

Conclusion: Successfully demonstrates that scalar auxiliary variable techniques can be integrated with neural ODEs to create stable differentiable models capable of learning nonlinear dynamics while maintaining accessibility to physical parameters.
